...
#include "RISE/SlayerQaCredentials.h"
#endif
#include <vector>
#include <string>
#include <algorithm>
...
void CLoginWin::Create()
{
	...
	size_t qaAccountIndex = 0;
#ifdef RISE_SLAYER_RUNTIME_QA
	char requestedAccount[MAX_ID_SIZE + 1] = {};
	if (GetEnvironmentVariableA("RISE_SLAYER_QA_ACCOUNT", requestedAccount,
		sizeof(requestedAccount)) == 0)
		strcpy_s(requestedAccount, sizeof(requestedAccount), "admin4");
	for (size_t i = 0; i < __ids.size() && i < __pws.size(); ++i)
	{
		if (_stricmp(__ids[i].c_str(), requestedAccount) == 0)
		{
			qaAccountIndex = i;
			break;
		}
	...
	if (!__ids.empty() && qaAccountIndex < __ids.size())
	{
		strncpy(m_ID, __ids[qaAccountIndex].c_str(), MAX_ID_SIZE); m_ID[MAX_ID_SIZE] = '\0';
	}
	...
	if (!__pws.empty() && qaAccountIndex < __pws.size())
	{
		strncpy(m_PW, __pws[qaAccountIndex].c_str(), MAX_PASSWORD_SIZE); m_PW[MAX_PASSWORD_SIZE] = '\0';
	}
	...
	{
#ifdef RISE_SLAYER_RUNTIME_QA
		this->FirstLoad = IsSlayerAutoQaEnabled() ? 2 : 3;
#else
		this->FirstLoad = 3;
#endif
	}
...
#ifdef RISE_SLAYER_RUNTIME_QA
	if (TrySlayerAutoQaLogin())
		return;
#else
	if (this->FirstLoad == 2 && CurrentProtocolState == RECEIVE_JOIN_SERVER_SUCCESS)
	{
		RequestLogin();
		this->FirstLoad = 3;
		return;
...
#ifdef RISE_SLAYER_RUNTIME_QA
bool CLoginWin::TrySlayerAutoQaLogin()
{
	if (!IsSlayerAutoQaEnabled() || this->FirstLoad != 2 ||
		CurrentProtocolState != RECEIVE_JOIN_SERVER_SUCCESS)
		return false;
	this->FirstLoad = 3;
	rise::slayerqa::AppendRuntimeQALog("login-auto-submit credential-store=registry account=admin4");
	RequestLogin();
	return true;
}
#endif
