...
struct VAOMesh
{
	...
	GLuint VertexCount;
	GLuint IndexCount;
	TextureScript* TextureScriptData;
	std::vector<VertexBMD> Vertices;
	std::vector<GLuint> Indices;
	std::vector<short> Bones;

	VAOMesh()
		: NoneBlendMesh(true), Texture(0), VAO(0), VBO(0), IBO(0),
		  VertexCount(0), IndexCount(0), TextureScriptData(NULL)
	{
	}
...
public:
    ...
    bool PlayAnimationStep(float* AnimationFrame, float* PriorAnimationFrame, unsigned short* PriorAction, float Speed, vec3_t Origin, vec3_t Angle, float stepFactor);
