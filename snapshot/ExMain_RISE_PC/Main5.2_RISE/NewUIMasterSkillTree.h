...
namespace SEASON3B
{
	...
	class CNewUIMasterSkillTree : public CNewUIObj
	{
		...
		bool IsInMasterSkill();
		void GetMasterAddPointEx(float& Str, float& Agi, float& Vit, float& Ene, float& Cmd);
		void GetMasterAddPoint(DWORD& Str, DWORD& Agi, DWORD& Vit, DWORD& Ene, DWORD& Cmd);
		...
		WORD CategoryPoint[MAX_MASTER_SKILL_CATEGORY];
