...
static_assert(CLASS_TEMPLENIGHT + 1 == rise::growlancer::kGrowLancerNativeClassType,
	"Grow Lancer selector must append without changing SS6 class IDs");
...
BYTE CCharacterManager::GetCharacterClass(const BYTE byClass)
{
	...
	break;
	case rise::growlancer::kGrowLancerSourceBaseClass:
	{
		// The S21 source proves base7; evolution class labels remain unpinned.
		byCharacterClass = (bySecondClass || byThirdClass) ?
			rise::growlancer::kUnmappedGrowLancerEvolutionClassType :
			rise::growlancer::kGrowLancerNativeClassType;
	}
	break;
	}
...
const char* CCharacterManager::GetCharacterClassText(const BYTE byClass)
{
	...
	if (byCharacterClass == CLASS_MAGIC)
	{
	...
	if (byCharacterClass == rise::growlancer::kGrowLancerNativeClassType)
		return "Grow Lancer";
	// 3151 "템플나이트"
...
{
	if ((pChar->Weapon[1].Type != MODEL_BOW + 7) && ((pChar->Weapon[1].Type >= MODEL_BOW) && (pChar->Weapon[1].Type < MODEL_BOW + MAX_ITEM_INDEX)) || gCustomBow.CheckIsBow(pChar->Weapon[1].Type) == 1)
	{
		return BOWTYPE_BOW;
	}
	else if ((pChar->Weapon[0].Type != MODEL_BOW + 15) && ((pChar->Weapon[0].Type >= MODEL_BOW + 8) && (pChar->Weapon[0].Type < MODEL_BOW + MAX_ITEM_INDEX)) || gCustomBow.CheckIsCrossBow(pChar->Weapon[0].Type) == 1)
	{
...
{
	if ((CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type != ITEM_BOW + 7) && ((CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type >= ITEM_BOW) && (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type < ITEM_BOW + MAX_ITEM_INDEX)) || gCustomBow.CheckIsBow(CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type) == 1)
	{
		return BOWTYPE_BOW;
	}
	else if ((CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type != ITEM_BOW + 15) && ((CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type >= ITEM_BOW + 8) && (CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type < ITEM_BOW + MAX_ITEM_INDEX)) || gCustomBow.CheckIsCrossBow(CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type) == 1)
	{
		return BOWTYPE_CROSSBOW;
	}
	return BOWTYPE_NONE;
...
{
	if (((pItem->Type >= ITEM_BOW) && (pItem->Type <= ITEM_BOW + 6)) || (pItem->Type == ITEM_BOW + 17) || ((pItem->Type >= ITEM_BOW + 20) && (pItem->Type <= ITEM_BOW + 23)) || (pItem->Type == ITEM_BOW + 24) || gCustomBow.CheckIsBow(pItem->Type) == 1)
	{
		return BOWTYPE_BOW;
	}

	else if (((pItem->Type >= ITEM_BOW + 8) && (pItem->Type <= ITEM_BOW + 14)) || (pItem->Type == ITEM_BOW + 16) || ((pItem->Type >= ITEM_BOW + 18) && (pItem->Type <= ITEM_BOW + 19)) || gCustomBow.CheckIsCrossBow(pItem->Type) == 1)
	{
		return BOWTYPE_CROSSBOW;
	}
	return BOWTYPE_NONE;
...
bool CCharacterManager::IsEquipedWing()
{
	...
	if ((pEquippedItem->Type >= ITEM_WING && pEquippedItem->Type <= ITEM_WING + 6)
		...
		|| (pEquippedItem->Type >= ITEM_WING + 49 && pEquippedItem->Type <= ITEM_WING + 50)
		|| (pEquippedItem->Type == ITEM_WING + 135 || pEquippedItem->Type == ITEM_WING + 154)
		|| ( gCustomWing->CheckCustomWingByItem(pEquippedItem->Type))
		)
	{
		return true;
	}
...
CHARACTER* CCharacterManager::FindCharacterByName(const char* szName)
{
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		CHARACTER* pChar = &CharactersClient[i];

		if (pChar->IsLife > 0 && strcmp(pChar->ID, szName) == 0)
		{
			return pChar;
		}
	}
