...
{
	if (this->IsSlayerClientClass(byClass))
	{
		// Keep generic class consumers on the safe DK-compatible enum; the
		// class-text path below supplies the Slayer label explicitly.
		return CLASS_KNIGHT;
	}
...
BYTE CCharacterManager::GetSkinModelIndex(const BYTE byClass)
{
	if (this->IsSlayerClientClass(byClass))
	{
		...
		// Seven legacy classes x three stages occupy 0..20. The private
		// Class09/209/309 loader owns the unused body slots 21..23.
		const BYTE stage = this->IsThirdClass(byClass) ? 2 :
			(this->IsSecondClass(byClass) ? 1 : 0);
		return static_cast<BYTE>(MODEL_BODY_NUM - 3 + stage);
	#else
		return CLASS_KNIGHT;
	#endif
	}
