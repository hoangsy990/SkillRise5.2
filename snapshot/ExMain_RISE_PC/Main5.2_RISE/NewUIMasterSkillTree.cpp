...
void SEASON3B::CNewUIMasterSkillTree::LoadImages()
{
	...
	LoadBitmapFile("RISE\\GrowLancer\\Master\\new_Master_Icon.jpg",
		rise::growlancer::kWrathMasterIconAtlasBitmap, GL_LINEAR);
	LoadBitmapFile("RISE\\GrowLancer\\Master\\new_Master_non_Icon.jpg",
		rise::growlancer::kWrathMasterDisabledIconAtlasBitmap, GL_LINEAR);
...
{
	DeleteBitmap(rise::growlancer::kWrathMasterIconAtlasBitmap, 0);
	DeleteBitmap(rise::growlancer::kWrathMasterDisabledIconAtlasBitmap, 0);
	for (int i = 0; i < 15; i++)
	{
...
void SEASON3B::CNewUIMasterSkillTree::RenderIcon()
{
	...
	for (std::map<BYTE, _MASTER_SKILLTREE_DATA>::iterator it = this->map_masterData.begin(); it != this->map_masterData.end(); it++)
	{
		...
		v20 = CalcY + 5.0;
		v19 = CalcX + 8.5;
		const int iconAtlas = Skill == rise::growlancer::kWrathMasterSkillId ?
			rise::growlancer::kWrathMasterIconAtlasBitmap : IMAGE_MASTER_INTERFACE + 2;
		SEASON3B::RenderImage(iconAtlas, v19, v20, 20.0, 28.0,
			v21, v22, 0.0390625, 0.053710938, textColor);
		//===Draw Skill Point
...
{
	if (index < 0 || index >= rise::growlancer::kSkillAttributeIdCapacity)
	{
		return 0;
	}
