...
class CGlobalBitmap  
{
	...
	std::map<std::string, BITMAP_t*> m_mapBitmapByName;
	std::map<std::string, BITMAP_t*> m_mapBitmapByBaseName;
	type_index_list m_listNonamedIndex;

	GLuint	m_uiAlternate, m_uiTextureIndexStream;
	DWORD	m_dwUsedTextureMemory;
	void*	m_pTurboJpegDecoder;
...
protected:
	...
	bool OpenJpegTurbo(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter = GL_NEAREST, GLuint uiWrapMode = GL_CLAMP_TO_EDGE);
	...
	bool OpenRise(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter = GL_NEAREST, GLuint uiWrapMode = GL_CLAMP_TO_EDGE);
