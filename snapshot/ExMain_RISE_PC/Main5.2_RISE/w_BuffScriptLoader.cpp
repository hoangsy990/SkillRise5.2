...
{
	std::string filename = "Data/RISE/Config/BuffEffect.bmd";
...
{
	const char* wrathName = rise::growlancer::WrathBuffName(static_cast<int>(type));
	if (wrathName)
	{
		BuffInfo info;
		info.s_BuffIndex = static_cast<short>(type);
		info.s_BuffEffectType = 123;
		info.s_BuffClassType = eBuffClass_Buff;
		// Native defaults keep item255/255 and notice0. Clear-policy mapping is
		// deliberately not enabled from an as-yet uninterpreted source column7.
		strcpy_s(info.s_BuffName, wrathName);
		strcpy_s(info.s_BuffDescript, rise::growlancer::kWrathBuffDescription);
		info.s_BuffDescriptlist.push_back(rise::growlancer::kWrathBuffDescription);
		return info;
	}
...
{
	// S21 classifier 0 maps to the native named normal-buff enum. Keep the
	// legacy count, disk records and all other IDs unchanged; metadata/transport
	// readiness is independent and remains disabled until lifecycle is wired.
	if (rise::growlancer::HasWrathBuffClassification(static_cast<int>(type)))
		return eBuffClass_Buff;
