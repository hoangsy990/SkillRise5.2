...
namespace
{
    inline unsigned int RotateLeft32(unsigned int value, unsigned int bits)
    {
        bits &= 31u;
        return bits == 0u ? value : ((value << bits) | (value >> (32u - bits)));
    }

    inline unsigned int RotateRight32(unsigned int value, unsigned int bits)
    {
        bits &= 31u;
        return bits == 0u ? value : ((value >> bits) | (value << (32u - bits)));
    }

    void ExpandS21Lea256Key(unsigned int roundKeys[32][6])
    {
        static const unsigned char keyBytes[32] = {
        ...
        static const unsigned int delta[8] = {
            0xC3EFE9DBu, 0x44626B02u, 0x79E27C8Au, 0x78DF30ECu,
            0x715EA49Eu, 0xC785DA0Au, 0xE04EF22Au, 0xE5C40957u
        };
        static const unsigned int rotations[6] = { 1u, 3u, 6u, 11u, 13u, 17u };
        unsigned int words[8];
        memcpy(words, keyBytes, sizeof(words));

        for (unsigned int round = 0; round < 32u; ++round)
        {
            for (unsigned int lane = 0; lane < 6u; ++lane)
            {
                const unsigned int wordIndex = (6u * round + lane) % 8u;
                words[wordIndex] = RotateLeft32(
                    words[wordIndex] + RotateLeft32(delta[round % 8u], round + lane),
                    rotations[lane]);
                roundKeys[round][lane] = words[wordIndex];
            }
        }
    }

    void DecryptS21Lea256Payload(unsigned char* destination, const unsigned char* source, unsigned int size)
    {
        unsigned int roundKeys[32][6];
        ExpandS21Lea256Key(roundKeys);

        for (unsigned int offset = 0; offset < size; offset += 16u)
        {
            unsigned int y[4];
            memcpy(y, source + offset, sizeof(y));
            for (int round = 31; round >= 0; --round)
            {
                const unsigned int x0 = y[3];
                const unsigned int x1 =
                    (RotateRight32(y[0], 9u) - (x0 ^ roundKeys[round][0])) ^ roundKeys[round][1];
                const unsigned int x2 =
                    (RotateLeft32(y[1], 5u) - (x1 ^ roundKeys[round][2])) ^ roundKeys[round][3];
                const unsigned int x3 =
                    (RotateLeft32(y[2], 3u) - (x2 ^ roundKeys[round][4])) ^ roundKeys[round][5];
                y[0] = x0;
                y[1] = x1;
                y[2] = x2;
                y[3] = x3;
            }
            memcpy(destination + offset, y, sizeof(y));
        }
    }
}
#endif
...
void BMD::Transform(float(*BoneMatrix)[3][4], vec3_t BoundingBoxMin, vec3_t BoundingBoxMax, OBB_t* OBB, bool Translate, float _Scale)
{
    ...
    if (OGL330::IsShader())
    {
        ...
        if (EditFlag != 2 && OGL330MODEL::ShouldSkipCpuSkinning(this))
        {
            ...
            fTransformedSize = max(max(OBB->XAxis[0], OBB->YAxis[1]), OBB->ZAxis[2]);
            return;
        }
    }
#endif
...
void BMD::RenderMesh(int meshIndex, int renderFlags, float alpha, int blendMeshIndex, float blendMeshAlpha, float blendMeshTextureCoordU, float blendMeshTextureCoordV, int explicitTextureIndex)
{
    ...
    if (OGL330::IsShader())
    {
        if (GMMeshShader->AddMeshCommand(this, meshIndex, renderFlags, alpha, blendMeshIndex, blendMeshAlpha, blendMeshTextureCoordU, blendMeshTextureCoordV, explicitTextureIndex))
        {
            return;
        }

        // Preserve draw order when this mesh must use the legacy renderer.
...
void BMD::RenderBody(int Flag, float Alpha, int BlendMesh, float BlendMeshLight, float BlendMeshTexCoordU, float BlendMeshTexCoordV, int HiddenMesh, int Texture)
{
    ...
    BeginRender(Alpha);
#ifdef jdk_shader_local330
    const bool shaderBatch = OGL330::IsShader();
    if (shaderBatch)
    {
    ...
    if (shaderBatch)
    {
        // Draw while the model-view matrix pushed by BeginRender is still active.
        GMMeshShader->FlushAllMesh();
...
{
    if (!g_pOption->GetRenderAllEffects() || !g_pOption->GetRenderShadowEffects())
    {
        return;
    }
    ...
    glColor4f(0.0f, 0.0f, 0.0f, 0.35f); // 50% opacity for shadows
...
bool BMD::Open(char* DirName, char* ModelFileName)
{
    ...
    if (_snprintf_s(ModelName, sizeof(ModelName), _TRUNCATE, "%s%s", DirName, ModelFileName) < 0)
    {
        return false;
    }
...
bool BMD::Open2(char* DirName, char* ModelFileName, bool bReAlloc)
{
    ...
    else if (Version == 0x0F)
    {
        ...
        memcpy(&encodedSize, Data + DataPtr, sizeof(encodedSize));
        DataPtr += sizeof(encodedSize);
        if (encodedSize != static_cast<unsigned int>(DataBytes - DataPtr) ||
            encodedSize == 0u || (encodedSize % 16u) != 0u)
        {
            delete[] Data;
            m_bCompletedAlloc = false;
            return false;
        }
        BYTE* decrypted = new BYTE[encodedSize];
        DecryptS21Lea256Payload(decrypted, Data + DataPtr, encodedSize);
        delete[] Data;
        Data = decrypted;
        DataBytes = static_cast<int>(encodedSize);
        DataPtr = 0;
    }
#endif

    if (DataBytes - DataPtr < 38)
    {
        delete[] Data;
        m_bCompletedAlloc = false;
        return false;
    ...
    if (NumMeshs < 0 || NumMeshs > 4096 || NumBones < 0 || NumBones > MAX_BONES ||
