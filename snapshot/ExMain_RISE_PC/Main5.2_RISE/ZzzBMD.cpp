...
{
    return PlayAnimationStep(AnimationFrame, PriorAnimationFrame, PriorAction,
        Speed, Origin, Angle, FPS_ANIMATION_FACTOR);
}

bool BMD::PlayAnimationStep(float* AnimationFrame, float* PriorAnimationFrame, unsigned short* PriorAction, float Speed, vec3_t Origin, vec3_t Angle, float stepFactor)
{
    bool Loop = true;
    ...
    *AnimationFrame += Speed * stepFactor;
    ...
    for (int j = 0; j < m->NumTriangles; j++)
    {
        ...
        for (int k = 0; k < tp->Polygon; k++)
        {
            ...
            case MODEL_GOLEM_STONE:
                ...
                {
                    if (iSubType == 0) {	//. \xBAҰ\xF1\xB7\xBD
                        CreateEffect(MODEL_GOLEM_STONE, VertexTransform[i][vi], angle, Light);
                    }
                    else if (iSubType == 1) {	//. \xB5\xB6\xB0\xF1\xB7\xBD
