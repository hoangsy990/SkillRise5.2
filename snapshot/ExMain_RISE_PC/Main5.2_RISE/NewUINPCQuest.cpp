...
void CNewUINPCQuest::RenderText()
{
	...
	char* NameTitlez = nullptr;

	if ((Hero->Class == CLASS_DARK_LORD || Hero->Class == CLASS_MAGIC || Hero->Class == CLASS_RAGEFIGHTER) && bCheckNPC)
	{
		NameTitlez = g_csQuest.GetNPCName(2);
	}
	else
	{
		NameTitlez = g_csQuest.GetNPCName(byCurQuestIndex);
	}

	g_pUIForm->Form(m_Pos.x, 0, 207.f, 359.f, INTERFACE_NPCQUEST, NameTitlez);
	...
	if ((Hero->Class != CLASS_DARK_LORD && Hero->Class != CLASS_MAGIC && Hero->Class != CLASS_RAGEFIGHTER) || !bCheckNPC)
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 33, g_csQuest.getQuestTitle(), NPCQUEST_WIDTH, 0, RT3_SORT_CENTER);
...
bool CNewUINPCQuest::RenderItemMobText()
{
	...
	{
		if (nClass < 0 || nClass >= MAX_CLASS || !pQuest->QuestAct[i].byRequestClass[nClass])
			continue;
