...
bool CSItemOption::OpenItemSetScript(bool bTestServer)
{
	...
	strFileName = std::string("Data\\RISE\\Config") + ReadLang + "\\ItemSetType.bmd";
	if (!OpenItemSetType(strFileName.c_str()))		return false;

	strFileName = std::string("Data\\RISE\\Config") + ReadLang + "\\ItemSetOption.bmd";
...
bool CSItemOption::IsDisableSkill(int Type, int Energy, int Charisma)
{
	...
	switch (Type)
	{
	case AT_SKILL_ENERGYBALL:	SkillEnergy = 0; break;
	case AT_SKILL_SUMMON:		SkillEnergy = 30; break;
	case AT_SKILL_SUMMON + 1:	SkillEnergy = 60; break;
	case AT_SKILL_SUMMON + 2:	SkillEnergy = 90; break;
	case AT_SKILL_SUMMON + 3:	SkillEnergy = 130; break;
	case AT_SKILL_SUMMON + 4:	SkillEnergy = 170; break;
	case AT_SKILL_SUMMON + 5:	SkillEnergy = 210; break;
	case AT_SKILL_SUMMON + 6:	SkillEnergy = 300; break;
	case AT_SKILL_SUMMON + 7:	SkillEnergy = 500; break;
	case AT_SKILL_STRONG_PIER:	SkillEnergy = 15; break;
...
bool	CSItemOption::GetSetItemName(char* strName, const int iType, const int setType)
{
	...
	if (setItemType == 0 || setItemType == 1)
	{
		...
		if (itemSType.byOption[setItemType] != 0xFF && itemSType.byOption[setItemType] != 0)
		{
			...
			memcpy(strName, itemOption.strSetName, sizeof(char) * 31);
			strName[31] = 0;

			int length = 0;
			while (length < 31 && strName[length] != 0)
			{
				++length;
			}
...
void CSItemOption::calcSetOptionList(BYTE* optionList)
{
	...
	const bool hasLegacyClassColumn = Class >= 0 && Class < MAX_CLASS;
	...
	for (int i = 0; i < 30; i += 3)
	{
		if (optionList[i + 1] >= 2)
		{
			...
			if (itemOption.byRequireClass[3] == 1 && Class == CLASS_MAGIC) RequireClass = 1;
			if (itemOption.byRequireClass[3] == 1 && Class == CLASS_MAGIC && ExClass) RequireClass = 1;
			...
			for (int j = 0; j < count; ++j)
			{
				...
				if (option[0] != 0xFF)
				{
					...
					{
						if (hasLegacyClassColumn && itemOption.byRequireClass[Class] && ExClass >= itemOption.byRequireClass[Class] - 1)
						{
							arruiSetItemTypeSequence[setType][optionCount[setType]] = iCurrentSetItemTypeSequence;
							bySetOptionListTmp[setType][optionCount[setType]] = option[0];
							bySetOptionListTmp2[setType][optionCount[setType]] = RequireClass;
...
int CSItemOption::RenderDefaultOptionText(const ITEM* ip, int TextNum)
{
	...
	if (GetDefaultOptionText(ip, TextList[TNum]))
	{
		...
		if ((ip->Type >= ITEM_HELPER + 300 && ip->Type <= ITEM_HELPER + 400) || (ip->Type >= ITEM_HELPER + 8 && ip->Type <= ITEM_HELPER + 9) || (ip->Type >= ITEM_HELPER + 12 && ip->Type <= ITEM_HELPER + 13) || (ip->Type >= ITEM_HELPER + 21 && ip->Type <= ITEM_HELPER + 27))
		{
...
//void CSItemOption::getAllAddStateOnlyAddValue(DWORD* AddStrength, DWORD* AddDexterity, DWORD* AddEnergy, DWORD* AddVitality, DWORD* AddCharisma)
//{
//	DWORD oriStr = *AddStrength;
//	DWORD oriDex = *AddDexterity;
//	DWORD oriEng = *AddEnergy;
//	DWORD oriVit = *AddVitality;
//	DWORD oriCha = *AddCharisma;
//
//	*AddStrength = *AddDexterity = *AddEnergy = *AddVitality = *AddCharisma = 0;
//	memset(m_bySetOptionListOnOff, 0, sizeof(BYTE) * 16);
//
...
void CSItemOption::CheckItemSetOptions(void)
{
	...
	DWORD AllStrength	= CharacterAttribute->Strength		+ CharacterAttribute->AddStrength;
	DWORD AllDexterity	= CharacterAttribute->Dexterity		+ CharacterAttribute->AddDexterity;
	DWORD AllEnergy		= CharacterAttribute->Energy		+ CharacterAttribute->AddEnergy;
	DWORD AllVitality	= CharacterAttribute->Vitality		+ CharacterAttribute->AddVitality;
	DWORD AllCharisma	= CharacterAttribute->Charisma		+ CharacterAttribute->AddCharisma;
	DWORD AllLevel		= CharacterAttribute->Level;
