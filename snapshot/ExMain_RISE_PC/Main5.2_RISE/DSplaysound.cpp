...
LPDIRECTSOUNDBUFFER     g_lpDSBuffer[rise::growlancer::kSoundBufferCapacity][MAX_CHANNEL];
LPDIRECTSOUND3DBUFFER   g_lpDS3DBuffer[rise::growlancer::kSoundBufferCapacity][MAX_CHANNEL];

DWORD                   g_dwBufferBytes;

OBJECT* Object3DSound[rise::growlancer::kSoundBufferCapacity][MAX_CHANNEL];
char                    BufferName[rise::growlancer::kSoundBufferCapacity][64];
int                     BufferChannel[rise::growlancer::kSoundBufferCapacity];
int                     MaxBufferChannel[rise::growlancer::kSoundBufferCapacity];
bool                    Enable3DSound[rise::growlancer::kSoundBufferCapacity];

LPKSPROPERTYSET		    g_lpPropertySet[rise::growlancer::kSoundBufferCapacity];
...
HRESULT InitDirectSound(HWND hDlg)
{
	...
	else
	{
		...
		for (int i = 0; i < rise::growlancer::kSoundBufferCapacity; ++i)
		{
...
VOID LoadWaveFile(int Buffer, TCHAR* strFileName, int MaxChannel, bool Enable)
{
	if (!g_EnableSound)
		return;
	if (Buffer < 0 || Buffer >= rise::growlancer::kSoundBufferCapacity)
		return;
...
HRESULT ReleaseBuffer(int Buffer)
{
	...
	if (Buffer < 0 || Buffer >= rise::growlancer::kSoundBufferCapacity) return false;
