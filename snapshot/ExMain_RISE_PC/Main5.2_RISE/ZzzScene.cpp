...
#ifdef RISE_SLAYER_RUNTIME_QA
	static bool slayerQaCharacterSelected = false;
	if (!slayerQaCharacterSelected && CurrentProtocolState == RECEIVE_CHARACTERS_LIST)
	{
		char requestedName[MAX_ID_SIZE + 1] = {};
		if (GetEnvironmentVariableA("RISE_SLAYER_QA_CHARACTER", requestedName,
			sizeof(requestedName)) == 0)
			strcpy_s(requestedName, sizeof(requestedName), "Slayer");
		for (int qaIndex = 0; qaIndex < MAX_CHARACTERS_CLIENT; ++qaIndex)
		{
			if (CharactersClient[qaIndex].Object.Live && CharactersClient[qaIndex].ID[0] &&
				!(CharactersClient[qaIndex].CtlCode & CTLCODE_01BLOCKCHAR) &&
				_stricmp(CharactersClient[qaIndex].ID, requestedName) == 0)
			{
				slayerQaCharacterSelected = true;
				SelectedHero = qaIndex;
				CUIMng::Instance().m_CharSelMainWin.UpdateDisplay();
				rise::slayerqa::AppendRuntimeQALog("character-auto-select requested=Slayer");
				StartGame();
				return;
			}
		}
...
void MoveMainScene()
{
...
#ifdef RISE_SLAYER_RUNTIME_QA
	rise::slayerqa::RunRuntimeQAAutoSequence();
#endif
	//init
	EarthQuake *= 0.2f;
	...
	if (Hero && Hero->Object.Live)
		rise::slayer::TickNativeRuntime(Hero->Key,
