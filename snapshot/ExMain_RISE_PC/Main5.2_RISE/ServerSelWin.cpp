...
#ifdef RISE_SLAYER_RUNTIME_QA
static bool IsSlayerAutoQaEnabled()
{
...
void CServerSelWin::SetServerBtnPosition()
{
	...
	{
		m_aServerBtn[i].SetPosition(nServerBtnPosX - 25, nServerBtnBasePosY + nServerBtnHeight * i);
...
#ifdef RISE_SLAYER_RUNTIME_QA
	if (m_iSelectServerBtnIndex == -1 && TrySlayerAutoQaSelect())
		return;
#endif
...
#ifdef RISE_SLAYER_RUNTIME_QA
	TrySlayerAutoQaSelect();
#endif
}

#ifdef RISE_SLAYER_RUNTIME_QA
bool CServerSelWin::TrySlayerAutoQaSelect()
{
	if (!IsSlayerAutoQaEnabled())
		return false;
	if (m_iSelectServerBtnIndex == -1)
	{
		for (int i = 0; i < SSW_SERVER_G_MAX; ++i)
		{
			if (g_ServerListManager->GetServerGroupByBtnPos(i) == NULL)
				continue;
			m_aServerGroupBtn[i].SetCheck(true);
			m_iSelectServerBtnIndex = i;
			rise::slayerqa::AppendRuntimeQALog("server-auto-select group=selected");
	...
	if (m_pSelectServerGroup == NULL)
		return false;
	for (int i = 0; i < m_icntServer; ++i)
	{
		CServerInfo* qaServer = m_pSelectServerGroup->GetServerInfo(i);
		if (qaServer == NULL || qaServer->m_iPercent >= 100)
			continue;
		CUIMng::Instance().HideWin(this);
		SendRequestServerAddress(qaServer->m_iConnectIndex);
		int censorship = CGameCensorship::STATE_12;
		if (m_pSelectServerGroup->m_bPvPServer)
			censorship = CGameCensorship::STATE_18;
		else if ((qaServer->m_byNonPvP & 0x01) != 0)
			censorship = CGameCensorship::STATE_15;
		const bool testServer = m_pSelectServerGroup->m_iSequence == 0;
		g_ServerListManager->SetSelectServerInfo(
			m_pSelectServerGroup->m_szName, qaServer->m_iIndex,
			censorship, qaServer->m_byNonPvP, testServer);
		rise::slayerqa::AppendRuntimeQALog("server-auto-select server=selected");
		return true;
	}
	return false;
}
