...
{   
    ...
    if (IsRelease(VK_LBUTTON))
    {
        ...
        if (iIconIndex != -1 && iIconIndex < MAX_SKILLS_SLOT)
        {
            ...
            if (g_pNewUIMuHelperSkillList)       // them check null
            {
...
void SEASON3B::CNewUIMuHelper::RenderBtnList()
{
    ...
    for (; li != m_ButtonList.end(); li++)
    {
        ...
        if (gCharacterManager.GetBaseClass(Hero->Class) < MAX_CLASS &&
            (cBTN->class_character[gCharacterManager.GetBaseClass(Hero->Class)]) && (cBTN->iNumTab == m_iCurrentOpenTab || cBTN->iNumTab == -1))
        {
            cBTN->btn->Render();
        }
    }
...
void SEASON3B::CNewUIMuHelper::RenderBoxList()
{
    ...
    for (; li != m_CheckBoxList.end(); li++)
    {
        ...
        if (gCharacterManager.GetBaseClass(Hero->Class) < MAX_CLASS &&
            (cBOX->class_character[gCharacterManager.GetBaseClass(Hero->Class)]) && (cBOX->iNumTab == m_iCurrentOpenTab || cBOX->iNumTab == -1))
        {
            cBOX->box->Render();
        }
    }
...
void SEASON3B::CNewUIMuHelper::RenderIconList()
{
    ...
    for (; li != m_IconList.end(); li++)
    {
        ...
        if (gCharacterManager.GetBaseClass(Hero->Class) < MAX_CLASS &&
            (cImage->class_character[gCharacterManager.GetBaseClass(Hero->Class)]) && (cImage->iNumTab == m_iCurrentOpenTab || cImage->iNumTab == -1))
        {
