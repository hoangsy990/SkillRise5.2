...
#define	REQUEST_CHARACTERS_LIST_HACK	        72
#define	REQUEST_LOGIN_CHARTEXT					73
...
typedef struct {
	...
	DWORD			ViewCurHP;
	DWORD			ViewCurSD;
	__int64			ViewDamageHP;
	__int64			ViewDamageSD;
...
// Grow Lancer positional echo keeps PRECEIVE_MAGIC_CONTINUE as an exact
// prefix and appends the S21-selected target.  Legacy 0x1E packets are not
// widened and continue to use PRECEIVE_MAGIC_CONTINUE unchanged.
typedef struct {
	PBMSG_HEADER  Header;
	BYTE          MagicH;
	BYTE          MagicL;
	...
	BYTE          TargetKeyH;
	BYTE          TargetKeyL;
} PRECEIVE_GROW_LANCER_MAGIC_CONTINUE, * LPPRECEIVE_GROW_LANCER_MAGIC_CONTINUE;
static_assert(sizeof(PRECEIVE_GROW_LANCER_MAGIC_CONTINUE) ==
	sizeof(PRECEIVE_MAGIC_CONTINUE) + 2,
	"Grow Lancer duration receive must preserve the native C1:1E prefix");
...
typedef struct 
{
	...
	DWORD ViewMaxBP;
	DWORD ViewMaxSD;
	__int64 ViewMasterExperience;
	__int64 ViewMasterNextExperience;
} 
...
struct PMSG_ADD_BANK_REQ
{
	PSBMSG_HEAD h;
	int	Number;
};
struct PMSG_EXT_INVENTORY_REQ
{
	PSBMSG_HEAD h;
	int Slot;
};
...
typedef struct
{
	...
	int type;
	int count;
}JEWELBANKWITHDRAW_SEND, LP_WITHDRAW_SEND;

typedef struct
{
	PSBMSG_HEAD header;
	int slot;
}JEWELBANK_SEND, LPRISE_BANK_SEND;

typedef struct
{
	int ItemCount[MAX_ITEM_SLOT];
	int ItemIndex[MAX_ITEM_SLOT];
	BYTE Other[3];
}JEWELBANK_DATA, LPRISE_SAVE_BANK;
...
typedef struct
{
	...
	int ItemCount[MAX_ITEM_SLOT];
	BYTE Other[3];
}JEWELBANK_RECV, LPRISE_BANK_RECV;
...
struct PKT_VIEW_BY_NAME
{
	PSBMSG_HEAD header;
	char Name[10];
};

struct SolvedPacketCount
{
	PSBMSG_HEAD header;
	DWORD ThaoTac;
	DWORD SoLan;
...
struct gPacketListItemClient
{
	...
	int WPYC = 0;
	int GPYC = 0;
	std::vector<gPacketListItemLocal> ListItemVongQuay;
...
struct gPacketListItemInfoClient
{
	int SpinIndex = 0;
	char SpinName[90]{};
	std::vector<ListVongQuaySend> LoadListSpin;

	void Clear()
	{
		SpinIndex = 0;
		memset(SpinName, 0, sizeof(SpinName));
		LoadListSpin.clear();
	}
};
...
struct DataSendClientResetChange
{
	PSBMSG_HEAD h;
	int ReqResetChange[MAX_RESET_CHANGE];
	int ReqResetCoin[MAX_RESET_CHANGE];
	int ReqResetUpPoint[MAX_RESET_CHANGE];
	int ReqItem[MAX_RESET_CHANGE];
	int ReqItemLevel[MAX_RESET_CHANGE];
};

struct SaveResetChange
{
	int ReqResetChange[MAX_RESET_CHANGE];
	int ReqResetCoin[MAX_RESET_CHANGE];
	int ReqResetUpPoint[MAX_RESET_CHANGE];
...
struct GET_DATA_CHANGE_CLASS
{
	PSBMSG_HEAD header;
	int Other[2];
};
struct GET_DATA_CHANGE_CLASS_SAVE
{
	int Other[2];
};
...
struct EVENTTIME_MONSTER_DATA
{
	int index;
	int count;
};

struct CustmEventTimeData
{
	int index;
	int time;
	int Map;
	...
	int monster_count;
	int monster_index;
	int monster_type_count;
	EVENTTIME_MONSTER_DATA monsters[10];
	BYTE Status;
};

struct CustmEventTimeReceived
{
	PSWMSG_HEAD h;
	int count;
	BYTE RegLineEvent;
...
struct UI_CONTRIB_ITEM
{
	ITEM* pItem;
	int			Points;
};

struct UI_STORE_ITEM
{
	ITEM* pItem;
	int	ClanTier;
	int	Points;
...
struct CLAN_RANK_TABLE
{
	BYTE total;
	CLAN_RANK_NODE Entries[50];
};
...
struct CLAN_MEMBER_TABLE
{
	BYTE total;
	int SumPoints;
	int ThreshPoints;
	short Tier;
	CLAN_MEMBER_NODE Members[80];
};
struct PKT_CLAN_REQ
{
	PSBMSG_HEAD hdr;
...
struct PKT_CLAN_SELF_ANS {
	PSWMSG_HEAD hdr;
	int			Tier;
	int			Points;
};
struct PKT_CLAN_GROUP_ANS {
	PSWMSG_HEAD hdr;
	int			total;
	int			SumPoints;
	int			ThreshPoints;
...
struct PMSG_EXTEND_SHOP_BUY
{
	PSBMSG_HEAD header;
	int IndexInList;
};

struct EXTEND_SHOP_ITEM
{
	short Index;
	BYTE Level;
	BYTE Skill;
...
struct PMSG_EXTEND_SHOP_EXCHANGE
{
	PSBMSG_HEAD header;
	int Amount;
};

struct CG_SET_COLOR_NAME_REQ
{
	PSBMSG_HEAD header;
	DWORD ColorPacked;
};

struct CG_RENAME_CHAR_REQ
{
	PSBMSG_HEAD header;
	char NewName[11];
};
...
#pragma pack(1)
struct PMSG_NAME_COLOR_INFO_RECV
{
	PSBMSG_HEAD header;
	BYTE CoinType;
	int CoinValue;
