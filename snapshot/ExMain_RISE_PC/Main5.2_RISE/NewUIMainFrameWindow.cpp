...
void SEASON3B::CNewUISkillList::LoadImages()
{
	...
	LoadBitmapFile("RISE\\GrowLancer\\Interface\\newui_skill00.jpg",
		rise::growlancer::kSkillIconAtlasBitmap, GL_LINEAR);
	LoadBitmapFile("RISE\\GrowLancer\\Interface\\newui_skill00_Non_Icon.jpg",
		rise::growlancer::kDisabledSkillIconAtlasBitmap, GL_LINEAR);
...
void SEASON3B::CNewUISkillList::UnloadImages()
{
	...
	DeleteBitmap(rise::growlancer::kSkillIconAtlasBitmap);
	DeleteBitmap(rise::growlancer::kDisabledSkillIconAtlasBitmap);
...
bool SEASON3B::CNewUISkillList::Render()
{
	...
	if (rise::growlancer::IsRuntimeQAIconPanelVisible())
	{
		const float iconWidth = 20.f;
		const float iconHeight = 28.f;
		const float gap = 2.f;
		const float rowWidth = 9.f * iconWidth + 8.f * gap;
		float centerX = 320.f;
		float baseY = 390.f;
		g_RisePegasusHud.GetSkillListAnchor(centerX, baseY);
		const float startX = centerX - rowWidth * 0.5f;
		for (int icon = 0; icon < 9; ++icon)
		{
			...
			const float v = (skillId / rise::growlancer::kSkillIconAtlasColumns) *
...
void SEASON3B::CNewUISkillList::RenderSkillIcon(int iIndex, float x, float y, float width, float height, int TypeMuHelper, bool bShowHotKey)
{
	...
	else if (bySkillType >= rise::growlancer::kSpinStepSkill &&
		bySkillType <= rise::growlancer::kBrecheSkill)
	{
		// S21 newui_skill00 is a direct-ID atlas: 25 columns of 20x28
		// cells in a 512x512 image. Magic_Icon for 271..279 is verified
		// equal to the client skill ID in the decoded 120-byte records.
		fU = (Skill_Icon % rise::growlancer::kSkillIconAtlasColumns) *
			rise::growlancer::kSkillIconWidth /
			static_cast<float>(rise::growlancer::kSkillIconAtlasSize);
		fV = (Skill_Icon / rise::growlancer::kSkillIconAtlasColumns) *
			rise::growlancer::kSkillIconHeight /
			static_cast<float>(rise::growlancer::kSkillIconAtlasSize);
		iKindofSkill = KOS_GROW_LANCER;
		skillAtlasSize = static_cast<float>(rise::growlancer::kSkillIconAtlasSize);
	}
	...
	break;
	case KOS_GROW_LANCER:
	{
		iSkillIndex = bCantSkill
			? rise::growlancer::kDisabledSkillIconAtlasBitmap
			: rise::growlancer::kSkillIconAtlasBitmap;
	}
	break;

	}
	if (bCantSkill == true && iKindofSkill != KOS_GROW_LANCER)
	{
		iSkillIndex += 6;
	}
	...
	if (iSkillIndex != 0)
	{
		...
		{
			RenderBitmap(iSkillIndex, x, y, width, height, fU, fV,
				uvW / skillAtlasSize, uvH / skillAtlasSize);
		}
	}
