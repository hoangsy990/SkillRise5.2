...
void CUIGuildInfo::RenderGuildInfoTab()
{
	...
	g_pRenderText->RenderText(ptOrigin.x, ptOrigin.y, szTemp, 140 * g_SetForm->x_fScreenRate_x, 0, RT3_SORT_LEFT);
	ptOrigin.y += 13;
	...
	ptOrigin.x -= 5;
	ptOrigin.y += 33;
	g_pRenderText->RenderText(ptOrigin.x + 5, ptOrigin.y, GlobalText[1323], 136 * g_SetForm->x_fScreenRate_x, 0, RT3_SORT_LEFT);
...
void CUIGuildInfo::Render()
{
	...
	if (Hero->GuildStatus != G_NONE)
	{
		...
		sprintf(szTemp, GlobalText[3176], GuildMark[Hero->GuildMarkIndex].GuildName, pKit->QN(GuildTotalScore));
		g_pRenderText->RenderText(ptOrigin.x, ptOrigin.y, szTemp, 120, 0, RT3_SORT_CENTER);
	}
...
bool CheckUseMasterSkill(CHARACTER* c, int Index)
{
	...
	bool bUse = false;
	if (Class >= MAX_CLASS)
		return false; // fixed SS6 RequireClass[7], no GL guild-skill alias
