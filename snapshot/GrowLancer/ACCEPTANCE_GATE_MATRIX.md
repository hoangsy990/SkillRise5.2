...
2026-09-16 Clash hidden native GPU fixture: PID17292 loaded/issued 16 front/rear
mesh draws with GL0/stable release; front positive 8/8, rear positive 6/8 but
zero pixels at 0°/180° synthetic angles. `verify_clash_native_fixture_draw.py`
passes exact-row integrity only. Clash GPU/owner visual gate remains OPEN;
source S21 cull/pose and actual gameplay-camera pixels are still required.
QA-only differential PID9060 with face cull disabled before mesh queue flush
produced the same rear0°/180° zeros; its `cull` row does not prove raster
state. Native RISE texture+bright source explicitly disables cull via
`EnableAlphaBlend`, independently pinned by `verify_clash_native_blend_cull.py`.
Native bone projection PID972 showed rear doubled area<0.1 exactly at 0°/180°
and >100000 at the six positive-pixel angles. Isolated hidden fixture PID972
Exit0, full draw/projection/GL0/release verifier PASS. This closes only the
synthetic native geometry contract, not S21 camera/owner Clash visual gate or
GS authority. No normal renderer behavior changed.
S21 rear `5FC` call argument chain now pins caster+0x164 angle pointer,
computed position, owner and target index+0x364; native private rear call
maps these to named owner.Angle/position/target. `verify_clash_rear_angle_origin.py`
PASS and native texture2|bright40=0x42; S21 actual draw-state/camera and
owner siege pixels remain OPEN.
Ordinary fixed-action preparation now requires the owning live player
`CHARACTER` to have exact unevolved class byte7, resolved by Hero/object
identity or remote CharactersClient/object identity. Static guard and isolated
normal/QA Win32 links PASS; explicit DK RuntimeQA preview remains unchanged.
This closes a wrong-class clip-attachment boundary only, not class7 creation,
dynamic Magic rate, server authority or owner visual acceptance.
Native class7 speed boundary: `verify_class7_join_speed_chain.py` pins final
GS speed -> join DWORD -> client named fields -> second `SetCharacterClass`
after `Hero=c` -> `CalculateAll/CalculateAttackSpeed`. With no class7 base
setter, additive modifiers can reuse already-final speed. This is a source
integration gap, not validated S21 rate or runtime speed PASS; class7
ordinary activation stays fail-closed.
`verify_class7_speed_packet_capacity.py` separately pins native F3:ED
speed-divisor payload/client lookup exactly14 bytes for classes0..6, with no
class7 pair14/15 or GS config; this is not final-speed join or skill-slot
capacity. No speculative packet expansion/DEX divisor was staged. Class7
stat-update/rate/wire ownership stays OPEN.
S21 literal global-stat speed xrefs: hash-pinned 17 near-speed candidates were
decoded in bounded blocks. Receiver82/sub04 writes two WORDs; selected UI,
cast-speed and model-scalar paths read them. This does not close indirect
writers or original GS sender/rate; `verify_s21_speed_pointer_candidate_roles.py`
is a source classification PASS only.
S21 head82/sub04 boundary: C1 subtype byte+3/C2 byte+4 select the same raw
buffer receiver, which always reads WORDs at+4/+8; selected branch has no
local packet-length compare. Actual C2 sub04 use/upstream validation and
initial sender remain OPEN; `verify_s21_speed_packet_offset_boundary.py`.
...
| Grow Lancer class mapping | OPEN / boundary audited | Owner chose native GL base7/DB112; source contract/class-data verifiers pin S21 Lua/rate keys and class wire algebra. `CalcCharacter.ini` explicitly delegates extra defaults to unsupplied original S21 `MuOnline.dbo.DefaultClassType`; local QA class112 row is not S21 authority. `CLASS_CONSUMER_CAPACITY_AUDIT.md` maps remaining create/login/packet/stat/skill consumers. New `verify_class7_speed_recalculation_gap.py` proves native client and GS omit class7 base-speed assignment before additive modifiers; S21 Lua DEX/20 branch is qualified anti-hack source, not client action-speed proof. `verify_s21_speed_temp_packet.py` further separates C2:AE stack-temp same-offset serialization from the actual GLOBAL receiver82/sub04 speed writer. Equipment and quests1–3 are excluded, not base learning/cast. | Original S21 default row plus native class7 create/login/persistence/packet, source-backed speed rate/rounding/global stat ownership and rate/skill authority before activation; no RF/DK fallback or speculative DEX/20 client mapping |
...
| Spin Step full visual chain | NATIVE SAMPLER/CROSS/POSE/FLAT-DEPTH FIXTURE PIXELS/CLEANUP PASS / OWNER VISUAL FAIL | S21 motion-blur NEAREST/CLAMP and private slot32986 GPU isolation PASS. Hash-pinned S21 `spin_cross` v0F/private v0C equal plaintext; current hidden cross PID3532/Magic PID4448 regressions PASS. Native merged-player action285/bone33 pose ribbon PID17052 has nonzero minWidth181.108, 114 fixture pixels, GL0/stable release; synthetic style1 blur PID22320 6,379 fixture pixels. New depth-writing Z0/-55° hidden control PID15520 has groundDepth0.501395, 103 textured blur pixels with/without plane, GL0/releaseStable1; an earlier untextured 115-pixel false-positive was rejected and fixture native state corrected. Depth-off pose PID12572 still gives 114 pixels. This excludes whole-ribbon occlusion by a flat plane under that synthetic pose, not gameplay terrain. `verify_spin_blur_world_transform.py` pins S21 translated endpoint/rotated-offset map to native BMD API. `verify_spin_caster_bone_33.py` pins source/native `knife_gdf` bone33 and byte-identical action186→285 clip; raw header padding is noncanonical. Older target-bound PID28160 had 20 stock-texture32019 blur draws/19 cross submits but owner saw only ring. New historical-cadence verifier pins action285 speed0.706, frame4.706 stage0 at250ms then frame5.001 stage1 at266ms, before action exit: old fanout gate was reached, not current gameplay pixel proof. Current QA-only cross gameplay query built, no new F7 row. Source receiver and pinned S21 frame41 confirm **base271 cast plus separate internal280 target contact**; QA F7/F9 are distinct, two-message verifier PASS. | Fresh local-only target-bound F7 **then F9 once on the same target**, F11: root ribbon/cross query/world/depth and separate contact fixed/360 owner pixels, probe ON/OFF, repeat/map and S21 same-frame comparison. F9 fixture is not GS hit authority; fixture raster is not caster/gameplay visual PASS. |
| Circle Shield full visual chain | RECEIVE ROOT-BEFORE-SETTER STATIC/BUILD PASS / NATIVE BITMAP GPU/ALLOCATOR QA BUILT / OWNER VISUAL OPEN | `verify_circle_receive_action_order.py` pins S21 local send-only and receive controller5DD BEFORE primary187/conditional316 setter; private receive primary286 now matches order, QA/nonQA isolated Win32 links exit0, auxiliary316 unmapped. S21 buff216/221/222 branch `13ECB52..13ECFBC` proves five flare01 bones and four `firehik_mono01` subtype12 particles, two per UpperArm26/35. Source/emission verifiers PASS. Historical private QA Engine EEF727AE... hash-staged with E3EC7999... rollback and full Data verifier PASS; hidden PID10688 exit0 shows 64x64 native mono CPU/GPU equality, GL error0 and stable release. QA-only bounded owner F7/F11 records distinguish ready/bones and 0..4 successful native particle indices plus FPS/distance/options; F11/map snapshots count `circleMono`/`circleContact`. | Fresh local-only Circle cast for actual emission result, buff receipt, paired shoulder pixels, cadence, subtype12 pool/repeat/map cleanup and matching S21 frames. F7 receive-path preview is not S21 local-root evidence; GPU/allocator wiring does not prove owner visibility; separate buff-contact receiver NOT wired. |
| Obsidian full visual chain | PRIMARY ACTION TRANSITION/ASSETS STATIC PASS / OWNER QA OPEN | `OBSIDIAN_REVERSE.md`, 12 shared-helper cases and pinned renderer verifier. Source base273 primary191/alternate317 enters preserving setter; corrected private191→290 to native `SetAction(true)` rather than unconditional current/prior-frame reset. `verify_obsidian_action_reset.py`, source alternate and renderer checks PASS; isolated QA/nonQA x86 links PASS, current private QA SHA0E16EA70... stage/hash PASS. Hidden shader PID24040 uploaded/released both Obsidian BMDs among18 visible types, but no Obsidian cast pixels. | Verify conditional317 native auxiliary state/clip (do not alias to290), then F7 aura/rotation/Thunder8 hand placement/ownership/lifetime, party buff receive and real pool/GPU/owner acceptance. |
| Magic Pin full visual chain | NATIVE FIXTURE/Calc_RenderObject/FLAT-DEPTH RASTER PASS / ORDINARY ACTIVATION OPEN / OWNER VISUAL FAIL | `MAGIC_PIN_REVERSE.md`; PID28160 stage0 skipped, PID36328 controlled QA reached two stage gates/all four caster BMD submits but owner still saw no foot triangle. Corrected package upload gives native VAOs; direct PID24324 and actual `Calc_RenderObject(false)` PID10200 fragments PASS. New private flat-depth PID3596 compares 16 Magic01/Root source-scale0.7 native draws, real depth-writing synthetic Z0 plane (readback1.0→0.554251 at a covered ground pixel) and positive samples in both passes/GL0/release stable1. This rules out entire model below a flat fixture plane, not real terrain/owner pixels. `verify_magic_normal_activation_gap.py` confirms ordinary non-QA root/action returns false and 40ms actor/quanta stepping is QA-only, so the current owner-tested path is not a deployable ordinary cast. | Verify class7 actor/stat/clip/bonus and ordinary skill-local clock before activation without DK QA borrowing; then actual owner foot/root GPU samples/world Z/angle/terrain depth/cull and fixed/360 gameplay pixels. Intermittent narrow-window creation, third stage/subtype1 and internal281 target explosion remain unaccepted; .706 speed can conditionally skip third gate even with whole quanta. Fixture raster cannot close visual FAIL or GS hit authority. |
| Clash full visual chain | ACTION SETTER STATIC/BUILD PASS / BYTE-LEVEL MOVE/VECTOR PURE X86 PASS / OWNER VISUAL OPEN | `CLASH_REVERSE.md`, `verify_clash_action_setter.py` pins S21 local target validation→action194 setter→controller5FA and receive setter→controller, mapped to private293 `SetAction(true)`; isolated QA/nonQA Win32 links exit0. `verify_clash_movement_payload.py` plus x86 W4/WX pure test pin terrain0x1C and dormant C1:59 mode1+six byte payload. `verify_clash_vector_and_followup.py` pins source length/normalize/scale/round and native dormant `GrowLancerClashDestination.h`; focused x86 W4/WX zero/cardinal/diagonal/negative/edge test passes. D6DC05 is SECOND C1:31 coordinate SEND, not local position update; both S21 opcodes collide with other native 5.2 meanings and remain disabled. Historical PID23952 selected-target QA had 871 native mesh submits and only alpha-zero skips | Stationary/moving target pixels, facing/push presentation, real class7 caster/cursor tile ownership, authoritative siege GS handler/rollback/native protocol and GPU/cache cleanup; pure source math is not gameplay movement or owner visual PASS |
| Harsh Strike full visual chain | ACTION SETTER STATIC/BUILD PASS / OWNER VISUAL OPEN | `HARSH_STRIKE_REVERSE.md`, native implementation; `verify_harsh_action_setter.py` pins both S21 local/receive action185 setter calls before controller5E0 and native private action284 `SetAction(true)`; QA/nonQA isolated Win32 links exit0 | Bone 8/5 blur, delayed child timing, actual owner pixels and cleanup acceptance; build is not visual parity |
| Shining Peak full visual chain | ACTION SETTER STATIC/BUILD PASS / OWNER QA OPEN | `SHINING_PEAK_REVERSE.md`, `verify_pin_joint_contract.py`, `verify_pin_seed_texture.py`, `verify_shining_action_setter.py`; both S21 local/receive action189 setter calls precede roots5F6/5F3, mapped to private native `SetAction(288,true)`; isolated QA/nonQA Win32 links exit0. Subtype2/3/4, common seed geometry/history and selected renderer audited. Hash-pinned sweep identified 65 canonical direct `+A44` operands but none in selected body, complete renderer or called tail-shift; nearby reader owner-model0xAE4 gated. Selected update/tail have no indirect calls; all 48 renderer indirect calls resolve through mapped OpenGL IAT (`map_s21_joint_renderer_imports.py`), not a joint vtable. | External global pool/registration callbacks may still read `+A44`; mixed-pool seed pixels, four frame gates, particle/model parity and cleanup remain OPEN. Numeric xrefs/local call audit or build are not gameplay parity. |
| Wrath full visual chain | PRIMARY ACTION SETTER STATIC/BUILD PASS / NATIVE MODEL GPU UPLOAD / GROUND PROBE BUILT / OWNER VISUAL OPEN | `WRATH_REVERSE.md`: `verify_wrath_action_selection.py` pins S21 local/receive conditional321/primary192 setter before controller5EA and native primary291 `SetAction(true)`; isolated QA/nonQA x86 links exit0, auxiliary321 remains unmapped. Source-proven ONE/ONE ground draw restores native blend; hash-pinned edge/material verifiers PASS. `wratha01` sole `alpha_line.jpg`, `wratha02` sole `dust01.tga`; corrected native package BMD VAOs/GPU and release hidden PASS. Old PID28160 ground recorded ONE/ONE/texture3207 but no material match/pixels. Current QA-only four-draw/five-point native ground pixel/fragment query and expected `brocken` texture comparison built/staged FB4C8E42...; source-bound verifier/Data stage PASS, no new owner F7. | Auxiliary321 predicate/clip, actual Wrath F7 owner square frame, bound-material match/ground fragment+pixel delta to attribute layer, buff recurrence/removal, pool reuse and S21 matching frames OPEN. Probe compilation and hidden model GPU upload are not visual parity or GS buff proof. |
