...
| Clash full visual chain | ACTION SETTER STATIC/BUILD PASS / OWNER VISUAL OPEN | `CLASH_REVERSE.md`, `verify_clash_action_setter.py` pins S21 local target validation→action194 setter→controller5FA and receive setter→controller, mapped to private293 `SetAction(true)`; isolated QA/nonQA Win32 links exit0. 12 tick+6 pose tests, selected renderer/model-pipeline verifier; historical PID23952 selected-target QA had 871 native mesh-submit successes and only alpha-zero calc skips | Stationary/moving target pixels, facing/push presentation, source tile movement packet, GPU/cache reuse and cleanup; no siege GameServer PASS inferred |
