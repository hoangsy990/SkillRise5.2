...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial)
{
	...
	case 0x19:
		...
		break;
	case rise::slayer::kPierceFanoutHead:
		if (size == sizeof(rise::slayer::PierceLaneRequestWire) &&
			lpMsg[0] == 0xC1 && lpMsg[1] == size &&
			lpMsg[3] == rise::slayer::kPierceLaneRequestSub)
			gSkillManager.CGSlayerPierceLaneRecv(lpMsg, size, aIndex);
		break;
	case 0x1B:
