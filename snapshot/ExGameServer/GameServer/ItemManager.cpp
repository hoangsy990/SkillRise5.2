...
#include <ChecklevelVip.h>
CItemManager gItemManager;
ITEM_ATTRIBUTE ItemAttribute[MAX_ITEM];
int g_MaxItemIndexOfEachItemType[MAX_ITEM_TYPE];

static void LogNpcShopSale(LPOBJ lpObj, CItem* lpItem, int slot, const char* result,
	const char* currency, DWORD value, DWORD zenBefore, DWORD zenAfter, DWORD zenCredited, DWORD zenDiscarded)
{
	gLog.Output(LOG_SELL_SHOP,
		"[NPCShopSell][%s] Account=[%s] Character=[%s] DBNumber=%d Shop=%d Map=%s(%d) Pos=%d/%d Slot=%d "
		"Item=%s Index=%d Section=%d Type=%d Serial=%08X Level=%d Durability=%.2f Skill=%d Luck=%d Option=%d "
		"Excellent=%u Set=%u Harmony=%u ItemOptionEx=%u SocketBonus=%u Sockets=[%u,%u,%u,%u,%u] "
		"PetLevel=%d PetExp=%d Periodic=%d PeriodicTime=%d Currency=%s Value=%u ZenBefore=%u ZenAfter=%u "
		"ZenCredited=%u ZenDiscarded=%u LockState=%d",
		result, lpObj->Account, lpObj->Name, lpObj->DBNumber, lpObj->TargetShopNumber,
		MapName[lpObj->Map], lpObj->Map, lpObj->X, lpObj->Y, slot,
		gItemManager.GetItemName(lpItem->m_Index), lpItem->m_Index,
		(lpItem->m_Index / MAX_ITEM_TYPE), (lpItem->m_Index % MAX_ITEM_TYPE), lpItem->m_Serial,
		lpItem->m_Level, lpItem->m_Durability, lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3,
		lpItem->m_NewOption, lpItem->m_SetOption, lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx,
		lpItem->m_SocketOptionBonus, lpItem->m_SocketOption[0], lpItem->m_SocketOption[1],
		lpItem->m_SocketOption[2], lpItem->m_SocketOption[3], lpItem->m_SocketOption[4],
		lpItem->m_PetItemLevel, lpItem->m_PetItemExp, lpItem->m_IsPeriodicItem, lpItem->m_PeriodicItemTime,
		currency, value, zenBefore, zenAfter, zenCredited, zenDiscarded, lpObj->Lock);
}
...
int CItemManager::GetItemDurability(int index, int level, int NewOption, int SetOption)
{
	...
	if (index == GET_ITEM(12, 144) || index == GET_ITEM(12, 146) || gPentagramSystem.IsPentagramItem(index) != 0)
	{
		return ItemInfo.Durability;
	}
	if (index == GET_ITEM(14, 21) || index == GET_ITEM(14, 29) || index == GET_ITEM(14, 100) || index == GET_ITEM(14, 215))
	{
		return 1;
	}
	if (index == GET_ITEM(13, 18) || index == GET_ITEM(13, 29) || index == GET_ITEM(13, 51) || index == GET_ITEM(14, 19) || index == GET_ITEM(14, 102) || index == GET_ITEM(14, 109) || index == GET_ITEM(14, 110))
	{
		return 1;
	}

	// RISE: Dong bo giong voi ban chuan - check MaxStack
	int MaxStack = gItemStack.GetItemMaxStack(index, level);
	if (MaxStack > 0)
	{
		if (index == GET_ITEM(4, 7) || index == GET_ITEM(4, 15)) // Arrow / Bolt
		{
			return MaxStack;
		}
		else
	...
	if (index != GET_ITEM(0, 19) && index != GET_ITEM(2, 13) && index != GET_ITEM(4, 18) && index != GET_ITEM(5, 10) && index != GET_ITEM(5, 36) && ItemInfo.Slot != 7)
	{
		if (SetOption != 0)
		{
			dur += 20;
...
bool CItemManager::CheckItemMoveToInventory(LPOBJ lpObj, CItem* lpItem, int slot)
{
	...
	int GetVip = gCheckItemVip.CheckLevelVIP(lpItem->m_Index, lpItem->m_Level);
	if (GetVip > lpObj->rCharOption)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(782), GetVip);
		return 0;
	}
	...
	{
		if (lpItem->m_Index == GET_ITEM(13, 2) || lpItem->m_Index == GET_ITEM(13, 3))
		{
			return 0;
		}
	}
	...
	{
		if (lpItem->m_Index == GET_ITEM(13, 10) || (lpItem->m_Index >= GET_ITEM(13, 39) && lpItem->m_Index <= GET_ITEM(13, 42)) || lpItem->m_Index == GET_ITEM(13, 68) || lpItem->m_Index == GET_ITEM(13, 76) || lpItem->m_Index == GET_ITEM(13, 122))
		{
			return 0;
		}
	}
	if (lpItem->m_Index == GET_ITEM(13, 10) || (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 0) || (lpItem->m_Index >= GET_ITEM(13, 38) && lpItem->m_Index <= GET_ITEM(13, 42)) || lpItem->m_Index == GET_ITEM(13, 68) || lpItem->m_Index == GET_ITEM(13, 76) || lpItem->m_Index == GET_ITEM(13, 107) || lpItem->m_Index == GET_ITEM(13, 122))
	{
...
bool CItemManager::CheckItemMoveToTrade(LPOBJ lpObj, CItem* lpItem, BYTE TargetFlag)
{
	...
	if (lpItem->IsPentagramItem() != 0) {
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 2, 0, pMessage->Text(499));
			return 0;
		}
	}
...
bool CItemManager::InventoryBreakItemStack(LPOBJ lpObj, CItem* lpItem, int slot)
{
	...
	if (lpItem->m_Durability > 1)
	{
		if (gItemStack.IsBreakUp(lpItem->m_Index, lpItem->m_Level) != 0)
		{
			...
			{
				this->DecreaseItemDur(lpObj, slot, 1);
...
bool CItemManager::InventoryAddItemStack(LPOBJ lpObj, int SourceSlot, int TargetSlot)
{
	...
	// RISE: Dam bao item stack luon co it nhat dur = 1 de phep cong stack ko bi loi (do BaseDurability = 0)
	if (lpObj->Inventory[SourceSlot].m_Durability < 1)
	{
		lpObj->Inventory[SourceSlot].m_Durability = 1;
	}
	if (lpObj->Inventory[TargetSlot].m_Durability < 1)
	{
		lpObj->Inventory[TargetSlot].m_Durability = 1;
	}
...
void CItemManager::ItemByteConvert(BYTE* lpMsg, CItem item)
{
	...
	#else
	if (item.IsSocketItem() == 0 && item.IsPentagramItem() == 0 && item.IsPentagramJewel() == 0 && item.IsPentagramMithril() == 0)
	{
		lpMsg[6] = item.m_JewelOfHarmonyOption;
	}
	else
...
void CItemManager::DBItemByteConvert(BYTE* lpMsg, CItem* lpItem)
{
	...
	#else
	if (lpItem->IsSocketItem() == 0 && lpItem->IsPentagramItem() == 0 && lpItem->IsPentagramJewel() == 0 && lpItem->IsPentagramMithril() == 0)
	{
		lpMsg[10] = lpItem->m_JewelOfHarmonyOption;
	}
	else
...
int CItemManager::RepairItem(LPOBJ lpObj, CItem* lpItem, int slot, int type)
{
	...
	if (lpItem->m_Index >= GET_ITEM(12, 130) && lpItem->m_Index <= GET_ITEM(12, 135))
	{
		return 0;
	}
	...
	if (lpItem->m_Index == GET_ITEM(13, 20) && (lpItem->m_Level >= 0 && lpItem->m_Level <= 2))
	{
		return 0;
	}
	if (lpItem->m_Index == GET_ITEM(13, 10) || (lpItem->m_Index >= GET_ITEM(13, 38) && lpItem->m_Index <= GET_ITEM(13, 42)) || lpItem->m_Index == GET_ITEM(13, 68) || lpItem->m_Index == GET_ITEM(13, 76) || lpItem->m_Index == GET_ITEM(13, 122))
	{
		return 0;
	}
...
BYTE CItemManager::MoveItemToInventoryFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag)
{
	...
	int bStackRet = this->InventoryAddItemStack(lpObj, SourceSlot, TargetSlot);
	if (bStackRet == 1)
	{
		return TargetSlot;
	}
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg, int aIndex)
{
	...
	if ((lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 0) || lpItem->m_Index == GET_ITEM(13, 38))
	{
	...
	GCPickItemInfoSend(aIndex, &item);
...
void CItemManager::GCPickItemInfoSend(int aIndex, CItem* lpItem)
{
	LPOBJ lpObj = &gObj[aIndex];
	gLog.Output(
		LOG_SELF_DROP,
		"[Da Nhat Item][%s/%s] Type:%d, Item:%s, Level:%d, %d, %d, %d, %d, %d, %03d, %03d, %03d, %03d, %03d, %03d, %03d, Serial:%08X, Map:%s, (%d,%d)",
		lpObj->Account,
		lpObj->Name,
		lpItem->m_Index,
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex)
{
	...
	if (INVENTORY_WEAR_RANGE(lpMsg->slot) != 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Do dang mac tren nguoi, ban khong the vut. Hay thao vao hom do truoc.");
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	if (lpObj->DieRegen != 0)
	...
	if (gBloodCastle.OpenRewardBox(lpObj,lpItem) != 0 || gDevilSquare.OpenRewardBox(lpObj,lpItem) != 0 || gItemBagManager.DropItemByItemIndex(lpItem->m_Index, lpItem->m_Level, lpObj, lpObj->Map, lpMsg->x, lpMsg->y) != 0)
	{
		if (lpItem->IsItem())
		{
			gLog.Output(
				LOG_SELF_DROP,
				"[Nem Item Ra Dat][%s/%s] Type: %d, Item: %s +%d, %d, %d, %d, %d, %d, %03d, %03d, %03d, %03d, %03d, %03d, %03d, Serial: %08X | Map:%s (%d,%d)",
				lpObj->Account,
				lpObj->Name,
				lpItem->m_Index,
				gItemManager.GetItemName(lpItem->m_Index),
		...
		int maxStack = gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level);
		if (maxStack > 0 && lpItem->m_Durability > 1.0f)
		{
			lpItem->m_Durability -= 1.0f;
			pMsg.result = 2;
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			this->GCItemDurSend(lpObj->Index, lpMsg->slot, (BYTE)lpItem->m_Durability, 0);
			return;
		}
		else
		{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 7) && (lpItem->m_Level == 0 || lpItem->m_Level == 1))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 11) && lpItem->m_Level == 1)
	{
		#if(GAMESERVER_TYPE==1)
	...
	else if (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 1)
	{
		if (lpObj->Level < 40)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(272));
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
		int maxStack = gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level);
	...
	else if (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 2)
	{
		if (lpObj->Level < 80)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(274));
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
		int maxStack = gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level);
	...
	else if (lpItem->m_Index == GET_ITEM(14, 28))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 63))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 99))
	{
		GCFireworksSend(lpObj, lpMsg->x, lpMsg->y);
		int maxStack = gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level);
		if (maxStack > 0 && lpItem->m_Durability > 1.0f)
...
bool CItemManager::CGPkDrop(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex)
{
	...
	if (lpItem->m_Index == GET_ITEM(13, 7) && (lpItem->m_Level == 0 || lpItem->m_Level == 1))
	{
		return  0;
	}
	else if (lpItem->m_Index == GET_ITEM(13, 11) && lpItem->m_Level == 1)
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg, int aIndex)
{
	...
	if (lpMsg->SourceFlag == 1 || lpMsg->TargetFlag == 1)
	{
	...
	if (lpMsg->SourceFlag == 2 || lpMsg->TargetFlag == 2)
	{
	...
	if (lpMsg->SourceFlag == 3 || (lpMsg->SourceFlag >= 6 && lpMsg->SourceFlag <= 20) || lpMsg->TargetFlag == 3 || (lpMsg->TargetFlag >= 6 && lpMsg->TargetFlag <= 20))
	{
	...
	if (lpMsg->SourceFlag == 4 || lpMsg->TargetFlag == 4)
	{
	...
	if (lpMsg->SourceFlag == 5 || lpMsg->TargetFlag == 5)
	{
	...
	if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 0)
	{
		if ((pMsg.result = this->MoveItemToInventoryFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			...
			for (std::vector<CustomStartSetItemDame_Data>::iterator it = gCustomStartSetItemDame.itemdata.begin(); it != gCustomStartSetItemDame.itemdata.end(); it++)
			{
				...
				{
					if (lpItemHelm->m_Level >= it->Level && lpItemHelm->m_NewOption >= it->Option)
					{
				...
				{
					if (lpItemArmor->m_Level >= it->Level && lpItemArmor->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
				...
				{
					if (lpItemPant->m_Level >= it->Level && lpItemPant->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
				...
				{
					if (lpItemGloves->m_Level >= it->Level && lpItemGloves->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
				...
				{
					if (lpItemBoot->m_Level >= it->Level && lpItemBoot->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->IncLife, it->Defense, it->IncResistCriticalDamage, it->IncResisteExcellentDamage);
				...
				{
					if (lpItemWing->m_Level >= it->Level && lpItemWing->m_NewOption >= it->Option)
					{
				...
				{
					if (lpItemKhien->m_Level >= it->Level && lpItemKhien->m_NewOption >= it->Option)
					{
				...
				{
					if (lpItemPen->m_Level >= it->Level && lpItemPen->m_NewOption >= it->Option)
					{
				...
				{
					if (lpItemRing1->m_Level >= it->Level && lpItemRing1->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->Defense, it->IncLife, 0);
				...
				{
					if (lpItemRing2->m_Level >= it->Level && lpItemRing2->m_NewOption >= it->Option)
					{
						gEffectManager.AddEffect(lpObj, 0, it->Effect, (int)(time(0) + it->Time), it->Dame, it->Defense, it->IncLife, 0);
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex)
{
	...
	else if (lpItem->m_Index == GET_ITEM(12, 30))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 15))
	{
		if (lpMsg->type == 0)
		{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 48))
	{
	...
	else if (lpItem->m_Index >= GET_ITEM(13, 54) && lpItem->m_Index <= GET_ITEM(13, 58))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 66))
	{
		gObjTeleport(aIndex, 62, 223, 23);
		this->DecreaseItemDur(lpObj, lpMsg->SourceSlot, 1);
	}
	else if (lpItem->m_Index == GET_ITEM(13, 69))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 70))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 7))
	{
		if (lpItem->m_Level == 0)
		{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 13))
	{
		if (gObjectManager.CharacterUseJewelOfBles(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot) != 0)
		{
			this->DecreaseItemDur(lpObj, lpMsg->SourceSlot, 1);
	...
	else if (lpItem->m_Index == GET_ITEM(14, 20))
	{
		if (lpItem->m_Level == 0)
		{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 43) || lpItem->m_Index == GET_ITEM(14, 44))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 91))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 160))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 161))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 162))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 163))
	{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 169))
	{
		if (gObjectManager.CharacterUseCreationCard(lpObj, lpItem) != 0)
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
	...
	else if (lpItem->m_Index == GET_ITEM(14, 209))
	{
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg, int aIndex, int ok)
{
	...
	DWORD value;
	int valuex;
	if (gItemStack.GetItemMaxStack(item.m_Index, item.m_Level) == 0 || item.m_Index == GET_ITEM(4, 7) || item.m_Index == GET_ITEM(4, 15))
	{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(634));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	if ((item.Coin1) > 0 && (lpObj->ThisCoin[0] < (item.Coin1 * valuex)))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(634));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	if ((item.Coin2) > 0 && (lpObj->ThisCoin[1] < (item.Coin2 * valuex)))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(634));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	if (item.Coin3 > 0 && (lpObj->ThisCoin[2] < (item.Coin3 * valuex)))
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(634));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	...
	if (item.m_Index >= GET_ITEM(13, 71) && item.m_Index <= GET_ITEM(13, 75))
	{
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg, int aIndex)
{
	...
	// Legacy clients recognize zero as rejection; never advertise a zero balance on failure.
	pMsg.result = 0;
	...
	if (lpObj->Lock > 0)
	{
		LogNpcShopSale(lpObj, lpItem, lpMsg->slot, "BLOCKED_LOCKED", "NONE", 0,
			lpObj->Money, lpObj->Money, 0, 0);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 2, 0, pMessage->Text(358));
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	...
	DWORD value;
	if (gItemStack.GetItemMaxStack(lpItem->m_Index, lpItem->m_Level) == 0 || lpItem->m_Index == GET_ITEM(4, 7) || lpItem->m_Index == GET_ITEM(4, 15))
	{
	...
	const char* currency = "ZEN";
	if (lpItem->Sell > 0 && lpItem->Coin1 > 0) currency = "WCOIN_C";
	else if (lpItem->Sell > 0 && lpItem->Coin2 > 0) currency = "WCOIN_P";
	else if (lpItem->Sell > 0 && lpItem->Coin3 > 0) currency = "GOBLIN_POINT";

	DWORD zenBefore = lpObj->Money;
	DWORD zenCredited = 0;
	DWORD zenDiscarded = 0;
	const DWORD zenCapacity = (zenBefore < (DWORD)MAX_MONEY) ? (DWORD)MAX_MONEY - zenBefore : 0;
	if (strcmp(currency, "ZEN") == 0 && gObjCheckMaxMoney(aIndex, value) == 0 && gServerInfo.m_AllowSellItemAtMaxZen == 0)
	{
		LogNpcShopSale(lpObj, lpItem, lpMsg->slot, "BLOCKED_MAX_ZEN", currency, value,
			zenBefore, zenBefore, 0, 0);
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	if (lpItem->Sell > 0)
	{
		...
		{
			zenCredited = (zenCapacity < value) ? zenCapacity : value;
			zenDiscarded = value - zenCredited;
			lpObj->Money += zenCredited;
		}
	}
	else
	{
	...
	LogNpcShopSale(lpObj, lpItem, lpMsg->slot, "SUCCESS", currency, value,
		zenBefore, lpObj->Money, zenCredited, zenDiscarded);
	this->InventoryDelItem(aIndex, lpMsg->slot);
	pMsg.result = 1;
	pMsg.money = lpObj->Money;
	DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
