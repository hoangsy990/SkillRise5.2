...
void gObjEventRunProc()
{
	...
	gCrywolf.MainProc();
#endif
	g_LoanChien->MainProc();
	gAUTOHP.MainProc();
	gDevilSquare.MainProc();
	gDoubleGoer.MainProc();
	gCastleStartGuild.MainProc();
	gDuel.MainProc();
	...
	gRaklion.MainProc();
	SkyEvent.MainProc();
	g_ThanMaChien->MainProc();
	g_BossGuild->MainProc();
	if (gServerInfo.AutoQuetItemDupe)
	{
		gObjSerialScanProc();
	}
}
DWORD gSerialScanTick = 0;
void gObjSerialScanProc()
{
	if ((GetTickCount() - gSerialScanTick) < gServerInfo.TimeQuetItemDupe * 1000)
	{
		return;
	}
	gSerialScanTick = GetTickCount();
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			DWORD serial = lpObj->Inventory[i].m_Serial;
			if (serial != 0)
			{
				gObjInventorySearchSerialNumber(lpObj, serial);
			}
		}
		for (int j = 0; j < WAREHOUSE_SIZE; j++)
		{
			DWORD serial = lpObj->Warehouse[j].m_Serial;
			if (serial != 0)
			{
				gObjWarehouseSearchSerialNumber(lpObj, serial);
			}
		}
	}
}
...
void gObjCloseProc()
{
	if (gCloseMsg != 0)
	{
		if ((--gCloseMsgTime) <= 1)
		{
			if (gCloseMsgTime == 1)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, pMessage->Text(480));
				LogAdd(LOG_RED, pMessage->Text(480));
			}
		}
		else
		{
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, pMessage->Text(481), gCloseMsgTime);
				LogAdd(LOG_RED, pMessage->Text(481), gCloseMsgTime);
			}
		}
		if (gCloseMsgTime < 0)
		{
...
void gObjCharZeroSet(int aIndex)
{
	...
	lpObj->ExWQuestCurrent = 0;
	lpObj->ExWQuestStart = 0;
	lpObj->ExWQuestNum = 0;
	for (int j = 0; j < EW_MAX_MISSION; j++)
	{
		lpObj->ExWQuestCount[j] = 0;
	}
...
void gObjClearPlayerOption(LPOBJ lpObj)
{
	...
	for (int n = 0; n < MAX_SKILL_DELAY_ID; n++)
	{
...
short gObjDel(int aIndex)
{
	...
	if (lpObj->Type == OBJECT_USER)
	{
		...
		{
			LogAddConnect(LOG_ALERT, "ExitAccount: %s - IP: %s", lpObj->Account, lpObj->IpAddr);
			gLog.Output(LOG_OUT_ACCOUNT, "Account: %s"
				, lpObj->Account
			);
		}
		memset(lpObj->Account, 0, sizeof(lpObj->Account));
...
{
	if (lpObj->Type == OBJECT_USER || lpObj->Type == OBJECT_BOTS)
	{
		gViewport.GCViewportSimplePlayerNewsSend(lpObj);
...
void gObjViewportListProtocol(int aIndex)
{
	...
	if (lpObj->Type == OBJECT_USER)
	{
		gViewport.GCViewportPlayerNewsSend(aIndex);
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (g_LoanChien->UserDieProc(lpObj, lpTarget))
	{
		return;
	}
	...
	if (g_ThanMaChien->CheckPlayerTarget(lpObj))
	{
		g_ThanMaChien->UserDieProc(lpObj, lpTarget);
	}

	if (lpTarget->Type == OBJECT_USER && lpObj->IsFakeOnline == true)
	{
		lpObj->IsFakeRegen = false;
	...
	if (lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		...
		g_UserRank->ProtocolUserRank(lpTarget->Index, lpTarget->Index);
		gLog.Output(LOG_OUT_PK, "[%s][%s] Kill [%s][%s]", lpTarget->Account, lpTarget->Name, lpObj->Account, lpObj->Name);
	}
	if (gServerInfo.m_DieUserSwitch == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		gNotice.GCNoticeSendToAll(eMessagePK, 0, 0, 20, 0, 0, "%s %s %d", lpTarget->Name, lpObj->Name, lpObj->Map);
	}
	if (gServerInfo.m_DieMonsterSwitch == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		PostMessagePK(lpTarget->Name, pMessage->Text(316), lpObj->Name);
	}
	int itemdrop = 1;
	int count = 24;
	int number = 0;
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (gCastleStartGuild.CheckPlayerTarget(lpObj, lpTarget, 0) != 0)
	{
		return;
	}
	...
	if (g_LoanChien->CheckPlayerJoined(lpObj, lpTarget) != 0)
	{
		return;
	}
	...
	if (g_BossGuild->CheckPlayerTarget(lpObj, lpTarget, 0) != 0)
	{
		return;
	}
#if(GAMESERVER_TYPE==1)
...
BOOL gObjMoveGate(int aIndex, int gate)
{
...
#if ENABLE_VOCANUS_MOVE
	if (lpObj->GensFamily == 0 && gMapManager.GetMapGensBattle(map) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(253));
		goto ERROR_JUMP;
	}
#endif
...
#endif
	if (map == MAP_ATLANS && lpObj->Inventory[8].IsItem() != 0 && (lpObj->Inventory[8].m_Index == GET_ITEM(13, 2) || lpObj->Inventory[8].m_Index == GET_ITEM(13, 3)))
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(266));
		goto ERROR_JUMP;
	}
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(276));
		goto ERROR_JUMP;
	}
	...
	if (map == MAP_RAKLION2)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(277));
			goto ERROR_JUMP;
		}
		else
		{
...
void gObjSkillUseProc(LPOBJ lpObj)
{
	...
	if (lpObj->Type == OBJECT_USER && lpObj->SkillSummonPartyTime > 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(273), lpObj->SkillSummonPartyTime);
		}
	}
...
bool gObjInventorySearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	for (int n = 0; n < INVENTORY_SIZE; n++)
	{
		if (lpObj->Inventory[n].m_Serial != 0 && lpObj->Inventory[n].m_Serial == serial)
		{
			countInv++;
		}
	}
	for (int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		if (lpObj->Warehouse[n].m_Serial != 0 && lpObj->Warehouse[n].m_Serial == serial)
		{
			countWh++;
		}
	}
	if (countInv > 0 && countWh > 0)
	{
		if (gServerInfo.DeleteDupeItem == 1)
		{
			for (int n = 0; n < WAREHOUSE_SIZE; n++)
			{
				if (lpObj->Warehouse[n].m_Serial == serial)
				{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(482));
		LogAdd(LOG_RED, "Cross Dupe Serial: [%s][%s] %02X", lpObj->Account, lpObj->Name, serial);
		return 0;
	}
	return 1;
}
bool gObjWarehouseSearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	if (countWh > 0 && countInv > 0)
	{
		if (gServerInfo.DeleteDupeItem == 1)
		{
			for (int n = 0; n < INVENTORY_SIZE; n++)
			{
				if (lpObj->Inventory[n].m_Serial == serial)
				{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(29));
		LogAdd(LOG_RED, "Cross Dupe Serial: [%s][%s] %02X", lpObj->Account, lpObj->Name, serial);
		return 0;
	}
	return 1;
	...
	for (n = 0; n < MAX_OBJECT; n++)
	{
		...
		if (lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if (lpObj->Type == OBJECT_USER)
			{
				...
				g_LuckySpin->MakeItem(lpObj->Index, 1);
				if (lpObj->ChatLimitTime > 0)
				{
					...
					if (lpObj->ChatLimitTimeSec > 60)
					{
						...
						{
							lpObj->ChatLimitTime = 0;
							gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(483));
						}
					}
				}
				#if(GAMESERVER_TYPE==1)
		...
		if (lpObj->Connected >= OBJECT_LOGGED && lpObj->Type == OBJECT_USER && lpObj->CloseCount > 0)
		{
			...
			else
			{
				...
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(659), (lpObj->CloseCount - 1));
				}
				else if (lpObj->CloseType == 4)
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(658), (lpObj->CloseCount - 1));
				}
				else
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(484), (lpObj->CloseCount - 1));
				}
			}
			lpObj->CloseCount--;
		}
		if (lpObj->Connected > OBJECT_LOGGED && lpObj->Type == OBJECT_USER)
		{
			...
			{
				LogAdd(LOG_RED, "[%s][%s] CheckSumTime Error (bypass disconnect)", lpObj->Account, lpObj->Name);
				lpObj->CheckSumTime = 0;
				lpObj->CheckSumTableNum = -1;
			}
			...
			if (lpObj->Type == OBJECT_USER)
			{
				if (lpObj->Connected >= OBJECT_LOGGED)
				{
					if (GetTickCount() - lpObj->ConnectTickCount > 60000)
					{
						...
						LogAdd(LOG_RED, "Game response error detected but bypassed [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
						gLog.Output(LOG_OUT_RESPONSE, "Game response error detected but bypassed [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
						lpObj->ConnectTickCount = GetTickCount();
					}
				}
				else
				{
					...
					{
						LogAdd(LOG_RED, "Response error after connection causes conclusion [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
						gLog.Output(LOG_OUT_RESPONSE, "Response error after connection causes conclusion [%d][%s][%s][%s]", lpObj->Index, lpObj->Account, lpObj->Name, lpObj->IpAddr);
						CloseClient(n);
						gObjDel(lpObj->Index);
					}
				}
...
void gObjCheckSelfDefense(LPOBJ lpObj, int aTargetIndex)
{
	...
	char szTemp[64];
	wsprintf(szTemp, pMessage->Text(485), lpObj->Name, gObj[aTargetIndex].Name);
...
void gObjTimeCheckSelfDefense(LPOBJ lpObj)
{
	...
	for (int n = 0; n < MAX_SELF_DEFENSE; n++)
	{
		if (lpObj->SelfDefense[n] >= 0)
		{
			...
			{
				wsprintf(szTemp, pMessage->Text(486), lpObj->Name);
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, szTemp);
...
{
	if (lpObj->Guild == 0 || lpTargetObj->Guild == 0)
	{
		return 0;
	}
	GUILD_INFO_STRUCT* g1 = lpObj->Guild;
	GUILD_INFO_STRUCT* g2 = lpTargetObj->Guild;
	int u1 = (g1->GuildUnion > 0 ? g1->GuildUnion : g1->Number);
	int u2 = (g2->GuildUnion > 0 ? g2->GuildUnion : g2->Number);
	if (u1 == u2)
	{
		return 1;
	}
	for (int n = 1; n < MAX_OBJECT; n++)
	{
		if (gObj[n].Connected < OBJECT_LOGGED) continue;
		if (gObj[n].Guild == 0) continue;
		GUILD_INFO_STRUCT* gx = gObj[n].Guild;
		int ux = (gx->GuildUnion > 0 ? gx->GuildUnion : gx->Number);
		if (ux == u1 && gx->GuildRival == u2)
		{
			return 2;
		}
	}
