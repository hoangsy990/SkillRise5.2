...
#include "RISE/ZenDrop.h"
#include "RISE/CustomItemPro.h"
#include "RISE/CustomItemSetPro.h"
...
void CObjectManager::ObjectSetStateProc()
{
	for (int n = 0; n < MAX_OBJECT; n++)
	{
		...
		if (lpObj->Type == OBJECT_USER)
		{
			...
			gEffectManager.ClearAllEffect(lpObj);
			CHelper::Instance()->DisableHelper(lpObj);
			...
			lpObj->DieRegen = 0;
			lpObj->State = OBJECT_CREATE;
			gGetInfoCharacter.GCCharacterRegenSend(lpObj);
			gObjViewportListProtocolCreate(lpObj);
			this->CharacterUpdateMapEffect(lpObj);
		}
	}
...
bool CObjectManager::CharacterGameClose(int aIndex)
{
	...
	{
		g_DungLuyen->SetStateInterface(lpObj->Index);
	}
	...
	g_ItemChanger->RollBackItem(lpObj->Index, 0);
	g_DungLuyen->RollBackItem(lpObj->Index, 0);
	g_MoveOptionNew->ClearDB(lpObj->Index, 0);
	...
	{
		g_Harmony->SetStateInterface(lpObj->Index);
	}
	...
	if (lpObj->Map == g_ThanMaChien->EventMap)
	{
		...
		g_ThanMaChien->DelUser(lpObj->Index);
	}
	for (int ev = 0; ev < MAX_CUSTOM_GREATPK; ev++)
	{
		if (g_LoanChien->MapEvent[ev] >= 0 && lpObj->Map == g_LoanChien->MapEvent[ev])
		{
			int gate, map, x, y, dir, level;
			if (gGate.GetGate(17, &gate, &map, &x, &y, &dir, &level) != 0)
			{
			...
			g_LoanChien->DelUser(&g_LoanChien->m_LoanChienInfo[ev], lpObj->Index);
			break;
		}
	}
	if (lpObj->Map == g_BossGuild->m_MapBossGuild)
	{
		int gate, map, x, y, dir, level;
		if (gGate.GetGate(17, &gate, &map, &x, &y, &dir, &level) != 0)
		{
	...
	gObjClearViewport(lpObj);
	LogAddConnect(LOG_USER, "Character: %s logout", lpObj->Name);
...
void CObjectManager::CharacterGameCloseSet(int aIndex, int type)
{
	...
	if (lpObj->Connected == OBJECT_ONLINE)
	{
		...
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(291));
			return;
		}
		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_WAREHOUSE)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(292));
			return;
		}
		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_CHAOS_BOX)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(293));
			return;
		}
		if (lpObj->Interface.use != 0 && lpObj->Interface.type == INTERFACE_TRAINER)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(294));
			return;
		}
		if ((GetTickCount() - lpObj->MySelfDefenseTime) < 5000)
		{
			gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, pMessage->Text(295));
			return;
		}
	}
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj)
{
	...
	else if (lpObj->Map == g_BossGuild->m_MapBossGuild)
	{
		result = g_BossGuild->GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	else if (lpObj->Map == MAP_GUILWAR)
	{
		result = gCastleStartGuild.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	else if (g_ThanMaChien->CheckPlayerTarget(lpObj))
	{
		result = g_ThanMaChien->GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
	else if (g_LoanChien->GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level))
	{
		result = 1;
	}
...
bool CObjectManager::CharacterLevelUp(LPOBJ lpObj, DWORD AddExperience, int MaxLevelUp, int ExperienceType)
{
	if (gMasterSkillTree.CheckMasterLevel(lpObj) == 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(296));
			return 1;
		}
		...
		gGetInfoCharacter.GCLevelUpSend(lpObj);
	}
	else
	{
		if(lpObj->MasterLevel >= gServerInfo.m_MasterSkillTreeMaxLevel)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(297));
			return 1;
		}
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj, int type, int amount)
{
	...
	if (stat == &lpObj->Leadership && ((*stat) + amount) > gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]) {
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(278), gServerInfo.m_MaxStatPointCMD[lpObj->AccountLevel]);
		return 0;
	}
	else if (((*stat) + amount) > gServerInfo.m_MaxStatPoint[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(278), gServerInfo.m_MaxStatPoint[lpObj->AccountLevel]);
		return 0;
	}
	(*stat) += amount;
...
{
	const int petSlots[] = { 8, EQUIPMENT_MUUN, EQUIPMENT_EAGLE };
	for (int pet = 0; pet < 3; ++pet)
	{
		const int slot = petSlots[pet];
		CItem* item = &lpObj->Inventory[slot];
		if (item->m_Index != GET_ITEM(13, 4) && gCustomPet.CheckCustomPetByItem(item->m_Index) == 0)
		{
			continue;
		}
		if (item->AddPetItemExp((AddExperience / 10)) == 0)
		{
			GCPetItemInfoSend(lpObj->Index, 1, 0, slot, item->m_PetItemLevel, item->m_PetItemExp, (BYTE)item->m_Durability);
		}
		else
		{
			if (gItemManager.IsValidItem(lpObj, item) == 0)
			{
				if (item->DecPetItemExp((AddExperience / 10)) != 0)
				{
					GCPetItemInfoSend(lpObj->Index, 1, 0, slot, item->m_PetItemLevel, item->m_PetItemExp, (BYTE)item->m_Durability);
				}
			}
			...
			{
				GCPetItemInfoSend(lpObj->Index, 1, 0xFE, slot, item->m_PetItemLevel, item->m_PetItemExp, (BYTE)item->m_Durability);
			}
		}
	}
...
char CObjectManager::gObjFixClassPacket(int DbClass)
{
	char Preview = 0;
	if (rise::slayerserver::IsSlayerDbClass(DbClass))
	{
		return static_cast<char>(rise::slayerserver::SlayerPreviewClassByteForDbClass(DbClass));
	}
	switch (DbClass)
	{
	case DB_DARK_WIZARD:
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex)
{
	...
	lpObj->CharSet[0] = gObjFixClassPacket(lpObj->DBClass);
	lpObj->CharSet[0] |= CS_SET_CHANGEUP(lpObj->ChangeUp);
	...
	for (int i = 0; i < 2; ++i)
	{
		const int slot = (i == 0) ? EQUIPMENT_MUUN : EQUIPMENT_EAGLE;
		const WORD itemIndex = lpObj->Inventory[slot].IsItem() ? (WORD)lpObj->Inventory[slot].m_Index : 0xFFFF;
		lpObj->PetCharSet[4 + (i * 2)] = LOBYTE(itemIndex);
		lpObj->PetCharSet[5 + (i * 2)] = HIBYTE(itemIndex);
	}
}
...
bool CObjectManager::CharacterUsePortal(LPOBJ lpObj, CItem* lpItem)
{
	...
	if (lpObj->Map == MAP_LORENCIA || lpObj->Map == MAP_WARDEAD)
	{
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj, int SourceSlot, int TargetSlot)
{
	...
	if (lpItem->m_Index == GET_ITEM(4, 15))
	{
		return 0;
	}
	if (lpItem->m_Index == GET_ITEM(4, 7))
	{
		return 0;
	}
	if (gServerInfo.m_BlessSoulRingPenAllow && (lpItem->m_Slot == 9 || lpItem->m_Slot == 10))
	{
		goto Next;
	}
	...
	if (lpItem->m_Index == GET_ITEM(0, 41))
	{
	...
	if (gCustomPet.PetIsInmortal(lpItem->m_Index))
	{
		lpItem->m_Durability = 255;
		return 1;
	}
	if (lpItem->m_Index == GET_ITEM(13, 37))
	{
		lpItem->m_Durability = 255;
		return 1;
	}
	...
	int IsRate = gServerInfo.m_JwBlessRingPen[lpObj->AccountLevel];
	if (lpItem->m_Option2 != 0)
	{
		IsRate += gServerInfo.m_AddLuckSuccessRate1[lpObj->AccountLevel];
	}
	if ((GetLargeRand() % 100) < IsRate)
	{
		lpItem->m_Level++;
	}
	else
...
bool CObjectManager::CharacterUseJewelOfSoul(LPOBJ lpObj, int SourceSlot, int TargetSlot)
{
	...
	int IsRate = gServerInfo.m_JwSoulsRingPen[lpObj->AccountLevel];
	if (lpItem->m_Option2 != 0)
	{
		IsRate += gServerInfo.m_AddLuckSuccessRate1[lpObj->AccountLevel];
	}
...
bool CObjectManager::CharacterUseTalismanOfResurrection(LPOBJ lpObj, int SourceSlot)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(249));
		return 0;
	}
...
void CObjectManager::CharacterCalcAttribute(int aIndex)
{
	...
	else if (Left->IsItem() == 0 && Right->m_Index == GET_ITEM(4, 15))
	{
		lpObj->HaveWeaponInHand = 0;
	}
	else if (Right->IsItem() == 0)
	{
		if (Left->m_Index == GET_ITEM(4, 7))
		{
			lpObj->HaveWeaponInHand = 0;
		}
		else if (Left->m_Index >= GET_ITEM(6, 0) && Left->m_Index < GET_ITEM(7, 0))
		{
			lpObj->HaveWeaponInHand = 0;
		}
	}
	...
	while (SkipScan != false)
	{
		...
		gEarringManager.CalcItemOption(lpObj, 1);
		gCustomStartItemDame.CaclCharacter(lpObj, 1);
		g_CustomDameItem->CalcCharacter(lpObj, 1);
		g_CustomSetDameItem->CalcCharacter(lpObj, 1);
		gCustomStartSetItemDame.CalcCustomSetItemOption(lpObj, 1);
		g_DanhHieu->AddOption(lpObj, 1);
		g_QuanHam->AddOption(lpObj, 1);
		g_TuLuyen->AddOption(lpObj, 1);
		g_HonHoan->AddOption(lpObj, 1);
		g_CharOption->ActiveBonus(lpObj, 1);
		for (int n : equipmentSlots)
		{
	...
	else if (lpObj->Class == CLASS_MG)
	{
		...
		lpObj->MGDamageMultiplierRate = 200 + (Energy / gServerInfo.m_MGDamageMultiplierConstA);
	...
	g_CustomDameItem->CalcCharacter(lpObj, 0);
	g_CustomSetDameItem->CalcCharacter(lpObj, 0);
	gCustomPet.CalcCustomPetOption(lpObj, 0);
	gEarringManager.CalcItemOption(lpObj, 0);
	gCustomStartItemDame.CaclCharacter(lpObj, 0);
	gCustomStartSetItemDame.CalcCustomSetItemOption(lpObj, 0);
	g_DanhHieu->AddOption(lpObj, 0);
	g_QuanHam->AddOption(lpObj, 0);
	g_TuLuyen->AddOption(lpObj, 0);
	g_HonHoan->AddOption(lpObj, 0);
	g_CharOption->ActiveBonus(lpObj, 0);
	...
	gGetInfoCharacter.GCNewCharacterCalcSend(lpObj);
	gGetInfoCharacter.SendStatCharacter(lpObj);

	g_ResetChange->GCSendToClient(lpObj);
	g_ChangeClass->GCSendToClient(lpObj);
}
void CObjectManager::CharacterInfoSetSkill(LPOBJ lpObj)
{
	// Slayer skills are keyed by the persisted S21 DB class, not by the
	// compatibility array slot stored in lpObj->Class.  Keep this branch first
	// so a Slayer never falls through to the legacy DK skill bundle.
	...
	else if (lpObj->Class == CLASS_DW)
	{
		...
		gSkillManager.AddSkill(lpObj, SKILL_NOVA, 0);
		gSkillManager.AddSkill(lpObj, SKILL_BLAST, 0);
		gSkillManager.AddSkill(lpObj, SKILL_MAGIC_CIRCLE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_EVIL_SPIRIT, 0);
		gSkillManager.AddSkill(lpObj, SKILL_MANA_SHIELD, 0);
	}
	else if (lpObj->Class == CLASS_DK)
	{
		gSkillManager.AddSkill(lpObj, SKILL_CRESCENT_MOON_SLASH, 0);
		gSkillManager.AddSkill(lpObj, SKILL_TWISTING_SLASH, 0);
		gSkillManager.AddSkill(lpObj, SKILL_RAGEFUL_BLOW, 0);
		gSkillManager.AddSkill(lpObj, SKILL_DEATH_STAB, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_LIFE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FROZEN_STAB, 0);
	}
	else if (lpObj->Class == CLASS_FE)
	{
		gSkillManager.AddSkill(lpObj, SKILL_STAR_FALL, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FIVE_SHOT, 0);
		gSkillManager.AddSkill(lpObj, SKILL_ICE_ARROW, 0);
		gSkillManager.AddSkill(lpObj, SKILL_PENETRATION, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_DEFENSE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_DAMAGE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_HEAL, 0);
		if (gQuest.CheckQuestListState(lpObj, 2, QUEST_FINISH) != 0)
		{
	...
	else if (lpObj->Class == CLASS_MG)
	{
		...
		gSkillManager.AddSkill(lpObj, SKILL_SWORD_SLASH, 0);
		gSkillManager.AddSkill(lpObj, SKILL_LIGHTNING_STORM, 0);
	}
	else if (lpObj->Class == CLASS_DL)
	{
		gSkillManager.AddSkill(lpObj, SKILL_FORCE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FIRE_BLAST, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FIRE_BURST, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FIRE_SCREAM, 0);
		gSkillManager.AddSkill(lpObj, SKILL_BIRDS, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_CRITICAL_DAMAGE, 0);
	}
	else if (lpObj->Class == CLASS_SU)
	{
		gSkillManager.AddSkill(lpObj, SKILL_MANA_GLAIVE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_SLEEP, 0);
		gSkillManager.AddSkill(lpObj, SKILL_RED_STORM, 0);
		gSkillManager.AddSkill(lpObj, SKILL_DRAIN_LIFE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_CHAIN_LIGHTNING, 0);
		gSkillManager.AddSkill(lpObj, SKILL_DAMAGE_REFLECT, 0);
		gSkillManager.AddSkill(lpObj, SKILL_SWORD_POWER, 0);
		gSkillManager.AddSkill(lpObj, SKILL_ELECTRIC_SURGE, 0);
	}
	else if (lpObj->Class == CLASS_RF)
	{
		gSkillManager.AddSkill(lpObj, SKILL_CHARGE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_CHAIN_DRIVER, 0);
		gSkillManager.AddSkill(lpObj, SKILL_DARK_SIDE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_DRAGON_LORE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_IGNORE_DEFENSE_RATE, 0);
		gSkillManager.AddSkill(lpObj, SKILL_FITNESS, 0);
		gSkillManager.AddSkill(lpObj, SKILL_GREATER_DEFENSE_SUCCESS_RATE, 0);
	}
}
bool CObjectManager::CharacterInfoSet(BYTE* aRecv, int aIndex)
{
	...
	// Keep the S21 DB identity intact for persistence and Slayer gates, but
	// never use class 9 as an index into legacy MAX_CLASS (7) arrays.  The
	// compatibility slot is the existing sword-class row; Slayer-specific
	// behavior checks DBClass below rather than borrowing its class identity.
	lpObj->Class			= rise::slayerserver::LegacyArrayClassForDbClass(lpMsg->Class);
	...
	lpObj->rHonHoan			= lpMsg->rHonHoan;
	lpObj->ExWQuestCurrent	= lpMsg->ExWQuestCurrent;
	lpObj->ExWQuestStart	= lpMsg->ExWQuestStart;
	lpObj->ExWQuestNum		= lpMsg->ExWQuestNum;
	lpObj->rCharOption		= lpMsg->rCharOption;
	lpObj->rCharPoint		= lpMsg->rCharPoint;
	lpObj->PKFreeMode		= lpMsg->rPKFreeMode;
	lpObj->rColorName		= lpMsg->rColorName;
	for (int m = 0; m < EW_MAX_MISSION; m++)
	{
		lpObj->ExWQuestCount[m] = lpMsg->ExWQuestCount[m];
	}
	...
	lpObj->ItemStart = lpMsg->ItemStart;
	lpObj->ItemStartCoin = lpMsg->ItemStartCoin;
	bool result = 0;
	int gate, map, x, y, dir, level;
	...
	lpObj->GuildNumber = 0;
	lpObj->Guild = 0;
	const bool isSlayerCharacter = rise::slayerserver::IsSlayerDbClass(lpObj->DBClass);
	for (int n = 0; n < MAX_SKILL_LIST; n++)
	{
		...
		// A persisted Slayer row can arrive from a character that previously
		// used the legacy class slot (or from a full-skill test fixture).  Do
		// not mix that legacy bundle into the Slayer skill list: it can consume
		// all 60 slots before the five imported S21 skills are installed.  Keep
		// only the five Slayer ids here and let CharacterInfoSetSkill add the
		// class starter/full bundle below.
		if (isSlayerCharacter && !rise::slayerserver::IsSlayerSkill(skill.m_index))
		{
			continue;
		}
		gSkillManager.AddSkill(lpObj, skill.m_index, skill.m_level);
	}
	if (gServerInfo.CreateCharacterFullSkill)
	{
		CharacterInfoSetSkill(lpObj);
	}
	else
	{
		// A newly created Slayer receives its S21 starter skill from DBClass.
		// Higher skills remain level/requirement gated when full-skill mode is off.
		if (rise::slayerserver::IsSlayerDbClass(lpObj->DBClass))
		{
			gSkillManager.AddSkill(lpObj, rise::slayerserver::kSwordInertia, 0);
		}
	...
	gDarkSpirit[aIndex].SetMode(DARK_SPIRIT_MODE_NORMAL, -1);
	g_JewelBank->CustomJewelBankInfoSend(lpObj->Index);
	g_WinQuest->GC_MiniInfo(lpObj->Index);
	lpObj->HPAutoRecuperationTime = GetTickCount();
	lpObj->MPAutoRecuperationTime = GetTickCount();
	lpObj->BPAutoRecuperationTime = GetTickCount();
	lpObj->SDAutoRecuperationTime = GetTickCount();
	...
	// Send AutoHp state to client on login
	gAUTOHP.GCAutoHpStateSend(lpObj->Index);
	g_CustomNameColor->GCSendCoinInfo(lpObj->Index);

	return 1;
}
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj, LPOBJ lpTarget, __int64 damage, int DamageType, int flag, int type, int skill, int ShieldDamage)
{
	if (lpObj->Connected != OBJECT_ONLINE)
	{
		return;
		...
		if (lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if (lpObj->CustomQuestMonsterIndex >= 0)
			{
				if (lpObj->CustomQuestMonsterIndex == lpTarget->Class && lpObj->CustomQuestMonsterQtd != 0)
				{
					...
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(655));
					}
					else
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(654), lpObj->CustomQuestMonsterQtd);
					}
				}
			}
			if (lpObj->CustomNpcQuestMonsterIndex >= 0)
			{
				if (lpObj->CustomNpcQuestMonsterIndex == lpTarget->Class && lpObj->CustomNpcQuestMonsterQtd != 0)
				{
					...
					{
						gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(654), lpObj->CustomNpcQuestMonsterQtd);
					}
				}
			}
	...
	else
	{
		if (DamageType == 0)
		{
			gGetInfoCharacter.GCDamageSend(SummonIndex, lpTarget->Index, flag, damage, type, ShieldDamage);
		}
		else if (DamageType == 1)
		{
			gGetInfoCharacter.GCMonsterDamageSend(lpTarget->Index, damage, ShieldDamage);
		}
		else if (DamageType == 2)
		{
			gGetInfoCharacter.GCDamageSend(SummonIndex, lpTarget->Index, flag, damage, 5, ShieldDamage);
		}
		else if (DamageType == 3)
		{
			gGetInfoCharacter.GCDamageSend(SummonIndex, lpTarget->Index, flag, damage, 6, ShieldDamage);
		}
