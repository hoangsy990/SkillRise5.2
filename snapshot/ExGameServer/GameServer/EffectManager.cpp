...
void CEffectManager::Load(char* path)
{
	...
	// Bat Flock needs an owned five-second periodic slot.  Seed it in code so
	// the isolated port does not depend on or overwrite the shared Effect.txt.
	EFFECT_INFO slayerBatFlock = {};
	slayerBatFlock.Index = EFFECT_SLAYER_BAT_FLOCK;
	slayerBatFlock.Group = EFFECT_SLAYER_BAT_FLOCK;
	slayerBatFlock.ItemIndex = -1;
	strcpy_s(slayerBatFlock.Name, "Slayer Bat Flock");
	slayerBatFlock.Save = 0;
	slayerBatFlock.Type = 0;
	slayerBatFlock.Flag = 0;
	slayerBatFlock.Count = -1;
	for (int n = 0; n < 4; ++n)
	{
...
{
	if (lpEffect->m_index != EFFECT_POISON && lpEffect->m_index != EFFECT_NEIL && lpEffect->m_index != EFFECT_SAHAMUTT && lpEffect->m_index != EFFECT_DEATH_STAB_ENHANCED && lpEffect->m_index != EFFECT_DRAIN_LIFE_ENHANCED && lpEffect->m_index != EFFECT_DRAGON_ROAR_ENHANCED && lpEffect->m_index != EFFECT_CHAIN_DRIVER_ENHANCED && lpEffect->m_index != EFFECT_POISON_ARROW && lpEffect->m_index != EFFECT_POISON_ARROW_IMPROVED && lpEffect->m_index != EFFECT_SLAYER_BAT_FLOCK)
	{
		return;
	}
