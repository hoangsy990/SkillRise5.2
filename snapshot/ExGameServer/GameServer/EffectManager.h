...
enum eEffectNumber
{
	...
	// Isolated S21 Slayer port. 221..223 are unused by the legacy effect catalog.
	EFFECT_SLAYER_BAT_FLOCK = 221,
	EFFECT_SLAYER_DEMOLISH = 222,
	EFFECT_SLAYER_DETECTION = 223,
};
...
class CEffectManagerUI
{
public:
	CEffectManagerUI();
	void Load(HWND hList);
	void Save(HWND hList);
	void Refresh(HWND hList);
	static INT_PTR CALLBACK EffectProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
public:
	std::vector<EFFECT_DB_INFO> m_Data;
};
extern CEffectManagerUI gEffectManagerUI;
