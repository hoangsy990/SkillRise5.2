...
int CMasterSkillTree::GetMasterSkillWeapon(LPOBJ lpObj, int index)
{
	...
	case SKILL_PHOENIX_SHOT:
		break;
	case SKILL_FLAME:
		WeaponSkill = ((gSkillManager.GetMasterSkill(lpObj, MASTER_SKILL_ADD_FLAME_IMPROVED1) == 0) ? WeaponSkill : MASTER_SKILL_ADD_FLAME_IMPROVED1);
		break;
	}
	return WeaponSkill;
}
	...
	for (int n = 0; n < MAX_MASTER_SKILL_LIST; n++)
	{
		if (lpObj->MasterSkill[n].IsMasterSkill() != 0)
		{
			...
			if (this->GetInfo(lpObj->MasterSkill[n].m_index, &MasterSkillTreeInfo) != 0)
			{
				...
				info.level = (lpObj->MasterSkill[n].m_level + 1 > 255) ? 255 : lpObj->MasterSkill[n].m_level + 1;
...
QWORD CMasterSkillTree::GetMasterLevelExpTlbInfo(int iMasterLevel)
{
