...
$expectedHash = 'DF3D1F863741E720EFC7B9ECC90117BB1BE9CA49852A86420FA8CA3E396D9F74'
...
if ((Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash -ne $expectedHash) {
    throw 'Read-only S21 master atlas hash mismatch'
}
if (Test-Path -LiteralPath $target) {
    if ((Get-FileHash -LiteralPath $target -Algorithm SHA256).Hash -ne $expectedHash) {
        throw 'Inactive isolated master atlas conflict'
    }
    Write-Output 'PASS: inactive Grow Lancer master atlas already hash-pinned; no writes'
    return
}
...
if ((Get-FileHash -LiteralPath $target -Algorithm SHA256).Hash -ne $expectedHash) {
    throw 'Inactive master atlas copy verification failed'
}
Write-Output 'PASS: inactive Grow Lancer master atlas staged; S21 source unchanged'
