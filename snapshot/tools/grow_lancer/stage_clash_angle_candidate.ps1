...
$backup = Join-Path $qaRoot 'Bin\Engine-Port S21.before-clash-angle-9F150003.exe'
$oldHash = '9F150003CC9ABDF49CBAAEDD69CFF4557487B5A5FC664190F13E0444FB0A7591'
$newHash = '23F598327786F6788476AC557CB0DA976AA15DEAF0CBAAA2D1D1C37D785A3893'
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $staged).Hash -ne $oldHash) { throw 'Staged executable hash mismatch' }
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $backup).Hash -ne $oldHash) { throw 'Rollback backup hash mismatch' }
if (@(Get-CimInstance Win32_Process -Filter "Name='Engine-Port S21.exe'").Count -ne 0) {
...
Write-Output "PASS staged private Clash angle fixture $newHash; rollback $backup"
