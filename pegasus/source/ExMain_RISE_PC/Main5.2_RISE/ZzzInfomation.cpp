...
#include "stdafx.h"
#include "RISE/PegasusHighSkillCatalog.h"
...
void OpenSkillScript(char* FileName)
{
	...
	if (fp != NULL)
	{
		...
		delete[] Buffer;
		rise::pegasus::ApplyHighSkillCatalog();
		rise::windsoul::ApplyBase737SkillCatalog();
	}
	else
	{
		char Text[256];
...
{
	if (iSkillIdx >= MAX_SKILL_TYPES || iSkillIdx < 0)
	{
		return FALSE;
	}
...
void OpenItemScript(char* FileName)
{
	...
	if (fp != NULL)
	{
		...
		const struct
		{
			...
			{ 12 * MAX_ITEM_INDEX + 83, 237, "Shining Tail" },
			{ 12 * MAX_ITEM_INDEX + 84, 238, "Rare Shining Tail" },
			{ 13 * MAX_ITEM_INDEX + 304, 239, "Earring of Radiance (R)" },
			{ 13 * MAX_ITEM_INDEX + 305, 240, "Earring of Radiance (L)" },
		};

		for (int i = 0; i < _countof(builtInItems); ++i)
		{
			ITEM_ATTRIBUTE& item = ItemAttribute[builtInItems[i].itemIndex];
			memset(&item, 0, sizeof(item));
			strcpy_s(item.Name, builtInItems[i].name);
			item.m_byItemSlot = builtInItems[i].slot;
			item.Width = 1;
			item.Height = 1;
			item.Durability = 255;
			item.Value = 1;
			...
			{
				item.RequireClass[classIndex] = 1;
			}
		}
	}
	else
...
void ItemConvert(ITEM* ip, BYTE Attribute1, BYTE Attribute2, BYTE Attribute3)
{
	...
	if (p->Defense > 0)
	{
		...
		else
		{
			...
			else if (gCustomWing->CheckCustomWingByItem(ip->Type))
			{
				ip->Defense += gCustomWing->GetCustomWingDefense(ip->Type, Level);	// ~ +9
			}
			else
			{
				ip->Defense += (min(9, Level) * 3);	// ~ +9
			}
			if ((ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 40) || ip->Type == ITEM_WING + 43 || gCustomWing->CheckCustomWingByItem(ip->Type)
				|| ip->Type == ITEM_WING + 50)
			{
				switch (Level - 9)
				{
	...
	if (p->MagicDefense > 0)
	{
		...
		switch (Level - 9)
		{
			case 6: ip->MagicDefense += 9; break;	// +15
			case 4: ip->MagicDefense += 7; break;	// +13
			case 3: ip->MagicDefense += 6; break;	// +12
			case 2: ip->MagicDefense += 5; break;	// +11
			case 1: ip->MagicDefense += 4; break;	// +10
			default: 
			break;
		};
	}
	...
	if (p->RequireLevel && ((ip->Type >= ITEM_SWORD && ip->Type < ITEM_WING)
		...
		|| (ip->Type >= ITEM_WING + 7 && ip->Type <= ITEM_WING + 40 || gCustomWing->CheckCustomWingByItem(ip->Type))
	...
	CUSTOM_SET_INFO* SetInfo = g_pSetEffect->GetInfoByItem(ip->Type);

	if (SetInfo || ip->Type == ITEM_SWORD + 22 || ip->Type == ITEM_SWORD + 23 || ip->Type == ITEM_STAFF + 12 || ip->Type == ITEM_BOW + 21 || ip->Type == ITEM_MACE + 14 || ip->Type == ITEM_STAFF + 19
	...
	if (excel > 0)
	{
		if (ip->RequireLevel > 0 && ip->Type != ITEM_HELPER + 37 || gCustomPet.GetInfoPetType(ip->Type) != 5)
			ip->RequireLevel += 20;
	}
	...
	else if (gCustomWing->CheckCustomWingByItem(ip->Type))
	{
		if (excelWing & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionValue(ip->Type, 0);
			ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionIndex(ip->Type, 0);
			ip->SpecialNum++;
		}
		if ((excelWing >> 1) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionValue(ip->Type, 1);
			ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionIndex(ip->Type, 1);
			ip->SpecialNum++;
		}
		if ((excelWing >> 2) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionValue(ip->Type, 2);
			ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionIndex(ip->Type, 2);
			ip->SpecialNum++;
		}
		if ((excelWing >> 3) & 0x01)
		{
			ip->SpecialValue[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionValue(ip->Type, 3);
			ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingNewOptionIndex(ip->Type, 3);
			ip->SpecialNum++;
		}
	}
	if ((Attribute1 >> 7) & 1)
	...
	if ((Attribute1 >> 2) & 1)
	{
		...
		if ((ip->Type >= ITEM_WING && ip->Type <= ITEM_WING + 6) || (ip->Type >= ITEM_WING + 41 && ip->Type <= ITEM_WING + 42) || gCustomWing->CheckCustomWingByItem(ip->Type))
		{
			ip->Special[ip->SpecialNum] = AT_LUCK; ip->SpecialNum++;
		}
		if (ip->Type == ITEM_HELPER + 30
	...
	if (Option3)
	{
		...
		else
		{
			...
			if (g_pCustomRingPen->CheckCustomRing(ip->Type) == 1 || (ip->Type >= ITEM_HELPER + 8 && ip->Type < ITEM_HELPER + MAX_ITEM_INDEX && ip->Type != ITEM_HELPER + 30))
			{
			...
			else if (gCustomWing->CheckCustomWingByItem(ip->Type))
			{
				...
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * 4;
					ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingOptionIndex(ip->Type, 0); ip->SpecialNum++;
				}
				else if ((excelWing >> 5) & 0x01)
				{
					ip->SpecialValue[ip->SpecialNum] = Option3 * 4;
					ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingOptionIndex(ip->Type, 1); ip->SpecialNum++;
				}
				else
				{
					ip->SpecialValue[ip->SpecialNum] = Option3;
					ip->Special[ip->SpecialNum] = gCustomWing->GetCustomWingOptionIndex(ip->Type, 2); ip->SpecialNum++;
				}
			}
		}
	}
	...
	//Ring
	if ((ip->Type >= ITEM_SHIELD && ip->Type < ITEM_BOOTS + MAX_ITEM_INDEX) || g_pCustomRingPen->CheckCustomRing(ip->Type) || (ip->Type >= ITEM_HELPER + 8 && ip->Type <= ITEM_HELPER + 9) || (ip->Type >= ITEM_HELPER + 21 && ip->Type <= ITEM_HELPER + 24))
	{
		if ((Attribute2 >> 5) & 1)
		{
	...
	//Pendan
	if ((ip->Type >= ITEM_SWORD && ip->Type < ITEM_STAFF + MAX_ITEM_INDEX) || g_pCustomRingPen->CheckCustomPen(ip->Type) || (ip->Type >= ITEM_HELPER + 12 && ip->Type <= ITEM_HELPER + 13) || (ip->Type >= ITEM_HELPER + 25 && ip->Type <= ITEM_HELPER + 28))
	{
		if ((Attribute2 >> 5) & 1)
		{
			ip->Special[ip->SpecialNum] = AT_EXCELLENT_DAMAGE; ip->SpecialNum++;
		}
		if ((ip->Type >= ITEM_STAFF && ip->Type < ITEM_STAFF + MAX_ITEM_INDEX) || g_pCustomRingPen->CheckCustomRingPen(ip->Type) == 3 || (ip->Type == ITEM_HELPER + 12) || (ip->Type == ITEM_HELPER + 25 || ip->Type == ITEM_HELPER + 27))
		{
			if ((Attribute2 >> 4) & 1)
			{
	...
	if ((ip->Type >= ITEM_BOW && ip->Type < ITEM_BOW + 8) ||
		ip->Type == ITEM_BOW + 17 ||
		(ip->Type >= ITEM_STAFF + 21 && ip->Type <= ITEM_STAFF + 29) ||
	...
	else if (ip->Type >= ITEM_BOOTS && ip->Type < ITEM_BOOTS + MAX_ITEM_INDEX)
	{
		...
		(ip->Type >= ITEM_WING + 49 && ip->Type <= ITEM_WING + 50) ||
		gCustomWing->CheckCustomWingByItem(ip->Type))
	{
	...
	else if ((ip->Type >= ITEM_HELPER + 8 && ip->Type < ITEM_HELPER + 12) ||
		(ip->Type == ITEM_HELPER + 20 && (Level == 0 || Level == 3)) ||
		(ip->Type >= ITEM_HELPER + 21 && ip->Type <= ITEM_HELPER + 24) 
		|| g_pCustomRingPen->CheckCustomRing(ip->Type)
		)
	{
		...
		|| g_pCustomRingPen->CheckCustomPen(ip->Type)
		)
	{
...
int GetExcellentAddValue(ITEM* ip)
{
	...
	if (ip->Type == ITEM_BOW + 6)
	{
		return 30;
	}

	if (ip->Type == ITEM_STAFF + 7)
	{
		return 25;
	}
...
void CalcDefense(ITEM* ip, ITEM_ATTRIBUTE* p)
{
	...
	if ((ip->Type >= ITEM_WING + 3 && ip->Type <= ITEM_WING + 6) || ip->Type == ITEM_WING + 42)
	{
		ip->Defense += (min(9, ip->Level) * 2);
	}
	else if (ip->Type == ITEM_HELPER + 30 || ip->Type == ITEM_WING + 49)
	{
		ip->Defense += (min(9, ip->Level) * 2);
	}
	else if ((ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 40) || ip->Type == ITEM_WING + 43 || ip->Type == ITEM_WING + 50)
	{
		ip->Defense += (min(9, ip->Level) * 4);
	}
	else
	{
		ip->Defense += (min(9, ip->Level) * 3);
	}

	if ((ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 40) || ip->Type == ITEM_WING + 43 || ip->Type == ITEM_WING + 50)
	{
		if (ip->Level - 9 > 0)
		{
...
void CalcRequirements(ITEM* ip, ITEM_ATTRIBUTE* p)
{
	...
	if (p->RequireLevel && ((ip->Type >= ITEM_SWORD && ip->Type < ITEM_WING) || (ip->Type == ITEM_HELPER + 37) || (ip->Type >= ITEM_WING + 7 && ip->Type <= ITEM_WING + 40) || (ip->Type >= ITEM_WING + 43 && ip->Type < ITEM_WING + 40) && (ip->Type != ITEM_WING + 49)))
	{
		ip->RequireLevel = p->RequireLevel;
	}
	else if (p->RequireLevel && ((ip->Type >= ITEM_WING && ip->Type <= ITEM_WING + 7) || (ip->Type >= ITEM_WING + 41 && ip->Type <= ITEM_WING + 42) || (ip->Type == ITEM_WING + 49) || ip->Type >= ITEM_HELPER))
	{
	...
	if (isExcellent > 0 && ip->RequireLevel > 0 && !IsWingItem(ip) && ip->Type != ITEM_HELPER + 37)
	{
		ip->RequireLevel += 20;
	}
}
...
void CalcPartType(ITEM* ip)
{
	...
	if (ip->Type >= ITEM_STAFF + 21 && ip->Type <= ITEM_STAFF + 29)
		...
		ip->Part = EQUIPMENT_HELPER;
	else if ((ip->Type >= ITEM_HELPER + 8 && ip->Type < ITEM_HELPER + 12) || (ip->Type == ITEM_HELPER + 20 && ip->Level == 0)
		|| (ip->Type == ITEM_HELPER + 20 && ip->Level == 3)
		)
		ip->Part = EQUIPMENT_RING_RIGHT;
...
int64_t ItemValue(ITEM* ip, int goldType)
{
	...
	if (ip->Type == ITEM_BOW + 7) //CustomArrow
	{
		int sellMoney = 0;
	...
	else if (ip->Type == ITEM_BOW + 15) //CustomArrow
	{
		int sellMoney = 0;
	...
	else if (ip->Type == ITEM_POTION + 13 || g_pCustomJewel->CheckCustomJewelByItem(ip->Type))
	{
		if (ip->Type == ITEM_POTION + 13)
		{
			Gold = 9000000;
		}
		if (g_pCustomJewel->CheckCustomJewelByItem(ip->Type))
		{
			Gold = g_pCustomJewel->GetCustomJewelSalePrice(ip->Type);
		}
	}
	...
	else if (((Type == 12 && (ip->Type > ITEM_WING + 6 && !(ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 43 || gCustomWing->CheckCustomWingByItem(ip->Type)))) || Type == 13 || Type == 15))
	{
	...
	else
	{
		...
		if ((Type == 12 && ip->Type <= ITEM_WING + 6) || ip->Type == ITEM_HELPER + 30 || (ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 43 || gCustomWing->CheckCustomWingByItem(ip->Type))
			|| (ip->Type == ITEM_WING + 50))
		{
		...
		for (int i = 0; i < ip->SpecialNum; i++)
		{
			...
			case AT_LIFE_REGENERATION:
				if ((Type == 12 && ip->Type <= ITEM_WING + 6) || (ip->Type >= ITEM_WING + 36 && ip->Type <= ITEM_WING + 43) || gCustomWing->CheckCustomWingByItem(ip->Type))
				{
	...
	if (ip->Type == ITEM_HELPER + 32)
		...
		Gold = 30000;
	else if (ip->Type == ITEM_HELPER + 37 || gCustomPet.GetInfoPetType(ip->Type) == 5)
		Gold = 50000;
	...
	if (ip->Type == ITEM_HELPER + 10 || ip->Type == ITEM_HELPER + 20 || ip->Type == ITEM_HELPER + 29)
		goto EXIT_CALCULATE;
	if (ip->Type == ITEM_BOW + 7 || ip->Type == ITEM_BOW + 15 || ip->Type >= ITEM_POTION) //CustomArrow
		goto EXIT_CALCULATE;
	...
	if (goldType == 1 && !(ip->Type >= ITEM_HELPER + 32 && ip->Type <= ITEM_HELPER + 37) || gCustomPet.GetInfoPetType(ip->Type) == 5)
	{
		//        char Text[100];
...
{
	char* name = gCustomMonster->GetMonsterName(type);
	if (name) 
		return name;
...
void CHARACTER_MACHINE::CalculateDamage()
{
	...
	DWORD Strength = Character.Strength + Character.AddStrength;
	DWORD Dexterity = Character.Dexterity + Character.AddDexterity;
	DWORD Energy = Character.Energy + Character.AddEnergy;
	...
	if (Right->Type != -1 && Right->Durability != 0)
	{
		...
		DamageMin = DamageMin - (DWORD)(DamageMin * percent);
		DamageMax = DamageMax - (DWORD)(DamageMax * percent);

		if (Right->Type >= ITEM_STAFF && Right->Type <= ITEM_STAFF + MAX_ITEM_INDEX)
		{
			Character.AttackDamageMinLeft += (DWORD)(DamageMin);
			Character.AttackDamageMaxLeft += (DWORD)(DamageMax);
		}
		else
		{
			Character.AttackDamageMinRight += DamageMin;
	...
	if (Left->Type != -1 && Left->Durability != 0)
	{
		...
		if (CLASS_SUMMONER != gCharacterManager.GetBaseClass(Character.Class))
		{
			...
			if (Left->Type >= ITEM_SWORD && Left->Type < ITEM_SHIELD)
			{
				...
				if (gCharacterManager.GetBaseClass(Character.Class) != CLASS_MAGIC)
				{
					Character.MagicDamageMin += DamageMin - (WORD)(DamageMin * percent);
					Character.MagicDamageMax += DamageMax - (WORD)(DamageMax * percent);
				}
...
void CHARACTER_MACHINE::CalculateAttackRating()
{
	DWORD Strength = Character.Strength + Character.AddStrength;
	DWORD Dexterity = Character.Dexterity + Character.AddDexterity;
	DWORD Charisma = Character.Charisma + Character.AddCharisma;
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	BYTE* speed = pMain->ThisValue;

	int index = -1;

	switch (CharacterClass)
	{
		case CLASS_WIZARD: index = 0; break;
		case CLASS_KNIGHT: index = 2; break;
		case CLASS_ELF: index = 4; break;
		case CLASS_MAGIC: index = 6; break;
		case CLASS_DARK_LORD: index = 8; break;
		case CLASS_SUMMONER: index = 10; break;
		case CLASS_RAGEFIGHTER: index = 12; break;
	}

	if (index >= 0 && speed[index] != 0 && speed[index + 1] != 0)
	{
		Character.AttackSpeed = Dexterity / speed[index];
		Character.MagicSpeed = Dexterity / speed[index + 1];
	}
...
void CHARACTER_MACHINE::CalculateSuccessfulBlocking()
{
	...
	if (Left->Type != -1 && Left->Durability != 0)
	{
		...
		DWORD SuccessBlocking = Left->SuccessfulBlocking - (WORD)(Left->SuccessfulBlocking * percent);
...
{
	//Character.Experience = Character.NextExperince; // RISE Exp nay bi sai.. loai bo 21.11.2025
...
bool CHARACTER_MACHINE::CheckEquipment(int index) const
{
	if (index >= 0 && index < MAX_EQUIPMENT)
		return true;

	const int clientIndex = index - 224;
	return clientIndex >= MAX_EQUIPMENT && clientIndex < MAX_NEW_EQUIPMENT;
}
...
ITEM* CHARACTER_MACHINE::GetOnlyEquipment(int index)
{
	...
	const int clientIndex = index - 224;
	if (clientIndex >= MAX_EQUIPMENT && clientIndex < MAX_NEW_EQUIPMENT)
		return &Equipment[clientIndex];
...
void CHARACTER_MACHINE::CalculateAll()
{
	...
	// OK Method 1
	g_pMasterSkillTreeInterface->GetMasterAddPoint(Character.AddStrength, Character.AddDexterity, Character.AddVitality, Character.AddEnergy, Character.AddCharisma);
	
	// OK Method 2
	/*
	float mStr = 0, mAgi = 0, mVit = 0, mEne = 0, mCmd = 0;

	g_pMasterSkillTreeInterface->GetMasterAddPointEx(mStr, mAgi, mVit, mEne, mCmd);

	Character.AddStrength	+= (DWORD)mStr;
	Character.AddDexterity	+= (DWORD)mAgi;
	Character.AddVitality	+= (DWORD)mVit;
	Character.AddEnergy		+= (DWORD)mEne;
	Character.AddCharisma	+= (DWORD)mCmd;
	*/
...
void CHARACTER_MACHINE::GetPhisyDamage(DWORD* _DamageMin, DWORD* _DamageMax)
{
	(*_DamageMin) = CharacterAttribute->ViewPhysiDamageMin;
	(*_DamageMax) = CharacterAttribute->ViewPhysiDamageMax;

	(*_DamageMin) += ((*_DamageMin) * CharacterAttribute->ViewMulPhysiDamage) * 0.01;
	(*_DamageMax) += ((*_DamageMax) * CharacterAttribute->ViewMulPhysiDamage) * 0.01;
	(*_DamageMin) -= ((*_DamageMin) * CharacterAttribute->ViewDivPhysiDamage) * 0.01;
	(*_DamageMax) -= ((*_DamageMax) * CharacterAttribute->ViewDivPhysiDamage) * 0.01;
}
