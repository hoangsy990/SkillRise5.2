...
#include "RISE/PegasusSwordBlowRuntime.h"
#include "RISE/PegasusSpiralChargeRuntime.h"
#include "RISE/PegasusCrusherChargeRuntime.h"
#include "RISE/PegasusShiningBirdRuntime.h"
#include "RISE/PegasusDexBoosterRuntime.h"
#include "RISE/PegasusChaosBladeRuntime.h"
#include "RISE/PegasusDragonViolentRuntime.h"
#include "RISE/PegasusElementalChargeRuntime.h"
#include "RISE/PegasusHavocSpearRuntime.h"
#include "RISE/PegasusSpearstormRuntime.h"
#include "RISE/PegasusRainingArrowRuntime.h"
#include "RISE/PegasusDeathsideRuntime.h"
...
#include "RISE/PegasusSoundRuntime.h"
...
#include "RISE/CustomJewel.h"
#include "RISE/CustomWing.h"
#include "ItemManager.h"
#include "RISE/CustomMonster.h"
#include "RISE/CustomBow.h"
#include "RISE/CustomPet.h"
#include <RISE/CustomGloves.h>
#include <RISE/CustomRingPen.h>
#include <RISE/RenderCape.h>
///////////////////////////////////////////
...
namespace
{
typedef bool (*PegasusModelEnsure)(int);

void PreflightPegasusModelRange(int skill, int firstModel, int lastModel,
	PegasusModelEnsure ensure)
{
	bool ready = true;
	for (int model = firstModel; model <= lastModel; ++model)
		ready = ensure(model) && ready;
	rise::pegasus::TraceRuntime("ASSET_PREFLIGHT", skill,
		lastModel - firstModel + 1, ready ? "PASS" : "FAIL");
}

void PreflightPegasusRuntimeAssets()
{
	using namespace rise::pegasus;
	PreflightPegasusModelRange(kSwordWrathSkill, kSwordWrathRootModel,
		kSwordWrathLayerModel, EnsureSwordWrathModel);
	PreflightPegasusModelRange(kSpiralChargeSkill, kSpiralChargeFirstModel,
		kSpiralChargeLastModel, EnsureSpiralChargeModel);
	PreflightPegasusModelRange(kCrusherChargeSkill, kCrusherChargeFirstModel,
		kCrusherChargeLastModel, EnsureCrusherChargeModel);
	TraceRuntime("ASSET_PREFLIGHT", kShiningBirdSkill, 1,
		EnsureShiningBirdModel() ? "PASS" : "FAIL");
	TraceRuntime("ASSET_PREFLIGHT", kDexBoosterSkill, 0,
		EnsureDexBoosterBitmaps() ? "PASS" : "FAIL");
	PreflightPegasusModelRange(kChaosBladeSkill, kChaosBladeFirstModel,
		kChaosBladeLastModel, EnsureChaosBladeModel);
	PreflightPegasusModelRange(kDragonViolentSkill, kDragonViolentFirstModel,
		kDragonViolentLastModel, EnsureDragonViolentModel);
	TraceRuntime("ASSET_PREFLIGHT", kElementalChargeSkill, 1,
		EnsureElementalChargeModel() ? "PASS" : "FAIL");
	PreflightPegasusModelRange(kHavocSpearSkill, kHavocFirstModel,
		kHavocLastModel, EnsureHavocSpearModel);
	PreflightPegasusModelRange(kSpearstormSkill, kSpearstormFirstModel,
		kSpearstormLastModel, EnsureSpearstormModel);
	PreflightPegasusModelRange(kRainingArrowSkill, kRainingArrowFirstModel,
		kRainingArrowLastModel, EnsureRainingArrowModel);
	TraceRuntime("ASSET_PREFLIGHT", kDeathsideSkill,
		kDeathsideLastModel - kDeathsideFirstModel + 1,
		EnsureDeathsideModels() ? "PASS" : "FAIL");
}
}
#endif
...
void OpenPlayers()
{
	ModelsDump = new BMD[MAX_MODELS + 1024 + rise::pegasus::kDynamicModelCapacity];
	Models = ModelsDump + (rand() % 1024);
	// The isolated Pegasus BMDs live in the bounded tail after MAX_MODELS.
	// Leaving that tail uninitialized lets stale NumMeshs/texture state make
	// AccessModel skip or partially open a dynamic model nondeterministically.
	ZeroMemory(Models, rise::pegasus::RuntimeModelLimit(MAX_MODELS) * sizeof(BMD));
	rise::pegasus::TraceRuntime("MODEL_STORAGE_INIT", MAX_MODELS,
		rise::pegasus::RuntimeModelLimit(MAX_MODELS), "PASS");

#if defined(PEGASUS_ISOLATED_TEST)
	// Pegasus player.bmd is an animation-only strict superset of the native
	// RISE file: the first 284 action payloads match byte-for-byte and Pegasus
	// adds actions 284..286.  Load it only from the isolated, hash-staged tree
	// so Raining Arrow can use its proven action 286 without touching the base
	// client Data directory.
	gLoadData.AccessModel(MODEL_PLAYER,
		"Data\\RISE\\Pegasus\\Recovered\\Data\\Player\\", "Player");
	rise::pegasus::TraceRuntime("PLAYER_ACTION_BMD",
		Models[MODEL_PLAYER].NumActions,
		rise::pegasus::kPegasusPlayerActionCount,
		Models[MODEL_PLAYER].NumActions ==
			rise::pegasus::kPegasusPlayerActionCount ? "PASS" : "FAIL");
#else
	gLoadData.AccessModel(MODEL_PLAYER, "Data\\Player\\", "Player");
#endif
	rise::pegasus::InstallSwordBlowFlightAction();
	...
	for (int i = 0; i < MAX_CLASS; ++i)
	{
		...
		if (CLASS_MAGIC != i && CLASS_DARK_LORD != i && CLASS_RAGEFIGHTER != i)
		{
...
#if defined(PEGASUS_ISOLATED_TEST)
	// Pegasus OpenPlayers 0x87B127..0x87B147 applies 0.4 to the complete
	// action tail through index 286.  RISE's older enum stops at 283, so apply
	// the proven tail explicitly after loading the 287-action carrier.
	if (Models[MODEL_PLAYER].NumActions >= rise::pegasus::kPegasusPlayerActionCount)
	{
		for (int i = MAX_PLAYER_ACTION;
			i < rise::pegasus::kPegasusPlayerActionCount; ++i)
			Models[MODEL_PLAYER].Actions[i].PlaySpeed = 0.4f;
	}
#endif
...
{
	LoadBitmapFile("Player\\skin_barbarian_01.jpg", BITMAP_SKIN);
	LoadBitmapFile("Player\\level_man022.jpg", BITMAP_SKIN + 1);
	LoadBitmapFile("Player\\skin_wizard_01.jpg", BITMAP_SKIN + 2);
	LoadBitmapFile("Player\\level_man01.jpg", BITMAP_SKIN + 3);
	LoadBitmapFile("Player\\skin_archer_01.jpg", BITMAP_SKIN + 4);
	LoadBitmapFile("Player\\level_man033.jpg", BITMAP_SKIN + 5);
	LoadBitmapFile("Player\\skin_special_01.jpg", BITMAP_SKIN + 6);
	LoadBitmapFile("Player\\level_man02.jpg", BITMAP_SKIN + 8);
	LoadBitmapFile("Player\\hair_r.jpg", BITMAP_HAIR);
	LoadBitmapFile("Player\\LevelClass107.jpg", BITMAP_SKIN + 12);
	LoadBitmapFile("Player\\LevelClass207.jpg", BITMAP_SKIN + 13);
	LoadBitmapFile("Player\\LevelClass207_1.jpg", BITMAP_SKIN + 14);
	...
	for (int j = 0; j < 3; ++j)
	{
		for (int i = 0; i < MAX_CLASS; ++i)
		{
			...
			if (1 == j && (CLASS_MAGIC == i || CLASS_DARK_LORD == i || CLASS_RAGEFIGHTER == i))
				continue;
	...
	{
		LoadBitmapFile("Player\\Robe01.jpg", BITMAP_ROBE);
		LoadBitmapFile("Player\\Robe02.jpg", BITMAP_ROBE + 1);
		LoadBitmapFile("Player\\Robe03.tga", BITMAP_ROBE + 2);
		LoadBitmapFile("Player\\DarklordRobe.tga", BITMAP_ROBE + 7);
		LoadBitmapFile("Item\\msword03.tga", BITMAP_ROBE + 8);
		LoadBitmapFile("Item\\dl_redwings02.tga", BITMAP_ROBE + 9);
		LoadBitmapFile("Item\\dl_redwings03.tga", BITMAP_ROBE + 10);
	}
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		::LoadBitmapFile(szFileName, BITMAP_INVEN_ARMOR + i);
		::sprintf(szFileName, "Player\\InvenPantsMale%d.tga", 40 + i);
		::LoadBitmapFile(szFileName, BITMAP_INVEN_PANTS + i);
	}

	::sprintf(szFileName, "Player\\Item312_Armoritem.tga");
	::LoadBitmapFile(szFileName, BITMAP_SKIN_ARMOR_DEVINE);
	::sprintf(szFileName, "Player\\Item312_Pantitem.tga");
	::LoadBitmapFile(szFileName, BITMAP_SKIN_PANTS_DEVINE);
	::sprintf(szFileName, "Player\\SkinClass706_upperitem.tga");
	::LoadBitmapFile(szFileName, BITMAP_SKIN_ARMOR_SUCCUBUS);
	::sprintf(szFileName, "Player\\SkinClass706_loweritem.tga");
	::LoadBitmapFile(szFileName, BITMAP_SKIN_PANTS_SUCCUBUS);
...
void OpenItems()
{
	...
	LoadBitmapFile("Item\\NCcape.tga", BITMAP_NCCAPE, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\monk_manto01.TGA", BITMAP_MANTO01, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\monke_manto.TGA", BITMAP_MANTOE, GL_LINEAR, GL_REPEAT);
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.AccessModel(ItemInfo->ItemIndex + MODEL_ITEM, "Data\\RISE\\Item\\", ItemInfo->ModelName);
		}
	}

	for (std::map<int, CUSTOM_RING_INFO>::iterator it = g_pCustomRingPen->m_CustomRingPenInfo.begin(); it != g_pCustomRingPen->m_CustomRingPenInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			gLoadData.AccessModel((it->second.ItemIndex + MODEL_ITEM), "Data\\RISE\\Item\\", it->second.ModelName);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			gLoadData.AccessModel(it->second.ItemIndex + MODEL_ITEM, "Data\\RISE\\Item\\", it->second.ModelName, -1);
			gLoadData.AccessModel(it->second.TextureLeftIndex + MODEL_ITEM, "Data\\RISE\\Item\\", it->second.TextureLeftName, -1);
			gLoadData.AccessModel(it->second.TextureRightIndex + MODEL_ITEM, "Data\\RISE\\Item\\", it->second.TextureRightName, -1);
		}
	}
...
#endif
	for (std::map<int, CUSTOM_PET_INFO>::iterator it = gCustomPet.m_CustomPetInfo.begin(); it != gCustomPet.m_CustomPetInfo.end(); it++)
	{
		gLoadData.AccessModel((it->second.ItemIndex + MODEL_ITEM), "Data\\RISE\\Item\\", it->second.ModelName, -1);
	}

	for (std::map<int, CUSTOM_WING_INFO>::iterator it = gCustomWing->m_CustomWingInfo.begin(); it != gCustomWing->m_CustomWingInfo.end(); it++)
	{
		auto lpInfo = &(*it).second;

		gLoadData.AccessModel((lpInfo->ItemIndex + MODEL_ITEM), "Data\\RISE\\Item\\", lpInfo->ModelName);

	}

	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		if (g_pCustomJewel->m_CustomJewelInfo[n].Index != -1)
		{
			gLoadData.AccessModel((g_pCustomJewel->m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Data\\RISE\\Item\\", g_pCustomJewel->m_CustomJewelInfo[n].ModelName);
		}
	}
}
...
void OpenItemTextures()
{
	...
	LoadBitmapFile("Item\\msword01_r.jpg", BITMAP_3RDWING_LAYER, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Item\\rollofpaper_R.jpg", BITMAP_ROOLOFPAPER_EFFECT_R, GL_LINEAR);
...
//---------------------------------------------------------------------------------
	...
	LoadBitmapFile("Item\\InGameShop\\membership_item_blue.jpg", BITMAP_PACKAGEBOX_BLUE, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\InGameShop\\membership_item_gold.jpg", BITMAP_PACKAGEBOX_GOLD, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\InGameShop\\membership_item_green.jpg", BITMAP_PACKAGEBOX_GREEN, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\InGameShop\\membership_item_pouple.jpg", BITMAP_PACKAGEBOX_PUPLE, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\InGameShop\\membership_item_red.jpg", BITMAP_PACKAGEBOX_RED, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\InGameShop\\membership_item_sky.jpg", BITMAP_PACKAGEBOX_SKY, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Item\\InGameShop\\mebership_3items_green.jpg", BITMAP_INGAMESHOP_PRIMIUM6, GL_LINEAR, GL_REPEAT);
	for (int k = 0; k < 4; k++)
	{
		gLoadData.OpenTexture(MODEL_POTION + 126 + k, "Item\\InGameShop\\");
	}

	LoadBitmapFile("Item\\InGameShop\\mebership_3items_red.jpg", BITMAP_INGAMESHOP_COMMUTERTICKET4, GL_LINEAR, GL_REPEAT);
	for (int k = 0; k < 3; k++)
	{
		gLoadData.OpenTexture(MODEL_POTION + 130 + k, "Item\\InGameShop\\");
	}

	LoadBitmapFile("Item\\InGameShop\\mebership_3items_yellow.jpg", BITMAP_INGAMESHOP_SIZECOMMUTERTICKET3, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Item\\rgb_mix.jpg", BITMAP_RGB_MIX, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Item\\PhoenixSoul_render.JPG", BITMAP_PHOENIXSOULWING, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile(szLuckySetPathName, BITMAP_INVEN_ARMOR + 6);
	sprintf(szLuckySetPathName, "Player\\LuckyItem\\65\\InvenPantsMale40_luck.tga");
	LoadBitmapFile(szLuckySetPathName, BITMAP_INVEN_PANTS + 6);
	sprintf(szLuckySetPathName, "Player\\LuckyItem\\70\\InvenArmorMale41_luck.tga");
	LoadBitmapFile(szLuckySetPathName, BITMAP_INVEN_ARMOR + 7);
	sprintf(szLuckySetPathName, "Player\\LuckyItem\\70\\InvenPantsMale41_luck.tga");
	LoadBitmapFile(szLuckySetPathName, BITMAP_INVEN_PANTS + 7);
#endif // LEM_ADD_LUCKYITEM
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.OpenTexture(ItemInfo->ItemIndex + MODEL_ITEM, "RISE\\Item\\");
		}
	}
	...
	for (std::map<int, CUSTOM_RING_INFO>::iterator it = g_pCustomRingPen->m_CustomRingPenInfo.begin(); it != g_pCustomRingPen->m_CustomRingPenInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			gLoadData.OpenTexture((it->second.ItemIndex + MODEL_ITEM), "RISE\\Item\\");
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		...
		{
			gLoadData.OpenTexture(it->second.ItemIndex + MODEL_ITEM, "RISE\\Item\\");
			gLoadData.OpenTexture(it->second.TextureLeftIndex + MODEL_ITEM, "RISE\\Item\\");
			gLoadData.OpenTexture(it->second.TextureRightIndex + MODEL_ITEM, "RISE\\Item\\");
		}
	}
	...
	for (std::map<int, CUSTOM_WING_INFO>::iterator it = gCustomWing->m_CustomWingInfo.begin(); it != gCustomWing->m_CustomWingInfo.end(); it++)
	{
		...
		gLoadData.OpenTexture((lpInfo->ItemIndex + MODEL_ITEM), "RISE\\Item\\");
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		if (g_pCustomJewel->m_CustomJewelInfo[n].Index != -1)
		{
			gLoadData.OpenTexture((g_pCustomJewel->m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "RISE\\Item\\");
		}
	}

	gCapeRender.LoadTexture();
}
...
void OpenMonsterModel(int Type)
{
	...
	#if M_CUSTOM_MONSTER
	CUSTOM_MONSTER_DATA* lpInfo = gCustomMonster->GetMonsterData(Type);	
	if (lpInfo != nullptr)
	{
		gLoadData.AccessModel(Index, lpInfo->ModelDir, lpInfo->ModelFile);
		gLoadData.OpenTexture(Index, lpInfo->ModelFolder);
	}
	else
	{
		#endif
	...
	{
		if (!b->Actions || b->NumActions == 0)
		{
			g_ConsoleDebug->Write(MCD_NORMAL, "Model %d , %d action", Type, b->NumActions);
		}
	}
	...
	case 42:
		...
		LoadBitmapFile("Monster\\bv01_2.jpg", BITMAP_MONSTER_SKIN, GL_LINEAR, GL_REPEAT);
		LoadBitmapFile("Monster\\bv02_2.jpg", BITMAP_MONSTER_SKIN + 1, GL_LINEAR, GL_REPEAT);
		Models[MODEL_MONSTER01 + Type].BoneHead = 20;
		break;
	case 43:
	...
	case 48:
		...
		break;
	case 52:
		LoadBitmapFile("Monster\\iui02.tga", BITMAP_ROBE + 3);
		LoadBitmapFile("Monster\\iui03.tga", BITMAP_ROBE + 5);
		b->Actions[MONSTER01_DIE].PlaySpeed = 0.22f;
...
void OpenSkills()
{
	...
	LoadBitmapFile("Skill\\jack04.jpg", BITMAP_JACK_1);
	LoadBitmapFile("Skill\\jack05.jpg", BITMAP_JACK_2);
	LoadBitmapFile("Monster\\iui02.tga", BITMAP_ROBE + 3);
	gLoadData.AccessModel(MODEL_POTION + 45, "Data\\Item\\", "hobakhead");
	gLoadData.OpenTexture(MODEL_POTION + 45, "Item\\");
	...
	LoadBitmapFile("Skill\\dkthreebody_r.jpg", BITMAP_MONSTER_SKIN + 2, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Effect\\Bugbear_R.jpg", BITMAP_BUGBEAR_R, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\pk_mon02_fire.jpg", BITMAP_PKMON01, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\pk_mon03_red.jpg", BITMAP_PKMON02, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\pk_mon02_green.jpg", BITMAP_PKMON03, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\pk_mon03_green.jpg", BITMAP_PKMON04, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\lavagiantAa_e.jpg", BITMAP_PKMON05, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\lavagiantBa_e.jpg", BITMAP_PKMON06, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\eff_magma_red.jpg", BITMAP_LAVAGIANT_FOOTPRINT_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\eff_magma_violet.jpg", BITMAP_LAVAGIANT_FOOTPRINT_V, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\raymond_sword_R.jpg", BITMAP_RAYMOND_SWORD, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\mist01.jpg", BITMAP_AG_ADDITION_EFFECT, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Skill\\flower1.tga", BITMAP_FLOWER01);
	LoadBitmapFile("Skill\\flower2.tga", BITMAP_FLOWER01 + 1);
	LoadBitmapFile("Skill\\flower3.tga", BITMAP_FLOWER01 + 2);
	...
	LoadBitmapFile("Effect\\sbumb.jpg", BITMAP_SBUMB, GL_LINEAR, GL_REPEAT);
	...
	LoadBitmapFile("Effect\\force_Pillar.jpg", BITMAP_FORCEPILLAR, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\!SwordEff.jpg", BITMAP_SWORDEFF, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\Damage1.jpg", BITMAP_DAMAGE1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\ground_wind.jpg", BITMAP_GROUND_WIND, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Kwave2.jpg", BITMAP_KWAVE2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Damage2.jpg", BITMAP_DAMAGE2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\volcano_core.jpg", BITMAP_VOLCANO_CORE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	gLoadData.AccessModel(MODEL_SHOCKWAVE03, "Data\\Effect\\", "shockwave03");
	gLoadData.OpenTexture(MODEL_SHOCKWAVE03, "Effect\\");
	LoadBitmapFile("Effect\\ground_smoke.tga", BITMAP_GROUND_SMOKE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\knightSt_blue.jpg", BITMAP_KNIGHTST_BLUE, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Effect\\pin_star.jpg", BITMAP_START_RISE_BIT + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Skill\\bossrock1_R.JPG", BITMAP_FISSURE_FIRE, GL_LINEAR, GL_CLAMP_TO_EDGE);
...
void OpenImages()
{
	LoadBitmapFile("Interface\\command.jpg", BITMAP_SKILL_INTERFACE + 2);
	LoadBitmapFile("Interface\\Item_Back01.jpg", BITMAP_INVENTORY);
	LoadBitmapFile("Interface\\Item_Money.jpg", BITMAP_INVENTORY + 11);
	LoadBitmapFile("Interface\\Item_box.jpg", BITMAP_INVENTORY + 17);
	LoadBitmapFile("Interface\\InventoryBox2.jpg", BITMAP_INVENTORY + 18);
	LoadBitmapFile("Interface\\Trading_line.jpg", BITMAP_INVENTORY + 19);
	LoadBitmapFile("Interface\\exit_01.jpg", BITMAP_INVENTORY_BUTTON);
	LoadBitmapFile("Interface\\exit_02.jpg", BITMAP_INVENTORY_BUTTON + 1);
	LoadBitmapFile("Interface\\accept_box01.jpg", BITMAP_INVENTORY_BUTTON + 10);
	LoadBitmapFile("Interface\\accept_box02.jpg", BITMAP_INVENTORY_BUTTON + 11);
	LoadBitmapFile("Interface\\mix_button1.jpg", BITMAP_INVENTORY_BUTTON + 12);
	LoadBitmapFile("Interface\\mix_button2.jpg", BITMAP_INVENTORY_BUTTON + 13);
	LoadBitmapFile("Interface\\lock_01.jpg", BITMAP_INVENTORY_BUTTON + 14);
	LoadBitmapFile("Interface\\lock_02.jpg", BITMAP_INVENTORY_BUTTON + 15);
	LoadBitmapFile("Interface\\lock_03.jpg", BITMAP_INVENTORY_BUTTON + 16);
	LoadBitmapFile("Interface\\lock_04.jpg", BITMAP_INVENTORY_BUTTON + 17);
	LoadBitmapFile("Interface\\guild.tga", BITMAP_GUILD);
}
...
void OpenSounds()
{
	...
	LoadWaveFile(rise::pegasus::kDeathsideAttack1Sound, "Data\\RISE\\Pegasus\\Sound\\ReaperAttack1.wav");
	LoadWaveFile(rise::pegasus::kDeathsideAttack2Sound, "Data\\RISE\\Pegasus\\Sound\\ReaperAttack2.wav");
	LoadWaveFile(rise::pegasus::kDeathsideAttack3Sound, "Data\\RISE\\Pegasus\\Sound\\ReaperAttack3.wav");
	LoadWaveFile(rise::pegasus::kDeathsideAppearSound, "Data\\RISE\\Pegasus\\Sound\\ReaperAppear.wav");
	LoadWaveFile(rise::pegasus::kDeathsideDisappearSound, "Data\\RISE\\Pegasus\\Sound\\Reaperdisappear.wav");
	LoadWaveFile(rise::pegasus::kRainingArrowSound, "Data\\RISE\\Pegasus\\Sound\\Elf\\RainingArrow.wav");
	LoadWaveFile(rise::pegasus::kRainingArrowHitSound, "Data\\RISE\\Pegasus\\Sound\\Elf\\RainingArrow-Hit.wav");
	LoadWaveFile(rise::pegasus::kHavocSpearSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_HavocSpear.wav");
	LoadWaveFile(rise::pegasus::kHavocSpearHit1Sound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_HavocSpear_Hit_1.wav");
	LoadWaveFile(rise::pegasus::kHavocSpearHit2Sound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_HavocSpear_Hit_2.wav");
	LoadWaveFile(rise::pegasus::kHavocSpearExplosionSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_HavocSpear_Explosion.wav");
	LoadWaveFile(rise::pegasus::kSpiralChargeSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_SpiralCharge.wav");
	LoadWaveFile(rise::pegasus::kCrusherChargeSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_CrasherCharge.wav");
	LoadWaveFile(rise::pegasus::kElementalChargeSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_ElementalCharge.wav");
	LoadWaveFile(rise::pegasus::kChaosBladeSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_ChaosBlade.wav");
	LoadWaveFile(rise::pegasus::kChaosBladeHit1Sound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_ChaosBlade_Hit_1.wav");
	LoadWaveFile(rise::pegasus::kChaosBladeHit2Sound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_ChaosBlade_Hit_2.wav");
	LoadWaveFile(rise::pegasus::kChaosBladeExplosionSound, "Data\\RISE\\Pegasus\\Sound\\MG_Renewal_ChaosBlade_Explosion.wav");
	LoadWaveFile(rise::pegasus::kDragonViolentSound, "Data\\RISE\\Pegasus\\Sound\\Kundun\\DragonViolent.wav");
	LoadWaveFile(rise::pegasus::kDragonViolentHitSound, "Data\\RISE\\Pegasus\\Sound\\Kundun\\DragonViolent_Hit.wav");
	LoadWaveFile(rise::pegasus::kShiningBirdSound, "Data\\RISE\\Pegasus\\Sound\\Kundun\\ShiningBird.wav");
	LoadWaveFile(rise::pegasus::kSpearstormSound, "Data\\RISE\\Pegasus\\Sound\\Kundun\\SpearStorm.wav");
	LoadWaveFile(rise::pegasus::kSpearstormHitSound, "Data\\RISE\\Pegasus\\Sound\\Kundun\\SpearStorm_Hit.wav");
}
...
void OpenFont()
{
	...
	LoadBitmapFile("Interface\\FontInput.tga", BITMAP_FONT, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\FontTest.tga", BITMAP_FONT + 1);
	LoadBitmapFile("Interface\\Hit.tga", BITMAP_FONT_HIT, GL_NEAREST, GL_CLAMP_TO_EDGE);
...
{
	//image
	::LoadBitmapFile("Interface\\cha_bt.tga", BITMAP_LOG_IN);
	::LoadBitmapFile("Interface\\server_b2_all.tga", BITMAP_LOG_IN + 1);
	::LoadBitmapFile("Interface\\server_b2_loding.jpg", BITMAP_LOG_IN + 2);
	::LoadBitmapFile("Interface\\server_deco_all.tga", BITMAP_LOG_IN + 3);
	::LoadBitmapFile("Interface\\server_menu_b_all.tga", BITMAP_LOG_IN + 4);
	::LoadBitmapFile("Interface\\server_credit_b_all.tga", BITMAP_LOG_IN + 5);
	::LoadBitmapFile("Interface\\deco.tga", BITMAP_LOG_IN + 6);
	::LoadBitmapFile("Interface\\login_back.tga", BITMAP_LOG_IN + 7);
	//::LoadBitmapFile("Interface\\newui_cha_textbox02.tga", BITMAP_LOG_IN + 8);
	::LoadBitmapFile("Interface\\none.tga", BITMAP_LOG_IN + 8);
	//::LoadBitmapFile("Interface\\login_me.tga", BITMAP_LOG_IN + 8);
	::LoadBitmapFile("Interface\\server_ex03.tga", BITMAP_LOG_IN + 11, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\server_ex01.tga", BITMAP_LOG_IN + 12);
	::LoadBitmapFile("Interface\\server_ex02.jpg", BITMAP_LOG_IN + 13, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\cr_mu_lo.tga", BITMAP_LOG_IN + 14, GL_LINEAR);
	::LoadBitmapFile("Interface\\button_close_navi.tga", BITMAP_START_RISE_LOGIN_WIN_NAVI, GL_LINEAR);
}
...
void ReleaseLogoSceneData()
{
	...
	DeleteBitmap(BITMAP_START_RISE_LOGIN_WIN_NAVI);
...
{
	LoadBitmapFile("Interface\\cha_id.tga", BITMAP_LOG_IN);
	LoadBitmapFile("Interface\\cha_bt.tga", BITMAP_LOG_IN + 1);
	LoadBitmapFile("Interface\\deco.tga", BITMAP_LOG_IN + 2);
	LoadBitmapFile("Interface\\b_create.tga", BITMAP_LOG_IN + 3);
	LoadBitmapFile("Interface\\server_menu_b_all.tga", BITMAP_LOG_IN + 4);
	LoadBitmapFile("Interface\\b_connect.tga", BITMAP_LOG_IN + 5);
	LoadBitmapFile("Interface\\b_delete.tga", BITMAP_LOG_IN + 6);
	LoadBitmapFile("Interface\\character_ex.tga", BITMAP_LOG_IN + 7);
	::LoadBitmapFile("Interface\\server_ex03.tga", BITMAP_LOG_IN + 11, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\server_ex01.tga", BITMAP_LOG_IN + 12);
	::LoadBitmapFile("Interface\\server_ex02.jpg", BITMAP_LOG_IN + 13, GL_NEAREST, GL_REPEAT);
	LoadBitmapFile("Effect\\Impack03.jpg", BITMAP_EXT_LOG_IN + 2);
	LoadBitmapFile("Logo\\chasellight.jpg", BITMAP_EFFECT);
...
void OpenBasicData(HDC hDC)
{
	...
	LoadBitmapFile("Interface\\Cursor.tga", BITMAP_CURSOR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorPush.tga", BITMAP_CURSOR + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorAttack.tga", BITMAP_CURSOR + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorGet.tga", BITMAP_CURSOR + 3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorTalk.tga", BITMAP_CURSOR + 4, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorRepair.tga", BITMAP_CURSOR + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorLeanAgainst.tga", BITMAP_CURSOR + 6, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorSitDown.tga", BITMAP_CURSOR + 7, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorDontMove.tga", BITMAP_CURSOR + 8, GL_LINEAR, GL_CLAMP_TO_EDGE);
	//interface
	LoadBitmapFile("Interface\\ok.jpg", BITMAP_INTERFACE + 11);
	LoadBitmapFile("Interface\\ok2.jpg", BITMAP_INTERFACE + 12);
	LoadBitmapFile("Interface\\cancel.jpg", BITMAP_INTERFACE + 13);
	LoadBitmapFile("Interface\\cancel2.jpg", BITMAP_INTERFACE + 14);
	LoadBitmapFile("Interface\\win_titlebar.jpg", BITMAP_INTERFACE_EX + 8);
	LoadBitmapFile("Interface\\win_button.tga", BITMAP_INTERFACE_EX + 9);
	LoadBitmapFile("Interface\\win_size.jpg", BITMAP_INTERFACE_EX + 10);
	LoadBitmapFile("Interface\\win_resize.tga", BITMAP_INTERFACE_EX + 11);
	LoadBitmapFile("Interface\\win_scrollbar.jpg", BITMAP_INTERFACE_EX + 12);
	LoadBitmapFile("Interface\\win_check.tga", BITMAP_INTERFACE_EX + 13);
	LoadBitmapFile("Interface\\win_mail.tga", BITMAP_INTERFACE_EX + 14);
	LoadBitmapFile("Interface\\win_mark.tga", BITMAP_INTERFACE_EX + 15);
	LoadBitmapFile("Interface\\win_letter.jpg", BITMAP_INTERFACE_EX + 16);
	LoadBitmapFile("Interface\\win_man.jpg", BITMAP_INTERFACE_EX + 17);
	LoadBitmapFile("Interface\\win_push.jpg", BITMAP_INTERFACE_EX + 18);
	LoadBitmapFile("Interface\\win_question.tga", BITMAP_INTERFACE_EX + 20);
	LoadBitmapFile("Local\\Webzenlogo.jpg", BITMAP_INTERFACE_EX + 22);
	...
	LoadBitmapFile("Interface\\CursorID.tga", BITMAP_INTERFACE_EX + 29);
	LoadBitmapFile("Interface\\bar.jpg", BITMAP_INTERFACE + 23);
	LoadBitmapFile("Interface\\back1.jpg", BITMAP_INTERFACE + 24);
	LoadBitmapFile("Interface\\back2.jpg", BITMAP_INTERFACE + 25);
	LoadBitmapFile("Interface\\back3.jpg", BITMAP_INTERFACE + 26);

	LoadBitmapFile("Effect\\Fire01.jpg", BITMAP_FIRE, GL_LINEAR, GL_CLAMP_TO_EDGE);			// GM3rdChangeUp, GMCrywolf1st,GMHellas,Kanturu 3rd
	LoadBitmapFile("Effect\\Fire02.jpg", BITMAP_FIRE + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);			// GM3rdChangeUp, GMCrywolf1st,GMHellas,Kanturu 3rd
	LoadBitmapFile("Effect\\Fire03.jpg", BITMAP_FIRE + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);			// GM3rdChangeUp, GMCrywolf1st,GMHellas,Kanturu 3rd
	LoadBitmapFile("Effect\\PoundingBall.jpg", BITMAP_POUNDING_BALL, GL_LINEAR, GL_CLAMP_TO_EDGE); // Kanturu 2nd
	LoadBitmapFile("Effect\\fi01.jpg", BITMAP_ADV_SMOKE, GL_LINEAR, GL_CLAMP_TO_EDGE);		// GM 3rd ChangeUp, CryingWolf2nd
	LoadBitmapFile("Effect\\fi02.tga", BITMAP_ADV_SMOKE + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);		// GM 3rd ChangeUp, CryingWolf2nd
	LoadBitmapFile("Effect\\fantaF.jpg", BITMAP_TRUE_FIRE, GL_LINEAR, GL_CLAMP_TO_EDGE);	// GM Aida, GMBattleCastle, ....
	LoadBitmapFile("Effect\\fantaB.jpg", BITMAP_TRUE_BLUE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\JointSpirit02.jpg", BITMAP_JOINT_SPIRIT2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Piercing.jpg", BITMAP_PIERCING, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Monster\\iui06.jpg", BITMAP_ROBE + 6);
	LoadBitmapFile("Effect\\Magic_b.jpg", BITMAP_MAGIC_EMBLEM);
	LoadBitmapFile("Player\\dark3chima3.tga", BITMAP_DARKLOAD_SKIRT_3RD, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Player\\kaa.tga", BITMAP_DARK_LOAD_SKIRT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\ShockWave.jpg", BITMAP_SHOCK_WAVE, GL_LINEAR);
	LoadBitmapFile("Effect\\Flame01.jpg", BITMAP_FLAME, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\flare01.jpg", BITMAP_LIGHT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Magic_Ground1.jpg", BITMAP_MAGIC, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Magic_Ground2.jpg", BITMAP_MAGIC + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Magic_Circle1.jpg", BITMAP_MAGIC + 2, GL_LINEAR);
#ifdef ASG_ADD_INFLUENCE_GROUND_EFFECT
	LoadBitmapFile("Effect\\guild_ring01.jpg", BITMAP_OUR_INFLUENCE_GROUND, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\enemy_ring02.jpg", BITMAP_ENEMY_INFLUENCE_GROUND, GL_LINEAR, GL_CLAMP_TO_EDGE);
#endif	// ASG_ADD_INFLUENCE_GROUND_EFFECT
	LoadBitmapFile("Effect\\Spark02.jpg", BITMAP_SPARK, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Spark03.jpg", BITMAP_SPARK + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\smoke01.jpg", BITMAP_SMOKE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\smoke02.tga", BITMAP_SMOKE + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\smoke05.tga", BITMAP_SMOKE + 4, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\blood01.tga", BITMAP_BLOOD, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\blood.tga", BITMAP_BLOOD + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Explotion01.jpg", BITMAP_EXPLOTION, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Skill\\twlighthik01.jpg", BITMAP_TWLIGHT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Skill\\2line_gost.jpg", BITMAP_2LINE_GHOST, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\damage01mono.jpg", BITMAP_DAMAGE_01_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\SwordEff_mono.jpg", BITMAP_SWORD_EFFECT_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\flamestani.jpg", BITMAP_FLAMESTANI, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Effect\\Spark.jpg", BITMAP_SPARK + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firehik02.jpg", BITMAP_FIRE_CURSEDLICH, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\totemgolem_leaf.tga", BITMAP_LEAF_TOTEMGOLEM, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\empact01.jpg", BITMAP_SUMMON_IMPACT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\loungexflow.jpg", BITMAP_SUMMON_SAHAMUTT_EXPLOSION, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\gostmark01.jpg", BITMAP_DRAIN_LIFE_GHOST, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\mzine_typer2.jpg", BITMAP_MAGIC_ZIN, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\shiny05.jpg", BITMAP_SHINY + 6, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\hikorora.jpg", BITMAP_ORORA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\lightmarks.jpg", BITMAP_LIGHT_MARKS, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\cursorpin01.jpg", BITMAP_TARGET_POSITION_EFFECT1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\cursorpin02.jpg", BITMAP_TARGET_POSITION_EFFECT2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\smokelines01.jpg", BITMAP_SMOKELINE1, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\smokelines02.jpg", BITMAP_SMOKELINE2, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\smokelines03.jpg", BITMAP_SMOKELINE3, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\lighting_mega01.jpg", BITMAP_LIGHTNING_MEGA1, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\lighting_mega02.jpg", BITMAP_LIGHTNING_MEGA2, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\lighting_mega03.jpg", BITMAP_LIGHTNING_MEGA3, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\firehik01.jpg", BITMAP_FIRE_HIK1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firehik03.jpg", BITMAP_FIRE_HIK3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firehik_mono01.jpg", BITMAP_FIRE_HIK1_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firehik_mono02.jpg", BITMAP_FIRE_HIK2_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firehik_mono03.jpg", BITMAP_FIRE_HIK3_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\clouds3.jpg", BITMAP_RAKLION_CLOUDS, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\icenightlight.jpg", BITMAP_IRONKNIGHT_BODY_BRIGHT, GL_LINEAR, GL_CLAMP_TO_EDGE);

	g_XmasEvent->LoadXmasEventEffect();

	LoadBitmapFile("Skill\\younghtest1.tga", BITMAP_GM_HAIR_1);
	LoadBitmapFile("Skill\\younghtest3.tga", BITMAP_GM_HAIR_3);
	LoadBitmapFile("Skill\\gmmzine.jpg", BITMAP_GM_AURORA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\explotion01mono.jpg", BITMAP_EXPLOTION_MONO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\Success_kantru.tga", BITMAP_KANTURU_SUCCESS, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\Failure_kantru.tga", BITMAP_KANTURU_FAILED, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\MonsterCount.tga", BITMAP_KANTURU_COUNTER, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Clud64.jpg", BITMAP_CLUD64, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\clouds.jpg", BITMAP_CLOUD, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Skill\\SwordEff.jpg", BITMAP_BLUE_BLUR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Impack03.jpg", BITMAP_IMPACT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\ScolTail.jpg", BITMAP_SCOLPION_TAIL, GL_LINEAR);
	LoadBitmapFile("Effect\\FireSnuff.jpg", BITMAP_FIRE_SNUFF, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\coll.jpg", BITMAP_SPOT_WATER, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\BowE.jpg", BITMAP_DS_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shockwave.jpg", BITMAP_DS_SHOCK, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\DinoE.jpg", BITMAP_EXPLOTION + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shiny01.jpg", BITMAP_SHINY, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shiny02.jpg", BITMAP_SHINY + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shiny03.jpg", BITMAP_SHINY + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\eye01.jpg", BITMAP_SHINY + 3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\ring.jpg", BITMAP_SHINY + 4, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shiny04.jpg", BITMAP_SHINY + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Chrome01.jpg", BITMAP_CHROME, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\blur01.jpg", BITMAP_BLUR, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\bab2.jpg", BITMAP_CHROME + 1, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\motion_blur.jpg", BITMAP_BLUR + 1, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\motion_blur_r.jpg", BITMAP_BLUR + 2, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\motion_mono.jpg", BITMAP_BLUR + 3, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\motion_blur_r3.jpg", BITMAP_BLUR + 6, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\gra.jpg", BITMAP_BLUR + 7, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\spinmark01.jpg", BITMAP_BLUR + 8, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\flamestani.jpg", BITMAP_BLUR + 9, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\sword_blur.jpg", BITMAP_BLUR + 10, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\joint_sword_red.jpg", BITMAP_BLUR + 11, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\motion_blur_r2.jpg", BITMAP_BLUR + 12, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\motion_blur_r3.jpg", BITMAP_BLUR + 13, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\blur02.jpg", BITMAP_BLUR2, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\lightning2.jpg", BITMAP_LIGHTNING + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Thunder01.jpg", BITMAP_ENERGY, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\Spark01.jpg", BITMAP_JOINT_SPARK, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\JointThunder01.jpg", BITMAP_JOINT_THUNDER, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\JointSpirit01.jpg", BITMAP_JOINT_SPIRIT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\JointLaser01.jpg", BITMAP_JOINT_ENERGY, GL_LINEAR);
	LoadBitmapFile("Effect\\JointEnergy01.jpg", BITMAP_JOINT_HEALING, GL_LINEAR);
	LoadBitmapFile("Effect\\JointLaser02.jpg", BITMAP_JOINT_LASER + 1, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Monster\\iui03.jpg", BITMAP_JANUSEXT, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Monster\\magic_H.tga", BITMAP_PHO_R_HAIR, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Item\\t_lower_14m.tga", BITMAP_PANTS_G_SOUL, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Skill\\Skull.jpg", BITMAP_SKULL, GL_NEAREST);
	LoadBitmapFile("Effect\\motion_blur_r2.jpg", BITMAP_JOINT_FORCE, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Effect\\Fire04.jpg", BITMAP_FIRECRACKER, GL_LINEAR);
	LoadBitmapFile("Effect\\Flare.jpg", BITMAP_FLARE, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Effect\\Chrome02.jpg", BITMAP_CHROME2, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Chrome03.jpg", BITMAP_CHROME3, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Chrome06.jpg", BITMAP_CHROME6, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Chrome07.jpg", BITMAP_CHROME7, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\energy01.jpg", BITMAP_CHROME_ENERGY, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\energy02.jpg", BITMAP_CHROME_ENERGY2, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Effect\\firecracker0001.jpg", BITMAP_FIRECRACKER0001, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0002.jpg", BITMAP_FIRECRACKER0002, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0003.jpg", BITMAP_FIRECRACKER0003, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0004.jpg", BITMAP_FIRECRACKER0004, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0005.jpg", BITMAP_FIRECRACKER0005, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0006.jpg", BITMAP_FIRECRACKER0006, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\firecracker0007.jpg", BITMAP_FIRECRACKER0007, GL_NEAREST, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Shiny05.jpg", BITMAP_SHINY + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Effect\\partCharge1\\bujuckline.jpg", BITMAP_LUCKY_SEAL_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Item\\nfm03.jpg", BITMAP_BLUECHROME, GL_NEAREST, GL_REPEAT);
	LoadBitmapFile("Effect\\flareBlue.jpg", BITMAP_FLARE_BLUE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\NSkill.jpg", BITMAP_FLARE_FORCE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Flare02.jpg", BITMAP_FLARE + 1, GL_LINEAR, GL_REPEAT);
	LoadBitmapFile("Monster\\King11.jpg", BITMAP_WHITE_WIZARD, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\Kni000.jpg", BITMAP_DEST_ORC_WAR0, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\Kni011.jpg", BITMAP_DEST_ORC_WAR1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\Kni022.jpg", BITMAP_DEST_ORC_WAR2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Skill\\pinkWave.jpg", BITMAP_PINK_WAVE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\flareRed.jpg", BITMAP_FLARE_RED, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Fire05.jpg", BITMAP_FIRE + 3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Hole.jpg", BITMAP_HOLE, GL_LINEAR, GL_REPEAT);
	//	LoadBitmapFile( "Monster\\mop011.jpg"        , BITMAP_OTHER_SKIN,    GL_LINEAR, GL_REPEAT );
	LoadBitmapFile("Effect\\WATERFALL1.jpg", BITMAP_WATERFALL_1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\WATERFALL2.jpg", BITMAP_WATERFALL_2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\WATERFALL3.jpg", BITMAP_WATERFALL_3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\WATERFALL4.jpg", BITMAP_WATERFALL_4, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\WATERFALL5.jpg", BITMAP_WATERFALL_5, GL_LINEAR, GL_CLAMP_TO_EDGE);
	/*
	LoadBitmapFile("Interface\\in_bar.tga"		, BITMAP_MVP_INTERFACE, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_bar2.jpg"		, BITMAP_MVP_INTERFACE + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_deco.tga"		, BITMAP_MVP_INTERFACE + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main.tga"		, BITMAP_MVP_INTERFACE + 3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_icon_bal1.tga"		, BITMAP_MVP_INTERFACE + 4, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_icon_dl1.tga"		, BITMAP_MVP_INTERFACE + 5, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_icon_dl2.tga"		, BITMAP_MVP_INTERFACE + 6, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_number1.tga"		, BITMAP_MVP_INTERFACE + 7, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_number2.tga"		, BITMAP_MVP_INTERFACE + 8, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main2.tga"		, BITMAP_MVP_INTERFACE + 9, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_failure.tga"		, BITMAP_MVP_INTERFACE + 10, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_success.tga"		, BITMAP_MVP_INTERFACE + 11, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\t_main.tga"		, BITMAP_MVP_INTERFACE + 12, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_no1.tga"		, BITMAP_MVP_INTERFACE + 13, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_no2.tga"		, BITMAP_MVP_INTERFACE + 14, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_no3.tga"		, BITMAP_MVP_INTERFACE + 15, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_ok1.tga"		, BITMAP_MVP_INTERFACE + 16, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_ok2.tga"		, BITMAP_MVP_INTERFACE + 17, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_ok3.tga"		, BITMAP_MVP_INTERFACE + 18, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_yes1.tga"		, BITMAP_MVP_INTERFACE + 19, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_yes2.tga"		, BITMAP_MVP_INTERFACE + 20, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_b_yes3.tga"		, BITMAP_MVP_INTERFACE + 21, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_main.tga"		, BITMAP_MVP_INTERFACE + 22, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_number1_1.tga"		, BITMAP_MVP_INTERFACE + 23, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_number2_1.tga"		, BITMAP_MVP_INTERFACE + 24, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\in_main_number0_2.tga"		, BITMAP_MVP_INTERFACE + 25, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_table.tga"		, BITMAP_MVP_INTERFACE + 26, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_rank.tga"		, BITMAP_MVP_INTERFACE + 27, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_D.tga"		, BITMAP_MVP_INTERFACE + 28, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_C.tga"		, BITMAP_MVP_INTERFACE + 29, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_B.tga"		, BITMAP_MVP_INTERFACE + 30, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_A.tga"		, BITMAP_MVP_INTERFACE + 31, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_S.tga"		, BITMAP_MVP_INTERFACE + 32, GL_LINEAR, GL_CLAMP_TO_EDGE);
	//icon_Rank_0
	LoadBitmapFile("Interface\\icon_Rank_0.tga"		, BITMAP_MVP_INTERFACE + 33, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_1.tga"		, BITMAP_MVP_INTERFACE + 34, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_2.tga"		, BITMAP_MVP_INTERFACE + 35, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_3.tga"		, BITMAP_MVP_INTERFACE + 36, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_4.tga"		, BITMAP_MVP_INTERFACE + 37, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_5.tga"		, BITMAP_MVP_INTERFACE + 38, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_6.tga"		, BITMAP_MVP_INTERFACE + 39, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_7.tga"		, BITMAP_MVP_INTERFACE + 40, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_8.tga"		, BITMAP_MVP_INTERFACE + 41, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_9.tga"		, BITMAP_MVP_INTERFACE + 42, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\icon_Rank_exp.tga"		, BITMAP_MVP_INTERFACE + 43, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\m_main_rank.tga"		, BITMAP_MVP_INTERFACE + 44, GL_LINEAR, GL_CLAMP_TO_EDGE);
	*/
	//m_main_rank
	//icon_Rank_exp
	LoadBitmapFile("Interface\\BattleSkill.tga", BITMAP_INTERFACE_EX + 34);
	LoadBitmapFile("Effect\\Flashing.jpg", BITMAP_FLASH, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\senatusmap.jpg", BITMAP_INTERFACE_EX + 35);
	LoadBitmapFile("Interface\\gate_button2.jpg", BITMAP_INTERFACE_EX + 36);
	LoadBitmapFile("Interface\\gate_button1.jpg", BITMAP_INTERFACE_EX + 37);
	LoadBitmapFile("Interface\\suho_button2.jpg", BITMAP_INTERFACE_EX + 38);
	LoadBitmapFile("Interface\\suho_button1.jpg", BITMAP_INTERFACE_EX + 39);
	LoadBitmapFile("Interface\\DoorCL.jpg", BITMAP_INTERFACE_EX + 40);
	LoadBitmapFile("Interface\\DoorOP.jpg", BITMAP_INTERFACE_EX + 41);
	//OpenJpeg( "Effect\\FireSnuff.jpg"       , BITMAP_FIRE_SNUFF,    GL_LINEAR, GL_CLAMP_TO_EDGE );
	LoadBitmapFile("Object31\\Flag.tga", BITMAP_INTERFACE_MAP + 0, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\CursorAttack2.tga", BITMAP_CURSOR2);
	LoadBitmapFile("Effect\\Cratered.tga", BITMAP_CRATER);
	LoadBitmapFile("Effect\\FormationMark.tga", BITMAP_FORMATION_MARK);
	LoadBitmapFile("Effect\\Plus.tga", BITMAP_PLUS);
	LoadBitmapFile("Effect\\eff_lighting.jpg", BITMAP_FENRIR_THUNDER);
	LoadBitmapFile("Effect\\eff_lightinga01.jpg", BITMAP_FENRIR_FOOT_THUNDER1);
	LoadBitmapFile("Effect\\eff_lightinga02.jpg", BITMAP_FENRIR_FOOT_THUNDER2);
	LoadBitmapFile("Effect\\eff_lightinga03.jpg", BITMAP_FENRIR_FOOT_THUNDER3);
	LoadBitmapFile("Effect\\eff_lightinga04.jpg", BITMAP_FENRIR_FOOT_THUNDER4);
	LoadBitmapFile("Effect\\eff_lightinga05.jpg", BITMAP_FENRIR_FOOT_THUNDER5);
	LoadBitmapFile("Interface\\Progress_Back.jpg", BITMAP_INTERFACE_EX + 42);
	LoadBitmapFile("Interface\\Progress.jpg", BITMAP_INTERFACE_EX + 43);
	LoadBitmapFile("Effect\\Inferno.jpg", BITMAP_INFERNO, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Lava.jpg", BITMAP_LAVA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Interface\\frame.tga", BITMAP_INTERFACE_MAP + 3);
	LoadBitmapFile("Interface\\i_attack.tga", BITMAP_INTERFACE_MAP + 4);
	LoadBitmapFile("Interface\\i_defense.tga", BITMAP_INTERFACE_MAP + 5);
	LoadBitmapFile("Interface\\i_wait.tga", BITMAP_INTERFACE_MAP + 6);
	LoadBitmapFile("Interface\\b_command01.jpg", BITMAP_INTERFACE_MAP + 8);
	LoadBitmapFile("Interface\\b_command02.jpg", BITMAP_INTERFACE_MAP + 9);
	LoadBitmapFile("Interface\\b_group02.jpg", BITMAP_INTERFACE_MAP + 10);
	LoadBitmapFile("Interface\\b_zoomout01.jpg", BITMAP_INTERFACE_MAP + 11);
	LoadBitmapFile("Interface\\hourglass.tga", BITMAP_INTERFACE_MAP + 7);
	LoadBitmapFile("Interface\\dot.tga", BITMAP_INTERFACE_EX + 44);
	LoadBitmapFile("Object9\\Impack03.jpg", BITMAP_LIGHT + 1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\buserbody_r.jpg", BITMAP_BERSERK_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\busersword_r.jpg", BITMAP_BERSERK_WP_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	//	LoadBitmapFile("Monster\\gigantiscorn_R.jpg", BITMAP_GIGANTIS_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\bladeeff2_r.jpg", BITMAP_BLADEHUNTER_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\illumi_R.jpg", BITMAP_TWINTAIL_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\prsona_R.jpg", BITMAP_PRSONA_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\prsonass_R.jpg", BITMAP_PRSONA_EFFECT2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("effect\\water.jpg", BITMAP_TWINTAIL_WATER, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\cra_04.jpg", BITMAP_LIGHT + 2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\Impack01.jpg", BITMAP_LIGHT + 3, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Interface\\message_ok_b_all.tga", BITMAP_BUTTON);
	::LoadBitmapFile("Interface\\loding_cancel_b_all.tga", BITMAP_BUTTON + 1);
	::LoadBitmapFile("Interface\\message_close_b_all.tga", BITMAP_BUTTON + 2);
	::LoadBitmapFile("Interface\\message_back.tga", BITMAP_MESSAGE_WIN);
	::LoadBitmapFile("Interface\\delete_secret_number.tga", BITMAP_MSG_WIN_INPUT);
	::LoadBitmapFile("Interface\\op1_stone.jpg", BITMAP_SYS_WIN, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\op1_back1.tga", BITMAP_SYS_WIN + 1);
	::LoadBitmapFile("Interface\\op1_back2.tga", BITMAP_SYS_WIN + 2);
	::LoadBitmapFile("Interface\\op1_back3.jpg", BITMAP_SYS_WIN + 3, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\op1_back4.jpg", BITMAP_SYS_WIN + 4, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\op1_b_all.tga", BITMAP_TEXT_BTN);
	::LoadBitmapFile("Interface\\op2_back1.tga", BITMAP_OPTION_WIN);
	::LoadBitmapFile("Interface\\op2_ch.tga", BITMAP_CHECK_BTN);
	::LoadBitmapFile("Interface\\op2_volume3.tga", BITMAP_SLIDER);
	::LoadBitmapFile("Interface\\op2_volume2.jpg", BITMAP_SLIDER + 1, GL_NEAREST, GL_REPEAT);
	::LoadBitmapFile("Interface\\op2_volume1.tga", BITMAP_SLIDER + 2);
	::LoadBitmapFile("Effect\\clouds2.jpg", BITMAP_EVENT_CLOUD, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\pin_lights.jpg", BITMAP_PIN_LIGHT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\deasuler_cloth.tga", BITMAP_DEASULER_CLOTH, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Item\\soketmagic_stape02.jpg", BITMAP_SOCKETSTAFF, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\lightmarks.jpg", BITMAP_LIGHTMARKS, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Effect\\lightmarks.jpg", BITMAP_LIGHTMARKS_FOREIGN, GL_LINEAR, GL_CLAMP_TO_EDGE);

	rUIMng.RenderTitleSceneUI(hDC, 1, 11);

	::LoadBitmapFile("Item\\partCharge1\\entrance_R.jpg", BITMAP_FREETICKET_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\juju_R.jpg", BITMAP_CHAOSCARD_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Item\\partCharge1\\monmark01a.jpg", BITMAP_LUCKY_SEAL_EFFECT43, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\monmark02a.jpg", BITMAP_LUCKY_SEAL_EFFECT44, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\monmark03a.jpg", BITMAP_LUCKY_SEAL_EFFECT45, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Item\\partCharge1\\bujuck01alpa.jpg", BITMAP_LUCKY_CHARM_EFFECT53, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\expensiveitem01_R.jpg", BITMAP_RAREITEM1_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\expensiveitem02a_R.jpg", BITMAP_RAREITEM2_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\expensiveitem02b_R.jpg", BITMAP_RAREITEM3_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\expensiveitem03a_R.jpg", BITMAP_RAREITEM4_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge1\\expensiveitem03b_R.jpg", BITMAP_RAREITEM5_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Item\\partCharge3\\alicecard_R.tga", BITMAP_CHARACTERCARD_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Item\\Ingameshop\\kacama_R.jpg", BITMAP_CHARACTERCARD_R_MA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\Ingameshop\\kacada_R.jpg", BITMAP_CHARACTERCARD_R_DA, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Item\\partCharge3\\jujug_R.jpg", BITMAP_NEWCHAOSCARD_GOLD_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge3\\jujul_R.jpg", BITMAP_NEWCHAOSCARD_RARE_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\partCharge3\\jujum_R.jpg", BITMAP_NEWCHAOSCARD_MINI_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Effect\\cherryblossom\\sakuras01.jpg", BITMAP_CHERRYBLOSSOM_EVENT_PETAL, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Effect\\cherryblossom\\sakuras02.jpg", BITMAP_CHERRYBLOSSOM_EVENT_FLOWER, GL_LINEAR, GL_CLAMP_TO_EDGE);

	::LoadBitmapFile("Object39\\k_effect_01.JPG", BITMAP_KANTURU_2ND_EFFECT1, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\deathbeamstone_R.jpg", BITMAP_ITEM_EFFECT_DBSTONE_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\hellhorn_R.jpg", BITMAP_ITEM_EFFECT_HELLHORN_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\phoenixfeather_R.tga", BITMAP_ITEM_EFFECT_PFEATHER_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\Deye_R.jpg", BITMAP_ITEM_EFFECT_DEYE_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Item\\wing3chaking2.jpg", BITMAP_ITEM_NIGHT_3RDWING_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("NPC\\lumi.jpg", BITMAP_CURSEDTEMPLE_NPC_MESH_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("item\\songko2_R.jpg", BITMAP_CURSEDTEMPLE_HOLYITEM_MESH_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("skill\\unitedsoldier_wing.tga", BITMAP_CURSEDTEMPLE_ALLIED_PHYSICSCLOTH);
	LoadBitmapFile("skill\\illusionistcloth.tga", BITMAP_CURSEDTEMPLE_ILLUSION_PHYSICSCLOTH);
	LoadBitmapFile("effect\\masker.jpg", BITMAP_CURSEDTEMPLE_EFFECT_MASKER, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("effect\\wind01.jpg", BITMAP_CURSEDTEMPLE_EFFECT_WIND, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Interface\\menu01_new2_SD.jpg", BITMAP_INTERFACE_EX + 46);

#ifdef ASG_ADD_GENS_SYSTEM
	std::string strFileName = "RISE\\MapName\\MapNameAddStrife.rise";
	::LoadBitmapFile(strFileName.c_str(), BITMAP_INTERFACE_EX + 47);
#endif	// ASG_ADD_GENS_SYSTEM

#ifdef ASG_ADD_GENS_MARK
	::LoadBitmapFile("Interface\\Gens_mark_D_new.tga", BITMAP_GENS_MARK_DUPRIAN);
	::LoadBitmapFile("Interface\\Gens_mark_V_new.tga", BITMAP_GENS_MARK_BARNERT);
#endif	// ASG_ADD_GENS_MARK

	LoadBitmapFile("Monster\\serufanarm_R.jpg", BITMAP_SERUFAN_ARM_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Monster\\serufanwand_R.jpg", BITMAP_SERUFAN_WAND_R, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("npc\\santa.jpg", BITMAP_GOOD_SANTA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("npc\\santa_baggage.jpg", BITMAP_GOOD_SANTA_BAGGAGE, GL_LINEAR, GL_CLAMP_TO_EDGE);

#ifdef PJH_ADD_PANDA_CHANGERING	
	LoadBitmapFile("Item\\pandabody_R.jpg", BITMAP_PANDABODY_R, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR
#endif //PJH_ADD_PANDA_CHANGERING

	LoadBitmapFile("Monster\\DGicewalker_body.jpg", BITMAP_DOPPELGANGER_ICEWALKER0, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR
	LoadBitmapFile("Monster\\DGicewalker_R.jpg", BITMAP_DOPPELGANGER_ICEWALKER1, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR
	LoadBitmapFile("Monster\\Snake1.jpg", BITMAP_DOPPELGANGER_SNAKE01, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR

	LoadBitmapFile("NPC\\goldboit.jpg", BITMAP_DOPPELGANGER_GOLDENBOX1, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR
	LoadBitmapFile("NPC\\goldline.jpg", BITMAP_DOPPELGANGER_GOLDENBOX2, GL_LINEAR, GL_LINEAR);	// OPENGL_ERROR

	//BITMAP_LIGHT_RED
	LoadBitmapFile("effect\\flare01_red.jpg", BITMAP_LIGHT_RED, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("effect\\gra.jpg", BITMAP_GRA, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("effect\\ring_of_gradation.jpg", BITMAP_RING_OF_GRADATION, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Interface\\InGameShop\\ingame_pack_check.tga", BITMAP_IGS_CHECK_BUTTON, GL_LINEAR);
	LoadBitmapFile("Monster\\AssassinLeader_body_R.jpg", BITMAP_ASSASSIN_EFFECT1, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_7_body.jpg", BITMAP_RAREITEM7, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_8_body.jpg", BITMAP_RAREITEM8, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_9_body.jpg", BITMAP_RAREITEM9, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_10_body.jpg", BITMAP_RAREITEM10, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_11_body.jpg", BITMAP_RAREITEM11, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\rareitem_ticket_12_body.jpg", BITMAP_RAREITEM12, GL_LINEAR, GL_CLAMP_TO_EDGE);

	LoadBitmapFile("Item\\partCharge8\\DoppelCard.jpg", BITMAP_DOPPLEGANGGER_FREETICKET, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\BarcaCard.jpg", BITMAP_BARCA_FREETICKET, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\partCharge8\\Barca7Card.jpg", BITMAP_BARCA7TH_FREETICKET, GL_LINEAR, GL_CLAMP_TO_EDGE);

#ifdef LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2
	LoadBitmapFile("Item\\ork_cham_R.jpg", BITMAP_ORK_CHAM_LAYER_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	//LoadBitmapFile("Item\\maple_cham_R.jpg",			BITMAP_MAPLE_CHAM_LAYER_R,			GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmapFile("Item\\goldenork_cham_R.jpg", BITMAP_GOLDEN_ORK_CHAM_LAYER_R, GL_LINEAR, GL_CLAMP_TO_EDGE);
	//LoadBitmapFile("Item\\goldenmaple_cham_R.jpg",	BITMAP_GOLDEN_MAPLE_CHAM_LAYER_R,	GL_LINEAR, GL_CLAMP_TO_EDGE);
#endif //LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2

#ifdef ASG_ADD_KARUTAN_MONSTERS
	::LoadBitmapFile("Monster\\BoneSE.jpg", BITMAP_BONE_SCORPION_SKIN_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\KryptaBall2.jpg", BITMAP_KRYPTA_BALL_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\bora_golem_effect.jpg", BITMAP_CONDRA_SKIN_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\bora_golem2_effect.jpg", BITMAP_CONDRA_SKIN_EFFECT2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\king_golem01_effect.jpg", BITMAP_NARCONDRA_SKIN_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\king_golem02_effect.jpg", BITMAP_NARCONDRA_SKIN_EFFECT2, GL_LINEAR, GL_CLAMP_TO_EDGE);
	::LoadBitmapFile("Monster\\king_golem03_effect.jpg", BITMAP_NARCONDRA_SKIN_EFFECT3, GL_LINEAR, GL_CLAMP_TO_EDGE);
#endif	// ASG_ADD_KARUTAN_MONSTERS

#ifdef ASG_ADD_KARUTAN_NPC
	::LoadBitmapFile("NPC\\voloE.jpg", BITMAP_VOLO_SKIN_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
#endif	// ASG_ADD_KARUTAN_NPC

	LoadBitmapFile("RISE\\Item\\Texture\\black_gold_crom05.jpg", BITMAP_black_gold_crom05, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\absolute02_swordrender.jpg", BITMAP_absolute02_swordrender, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\black_gold_crom03.jpg", BITMAP_black_gold_crom03, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\gra.jpg", BITMAP_gra, GL_LINEAR, GL_CLAMP_TO_EDGE, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\gra2.jpg", BITMAP_gra2, GL_LINEAR, GL_CLAMP_TO_EDGE, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\gra3.jpg", BITMAP_gra3, GL_LINEAR, GL_CLAMP_TO_EDGE, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\ground_wind2.jpg", BITMAP_ground_wind2, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\magicianwing4da_render.jpg", BITMAP_magicianwing4da_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\dknightwing4db_render.jpg", BITMAP_dknightwing4db_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\elfwing4da_render01.jpg", BITMAP_elfwing4da_render01, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\elfwing4da_render02.jpg", BITMAP_elfwing4da_render02, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\alicewing4db_render.jpg", BITMAP_alicewing4db_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\soulknightswordB_render.jpg", BITMAP_soulknightswordB_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\soulknightswordA_render.jpg", BITMAP_soulknightswordA_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\soulmagicswordA_render.jpg", BITMAP_soulmagicswordA_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\hs_soularcherbowA01_render.jpg", BITMAP_hs_soularcherbowA01_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_SoulWizardStaff01_render.jpg", BITMAP_sd_SoulWizardStaff01_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_soulsummonerstickA02_render.jpg", BITMAP_sd_soulsummonerstickA02_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\hs_blueeyeknightA02_render.jpg", BITMAP_hs_blueeyeknightA02_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\hs_blueeyeknightA03r.jpg", BITMAP_hs_blueeyeknightA03r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\blueeyemagicsword02r.jpg", BITMAP_blueeyemagicsword02r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\blueeyemagicsword03r.jpg", BITMAP_blueeyemagicsword03r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\alpha_line2.jpg", BITMAP_alpha_line2, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\blueeyesceptre_render.jpg", BITMAP_blueeyesceptre_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\Blue_eyebow02r.jpg", BITMAP_Blue_eyebow02r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\blue_eye_ws02_render.jpg", BITMAP_blue_eye_ws02_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\bsummoners02_render.jpg", BITMAP_bsummoners02_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\Blueeyeclaw_render.jpg", BITMAP_Blueeyeclaw_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\fire_15frame_mono.jpg", BITMAP_fire_15frame_mono, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\mace_mastr02s_render.jpg", BITMAP_mace_mastr02s_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\Darkangel_StaffA_render.jpg", BITMAP_Darkangel_StaffA_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_soullance01_render.jpg", BITMAP_sd_soullance01_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\hs_blueeyelanceA02r.jpg", BITMAP_hs_blueeyelanceA02r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\mace5master02_render.jpg", BITMAP_mace5master02_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\runewing3db_render.jpg", BITMAP_runewing3db_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\runewing4db_render.jpg", BITMAP_runewing4db_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\runewing4dc_star01.jpg", BITMAP_runewing4dc_star01, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\runewing4dc_star02.jpg", BITMAP_runewing4dc_star02, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\season_wing_period_render.jpg", BITMAP_season_wing_period_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\firek_sq_mono.jpg", BITMAP_firek_sq_mono, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\BoodAnelsword_render.jpg", BITMAP_BoodAnelsword_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\BAGsword_render.jpg", BITMAP_BAGsword_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\bloodclaw2_render.jpg", BITMAP_bloodclaw2_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\booldangel_sapterr.jpg", BITMAP_booldangel_sapterr, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\blood_bow_render.jpg", BITMAP_blood_bow_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\bloodangelstaff01_render.jpg", BITMAP_bloodangelstaff01_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\bloodangelstaff02.jpg", BITMAP_bloodangelstaff02, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\bloodangelstick01_render.jpg", BITMAP_bloodangelstick01_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\AngelicBow_overlay.jpg", BITMAP_AngelicBow_overlay, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_wing3C_render.jpg", BITMAP_sd_wing3C_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_wing3B_render.jpg", BITMAP_sd_wing3B_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\K_StylizedMoon_E02.jpg", BITMAP_K_StylizedMoon_E02, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\K_StylizedMoon_SwordB02_Render01.jpg", BITMAP_K_StylizedMoon_SwordB02_Render01, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\K_StylizedMoon_SwordB02_Render02.jpg", BITMAP_K_StylizedMoon_SwordB02_Render02, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_wing4D_render.jpg", BITMAP_sd_wing4D_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\sd_wing4C_Red_render.jpg", BITMAP_sd_wing4C_Red_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("RISE\\Item\\Texture\\growlancerwing4db_render.jpg", BITMAP_growlancerwing4db_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\flame_chrome.jpg", BITMAP_flame_chrome, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\line_of_big_R.jpg", BITMAP_line_of_big_R, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\black_gold_crom04_mono.jpg", BITMAP_black_gold_crom04_mono, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\black_gold_crom04.jpg", BITMAP_black_gold_crom04, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\MMsSs2_r.jpg", BITMAP_MMsSs2_r, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\wings_van_render.jpg", BITMAP_wings_van_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\benzia_wing4_render.jpg", BITMAP_benzia_wing4_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\benzia_wing4a_render.jpg", BITMAP_benzia_wing4a_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\benzia_wing4b_render.jpg", BITMAP_benzia_wing4b_render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\0120.jpg", BITMAP_0120, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\0143.jpg", BITMAP_0143, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\ChaWing_renderbright.jpg", BITMAP_ChaWing_renderbright, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\KCWWingP_Render.jpg", BITMAP_KCWWingP_Render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\KCWWingR_Render.jpg", BITMAP_KCWWingR_Render, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\black_gold_crom02_mono.jpg", BITMAP_black_gold_crom02_mono, GL_LINEAR, GL_REPEAT, false, false);
	LoadBitmapFile("Effect\\black_gold_crom02.jpg", BITMAP_black_gold_crom02, GL_LINEAR, GL_REPEAT, false, false);
	...
	sprintf(Text, "Data\\RISE\\Config%s\\Dialog.bmd", ReadLang);
	OpenDialogFile(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\Item.bmd", ReadLang);
	OpenItemScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\MoveReq.bmd", ReadLang);
	SEASON3B::CMoveCommandData::OpenMoveReqScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\NpcName.txt", ReadLang);
	OpenMonsterScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\Quest.bmd", ReadLang);
	g_csQuest.OpenQuestScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\Skill.bmd", ReadLang);
	OpenSkillScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\SocketItem.bmd", ReadLang);
	...
	sprintf(Text, "Data\\RISE\\Config%s\\ToolTipText.txt", ReadLang);
	pStrText.Load(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\TextHtml.txt", ReadLang);
	pGlobalText.Load(Text);

	g_QuestMng.LoadQuestScript();

	sprintf(Text, "Data\\RISE\\Config%s\\Gate.bmd", ReadLang);
	OpenGateScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\Filter.bmd", ReadLang);
	OpenFilterFile(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\FilterName.bmd", ReadLang);
	OpenNameFilterFile(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\MonsterSkill.bmd", ReadLang);
	OpenMonsterSkillScript(Text);

	sprintf(Text, "Data\\RISE\\Config%s\\MasterSkillTreeData.bmd", ReadLang);
	g_pMasterSkillTreeInterface->OpenMasterSkillTreeData(Text);
	sprintf(Text, "Data\\RISE\\Config%s\\MasterSkillTooltip.bmd", ReadLang);
...
void OpenTextData()
{
	...
	sprintf(Text, "Data\\RISE\\Config%s\\Text.bmd", ReadLang);
