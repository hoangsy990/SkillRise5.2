...
LPDIRECTSOUNDBUFFER     g_lpDSBuffer[rise::pegasus::kSoundBufferCapacity][MAX_CHANNEL];
LPDIRECTSOUND3DBUFFER   g_lpDS3DBuffer[rise::pegasus::kSoundBufferCapacity][MAX_CHANNEL];

DWORD                   g_dwBufferBytes;

OBJECT* Object3DSound[rise::pegasus::kSoundBufferCapacity][MAX_CHANNEL];
char                    BufferName[rise::pegasus::kSoundBufferCapacity][64];
int                     BufferChannel[rise::pegasus::kSoundBufferCapacity];
int                     MaxBufferChannel[rise::pegasus::kSoundBufferCapacity];
bool                    Enable3DSound[rise::pegasus::kSoundBufferCapacity];

LPKSPROPERTYSET		    g_lpPropertySet[rise::pegasus::kSoundBufferCapacity];

waveIO* wavefile = NULL;
...
HRESULT InitDirectSound(HWND hDlg)
{
	...
	else
	{
		...
		for (int i = 0; i < rise::pegasus::kSoundBufferCapacity; ++i)
		{
...
VOID LoadWaveFile(int Buffer, TCHAR* strFileName, int MaxChannel, bool Enable)
{
	if (!g_EnableSound)
		return;
	if (Buffer < 0 || Buffer >= rise::pegasus::kSoundBufferCapacity)
		return;
...
HRESULT ReleaseBuffer(int Buffer)
{
	...
	if (Buffer < 0 || Buffer >= rise::pegasus::kSoundBufferCapacity) return false;
...
{
	const bool pegasusSound = Buffer >= rise::pegasus::kSoundBufferBase &&
		Buffer < rise::pegasus::kSoundBufferCapacity;
	if (!g_EnableSound)
	{
		if (pegasusSound)
			rise::pegasus::TraceRuntime("SOUND_PLAY", Buffer,
				Buffer - rise::pegasus::kSoundBufferBase, "DISABLED");
		return false;
	}
	if (Buffer < 0 || Buffer >= rise::pegasus::kSoundBufferCapacity) return false;
	...
	if (NULL == g_lpDSBuffer[Buffer][BufferChannel[Buffer]])
	{
		if (pegasusSound)
			rise::pegasus::TraceRuntime("SOUND_PLAY", Buffer,
				Buffer - rise::pegasus::kSoundBufferBase, "BUFFER_MISSING");
		return E_FAIL;
	}
	...
	if (FAILED(hr = g_lpDSBuffer[Buffer][BufferChannel[Buffer]]->Play(0, 0, dwLooped)))
	{
		if (pegasusSound)
			rise::pegasus::TraceRuntime("SOUND_PLAY", Buffer,
				Buffer - rise::pegasus::kSoundBufferBase, "PLAY_FAILED");
	...
	if (pegasusSound)
		rise::pegasus::TraceRuntime("SOUND_PLAY", Buffer,
			Buffer - rise::pegasus::kSoundBufferBase, "PASS");
	return S_OK;
}
