...
bool AttackCharacterRange(int Index, vec3_t Position, float Range, BYTE Serial, short PKKey, WORD SkillSerialNum)
{
    ...
    if (Count > 0)
    {
        ...
        else
        {
            ...
            {
                if (Skill == AT_SKILL_REDUCEDEFENSE || Skill == MASTER_SKILL_ADD_FIRE_SLASH_IMPROVED || Skill == MASTER_SKILL_ADD_FIRE_SLASH_ENHANCED)
                {
                   // return true;
                }
...
{
	if (o && o->Live && rise::pegasus::IsElementalChargeChild(o->Type))
	{
		rise::pegasus::TraceRuntime("ELEMENTAL_CHILD_DESTROY", o->Type,
			o->SubType, "RELEASED");
	}
	if (o && o->Live && o->Type >= MAX_MODELS &&
		rise::pegasus::IsRuntimeModelId(
			static_cast<unsigned int>(o->Type), MAX_MODELS))
	{
		rise::pegasus::TraceRuntime("EFFECT_DESTROY", o->Type,
			o->SubType, "RELEASED");
		rise::pegasus::TraceRuntime("EFFECT_LIFECYCLE_DESTROY", o->Type,
			o->Type - MAX_MODELS, "RELEASED");
	}
	else if (o && o->Live && o->Type >= MODEL_PEGASUS_SWORD_BLOW_BIG &&
		o->Type <= MODEL_PEGASUS_SWORD_BLOW_TARGET2)
	{
		rise::pegasus::TraceRuntime("SWORD_BLOW_DESTROY", o->Type,
			o->Type - MODEL_PEGASUS_SWORD_BLOW_BIG, "RELEASED");
	}
	switch (o->Type)
    {
...
{
    const bool traceSwordBlowModel =
        Type >= MODEL_PEGASUS_SWORD_BLOW_BIG &&
        Type <= MODEL_PEGASUS_SWORD_BLOW_TARGET2;
    const bool tracePegasusModel = traceSwordBlowModel ||
        (Type >= MAX_MODELS &&
        rise::pegasus::IsRuntimeModelId(static_cast<unsigned int>(Type), MAX_MODELS));
    bool pegasusModel = true;
    bool pegasusModelReady = true;
    if (Type == rise::pegasus::kSwordBlowArchangelWingModel)
        pegasusModelReady = rise::pegasus::EnsureSwordBlowArchangelWingModel();
    else if (Type >= MODEL_PEGASUS_SWORD_BLOW_BIG && Type <= MODEL_PEGASUS_SWORD_BLOW_TARGET2)
        pegasusModelReady = rise::pegasus::EnsureSwordBlowModel(Type);
    else if (Type >= rise::pegasus::kSwordWrathRootModel && Type <= rise::pegasus::kSwordWrathLayerModel)
        pegasusModelReady = rise::pegasus::EnsureSwordWrathModel(Type);
    else if (Type >= rise::pegasus::kSpiralChargeFirstModel && Type <= rise::pegasus::kSpiralChargeLastModel)
        pegasusModelReady = rise::pegasus::EnsureSpiralChargeModel(Type);
    else if (rise::pegasus::IsCrusherChargeModelId(Type))
        pegasusModelReady = rise::pegasus::EnsureCrusherChargeModel(Type);
    else if (Type == rise::pegasus::kShiningBirdModel)
        pegasusModelReady = rise::pegasus::EnsureShiningBirdModel();
    else if (Type >= rise::pegasus::kChaosBladeFirstModel && Type <= rise::pegasus::kChaosBladeLastModel)
        pegasusModelReady = rise::pegasus::EnsureChaosBladeModel(Type);
    else if (Type >= rise::pegasus::kDragonViolentFirstModel && Type <= rise::pegasus::kDragonViolentLastModel)
        pegasusModelReady = rise::pegasus::EnsureDragonViolentModel(Type);
    else if (Type == rise::pegasus::kElementalChargeModel)
        pegasusModelReady = rise::pegasus::EnsureElementalChargeModel();
    else if (Type >= rise::pegasus::kHavocFirstModel && Type <= rise::pegasus::kHavocLastModel)
        pegasusModelReady = rise::pegasus::EnsureHavocSpearModel(Type);
    else if (Type >= rise::pegasus::kSpearstormFirstModel && Type <= rise::pegasus::kSpearstormLastModel)
        pegasusModelReady = rise::pegasus::EnsureSpearstormModel(Type);
    else if (Type >= rise::pegasus::kRainingArrowFirstModel && Type <= rise::pegasus::kRainingArrowLastModel)
        pegasusModelReady = rise::pegasus::EnsureRainingArrowModel(Type);
    else if (Type >= rise::pegasus::kDeathsideFirstModel && Type <= rise::pegasus::kDeathsideLastModel)
        pegasusModelReady = rise::pegasus::EnsureDeathsideModels();
    else
        pegasusModel = false;
    if (pegasusModel && !pegasusModelReady)
    {
        rise::pegasus::TraceRuntime("MODEL_ENSURE", Type, SubType, "FAIL");
        return;
    }
    for (int icntEffect = 0; icntEffect < MAX_EFFECTS; icntEffect++)
    {
        ...
        if (!o->Live)
        {
            ...
            switch (Type)
            {
            case rise::pegasus::kElementalFlare03Bitmap:
            case rise::pegasus::kElementalShockwaveBitmap:
                rise::pegasus::InitializeElementalChargeChild(*o);
                break;
            case rise::pegasus::kSwordBlowArchangelWingModel:
                rise::pegasus::InitializeSwordBlowArchangelWings(*o);
                break;
            case MODEL_PEGASUS_SWORD_BLOW_BIG:
                rise::pegasus::CreateSwordBlowChildren(*o);
                break;
            case MODEL_PEGASUS_SWORD_BLOW_SMALL:
            {
                const float lifetime = static_cast<float>(static_cast<int>(
                    (0.2f + static_cast<float>(rand() % 9) / 10.0f) * 20.0f));
                const float speed = 40.0f + static_cast<float>(rand() % 71);
                rise::pegasus::InitializeSwordBlowSmallSword(*o, lifetime, speed);
                break;
            }
            case MODEL_PEGASUS_SWORD_BLOW_TARGET1:
            case MODEL_PEGASUS_SWORD_BLOW_TARGET2:
                rise::pegasus::InitializeSwordBlowTarget(*o);
                break;
            case MODEL_PEGASUS_SWORD_BLOW_PLANE:
                rise::pegasus::InitializeSwordBlowPlane(*o);
                break;
            case MODEL_PEGASUS_SWORD_BLOW_RING:
                rise::pegasus::InitializeSwordBlowRing(*o);
                break;
            case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT1:
            case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT2:
            case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT3:
                rise::pegasus::InitializeSwordBlowMultishot(*o);
                break;
            case rise::pegasus::kSwordWrathRootModel:
                rise::pegasus::InitializeSwordWrath(*o);
                rise::pegasus::CreateSwordWrathLayers(*o);
                break;
            case rise::pegasus::kSwordWrathLayerModel:
                rise::pegasus::InitializeSwordWrath(*o);
                break;
            case rise::pegasus::kSpiralChargeRootModel:
                rise::pegasus::InitializeSpiralCharge(*o);
                rise::pegasus::CreateSpiralChargeLayers(*o);
                break;
            case rise::pegasus::kSpiralChargeFlareModel:
            case rise::pegasus::kSpiralChargeShockwaveModel:
            case rise::pegasus::kSpiralChargeSteamModel:
            case rise::pegasus::kSpiralChargeSmokeModel:
            case rise::pegasus::kSpiralChargeAnchorModel:
                rise::pegasus::InitializeSpiralCharge(*o);
                break;
            case rise::pegasus::kCrusherChargeCoreModel:
            case rise::pegasus::kCrusherChargeRingModel:
            case rise::pegasus::kCrusherChargeShardModel:
            case rise::pegasus::kCrusherChargeAuraModel:
                rise::pegasus::InitializeCrusherCharge(*o);
                break;
            case rise::pegasus::kShiningBirdModel:
                rise::pegasus::InitializeShiningBird(*o);
                break;
            case rise::pegasus::kDexBoosterAuraModel:
            case rise::pegasus::kDexBoosterOrchestratorModel:
            case rise::pegasus::kDexBoosterGroundModel:
            case rise::pegasus::kDexBoosterFlareModel:
                rise::pegasus::InitializeDexBooster(*o);
                break;
            case rise::pegasus::kChaosBladeRootModel:
            case rise::pegasus::kChaosBladeArcModel:
            case rise::pegasus::kChaosBladeCrackModel:
                rise::pegasus::InitializeChaosBlade(*o);
                break;
            case rise::pegasus::kDragonViolentBaseModel:
            case rise::pegasus::kDragonViolentHeadModel:
            case rise::pegasus::kDragonViolentCrackModel:
            case rise::pegasus::kDragonViolentWindModel:
            case rise::pegasus::kDragonViolentLayerModel:
            case rise::pegasus::kDragonViolentEndModel:
                rise::pegasus::InitializeDragonViolent(*o);
                break;
            case rise::pegasus::kElementalChargeModel:
                rise::pegasus::InitializeElementalCharge(*o);
                break;
            case rise::pegasus::kHavocNovaModel:
            case rise::pegasus::kHavocLineModel:
            case rise::pegasus::kHavocShockwaveModel:
            case rise::pegasus::kHavocRootModel:
            case rise::pegasus::kHavocImpactModel:
            case rise::pegasus::kHavocEndModel:
                rise::pegasus::InitializeHavocSpear(*o);
                break;
            case rise::pegasus::kSpearstormRootModel:
            case rise::pegasus::kSpearstormBladeModel:
            case rise::pegasus::kSpearstormCrackModel:
                rise::pegasus::InitializeSpearstorm(*o);
                break;
            case rise::pegasus::kRainingArrowRootModel:
            case rise::pegasus::kRainingArrowFollowModel:
            case rise::pegasus::kRainingArrowImpactModel:
            case rise::pegasus::kRainingArrowGroundModel:
            case rise::pegasus::kRainingArrowRing01Model:
            case rise::pegasus::kRainingArrowRing02Model:
            case rise::pegasus::kRainingArrowRing03Model:
            case rise::pegasus::kRainingArrowRing04Model:
            case rise::pegasus::kRainingArrowRing05Model:
            case rise::pegasus::kRainingArrowRing06Model:
            case rise::pegasus::kRainingArrowRing07Model:
            case rise::pegasus::kRainingArrowRing08Model:
                rise::pegasus::InitializeRainingArrow(*o);
                break;
            case rise::pegasus::kDeathsideReaperModel:
            case rise::pegasus::kDeathsideHeadModel:
            case rise::pegasus::kDeathsideScythe01Model:
            case rise::pegasus::kDeathsideScythe02Model:
            case rise::pegasus::kDeathsideScythe03Model:
                rise::pegasus::InitializeDeathside(*o);
                break;
            case MODEL_DRAGON:
            {
            ...
            case MODEL_DESAIR:
            {
                ...
                else if (FPS_ANIMATION_FACTOR > 0.5f)
                {
                    o->Scale = 0.8f;
                }
                else
            ...
            case MODEL_LIGHTNING_SHOCK:
            {
                ...
                else if (o->SubType == 1)
                {
                    ...
                    for (int i = 0; i < 5; ++i)
                    {
                        Vector(0.f, 230.f, 0.f, vDirection);
            ...
            case BITMAP_GATHERING:
                ...
                case 0:
                    ...
                    AngleMatrix(o->Angle, Matrix);
                    VectorRotate(p1, Matrix, p2);
                    VectorAddScaled(o->Position, p2, o->Position, 1.0f);
                        ...
                        if (Type == MODEL_MULTI_SHOT2)
                        {
                            ...
                            else if (Type == MODEL_DARK_SCREAM || Type == MODEL_DARK_SCREAM_FIRE)
                            {
                                ...
                                if (Type == MODEL_DARK_SCREAM)
                                {
                                    ...
                                    CreateParticleFpsChecked(BITMAP_BLUE_BLUR, o->Position, o->Angle, Light, 1, 1.f);   //fpschecked fix hoa am DL aLongfix5)
                                }
                            }
            ...
            case MODEL_BLIZZARD:
                if (o->SubType == 0 || o->SubType == 2)
                {
                    ...
                    rangeX = 400; rangeY = 200; rangeZ = 700;
                    o->Scale = 0.5f;
            ...
            case MODEL_PIER_PART:
                ...
                else if (o->SubType == 1)
                {
                    ...
                    o->Scale = 0.65f;
                    o->Alpha = 0.9f;
                    Vector(rand() % 4 - 2.f, rand() % 4 - 2.f, rand() % 4 - 2.f, o->Direction);
            ...
            case MODEL_FEATHER:
            {
                ...
                case 3:
                {
                    ...
                    o->EyeRight[0] = (float)(rand() % 20 - 10);
                    o->EyeRight[1] = (float)(rand() % 20 - 10);
                    o->EyeRight[2] = (float)(rand() % 20 - 10);
                }break;
                }
            }break;
            ...
            if (tracePegasusModel)
            {
                rise::pegasus::TraceRuntime(traceSwordBlowModel ?
                    "SWORD_BLOW_CREATE" : "EFFECT_CREATE", Type,
                    traceSwordBlowModel ? Type - MODEL_PEGASUS_SWORD_BLOW_BIG : SubType,
                    "ALLOCATED");
                if (!traceSwordBlowModel)
                    rise::pegasus::TraceRuntime("EFFECT_LIFECYCLE_CREATE", Type,
                        Type - MAX_MODELS, "ALLOCATED");
            }
            return;
        }
    }
    ...
    else
    {
        if (rand_fps_check(2))
            if (o->Owner == &Hero->Object)
            {
                ...
                if ((Skill == AT_SKILL_PIERCING || Skill == MASTER_SKILL_ADD_PENETRATION_IMPROVED) && o->Type == MODEL_ARROW_BOMB)
                {
                ...
                if ((Skill == AT_SKILL_PIERCING || Skill == MASTER_SKILL_ADD_PENETRATION_IMPROVED
...
void MoveEffect(OBJECT* o, int iIndex)
{
    ...
    case rise::pegasus::kElementalFlare03Bitmap:
    case rise::pegasus::kElementalShockwaveBitmap:
        rise::pegasus::UpdateElementalChargeChild(*o,
            FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kSwordBlowArchangelWingModel:
        rise::pegasus::UpdateSwordBlowArchangelWings(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_BIG:
        rise::pegasus::UpdateSwordBlowBigSword(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_SMALL:
        rise::pegasus::UpdateSwordBlowSmallSword(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_TARGET1:
    case MODEL_PEGASUS_SWORD_BLOW_TARGET2:
        rise::pegasus::UpdateSwordBlowTarget(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_PLANE:
        rise::pegasus::UpdateSwordBlowPlane(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_RING:
        rise::pegasus::UpdateSwordBlowRing(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT1:
    case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT2:
    case MODEL_PEGASUS_SWORD_BLOW_MULTISHOT3:
        rise::pegasus::UpdateSwordBlowMultishot(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kSwordWrathRootModel:
    case rise::pegasus::kSwordWrathLayerModel:
        rise::pegasus::UpdateSwordWrath(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kSpiralChargeRootModel:
    case rise::pegasus::kSpiralChargeFlareModel:
    case rise::pegasus::kSpiralChargeShockwaveModel:
    ...
    case rise::pegasus::kSpiralChargeSmokeModel:
    case rise::pegasus::kSpiralChargeAnchorModel:
        rise::pegasus::UpdateSpiralCharge(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kCrusherChargeCoreModel:
    case rise::pegasus::kCrusherChargeRingModel:
    case rise::pegasus::kCrusherChargeShardModel:
    case rise::pegasus::kCrusherChargeAuraModel:
        rise::pegasus::UpdateCrusherCharge(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kShiningBirdModel:
        rise::pegasus::UpdateShiningBird(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kDexBoosterAuraModel:
    case rise::pegasus::kDexBoosterOrchestratorModel:
    case rise::pegasus::kDexBoosterGroundModel:
    case rise::pegasus::kDexBoosterFlareModel:
        rise::pegasus::UpdateDexBooster(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kChaosBladeRootModel:
    case rise::pegasus::kChaosBladeArcModel:
    case rise::pegasus::kChaosBladeCrackModel:
        rise::pegasus::UpdateChaosBlade(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kDragonViolentBaseModel:
    case rise::pegasus::kDragonViolentHeadModel:
    case rise::pegasus::kDragonViolentCrackModel:
    ...
    case rise::pegasus::kDragonViolentLayerModel:
    case rise::pegasus::kDragonViolentEndModel:
        rise::pegasus::UpdateDragonViolent(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kElementalChargeModel:
        rise::pegasus::UpdateElementalCharge(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kHavocNovaModel:
    case rise::pegasus::kHavocLineModel:
    case rise::pegasus::kHavocShockwaveModel:
    ...
    case rise::pegasus::kHavocImpactModel:
    case rise::pegasus::kHavocEndModel:
        rise::pegasus::UpdateHavocSpear(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kSpearstormRootModel:
    case rise::pegasus::kSpearstormBladeModel:
    case rise::pegasus::kSpearstormCrackModel:
        rise::pegasus::UpdateSpearstorm(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kRainingArrowRootModel:
    case rise::pegasus::kRainingArrowFollowModel:
    case rise::pegasus::kRainingArrowImpactModel:
    ...
    case rise::pegasus::kRainingArrowRing07Model:
    case rise::pegasus::kRainingArrowRing08Model:
        rise::pegasus::UpdateRainingArrow(*o, FPS_ANIMATION_FACTOR);
        break;
    case rise::pegasus::kDeathsideReaperModel:
    case rise::pegasus::kDeathsideHeadModel:
    case rise::pegasus::kDeathsideScythe01Model:
    case rise::pegasus::kDeathsideScythe02Model:
    case rise::pegasus::kDeathsideScythe03Model:
        rise::pegasus::UpdateDeathside(*o, FPS_ANIMATION_FACTOR);
        break;
    case MODEL_DRAGON:
    {
        vec3_t vTemp;
            ...
            if (o->SubType == 1)
            {
                ...
                if (o->LifeTime >= 3 && (int)o->LifeTime % 2 == 0)
                {
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        o->Owner->Angle[2] = Angle[2] + 22.f;
                        break;
                    case 2:
                        o->Owner->Angle[2] = Angle[2] + 37.f;
                        break;
                    case 3:
                        o->Owner->Angle[2] = Angle[2] - 7.f;
                        break;
    ...
    case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT:
        ...
        else if (o->SubType == 1)
        {
            if (rand_fps_check(2))
            {
                ...
                if (iNumBones <= 0) break; // RISE Fix Crash Login BONE
    ...
    case BITMAP_MAGIC:
        ...
        else if (o->SubType == 12)
        {
            ...
            o->Scale += (o->Alpha * 1.7f) * FPS_ANIMATION_FACTOR;
            if (o->Scale > 5.0f)
            {
                //o->Scale = 5.0f;
    ...
    case MODEL_DESAIR:
    {
        ...
        if (oj->Live == true)
        {
            ...
            if (o->SubType == 2)
            {
                if ((int)o->LifeTime < 47)
                    o->Alpha -= 0.1f;
            }
            else if ((int)o->LifeTime % 4 == 0 && (int)o->LifeTime > 30)
            {
            	Vector(0.35f,0.35f,0.35f,Light);
                CreateEffect(MODEL_FEATHER, o->Position, o->Angle, o->Light, 2, NULL, -1, 0, 0, 0, 0.9f);
    ...
    case BITMAP_JOINT_THUNDER:
    {
        ...
        vec3_t vOffset;
        vOffset[0] = rand() % 160 - 80; 
        vOffset[1] = rand() % 160 - 80; 
        vOffset[2] = 0.0f;               
        VectorAdd(o->StartPosition, vOffset, vStartPosition); 
        VectorAdd(o->Position, vOffset, vPosition);    
        vPosition[0] += rand() % 50 - 25;
        vPosition[1] += rand() % 50 - 25;

        float AttackSpeedMg1 = CharacterAttribute->AttackSpeed;
        int tiaset = static_cast<int>(round(0.009f * AttackSpeedMg1 + 1.1f));
    ...
    case MODEL_SKILL_WHEEL2:
        ...
        o->Angle[2] -= 25 * FPS_ANIMATION_FACTOR;
        if (rand_fps_check(1)) 
        {
            Vector(Luminosity * 0.0f, Luminosity * 0.0f, Luminosity * 0.0f, Light);
            AddTerrainLight(o->Position[0], o->Position[1], Light, 0, PrimaryTerrainLight);
        }
        Vector(1.f, 1.f, 1.f, Light);
        ...
        if (gMapManager.InHellas())
        {
            ...
            CreateParticle(BITMAP_WATERFALL_5, Position, Angle, Light, 10);
        }
        else
        {
            for (int j = 0; j < 4; j++)
    ...
    case MODEL_SKILL_WHEEL2:
        ...
        Vector(Luminosity * 0.15f, Luminosity * 0.15f, Luminosity * 0.15f, Light);
        AddTerrainLight(o->Position[0], o->Position[1], Light, 3, PrimaryTerrainLight);
        Vector(1.f, 1.f, 1.f, Light);
    ...
    case MODEL_LACEARROW:
        ...
        else if (o->Type == MODEL_LACEARROW)
        {
            ...
            if ((int)o->LifeTime > 15 && (int)o->LifeTime < 30 && rand() % 10 == 0)
            {
                CreateEffectFpsChecked(MODEL_WAVES, o->Position, o->Angle, o->Light, 3, NULL, 0);
            }
    ...
    case MODEL_ARROW_DARKSTINGER: // Cung 23
    {
        CheckClientArrow(o);
       ...
       if ((int)o->LifeTime > 20 && (int)o->LifeTime < 29)
        {
            Vector(0.6f, 0.7f, 0.9f, Light);
            ...
            if (rand() % 8 == 0)
            {
                CreateEffect(MODEL_FEATHER, vPos, o->Angle, Light, 0, NULL, -1, 0, 0, 0, 0.6f); //bo Fpschecked fix hieu ung cung thien vu elf aLongfix5)
            }
        }
        ...
        if ((int)o->LifeTime == 30 )
        {
            ...
            if (FPS_ANIMATION_FACTOR > 0.5f)
            {
                CreateJoint(BITMAP_FLARE + 1, vPos, vPos, o->Angle, 18, o, 90.f, 80, 0, 0, -1, Light); 
            }
            else 
            {
                if (rand() % 4 < 3)
        ...
        if (o->SubType == 0 || o->SubType == 2 || o->SubType == 4 || o->SubType == 6 || o->SubType == 7 || o->SubType == 8)
        {
            ...
            if (success)
            {
                ...
                if (o->SubType == 6 || o->SubType == 7 || o->SubType == 8)
                {
                    ...
                    for (int i = 0; i < smokeNum; ++i)
                    {
                        Position[0] = o->Position[0] + (rand() % 120 - 60);
                        Position[1] = o->Position[1] + (rand() % 120 - 60);
                        Position[2] = o->Position[2] + 10;

                        Vector(0.1f, 0.5f, 0.1f, Light);
                        CreateParticleFpsChecked(BITMAP_SMOKE, Position, o->Angle, Light, 11, (float)(rand() % 52 + 80) * 0.05f);
                    }
    ...
    case MODEL_BLIZZARD:
        if (o->SubType == 0)
        {
            ...
            o->StartPosition[0] -= (1.f) * FPS_ANIMATION_FACTOR;
    ...
    case MODEL_DOWN_ATTACK_DUMMY_L:
    {
        ...
        {
            if (o->Owner && o->Owner->m_sTargetIndex >= 0)
            {
    ...
    case MODEL_DOWN_ATTACK_DUMMY_R:
    {
        ...
        if (o->AnimationFrame >= 3.6f && o->LifeTime > 50)
        {
            if (o->Owner->m_sTargetIndex < 0)
                break;
    ...
    else
    {
        ...
        if (o->Type<BITMAP_BOSS_LASER || o->Type>BITMAP_BOSS_LASER + 2)
        {
            ...
            case MODEL_SPEARSKILL:
                ...
                break;
            case MODEL_PEGASUS_SWORD_BLOW_SMALL:
                // Its update case already advances the position.
                break;
            case rise::pegasus::kSpearstormRootModel:
            case rise::pegasus::kSpearstormBladeModel:
            case rise::pegasus::kSpearstormCrackModel:
                // UpdateSpearstorm owns the recovered 676041..67623F motion.
                // Generic MoveParticle rotates/applies Direction a second time
                // and keeps pushing embedded blades underground after impact.
                break;
            case rise::pegasus::kSwordBlowArchangelWingModel:
                // Its update case follows the caster and never owns XY motion.
                break;
            case rise::pegasus::kRainingArrowRootModel:
            case rise::pegasus::kRainingArrowFollowModel:
            case rise::pegasus::kRainingArrowImpactModel:
            ...
            case rise::pegasus::kRainingArrowRing07Model:
            case rise::pegasus::kRainingArrowRing08Model:
                // Their recovered update cases own all movement.
                break;
            case MODEL_ARROW_HOLY:
            {
                float Matrix[3][4];
    ...
    rise::pegasus::UpdateRainingArrowProjectile(*o);
...
void RenderWheelWeapon(OBJECT* o)
{
    ...
    o->Direction[2] = 2 * FPS_ANIMATION_FACTOR;
    o->Angle[2] += o->Direction[2] + 30;
    o->Angle[1] = 90;
    o->Position[2] += 70.f;
    float Alpha = o->Alpha;
    ...
    RenderPartObject(o, Type, NULL, Light, Alpha, o->Owner->WeaponLevel << 0, 0, 0, true, true, true);
...
void RenderEffects(bool bRenderBlendMesh)
{
    ...
    for (int i = 0; i < iEffectSize; ++i)
    {
        ...
        if (o->Live)
        {
            ...
            if (o->Visible)
            {
                if (rise::pegasus::IsElementalChargeChild(o->Type))
                {
                    // The source carrier renders only the sprite emitted by
                    // its update callback; never index Models[] by bitmap ID.
                    continue;
                }
                if (o->Type == rise::pegasus::kSwordBlowArchangelWingModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER",
                            *o, MAX_MODELS,
                            rise::pegasus::RenderSwordBlowArchangelWings(*o));
                    continue;
                }
                if (o->Type >= MODEL_PEGASUS_SWORD_BLOW_BIG &&
                    o->Type <= MODEL_PEGASUS_SWORD_BLOW_TARGET2)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRangeRenderStateOnce(
                            "SWORD_BLOW_RENDER", *o,
                            MODEL_PEGASUS_SWORD_BLOW_BIG,
                            MODEL_PEGASUS_SWORD_BLOW_TARGET2,
                            rise::pegasus::RenderSwordBlowModel(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kSwordWrathRootModel &&
                    o->Type <= rise::pegasus::kSwordWrathLayerModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderSwordWrath(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kSpiralChargeFirstModel &&
                    o->Type <= rise::pegasus::kSpiralChargeLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderSpiralCharge(*o));
                    continue;
                }
                if (rise::pegasus::IsCrusherChargeModelId(o->Type))
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderCrusherCharge(*o));
                    continue;
                }
                if (o->Type == rise::pegasus::kShiningBirdModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderShiningBird(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kDexBoosterFirstModel &&
                    o->Type <= rise::pegasus::kDexBoosterLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderDexBooster(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kChaosBladeFirstModel &&
                    o->Type <= rise::pegasus::kChaosBladeLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderChaosBlade(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kDragonViolentFirstModel &&
                    o->Type <= rise::pegasus::kDragonViolentLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderDragonViolent(*o));
                    continue;
                }
                if (o->Type == rise::pegasus::kElementalChargeModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderElementalCharge(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kHavocFirstModel &&
                    o->Type <= rise::pegasus::kHavocLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderHavocSpear(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kSpearstormFirstModel &&
                    o->Type <= rise::pegasus::kSpearstormLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderSpearstorm(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kRainingArrowFirstModel &&
                    o->Type <= rise::pegasus::kRainingArrowLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderRainingArrow(*o));
                    continue;
                }
                if (o->Type >= rise::pegasus::kDeathsideFirstModel &&
                    o->Type <= rise::pegasus::kDeathsideLastModel)
                {
                    if (!bRenderBlendMesh)
                        rise::pegasus::TraceRuntimeModelRenderStateOnce("EFFECT_RENDER", *o,
                            MAX_MODELS, rise::pegasus::RenderDeathside(*o));
                    continue;
                }
                if (bRenderBlendMesh)
                {
