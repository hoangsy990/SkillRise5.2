...
#include "RISE/PegasusRainingArrowMath.h"
...
void RenderSprite(OBJECT *o,OBJECT *Owner)
{
    ...
    if (rise::pegasus::IsRainingArrowDamageBitmap(o->Type))
    {
        const int frame = o->SubType < 0 ? 0 :
            (o->SubType >= rise::pegasus::kRainingArrowDamageAtlasFrames ?
                rise::pegasus::kRainingArrowDamageAtlasFrames - 1 : o->SubType);
        const float cell = 1.0f /
            static_cast<float>(rise::pegasus::kRainingArrowDamageAtlasColumns);
        const float u = static_cast<float>(frame %
            rise::pegasus::kRainingArrowDamageAtlasColumns) * cell;
        const float v = static_cast<float>(frame /
            rise::pegasus::kRainingArrowDamageAtlasColumns) * cell;
        // Pegasus 0x7CE934..0x7CE940 scales geometry as well as UVs by
        // 0.25 (0xB4AD94). The bitmap is a 4x4 atlas, not one large frame.
        RenderSprite(o->Type, o->Position, Width * cell, Height * cell, o->Light,
            o->Angle[2], u, v, cell, cell);
    }
    else if ( o->Type==BITMAP_FORMATION_MARK )
    {
