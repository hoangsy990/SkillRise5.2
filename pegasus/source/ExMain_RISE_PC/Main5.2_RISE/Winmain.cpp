...
static int __cdecl ConfigurePegasusIsolatedWorkingDirectory()
{
	char modulePath[MAX_PATH] = { 0 };
	const DWORD moduleLength = GetModuleFileNameA(NULL,
...
// CMixRecipeMgr and other native systems open relative Data paths from global
// constructors, before WinMain.  Run in the CRT C-initializer range so the
// isolated client directory is established before any C++ initializer.
typedef int (__cdecl* PegasusCInitializer)(void);
#pragma section(".CRT$XIB", long, read)
extern "C" __declspec(allocate(".CRT$XIB")) PegasusCInitializer
	g_PegasusWorkingDirectoryInitializer =
		ConfigurePegasusIsolatedWorkingDirectory;
#if defined(_M_IX86)
#pragma comment(linker, "/include:_g_PegasusWorkingDirectoryInitializer")
#else
#pragma comment(linker, "/include:g_PegasusWorkingDirectoryInitializer")
#endif
#endif
...
GLvoid KillGLWindow(GLvoid)
{
	if (g_hRC)
	{
		...
		if (Models != NULL)
		{
			for (int i = MODEL_LOGO; i < rise::pegasus::RuntimeModelLimit(MAX_MODELS); ++i)
			{
...
#if defined(PEGASUS_ISOLATED_TEST)
	sprintf_s(WindowNameMU, MAX_LEN_CHAR, "PEGASUS QA - %s",
...
#if defined(PEGASUS_ISOLATED_TEST)
		"Software\\Microsoft\\Windows\\Windows Error Reporting\\LocalDumps\\Engine-Pegasus.exe",
#else
...
{
#if defined(PEGASUS_ISOLATED_TEST)
	// Defensive repeat after CRT initialization in case another startup hook
	...
	SkillAttribute = new SKILL_ATTRIBUTE[MAX_SKILL_TYPES];
	...
	memset(SkillAttribute, 0, sizeof(SKILL_ATTRIBUTE) * (MAX_SKILL_TYPES));
