...
void CGToolKit::WindowRename()
{
	...
	if (SceneFlag == 5)
	{
		...
		{
			sprintf_s(WindowName, sizeof(WindowName), "%s - %s || Level: %d || Reset: %d/%d || FPS : %.1f || Mouse: %d %d [%d] - MapID: %d [World%d]", gProtect->m_MainInfo.WindowName, Hero->ID, CharacterAttribute->Level, CharacterAttribute->ViewReset, CharacterAttribute->ViewMaxReset, FPS_AVG > gProtect->m_MainInfo.DefaultFPS ? gProtect->m_MainInfo.DefaultFPS : FPS_AVG, MouseX, MouseY, MouseLButtonPush, gMapManager.WorldActive, gMapManager.WorldActive + 1);
		}
		else
		{
			if (gProtect->m_MainInfo.ButtonCharracter[17])
			{
				sprintf_s(WindowName, sizeof(WindowName), "%s - %s || Level: %d || Reset: %d/%d || WC: %d || WP: %d || WG: %d || Ruud: %d", gProtect->m_MainInfo.WindowName, Hero->ID, CharacterAttribute->Level, CharacterAttribute->ViewReset, CharacterAttribute->ViewMaxReset, pMain->ThisCoin[0], pMain->ThisCoin[1], pMain->ThisCoin[2], pMain->ThisCoin[3]);
			}
			else
			{
				sprintf_s(WindowName, sizeof(WindowName), "%s - %s || Level: %d || Reset: %d/%d", gProtect->m_MainInfo.WindowName, Hero->ID, CharacterAttribute->Level, CharacterAttribute->ViewReset, CharacterAttribute->ViewMaxReset);
			}
		}
	}
	else
	{
		sprintf_s(WindowName, sizeof(WindowName), "%s", gProtect->m_MainInfo.WindowName);
	}

#if defined(PEGASUS_ISOLATED_TEST)
	char PegasusWindowName[500];
	sprintf_s(PegasusWindowName, sizeof(PegasusWindowName),
		"PEGASUS QA - %s", WindowName);
	SetWindowText(g_hWnd, PegasusWindowName);
#else
...
{
#if defined(PEGASUS_ISOLATED_TEST)
	std::string filePath = "Data/RISE/Pegasus/Recovered/Data/Player/player.bmd";
	unsigned int expectedCRC = 0xA7C3C76D;
#else
...
#if defined(PEGASUS_ISOLATED_TEST)
		MessageBoxA(NULL,
			"Pegasus player.bmd is missing or failed its pinned CRC check.",
			"RISE Pegasus QA", MB_OK | MB_ICONERROR);
#else
