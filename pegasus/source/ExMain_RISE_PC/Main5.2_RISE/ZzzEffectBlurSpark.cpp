...
static bool IsChaosBladeBlur(int type)
{
    return type == rise::pegasus::kChaosBladeBlurLongBitmap ||
        type == rise::pegasus::kChaosBladeBlurBlackBitmap;
}
...
static float g_chaosBlurTick[MAX_OBJECTBLURS] = {};

#if defined(PEGASUS_ISOLATED_TEST)
static void TraceDeathsideRibbonSubmission(const OBJECT_BLUR& blur)
{
    if (!blur.Owner || blur.Number < 8) return;
    const int variant = blur.Owner->Type - rise::pegasus::kDeathsideScythe01Model;
    const int subtype = blur.SubType;
    static bool sampled[3][7] = {};
    if (variant < 0 || variant >= 3 || subtype < 0 || subtype >= 7 ||
        sampled[variant][subtype]) return;
    sampled[variant][subtype] = true;
    GLint program = 0, texture = 0, src = 0, dst = 0, depth = 0, unit = 0;
    ...
    _snprintf_s(detail, sizeof(detail), _TRUNCATE,
        "points=%d life=%d program=%d texture=%d unit=%d blend=%d/%d enabled=%d depth=%d/%d mask=%d alphaTest=%d RGB=(%.3f,%.3f,%.3f)",
        blur.Number, blur.LifeTime, program, texture, unit, src, dst,
        static_cast<int>(glIsEnabled(GL_BLEND)),
        static_cast<int>(glIsEnabled(GL_DEPTH_TEST)), depth, static_cast<int>(mask),
        static_cast<int>(glIsEnabled(GL_ALPHA_TEST)),
        blur.Light[0], blur.Light[1], blur.Light[2]);
    rise::pegasus::TraceRuntime("DEATHSIDE_RIBBON_DRAW_STATE", blur.Owner->Type,
        subtype, detail);
    vec3_t lower, upper;
    VectorCopy(blur.p1[0], lower);
    VectorCopy(blur.p1[0], upper);
    for (int point = 0; point < blur.Number; ++point)
        for (int axis = 0; axis < 3; ++axis)
        {
            lower[axis] = min(lower[axis], min(blur.p1[point][axis], blur.p2[point][axis]));
            upper[axis] = max(upper[axis], max(blur.p1[point][axis], blur.p2[point][axis]));
        }
    _snprintf_s(detail, sizeof(detail), _TRUNCATE,
        "owner=(%.2f,%.2f,%.2f) min=(%.2f,%.2f,%.2f) max=(%.2f,%.2f,%.2f) firstA=(%.2f,%.2f,%.2f) firstB=(%.2f,%.2f,%.2f)",
        blur.Owner->Position[0], blur.Owner->Position[1], blur.Owner->Position[2],
        lower[0], lower[1], lower[2], upper[0], upper[1], upper[2],
        blur.p1[0][0], blur.p1[0][1], blur.p1[0][2],
        blur.p2[0][0], blur.p2[0][1], blur.p2[0][2]);
    rise::pegasus::TraceRuntime("DEATHSIDE_RIBBON_DRAW_BOUNDS", blur.Owner->Type,
        subtype, detail);
}
#endif

int CountPegasusObjectBlurs()
{
    int count = 0;
    for (int i = 0; i < MAX_OBJECTBLURS; ++i)
        if (ObjectBlur[i].Live && (IsChaosBladeBlur(ObjectBlur[i].Type) ||
            rise::pegasus::IsDeathsideBlurBitmap(ObjectBlur[i].Type)))
            ++count;
    return count;
}
...
void MoveObjectBlurs()
{
	for(int i=0;i<MAX_OBJECTBLURS;i++)
	{
		...
		if(b->Live)
		{
            // Pegasus 0x79439D..0x7943C2 gates these two textures on
            // accumulated animation time. Other native blur timing is unchanged.
            if (IsChaosBladeBlur(b->Type))
            {
                g_chaosBlurTick[i] += static_cast<float>(FPS_ANIMATION_FACTOR);
                if (g_chaosBlurTick[i] < 1.0f)
                    continue;
                g_chaosBlurTick[i] -= 1.0f;
            }
			b->LifeTime--;
			b->Number--;
...
void RenderObjectBlurs()
{
	...
	for(int i=0;i<MAX_OBJECTBLURS;i++)
	{
		...
		{
            Type = b->Type;
			const bool deathsideBlur = rise::pegasus::IsDeathsideBlurBitmap(Type);
			...
			if (Type == rise::pegasus::kChaosBladeBlurBlackBitmap)
            {
                // 0x794728 -> 0x87F992: glBlendFunc(GL_ZERO,
                // GL_ONE_MINUS_SRC_COLOR), NOT alpha-test.
                EnableAlphaBlendMinus();
            }
            else if (deathsideBlur)
			{
                // 0x65FF4D recognizes 0x7F03; 0x79471F branches to
                // 0x794728 -> 0x87F992: GL_ZERO, GL_ONE_MINUS_SRC_COLOR.
                // Alpha-test makes the RGB-only blur's black background opaque.
                EnableAlphaBlendMinus();
			}
			else
			{
			...
			if(b->Number >= 2)
			{
				...
				for(int j=0;j<b->Number-1;j++)
				{
                    ...
                    if (deathsideBlur)
                    {
                        NotifyPegasusQaDeathsideRibbonDraw();
                        TraceDeathsideRibbonSubmission(*b);
                    }
#endif
      				glBegin(GL_TRIANGLE_FAN);
					float Light;
					float TexU;
					Light = (b->Number-j)/(float)b->Number;
                    if (b->Type == rise::pegasus::kChaosBladeBlurLongBitmap)
                        Light = b->LifeTime / static_cast<float>(b->_iLimitLifeTime);
					glColor3f(b->Light[0]*Light,b->Light[1]*Light,b->Light[2]*Light);
					TexU = (j)/(float)b->Number;
					glTexCoord2f(TexU,1.f);glVertex3fv(b->p1[j]);
					glTexCoord2f(TexU,0.f);glVertex3fv(b->p2[j]);
