...
#if defined(PEGASUS_ISOLATED_TEST)
	if (m_iSelectServerBtnIndex == -1 && TryPegasusAutoQaSelect())
		return;
#endif
...
#if defined(PEGASUS_ISOLATED_TEST)
	TryPegasusAutoQaSelect();
#endif
}

#if defined(PEGASUS_ISOLATED_TEST)
bool CServerSelWin::TryPegasusAutoQaSelect()
{
	...
	if (m_iSelectServerBtnIndex == -1)
	{
		for (int i = 0; i < SSW_SERVER_G_MAX; ++i)
		{
			...
			rise::pegasus::TraceRuntime("AUTO_QA_SERVER_GROUP", i, 0,
				"SELECTED");
	...
	for (int i = 0; i < m_icntServer; ++i)
	{
		...
		rise::pegasus::TraceRuntime("AUTO_QA_SERVER", i,
			qaServer->m_iConnectIndex, "SELECTED");
		return true;
	}
	return false;
}
