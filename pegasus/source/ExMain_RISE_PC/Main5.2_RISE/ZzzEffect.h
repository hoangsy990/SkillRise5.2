...
int CreateParticle(int Type,vec3_t Position,vec3_t Angle,vec3_t Light,int SubType=0,float Scale=1.f,OBJECT *Owner=NULL,int PegasusVariant=-32768,bool PegasusOwned=false);
...
void RenderLeaves();
bool MoveLeaves();
void CreatePoint(vec3_t Position,__int64 Value,vec3_t Color,float scale=15.f, bool bMove=true, bool bRepeatedly = false);
