...
#include <RISE/CustomItemRenderMesh.h>
#include <RISE/CustomRenderEffect.h>
#include <RISE/CustomMonsterGold.h>
...
void BodyLight(OBJECT* o, BMD* b)
{
    ...
    if (gMapManager.WorldActive == WD_94EX702_LOGIN_SCENE && o->Type == 2)
    {
        Vector(0.95f, 0.93f, 1.0f, b->BodyLight);
        return;
    }
#endif
...
void Draw_RenderObject(OBJECT* o, bool Translate, int Select, int ExtraMon)
{
    ...
    if (!g_pOption->GetRenderHideWorld() && SceneFlag == MAIN_SCENE && o->Kind != KIND_MONSTER && o->Kind != KIND_NPC)
    {
        return;
    }
                        ...
                        if (M31HuntingGround::RenderHuntingGroundObjectMesh(o, b, ExtraMon)) {}
                            ...
                            if (M34CryingWolf2nd::RenderCryingWolf2ndObjectMesh(o, b)) {}
                                ...
                                else if (o->Type == MODEL_DESAIR)
                                {
                                    ...
                                    if (o->SubType == 2)
                                    {
                                        // Wind Soul's desair child is a
                                        // luminous part of the cyan trail.
                                        // The legacy subtype-0 path below
                                        // intentionally renders black and
                                        // cannot be reused for this child.
                                        VectorCopy(o->Light, b->BodyLight);
                                        b->RenderBody(RENDER_TEXTURE | RENDER_BRIGHT,
                                            o->Alpha, o->BlendMesh, 1.0f,
                                            0.f, 0.f, o->HiddenMesh);
                                    }
                                    else
                                    {
                                        Vector(0.f, 0.f, 0.f, b->BodyLight);
                                        o->Alpha = 1.f;
                                        b->RenderBody(RENDER_TEXTURE, o->Alpha,
                                            o->BlendMesh, 1.0f, 0.f, 0.f,
                                            o->HiddenMesh);
                                    }
                                }
                                ...
                                else if (o->Type == MODEL_BLADE_SKILL) // remove vet sang MultiShot EF
                                {
                                    /*VectorCopy(o->Light, b->BodyLight);
    ...
    if (gCustomPet.CheckCustomPetByItem(o->Type - MODEL_ITEM))
    {
        ...
        {
            if (it->ItemIndex == (o->Type - MODEL_ITEM))
            {
                vec3_t p1, p2;
                BMD* b = &Models[o->Type];

                Vector(it->ColorR, it->ColorG, it->ColorB, o->Light);
                Vector(0.0, 0.0, 0.0, p1);
                b->TransformPosition(BoneTransform[it->Join], p1, p2);

                int v25 = timeGetTime() % 0x50;

                if (it->TypeEffect == 1)
                {
                    CreateParticle(it->EffectCode, p2, o->Angle, o->Light, 0, it->EffectSize, o);
                }
                else if (it->TypeEffect == 2)
                {
                    CreateSprite(it->EffectCode, p2, it->EffectSize, o->Light, o);
                }
                else if (it->TypeEffect == 3)
                {
                    if (v25 >= 0 && v25 < it->LargeRand)
                    {
                        CreateEffect(it->EffectCode, p2, o->Angle, o->Light, 3);
                    }
                }
            }
        }
        for (std::vector<RENDER_MESH>::iterator it = gCustomCEffectPet.m_RenderMeshPet.begin(); it != gCustomCEffectPet.m_RenderMeshPet.end(); it++)
        {
            if (it->ItemIndex == (o->Type - MODEL_ITEM))
            {
                b->BeginRender(1.f);
                b->StreamMesh = 0;
                b->BodyLight[0] = (float)(it->ColorR / 255.0f);
                b->BodyLight[1] = (float)(it->ColorG / 255.0f);
                b->BodyLight[2] = (float)(it->ColorB / 255.0f);

                b->BodyOrigin[0] = (float)(it->ColorR / 255.0f);
                b->BodyOrigin[1] = (float)(it->ColorG / 255.0f);
                b->BodyOrigin[2] = (float)(it->ColorB / 255.0f);
                b->RenderMesh(it->EffectIndex, it->EffectType, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
                b->StreamMesh = -1;
                b->EndRender();
            }
        }
...
void RenderObjectVisual(OBJECT* o)
{
    ...
    default:
#if RISE_ENABLE_EX702_SELECT_SCENE
		// The Ex700 CScene hook draws its custom server-scene overlay first and
		// then delegates to the original Crying Wolf visual handler.
		if (gMapManager.WorldActive == WD_94EX702_LOGIN_SCENE)
		{
			TheMapProcess().RenderObjectVisual(o, b);
			M34CryingWolf2nd::RenderCryingWolf2ndObjectVisual(o, b);
			return;
		}
#endif
...
#if RISE_ENABLE_EX702_SELECT_SCENE
static bool IsEx702CharacterSceneForcedObject(int type)
{
    if (type == 4 || type == 7 || type == 8 || type == 12 ||
        type == 33 || type == 36 ||
        type == 60 || type == 61 || type == 71 ||
        type == 98 || type == 129)
    {
        return true;
    }

    return (type >= 27 && type <= 31);
}

static bool IsEx702CharacterSceneSuppressedObject(int type)
{
    return false;
}
#endif
...
#if RISE_ENABLE_EX702_SELECT_SCENE
            bool ex702CharacterSceneBlock = (gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE);
#endif
            if (g_Direction.m_CKanturu.IsMayaScene()
...
#if RISE_ENABLE_EX702_SELECT_SCENE
                || gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE
#endif
                || IsIceCity()
                || gMapManager.IRISEField()
                || IsDoppelGanger2()
                )
            {
                OBJECT* o = ob->Head;
                ...
                while (1)
                {
                    ...
                    {
#if RISE_ENABLE_EX702_SELECT_SCENE
                        if (ex702CharacterSceneBlock)
                        {
                            if (o->Live && IsEx702CharacterSceneForcedObject(o->Type))
                            {
                                RenderObject(o);
                        ...
                        if (g_Direction.m_CKanturu.IsMayaScene() && (o->Type == 0 || o->Type == MODEL_STORM2))
                        {
                            ...
                            else if (gMapManager.IRISEField() && (o->Type == 16 || o->Type == 67 || o->Type == 68) && TestFrustrum2D(o->Position[0] * 0.01f, o->Position[1] * 0.01f, -600.f))
                            {
                                RenderObject(o);
                                RenderObjectVisual(o);
                            }
                        ...
                        if (o->Live)
                        {
                            ...
                            if ((gMapManager.WorldActive == WD_51HOME_6TH_CHAR
                                ...
                                if ((gMapManager.WorldActive == WD_57ICECITY || gMapManager.WorldActive == WD_58ICECITY_BOSS) && (o->Type == 30 || o->Type == 31 || o->Type == 76));
                                    ...
                                    if ((gMapManager.IRISEField() || IsDoppelGanger2()) && (o->Type == 16 || o->Type == 67 || o->Type == 68));
                                    else
#ifdef PJH_NEW_SERVER_SELECT_MAP
                                        if (gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE);
#else
...
#endif //PJH_NEW_SERVER_SELECT_MAP
#if RISE_ENABLE_EX702_SELECT_SCENE
                                        else if (gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE && IsEx702CharacterSceneForcedObject(o->Type));
                                        else if (gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE && IsEx702CharacterSceneSuppressedObject(o->Type));
#endif
                                        else
...
#if RISE_ENABLE_EX702_SELECT_SCENE
        || gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE
        || gMapManager.WorldActive == WD_94EX702_LOGIN_SCENE
#endif
        ))
        return;
...
#if RISE_ENABLE_EX702_SELECT_SCENE
            if (gMapManager.WorldActive == WD_93EX702_CHARACTER_SCENE)
            {
                OBJECT* o = ob->Head;
                ...
                while (1)
                {
                    if (o != NULL)
                    {
                        if (o->Live && o->m_bRenderAfterCharacter == true && IsEx702CharacterSceneForcedObject(o->Type))
                        {
                        ...
                        if (o->Live && o->m_bRenderAfterCharacter == true)
                        {
                            ...
                            if ((gMapManager.WorldActive == WD_51HOME_6TH_CHAR
                                ...
                                if (IsIceCity() && o->Type == 76);
                                    ...
                                    if (gMapManager.IRISEField() && (o->Type == 16 || o->Type == 67 || o->Type == 68));
                                    else
...
void ItemAngle(OBJECT* o)
{
    ...
    if (gCustomPet.CheckCustomPetByItem(o->Type - MODEL_ITEM)) 
    {
        float customScale = gCustomPet.GetScaleDrop(o->Type - MODEL_ITEM);
        if (customScale > 0) o->Scale = customScale;
    }
#ifdef LEM_ADD_LUCKYITEM
...
void CreateItem(ITEM_t* ip, BYTE* Item, vec3_t Position, int CreateFlag)
{
    ...
    else
    {
        ...
        if (CreateFlag)
        {
            if (Type == ITEM_POTION + 13 || Type == ITEM_POTION + 14 || Type == ITEM_POTION + 16 || Type == ITEM_WING + 15 || Type == ITEM_POTION + 22 || Type == ITEM_POTION + 31 || g_pCustomJewel->CheckCustomJewelByItem(Type))
...
void PartObjectColor(int Type, float Alpha, float Bright, vec3_t Light, bool ExtraMon)
{
    ...
    if (gCustomItem.GetCustomItemColor((Type - MODEL_ITEM), (float*)Light))
    {
        return;
    }

    if (ExtraMon && (Type == MODEL_MONSTER01 + 27))
    {
        Color = 8;
    }
    else if (Type == MODEL_MONSTER01 + 27)
    {
        Color = 1;
    }
    else if (Type == MODEL_MONSTER01 + 35)
    {
        Color = 5;
    }
    ...
    else
    {
        ...
        int Section     = ItemType / MAX_ITEM_INDEX;
        int Index       = ItemType % MAX_ITEM_INDEX;
        ...
        if (Section == 0)
        {
            switch (Index)
            {
                ...
                case 13: Color = 1; break;
                case 14: Color = 2; break;
                case 15: Color = 11; break;
                case 16: Color = 11; break;
                case 17: Color = 0; break;
                case 18: Color = 11; break;
                case 19: Color = 0; break;
                case 20: Color = 0; break;
                case 21: Color = 0; break;
                case 22: Color = 0; break;
                case 23: Color = 0; break;
                case 24: Color = 0; break;
                case 25: Color = 0; break;
                case 31: Color = 0; break;
                case 32: Color = 1; break;
                case 33: Color = 0; break;
                case 34: Color = 6; break;
                case 35: Color = 89; break;
                case 47: Color = 0; break;
                case 51: Color = 8; break;
                case 60: Color = 0; break;
                case 61: Color = 8; break;
                case 83: Color = 8; break;
                case 81: Color = 1; break;
            }
        }
        else
        if (Section == 2)
        {
            switch (Index)
            {
                case 8:   Color = 15; break;
                case 9:   Color = 15; break;
                case 10:  Color = 15; break;
                case 15:  Color = 28; break;
                case 17:  Color = 30; break;
                case 25:  Color = 8; break;
            }
        }
        else
        if (Section == 3)
        ...
        if (Section == 4)
        {
            switch (Index)
            {
                ...
                case 19: Color = 15; break;
                case 20: Color = 31; break;
                case 21: Color = 15; break;
                case 22: Color = 26; break;
                case 23: Color = 30; break;
                case 24: Color = 15; break;
                case 30: Color = 8; break;
            }
        }
        else
        if (Section == 5)
        {
            switch (Index)
            {
                ...
                case 10: Color = 0; break;
                case 11: Color = 15; break;
                case 12: Color = 15; break;
                case 13: Color = 5; break;
                case 14: Color = 28; break;
                case 15: Color = 15; break;
                case 16: Color = 30; break;
                case 17: Color = 3; break;
                case 18: Color = 30; break;
                case 19: Color = 15; break;
                case 30: Color = 40; break;
                case 31: Color = 15; break;
                case 34: Color = 5; break;
                case 36: Color = 0; break;
                case 49: Color = 8; break;
                case 50: Color = 8; break;
            }
        }
        else
        if (Section == 6)
        ...
        if (Section >= 7 && Section <= 11)
        {
            switch (Index)
            {
                ...
                case 18: Color = 110; break;
                case 19: Color = 36; break;
                case 20: Color = 36; break;
                case 21: Color = 47; break;
                case 22: Color = 17; break;
                case 23: Color = 23; break;
                case 24: Color = 31; break;
                case 25: Color = 11; break;
                case 26: Color = 12; break;
                case 27: Color = 10; break;
                case 28: Color = 38; break;
                case 29: Color = 183; break;
                case 30: Color = 21; break;
                case 31: Color = 26; break;
                case 32: Color = 34; break;
                case 33: Color = 22; break;
                case 34: Color = 24; break;
                case 35: Color = 14; break;
                case 36: Color = 26; break;
                case 37: Color = 30; break;
                case 38: Color = 28; break;
                case 39: Color = 26; break;
                case 40: Color = 23; break;
                case 41: Color = 24; break;
                case 42: Color = 32; break;
                case 43: Color = 30; break;
                case 44: Color = 23; break;
                case 45: Color = 23; break;
                case 46: Color = 42; break;
                case 47: Color = 21; break;
                case 48: Color = 40; break;
                case 49: Color = 30; break;
                case 50: Color = 26; break;
                case 51: Color = 30; break;
                case 52: Color = 23; break;
                case 53: Color = 30; break;
                case 59: Color = 17; break;
                case 60: Color = 42; break;
                case 61: Color = 25; break;
                case 73: Color = 12; break;
            }
        }
    }
    ...
    switch (Color)
    {
        ...
        case 46: Vector(Bright * 0.6f, Bright * 0.9f, Bright * 0.7f, Light); break;
        case 47: Vector(Bright * 0.9f, Bright * 0.5f, Bright * 0.4f, Light); break;
        case 48: Vector(Bright * 0.4f, Bright * 0.6f, Bright * 0.9f, Light); break;
        case 49: Vector(Bright * 0.7f, Bright * 0.7f, Bright * 0.2f, Light); break;
        case 50: Vector(Bright * 0.3f, Bright * 0.9f, Bright * 0.4f, Light); break;
        case 51: Vector(Bright * 0.9f, Bright * 0.3f, Bright * 0.3f, Light); break;
        case 52: Vector(Bright * 0.4f, Bright * 0.8f, Bright * 0.9f, Light); break;
        case 53: Vector(Bright * 0.2f, Bright * 0.6f, Bright * 0.8f, Light); break;
        case 54: Vector(Bright * 0.8f, Bright * 0.5f, Bright * 0.1f, Light); break;
        case 55: Vector(Bright * 0.9f, Bright * 0.8f, Bright * 0.6f, Light); break;
        case 56: Vector(Bright * 0.5f, Bright * 0.3f, Bright * 0.9f, Light); break;
        case 57: Vector(Bright * 0.6f, Bright * 0.9f, Bright * 0.3f, Light); break;
        case 58: Vector(Bright * 0.9f, Bright * 0.6f, Bright * 0.2f, Light); break;
        case 59: Vector(Bright * 0.4f, Bright * 0.3f, Bright * 0.7f, Light); break;
        case 60: Vector(Bright * 0.3f, Bright * 0.5f, Bright * 0.8f, Light); break;
        case 61: Vector(Bright * 0.9f, Bright * 0.4f, Bright * 0.4f, Light); break;
        case 62: Vector(Bright * 0.6f, Bright * 0.6f, Bright * 0.2f, Light); break;
        case 63: Vector(Bright * 0.5f, Bright * 0.9f, Bright * 0.6f, Light); break;
        case 64: Vector(Bright * 0.7f, Bright * 0.3f, Bright * 0.7f, Light); break;
        case 65: Vector(Bright * 0.8f, Bright * 0.4f, Bright * 0.8f, Light); break;
        case 66: Vector(Bright * 0.9f, Bright * 0.7f, Bright * 0.5f, Light); break;
        case 67: Vector(Bright * 0.7f, Bright * 0.9f, Bright * 0.5f, Light); break;
        case 68: Vector(Bright * 0.2f, Bright * 0.5f, Bright * 0.6f, Light); break;
        case 69: Vector(Bright * 0.3f, Bright * 0.4f, Bright * 0.8f, Light); break;
        case 70: Vector(Bright * 0.6f, Bright * 0.2f, Bright * 0.5f, Light); break;
        case 71: Vector(Bright * 0.5f, Bright * 0.8f, Bright * 0.2f, Light); break;
        case 72: Vector(Bright * 0.8f, Bright * 0.6f, Bright * 0.3f, Light); break;
        case 73: Vector(Bright * 0.7f, Bright * 0.3f, Bright * 0.4f, Light); break;
        case 74: Vector(Bright * 0.6f, Bright * 0.7f, Bright * 0.9f, Light); break;
        case 75: Vector(Bright * 0.4f, Bright * 0.3f, Bright * 0.6f, Light); break;
        case 76: Vector(Bright * 0.8f, Bright * 0.7f, Bright * 0.2f, Light); break;
        case 77: Vector(Bright * 0.6f, Bright * 0.9f, Bright * 0.6f, Light); break;
        case 78: Vector(Bright * 0.3f, Bright * 0.3f, Bright * 0.9f, Light); break;
        case 79: Vector(Bright * 0.5f, Bright * 0.6f, Bright * 0.5f, Light); break;
        case 80: Vector(Bright * 0.7f, Bright * 0.9f, Bright * 0.3f, Light); break;
        case 81: Vector(Bright * 0.9f, Bright * 0.5f, Bright * 0.7f, Light); break;
        case 82: Vector(Bright * 0.6f, Bright * 0.3f, Bright * 0.8f, Light); break;
        case 83: Vector(Bright * 0.2f, Bright * 0.6f, Bright * 0.5f, Light); break;
        case 84: Vector(Bright * 0.5f, Bright * 0.2f, Bright * 0.7f, Light); break;
        case 85: Vector(Bright * 0.9f, Bright * 0.7f, Bright * 0.6f, Light); break;
        case 86: Vector(Bright * 0.7f, Bright * 0.6f, Bright * 0.7f, Light); break;
        case 87: Vector(Bright * 0.8f, Bright * 0.8f, Bright * 0.5f, Light); break;
        case 88: Vector(Bright * 0.9f, Bright * 0.2f, Bright * 0.2f, Light); break;
        case 89: Vector(Bright * 0.4f, Bright * 0.7f, Bright * 0.4f, Light); break;
        case 90: Vector(Bright * 0.3f, Bright * 0.5f, Bright * 0.9f, Light); break;
        case 91: Vector(Bright * 0.6f, Bright * 0.4f, Bright * 0.3f, Light); break;
        case 92: Vector(Bright * 0.7f, Bright * 0.7f, Bright * 0.7f, Light); break;
        case 93: Vector(Bright * 0.3f, Bright * 0.9f, Bright * 0.6f, Light); break;
        case 94: Vector(Bright * 0.6f, Bright * 0.3f, Bright * 0.6f, Light); break;
        case 95: Vector(Bright * 0.2f, Bright * 0.8f, Bright * 0.3f, Light); break;
        case 96: Vector(Bright * 0.9f, Bright * 0.6f, Bright * 0.4f, Light); break;
        case 97: Vector(Bright * 0.4f, Bright * 0.5f, Bright * 0.7f, Light); break;
        case 98: Vector(Bright * 0.5f, Bright * 0.3f, Bright * 0.4f, Light); break;
        case 99: Vector(Bright * 0.3f, Bright * 0.3f, Bright * 0.6f, Light); break;
        case 100: Vector(Bright * 0.7f, Bright * 0.9f, Bright * 0.7f, Light); break;
        case 101: Vector(Bright * 0.80f, Bright * 0.80f, Bright * 0.80f, Light); break;
        case 102: Vector(Bright * 0.00f, Bright * 0.00f, Bright * 0.00f, Light); break;
        case 103: Vector(Bright * 0.40f, Bright * 0.40f, Bright * 0.40f, Light); break;
        case 104: Vector(Bright * 0.80f, Bright * 0.00f, Bright * 0.00f, Light); break;
        case 105: Vector(Bright * 0.80f, Bright * 0.40f, Bright * 0.00f, Light); break;
        case 106: Vector(Bright * 0.80f, Bright * 0.80f, Bright * 0.00f, Light); break;
        case 107: Vector(Bright * 0.00f, Bright * 0.80f, Bright * 0.00f, Light); break;
        case 108: Vector(Bright * 0.00f, Bright * 0.80f, Bright * 0.80f, Light); break;
        case 109: Vector(Bright * 0.00f, Bright * 0.00f, Bright * 0.80f, Light); break;
        case 110: Vector(Bright * 0.60f, Bright * 0.00f, Bright * 0.80f, Light); break;
        case 111: Vector(Bright * 0.80f, Bright * 0.00f, Bright * 0.40f, Light); break;
        case 112: Vector(Bright * 0.50f, Bright * 0.25f, Bright * 0.00f, Light); break;
        case 113: Vector(Bright * 0.40f, Bright * 0.00f, Bright * 0.40f, Light); break;
        case 114: Vector(Bright * 0.00f, Bright * 0.40f, Bright * 0.20f, Light); break;
        case 115: Vector(Bright * 0.20f, Bright * 0.40f, Bright * 0.00f, Light); break;
        case 116: Vector(Bright * 0.60f, Bright * 0.60f, Bright * 0.60f, Light); break;
        case 117: Vector(Bright * 0.30f, Bright * 0.30f, Bright * 0.50f, Light); break;
        case 118: Vector(Bright * 0.50f, Bright * 0.30f, Bright * 0.30f, Light); break;
        case 119: Vector(Bright * 0.30f, Bright * 0.50f, Bright * 0.30f, Light); break;
        case 120: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.10f, Light); break;
        case 121: Vector(Bright * 0.80f, Bright * 0.60f, Bright * 0.00f, Light); break;
        case 122: Vector(Bright * 0.00f, Bright * 0.60f, Bright * 0.80f, Light); break;
        case 123: Vector(Bright * 0.40f, Bright * 0.20f, Bright * 0.80f, Light); break;
        case 124: Vector(Bright * 0.20f, Bright * 0.80f, Bright * 0.40f, Light); break;
        case 125: Vector(Bright * 0.60f, Bright * 0.00f, Bright * 0.20f, Light); break;
        case 126: Vector(Bright * 0.80f, Bright * 0.20f, Bright * 0.60f, Light); break;
        case 127: Vector(Bright * 0.60f, Bright * 0.40f, Bright * 0.80f, Light); break;
        case 128: Vector(Bright * 0.20f, Bright * 0.60f, Bright * 0.80f, Light); break;
        case 129: Vector(Bright * 0.80f, Bright * 0.80f, Bright * 0.60f, Light); break;
        case 130: Vector(Bright * 0.40f, Bright * 0.60f, Bright * 0.20f, Light); break;
        case 131: Vector(Bright * 0.70f, Bright * 0.30f, Bright * 0.10f, Light); break;
        case 132: Vector(Bright * 0.10f, Bright * 0.70f, Bright * 0.30f, Light); break;
        case 133: Vector(Bright * 0.30f, Bright * 0.10f, Bright * 0.70f, Light); break;
        case 134: Vector(Bright * 0.10f, Bright * 0.30f, Bright * 0.70f, Light); break;
        case 135: Vector(Bright * 0.70f, Bright * 0.10f, Bright * 0.30f, Light); break;
        case 136: Vector(Bright * 0.30f, Bright * 0.70f, Bright * 0.10f, Light); break;
        case 137: Vector(Bright * 0.25f, Bright * 0.25f, Bright * 0.25f, Light); break;
        case 138: Vector(Bright * 0.75f, Bright * 0.75f, Bright * 0.75f, Light); break;
        case 139: Vector(Bright * 0.55f, Bright * 0.20f, Bright * 0.55f, Light); break;
        case 140: Vector(Bright * 0.20f, Bright * 0.55f, Bright * 0.55f, Light); break;
        case 141: Vector(Bright * 0.55f, Bright * 0.55f, Bright * 0.20f, Light); break;
        case 142: Vector(Bright * 0.80f, Bright * 0.10f, Bright * 0.10f, Light); break;
        case 143: Vector(Bright * 0.10f, Bright * 0.80f, Bright * 0.10f, Light); break;
        case 144: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.80f, Light); break;
        case 145: Vector(Bright * 0.50f, Bright * 0.10f, Bright * 0.80f, Light); break;
        case 146: Vector(Bright * 0.10f, Bright * 0.50f, Bright * 0.80f, Light); break;
        case 147: Vector(Bright * 0.80f, Bright * 0.10f, Bright * 0.50f, Light); break;
        case 148: Vector(Bright * 0.50f, Bright * 0.80f, Bright * 0.10f, Light); break;
        case 149: Vector(Bright * 0.10f, Bright * 0.80f, Bright * 0.50f, Light); break;
        case 150: Vector(Bright * 0.80f, Bright * 0.50f, Bright * 0.10f, Light); break;
        case 151: Vector(Bright * 0.33f, Bright * 0.33f, Bright * 0.33f, Light); break;
        case 152: Vector(Bright * 0.66f, Bright * 0.66f, Bright * 0.66f, Light); break;
        case 153: Vector(Bright * 0.70f, Bright * 0.00f, Bright * 0.00f, Light); break;
        case 154: Vector(Bright * 0.00f, Bright * 0.70f, Bright * 0.00f, Light); break;
        case 155: Vector(Bright * 0.00f, Bright * 0.00f, Bright * 0.70f, Light); break;
        case 156: Vector(Bright * 0.70f, Bright * 0.70f, Bright * 0.00f, Light); break;
        case 157: Vector(Bright * 0.00f, Bright * 0.70f, Bright * 0.70f, Light); break;
        case 158: Vector(Bright * 0.70f, Bright * 0.00f, Bright * 0.70f, Light); break;
        case 159: Vector(Bright * 0.35f, Bright * 0.15f, Bright * 0.05f, Light); break;
        case 160: Vector(Bright * 0.15f, Bright * 0.35f, Bright * 0.05f, Light); break;
        case 161: Vector(Bright * 0.05f, Bright * 0.15f, Bright * 0.35f, Light); break;
        case 162: Vector(Bright * 0.35f, Bright * 0.05f, Bright * 0.15f, Light); break;
        case 163: Vector(Bright * 0.05f, Bright * 0.35f, Bright * 0.15f, Light); break;
        case 164: Vector(Bright * 0.15f, Bright * 0.05f, Bright * 0.35f, Light); break;
        case 165: Vector(Bright * 0.20f, Bright * 0.10f, Bright * 0.30f, Light); break;
        case 166: Vector(Bright * 0.30f, Bright * 0.20f, Bright * 0.10f, Light); break;
        case 167: Vector(Bright * 0.10f, Bright * 0.30f, Bright * 0.20f, Light); break;
        case 168: Vector(Bright * 0.20f, Bright * 0.30f, Bright * 0.10f, Light); break;
        case 169: Vector(Bright * 0.30f, Bright * 0.10f, Bright * 0.20f, Light); break;
        case 170: Vector(Bright * 0.10f, Bright * 0.20f, Bright * 0.30f, Light); break;
        case 171: Vector(Bright * 0.60f, Bright * 0.60f, Bright * 0.20f, Light); break;
        case 172: Vector(Bright * 0.20f, Bright * 0.60f, Bright * 0.60f, Light); break;
        case 173: Vector(Bright * 0.60f, Bright * 0.20f, Bright * 0.60f, Light); break;
        case 174: Vector(Bright * 0.50f, Bright * 0.10f, Bright * 0.10f, Light); break;
        case 175: Vector(Bright * 0.10f, Bright * 0.50f, Bright * 0.10f, Light); break;
        case 176: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.50f, Light); break;
        case 177: Vector(Bright * 0.00f, Bright * 0.20f, Bright * 0.10f, Light); break;
        case 178: Vector(Bright * 0.10f, Bright * 0.00f, Bright * 0.20f, Light); break;
        case 179: Vector(Bright * 0.20f, Bright * 0.10f, Bright * 0.00f, Light); break;
        case 180: Vector(Bright * 0.05f, Bright * 0.05f, Bright * 0.05f, Light); break;
        case 181: Vector(Bright * 0.15f, Bright * 0.15f, Bright * 0.15f, Light); break;
        case 182: Vector(Bright * 0.25f, Bright * 0.25f, Bright * 0.25f, Light); break;
        case 183: Vector(Bright * 0.35f, Bright * 0.35f, Bright * 0.35f, Light); break;
        case 184: Vector(Bright * 0.45f, Bright * 0.45f, Bright * 0.45f, Light); break;
        case 185: Vector(Bright * 0.55f, Bright * 0.55f, Bright * 0.55f, Light); break;
        case 186: Vector(Bright * 0.65f, Bright * 0.65f, Bright * 0.65f, Light); break;
        case 187: Vector(Bright * 0.75f, Bright * 0.75f, Bright * 0.75f, Light); break;
        case 188: Vector(Bright * 0.85f, Bright * 0.85f, Bright * 0.85f, Light); break;
        case 189: Vector(Bright * 0.95f, Bright * 0.95f, Bright * 0.95f, Light); break;
        case 190: Vector(Bright * 1.00f, Bright * 1.00f, Bright * 1.00f, Light); break;
        case 191: Vector(Bright * 0.10f, Bright * 0.00f, Bright * 0.00f, Light); break;
        case 192: Vector(Bright * 0.00f, Bright * 0.10f, Bright * 0.00f, Light); break;
        case 193: Vector(Bright * 0.00f, Bright * 0.00f, Bright * 0.10f, Light); break;
        case 194: Vector(Bright * 0.20f, Bright * 0.10f, Bright * 0.10f, Light); break;
        case 195: Vector(Bright * 0.10f, Bright * 0.20f, Bright * 0.10f, Light); break;
        case 196: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.20f, Light); break;
        case 197: Vector(Bright * 0.20f, Bright * 0.20f, Bright * 0.00f, Light); break;
        case 198: Vector(Bright * 0.00f, Bright * 0.20f, Bright * 0.20f, Light); break;
        case 199: Vector(Bright * 0.20f, Bright * 0.00f, Bright * 0.20f, Light); break;
        case 200: Vector(Bright * 0.50f, Bright * 0.25f, Bright * 0.15f, Light); break;
        case 201: Vector(Bright * 0.15f, Bright * 0.50f, Bright * 0.25f, Light); break;
        case 202: Vector(Bright * 0.25f, Bright * 0.15f, Bright * 0.50f, Light); break;
        case 203: Vector(Bright * 0.30f, Bright * 0.45f, Bright * 0.20f, Light); break;
        case 204: Vector(Bright * 0.20f, Bright * 0.30f, Bright * 0.45f, Light); break;
        case 205: Vector(Bright * 0.45f, Bright * 0.20f, Bright * 0.30f, Light); break;
        case 206: Vector(Bright * 0.60f, Bright * 0.10f, Bright * 0.10f, Light); break;
        case 207: Vector(Bright * 0.10f, Bright * 0.60f, Bright * 0.10f, Light); break;
        case 208: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.60f, Light); break;
        case 209: Vector(Bright * 0.60f, Bright * 0.60f, Bright * 0.10f, Light); break;
        case 210: Vector(Bright * 0.10f, Bright * 0.60f, Bright * 0.60f, Light); break;
        case 211: Vector(Bright * 0.60f, Bright * 0.10f, Bright * 0.60f, Light); break;
        case 212: Vector(Bright * 0.50f, Bright * 0.50f, Bright * 0.10f, Light); break;
        case 213: Vector(Bright * 0.10f, Bright * 0.50f, Bright * 0.50f, Light); break;
        case 214: Vector(Bright * 0.50f, Bright * 0.10f, Bright * 0.50f, Light); break;
        case 215: Vector(Bright * 0.35f, Bright * 0.40f, Bright * 0.45f, Light); break;
        case 216: Vector(Bright * 0.45f, Bright * 0.35f, Bright * 0.40f, Light); break;
        case 217: Vector(Bright * 0.40f, Bright * 0.45f, Bright * 0.35f, Light); break;
        case 218: Vector(Bright * 0.30f, Bright * 0.30f, Bright * 0.60f, Light); break;
        case 219: Vector(Bright * 0.60f, Bright * 0.30f, Bright * 0.30f, Light); break;
        case 220: Vector(Bright * 0.30f, Bright * 0.60f, Bright * 0.30f, Light); break;
        case 221: Vector(Bright * 0.20f, Bright * 0.40f, Bright * 0.50f, Light); break;
        case 222: Vector(Bright * 0.50f, Bright * 0.20f, Bright * 0.40f, Light); break;
        case 223: Vector(Bright * 0.40f, Bright * 0.50f, Bright * 0.20f, Light); break;
        case 224: Vector(Bright * 0.15f, Bright * 0.35f, Bright * 0.55f, Light); break;
        case 225: Vector(Bright * 0.55f, Bright * 0.15f, Bright * 0.35f, Light); break;
        case 226: Vector(Bright * 0.35f, Bright * 0.55f, Bright * 0.15f, Light); break;
        case 227: Vector(Bright * 0.25f, Bright * 0.45f, Bright * 0.65f, Light); break;
        case 228: Vector(Bright * 0.65f, Bright * 0.25f, Bright * 0.45f, Light); break;
        case 229: Vector(Bright * 0.45f, Bright * 0.65f, Bright * 0.25f, Light); break;
        case 230: Vector(Bright * 0.05f, Bright * 0.25f, Bright * 0.75f, Light); break;
        case 231: Vector(Bright * 0.75f, Bright * 0.05f, Bright * 0.25f, Light); break;
        case 232: Vector(Bright * 0.25f, Bright * 0.75f, Bright * 0.05f, Light); break;
        case 233: Vector(Bright * 0.15f, Bright * 0.15f, Bright * 0.65f, Light); break;
        case 234: Vector(Bright * 0.65f, Bright * 0.15f, Bright * 0.15f, Light); break;
        case 235: Vector(Bright * 0.15f, Bright * 0.65f, Bright * 0.15f, Light); break;
        case 236: Vector(Bright * 0.30f, Bright * 0.10f, Bright * 0.60f, Light); break;
        case 237: Vector(Bright * 0.10f, Bright * 0.60f, Bright * 0.30f, Light); break;
        case 238: Vector(Bright * 0.60f, Bright * 0.30f, Bright * 0.10f, Light); break;
        case 239: Vector(Bright * 0.00f, Bright * 0.20f, Bright * 0.70f, Light); break;
        case 240: Vector(Bright * 0.20f, Bright * 0.70f, Bright * 0.00f, Light); break;
        case 241: Vector(Bright * 0.70f, Bright * 0.00f, Bright * 0.20f, Light); break;
        case 242: Vector(Bright * 0.35f, Bright * 0.65f, Bright * 0.55f, Light); break;
        case 243: Vector(Bright * 0.65f, Bright * 0.55f, Bright * 0.35f, Light); break;
        case 244: Vector(Bright * 0.55f, Bright * 0.35f, Bright * 0.65f, Light); break;
        case 245: Vector(Bright * 0.70f, Bright * 0.40f, Bright * 0.10f, Light); break;
        case 246: Vector(Bright * 0.10f, Bright * 0.70f, Bright * 0.40f, Light); break;
        case 247: Vector(Bright * 0.40f, Bright * 0.10f, Bright * 0.70f, Light); break;
        case 248: Vector(Bright * 0.70f, Bright * 0.10f, Bright * 0.40f, Light); break;
        case 249: Vector(Bright * 0.40f, Bright * 0.70f, Bright * 0.10f, Light); break;
        case 250: Vector(Bright * 0.10f, Bright * 0.40f, Bright * 0.70f, Light); break;
        case 251: Vector(Bright * 0.60f, Bright * 0.20f, Bright * 0.50f, Light); break;
        case 252: Vector(Bright * 0.50f, Bright * 0.60f, Bright * 0.20f, Light); break;
        case 253: Vector(Bright * 0.20f, Bright * 0.50f, Bright * 0.60f, Light); break;
        case 254: Vector(Bright * 0.10f, Bright * 0.10f, Bright * 0.90f, Light); break;
        case 255: Vector(Bright * 0.90f, Bright * 0.10f, Bright * 0.10f, Light); break;
    }
}
...
void RenderPartObjectBody(BMD* b, OBJECT* o, int Type, float Alpha, int RenderType)
{
   ...
   // g_pNewUIRenderColorItem->RenderCEfect(b, o, Type, Alpha, RenderType);
    ...
    else if (g_pCustomRenderEffect->RenderItemMesh(Type - MODEL_ITEM, b, o))
    {
    }

    else if (gCapeRender.RenderModelCapeMesh(Type, b, o))
    {
        //RISE usually suck cunt here.
    }
    ...
    //else if (g_pNewUIRenderColorItem->IsRenderWeapon(Type))
    //{
    //    // No need
    //}
    ...
    else if (Type == MODEL_SWORD + 21 || Type == MODEL_HELPER + 152)
    {
        b->RenderBody(RENDER_TEXTURE, Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, o->HiddenMesh);
    ...
    else if (Type == MODEL_HELPER + 66 || Type == MODEL_HELPER + 154|| Type == MODEL_HELPER + 155)
    {
        b->RenderMesh(0, RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
    ...
    else if (Type == MODEL_HELM + 61)
    {
        ...
        || gCustomBattleGloves.CheckGloves(g_CMonkSystem.EqualItemModelType(Type) - MODEL_ITEM)
        )
    {
        b->RenderBody(RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, o->HiddenMesh);
    }
...
{
    int MonsterGolden = g_pMonsterGlow->HasBrightness(iMonsterIndex);
    ...
    else if (MonsterGolden != 0 || iMonsterIndex >= 493 && iMonsterIndex <= 502)
    {
    ...
    else if (Type >= MODEL_SWORD + 32 && Type <= MODEL_SWORD + 35 || gCustomBattleGloves.CheckGloves(Type - MODEL_ITEM))
    {
        b->RenderBody(RenderType, Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
    }
    ...
    else if (g_pCustomRenderEffect->RenderItemBodyEffect(Type, b, o, Alpha, RenderType, Texture))
    {
...
void NextGradeObjectRender(CHARACTER* c)
{
    ...
    //RISE Glow Item +15 Removed

    if (!g_pOption->GetRenderEndGlow())
    {
        return;
    }
...
void RenderPartObjectEffect(OBJECT* o, int Type, vec3_t Light, float Alpha, int ItemLevel, int Option1, int ExtOption, int Select, int RenderType)
{
    ...
    gCapeRender.RenderEffectWingAndCape(Type, b, o);
    ...
    case MODEL_WING + 49:Level = 0; break;
    ...
    else if (o->Type == MODEL_POTION + 220)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
    ...
    else if (o->Type == MODEL_POTION + 221)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
    ...
    else if (o->Type == MODEL_POTION + 222)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.6f, 0.0f, 0.8f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 223)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(1.0f, 0.8f, 0.2f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 224)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.7f, 0.7f, 0.7f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 225)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.9f, 0.4f, 0.1f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 226)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.0f, 1.0f, 0.0f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 227)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.0f, 1.0f, 1.0f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 228)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.6f, 0.3f, 0.0f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 229)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.6f, 0.7f, 0.5f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type == MODEL_POTION + 230)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        b->LightEnable = true;
        Vector(0.1f, 0.2f, 0.5f, b->BodyLight);
        o->Alpha = 0.5f;
        b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        return;
    }
    else if (o->Type >= MODEL_POTION + 268 && o->Type <= MODEL_POTION + 273)
    {
        Vector(1.f, 1.f, 1.f, b->BodyLight);
        b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        ...
        if (o->Type == MODEL_POTION + 244)        Vector(0.4f, 1.0f, 0.9f, b->BodyLight);
    ...
    /*else if (
        o->Type == MODEL_POTION + 244 ||
        o->Type == MODEL_POTION + 245 ||
        o->Type == MODEL_POTION + 246 ||
        o->Type == MODEL_POTION + 247 ||
        o->Type == MODEL_POTION + 248 ||
        o->Type == MODEL_POTION + 249 ||
        o->Type == MODEL_POTION + 250 ||
        o->Type == MODEL_POTION + 251 ||
        o->Type == MODEL_POTION + 252 ||
        o->Type == MODEL_POTION + 259
        )
        {
            Vector(1.f, 1.f, 1.f, b->BodyLight);
            b->RenderBody(RENDER_TEXTURE, o->Alpha, -1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
            ...
            if (o->Type == MODEL_POTION + 244)        Vector(0.4f, 1.0f, 0.9f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 245)   Vector(1.0f, 0.8f, 0.2f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 246)   Vector(0.2f, 1.0f, 0.2f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 247)   Vector(0.0f, 0.7f, 0.9f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 248)   Vector(1.0f, 0.4f, 0.1f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 249)   Vector(0.7f, 0.2f, 1.0f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 250)   Vector(0.9f, 0.4f, 0.6f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 251)   Vector(0.5f, 0.3f, 0.1f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 252)   Vector(0.5f, 0.6f, 0.8f, b->BodyLight);
            else if (o->Type == MODEL_POTION + 259)   Vector(0.4f, 0.5f, 0.3f, b->BodyLight);

            o->Alpha = 0.5f;

            b->RenderMesh(0, RENDER_BRIGHT | RENDER_CHROME,
                o->Alpha, -1,
                o->BlendMeshLight,
    ...
    else if (o->Type == MODEL_WING + 5 && gMapManager.InBloodCastle() == false)
    {
    ...
    else if ((Type == MODEL_WING + 36 || Type == MODEL_WING + 157) && gMapManager.InBloodCastle() == false) // RISE Fix Wing 3 BC Crash
    {
        vec3_t vRelativePos, vPos, vLight;
        Vector(0.f, 0.f, 0.f, vRelativePos);
        Vector(0.f, 0.f, 0.f, vPos);
    ...
    if (!o->EnableShadow)
    {
            ...
            )
        {
            if ((Option1 & 63) > 0 && (o->Type<MODEL_WING || o->Type>MODEL_WING + 6) && o->Type != MODEL_HELPER + 30
                ...
                && !gCapeRender.IncludeCape(o->Type)
                )
            {
                ...
                {
                    if (!g_pCustomRenderEffect->RenderItemBodyEffect(Type, b, o, Alpha, RENDER_TEXTURE | RENDER_BRIGHT, -1))
...
void RenderPartObjectEdge(BMD* b, OBJECT* o, int Flag, bool Translate, float Scale)
{
    ...
    {
        if (!g_pCustomRenderEffect->RenderItemBodyEffect(o->Type, b, o, o->Alpha, Flag, -1))
