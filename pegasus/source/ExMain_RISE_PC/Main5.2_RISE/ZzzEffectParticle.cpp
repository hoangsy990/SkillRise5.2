...
#include "RISE/PegasusSwordWrathMath.h"
#include "RISE/PegasusDeathsideRuntime.h"
#include "RISE/PegasusDexBoosterRuntime.h"
#include "RISE/PegasusDexBoosterContract.h"
#include "RISE/PegasusSpiralChargeContract.h"
...
namespace
{
    ...
    bool g_pegasusQaParticleSlots[MAX_PARTICLES] = {};
}

bool IsPegasusQaParticleSlot(int index)
{
    return index >= 0 && index < MAX_PARTICLES &&
        g_pegasusQaParticleSlots[index];
}
...
int CreateParticle(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, float Scale, OBJECT* Owner, int PegasusVariant, bool PegasusOwned)
{
    ...
    int particleIndex = g_particleCreateCursor;
    if (particleIndex >= particleCapacity)
    {
        particleIndex = 0;
    }

    for (int scanned = 0; scanned < particleCapacity; ++scanned)
    {
        int i = particleIndex + scanned;
        if (i >= particleCapacity)
        {
            i -= particleCapacity;
        }
        ...
        if (!o->Live)
        {
            const bool pegasusTracked = PegasusOwned ||
                rise::pegasus::IsPegasusPrivateEffectTexture(Type) ||
                rise::pegasus::IsDeathsideParticleBitmap(Type) ||
                rise::pegasus::IsSwordWrathSmokeBitmap(Type) ||
                rise::pegasus::IsSwordWrathPinStarBitmap(Type) ||
                (Type == BITMAP_WATERFALL_4 &&
                    SubType == rise::pegasus::kDeathsideSmokeSubtype);
            g_pegasusQaParticleSlots[i] = pegasusTracked;
            g_particleCreateCursor = i + 1;
            if (g_particleCreateCursor >= particleCapacity)
            {
                g_particleCreateCursor = 0;
            }
            ...
            if (Type == BITMAP_WATERFALL_4 && SubType == 0x74)
            {
                // Pegasus Deathside 0x65FDA6..0x65FDC1.
                o->bEnableMove = false;
                o->LifeTime = 40.0f;
                o->Scale = 0.51f;
                o->Alpha = 0.0f;
            }
            else if (rise::pegasus::IsDeathsideGroundSmokeBitmap(Type) &&
                SubType == rise::pegasus::kDeathsideGroundSmokeSubtype)
            {
                // Pegasus 0x65FDE6..0x65FE21.
                o->bEnableMove = false;
                o->LifeTime = 40.0f;
                o->Scale = 0.6f;
                o->Alpha = 0.0f;
            ...
            else if (rise::pegasus::IsDeathsideSmokeBitmap(Type) &&
                SubType == rise::pegasus::kDeathsideSmokeSubtype)
            {
                // Pegasus 0x65FDC6..0x65FDE1.
                o->bEnableMove = false;
                o->LifeTime = 16.0f;
                o->Scale = 0.48f;
                o->Alpha = 0.0f;
            ...
            if (rise::pegasus::IsSwordWrathSmokeBitmap(Type))
            {
                o->bEnableMove = false;
                o->LifeTime = static_cast<float>(
                    rise::pegasus::kSwordWrathSmokeLifetime);
                o->Scale = 2.0f + static_cast<float>(rand() % 1001) * 0.003f;
                o->Gravity = 2.0f + static_cast<float>(rand() % 1001) * 0.003f;
                o->Alpha = 1.0f;
                o->Rotation = static_cast<float>(rand() % 360);
            }
            else if (rise::pegasus::IsSwordWrathPinStarBitmap(Type))
            {
                o->bEnableMove = false;
                o->LifeTime = static_cast<float>(
                    rise::pegasus::kSwordWrathPinStarLifetime);
                o->Alpha = 1.0f;
            }

            if (PegasusVariant != -32768)
            {
                o->iNumBone = PegasusVariant;
            }
            ...
            switch (o->Type)
            {
            case rise::pegasus::kSpiralChargeSteamBitmap:
                if (o->SubType == rise::pegasus::kSpiralChargeSteamParticleSubtype)
                {
                    o->bEnableMove = false;
                    o->LifeTime = rise::pegasus::kSpiralChargeSteamParticleLifetime;
                    o->Alpha = 1.0f;
                    o->Gravity = Scale;
                }
                break;
            case rise::pegasus::kSpiralChargeSmokeBitmap:
                if (o->SubType == rise::pegasus::kSpiralChargeSmokeParticleSubtype)
                {
                    o->bEnableMove = false;
                    o->LifeTime = rise::pegasus::kSpiralChargeSmokeParticleLifetime;
                    o->Alpha = 0.0f;
                    o->Scale = static_cast<float>((rand() % 51) + 50) *
                        0.01f * Scale;
                    o->Rotation = static_cast<float>(rand() % 361);
                }
                break;
            case rise::pegasus::kDexBoosterImpactBitmap:
                if (o->SubType == 0)
                {
                    // Pegasus 0x66184D..0x66196C: Impack03 subtype 0 is a
                    // sixty-tick radial particle. Disable generic movement;
                    // the exact velocity/gravity update is handled below.
                    o->bEnableMove = false;
                    o->LifeTime = rise::pegasus::kDexBoosterImpactParticleLifetime;
                    o->Scale = static_cast<float>((rand() % 64) + 64) *
                        0.005f * 0.15f;
                    o->Alpha = 1.0f;
                    o->Rotation = static_cast<float>(rand() % 360);
                    o->Gravity = static_cast<float>((rand() % 30) - 10) * 0.3f;
                    const float heading = static_cast<float>(rand() % 360) *
                        Q_PI / 180.0f;
                    const float speed = static_cast<float>((rand() % 20) + 12) * 0.1f;
                    o->Velocity[0] = cosf(heading) * speed;
                    o->Velocity[1] = sinf(heading) * speed;
                    o->Velocity[2] = 0.0f;
                }
                break;
            case rise::pegasus::kChaosBladeFlareBlueBitmap:
                o->bEnableMove = false;
                o->LifeTime = 20.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladePinStarBitmap:
                o->bEnableMove = false;
                o->LifeTime = 12.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladeSteamBitmap:
                o->bEnableMove = false;
                o->LifeTime = 10.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladeSmokeBitmap:
                o->bEnableMove = false;
                o->LifeTime = 20.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladeShockwaveBitmap:
                o->bEnableMove = false;
                o->LifeTime = 8.0f;
                o->Scale = 0.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladeSmokeLineBitmap:
                o->bEnableMove = false;
                o->LifeTime = 20.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kChaosBladeSmokeLine03Bitmap:
                o->bEnableMove = false;
                o->LifeTime = 30.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kShiningBirdSmokeBitmap:
                o->bEnableMove = false;
                o->LifeTime = 20.0f;
                o->Scale = 2.0f;
                o->Alpha = 1.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                break;
            case rise::pegasus::kShiningBirdPinStarBitmap:
                o->bEnableMove = false;
                o->LifeTime = 25.0f;
                o->Scale = 0.6f;
                o->Alpha = 1.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                Vector(0.0f, 0.0f, -6.6f, o->Velocity);
                break;
            case rise::pegasus::kDragonViolentFlareBitmap:
                // Pegasus 0x663224: 0x7F41 flare01.
                o->bEnableMove = false;
                o->LifeTime = 15.0f;
                o->Scale = 3.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kDragonViolentLightningBitmap:
                // Pegasus 0x6631F4: 0x7F42 lighting_mega03.
                o->bEnableMove = false;
                o->LifeTime = 5.0f;
                o->Scale = 1.0f;
                o->Alpha = 1.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                break;
            case rise::pegasus::kDragonViolentSmokeBitmap:
                // Pegasus 0x6631C3: 0x7F43 smoke01.
                o->bEnableMove = false;
                o->LifeTime = 40.0f;
                o->Scale = static_cast<float>((rand() % 32) + 8) * 0.07f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kDragonViolentImpactBitmap:
                // Pegasus 0x663198: 0x7F44 Impack03.
                o->bEnableMove = false;
                o->LifeTime = 8.0f;
                o->Scale = static_cast<float>((rand() % 81) + 60) * 0.01f;
                o->Alpha = 1.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                break;
            case rise::pegasus::kDragonViolentComboBitmap:
                // Pegasus 0x66318C: 0x7F45 Combo4 grows from zero.
                o->bEnableMove = false;
                o->LifeTime = 20.0f;
                o->Scale = 0.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kSwordBlowSparkBitmap:
                // Pegasus 0x678875..0x678990 (Spark03 subtypes 36/37).
                if (o->SubType == 36)
                {
                    o->Scale = static_cast<float>((rand() % 4) + 3) * 0.2f;
                }
                else
                {
                    int precision = static_cast<int>(Scale);
                    if (precision < 1) precision = 1;
                    o->Scale = (Scale - static_cast<float>(rand() % precision) * 0.5f) * 0.1f;
                }
                o->LifeTime = static_cast<float>((rand() % 12) + 16);
                o->Gravity = static_cast<float>((rand() % 16) + 6);
                o->TurningForce[0] = static_cast<float>(rand() % 20);
                o->Rotation = 0.0f;
                o->Alpha = 1.0f;
                break;
            case rise::pegasus::kSwordBlowGroundSmokeBitmap:
                // Pegasus 0x67884B..0x678890 (resource 0x7F31).
                o->LifeTime = 25.0f;
                o->Rotation = static_cast<float>(rand() % 360);
                o->Alpha = 1.0f;
                break;
            ...
            case BITMAP_SMOKE:
                ...
                {
                case 80:
                    // Wind Soul S21 smoke constructor 0x165AC25.  This must
                    // live in BITMAP_SMOKE, not the adjacent cloud bitmap
                    // switch: CreateParticle is called with BITMAP_SMOKE.
                    o->LifeTime = 40;
                    o->Scale = (float)(rand() % 64 + 64) * 0.01f;
                    o->Rotation = (float)(rand() % 360);
                    o->Gravity = (float)(rand() % 32 + 60) * 0.1f;
            ...
            case BITMAP_SPARK + 1:
                ...
                case 30:
                {
                    o->LifeTime = rand() % 5 + 30;

                    o->Scale = (float)(rand() % 15) / 10.0f + 0.2f;
            ...
            case BITMAP_EXPLOTION:
                o->LifeTime = 15 + rand() % 15;
                o->Scale = Scale * (0.6f + ((float)(rand() % 7) / 10.f));
                if (o->SubType == 2)
                {
                    o->LifeTime = 30;
                }
            ...
            if (pegasusTracked)
                rise::pegasus::TraceRuntime("PEGASUS_PARTICLE_CREATE",
                    Type, SubType, "ALLOCATED");
#endif
            return i;
        }
    }
#if defined(PEGASUS_ISOLATED_TEST)
    if (PegasusOwned || rise::pegasus::IsPegasusPrivateEffectTexture(Type) ||
        rise::pegasus::IsDeathsideParticleBitmap(Type) ||
        rise::pegasus::IsSwordWrathSmokeBitmap(Type) ||
        rise::pegasus::IsSwordWrathPinStarBitmap(Type) ||
        (Type == BITMAP_WATERFALL_4 &&
    ...
    {
        rise::pegasus::TraceRuntime("PEGASUS_PARTICLE_CREATE",
            Type, SubType, "POOL_FULL");
        // Failure telemetry only. Do not bypass the native adaptive budget or
        // confuse an exhausted search prefix with every pool slot being live.
        static bool budgetFailureSampled = false;
        if (!budgetFailureSampled)
        {
            budgetFailureSampled = true;
            int liveInside = 0, liveOutside = 0, pegasusLive = 0;
            for (int slot = 0; slot < MAX_PARTICLES; ++slot)
            {
                if (!Particles[slot].Live) continue;
                if (slot < particleCapacity) ++liveInside;
                else ++liveOutside;
                if (g_pegasusQaParticleSlots[slot]) ++pegasusLive;
            }
            char detail[384];
            _snprintf_s(detail, sizeof(detail), _TRUNCATE,
                "capacity=%d max=%d liveInside=%d liveOutside=%d pegasusLive=%d fps=%.3f fpsAvg=%.3f factor=%.5f position=(%.2f,%.2f,%.2f) camera=(%.2f,%.2f,%.2f)",
                particleCapacity, MAX_PARTICLES, liveInside, liveOutside,
                pegasusLive, static_cast<double>(FPS), static_cast<double>(FPS_AVG),
                static_cast<double>(FPS_ANIMATION_FACTOR),
                Position[0], Position[1], Position[2],
                CameraPosition[0], CameraPosition[1], CameraPosition[2]);
            rise::pegasus::TraceRuntime("PEGASUS_PARTICLE_BUDGET_FAILURE",
                Type, SubType, detail);
        }
    }
#endif
...
void MoveParticles()
{
    ...
    for (int i = 0; i < MAX_PARTICLES; i++)
    {
        ...
        if (o->Live)
        {
            ...
            if (rise::pegasus::IsSwordWrathSmokeBitmap(static_cast<int>(o->Type)))
            {
                o->Scale += (o->LifeTime > rise::pegasus::kSwordWrathSmokeScaleSplit
                    ? rise::pegasus::kSwordWrathSmokeScaleDelta
                    : -rise::pegasus::kSwordWrathSmokeScaleDelta) *
                    FPS_ANIMATION_FACTOR;
                o->Position[2] += o->Gravity * FPS_ANIMATION_FACTOR;
                if (o->Scale <= 0.0f) o->Live = false;
            }
            else if (rise::pegasus::IsSwordWrathPinStarBitmap(static_cast<int>(o->Type)))
            {
                o->Scale -= rise::pegasus::kSwordWrathPinStarScaleDelta *
                    FPS_ANIMATION_FACTOR;
                o->Position[2] += rise::pegasus::kSwordWrathPinStarRise *
                    FPS_ANIMATION_FACTOR;
                if (o->Scale <= 0.0f) o->Live = false;
            }

            const bool deathsideWaterfall =
                o->Type == BITMAP_WATERFALL_4 && o->SubType == 0x74;
            const bool deathsideGroundSmoke =
                rise::pegasus::IsDeathsideGroundSmokeBitmap(static_cast<int>(o->Type)) &&
                o->SubType == rise::pegasus::kDeathsideGroundSmokeSubtype;
            const bool deathsideSmoke =
                rise::pegasus::IsDeathsideSmokeBitmap(static_cast<int>(o->Type)) &&
                o->SubType == rise::pegasus::kDeathsideSmokeSubtype;
            if (deathsideWaterfall || deathsideGroundSmoke || deathsideSmoke)
            {
                // Exact 0x66013C..0x66021E remaining-life envelope. All three
                // particles fade in until ratio .3, fade out afterward, grow
                // toward their own recovered end scale, and move only on Z.
                const float duration = deathsideSmoke ? 16.0f : 40.0f;
                const float startScale = deathsideWaterfall ? 0.51f :
                    deathsideSmoke ? 0.48f : 0.6f;
                const float endScale = deathsideWaterfall ? 2.1f :
                    deathsideSmoke ? 1.18f : 2.81f;
                float ratio = o->LifeTime / duration;
                if (ratio < 0.0f) ratio = 0.0f;
                if (ratio > 1.0f) ratio = 1.0f;
                o->Scale = startScale + (endScale - startScale) * (1.0f - ratio);
                o->Alpha = ratio <= 0.3f ? ratio / 0.3f :
                    (1.0f - ratio) / 0.7f;
                const float vertical = deathsideWaterfall ? -3.9f :
                    deathsideSmoke ? 0.44f : -5.0f;
                o->Position[2] += vertical * FPS_ANIMATION_FACTOR;
            }
            ...
            case rise::pegasus::kSpiralChargeSteamBitmap:
                if (o->SubType == rise::pegasus::kSpiralChargeSteamParticleSubtype)
                {
                    o->Alpha = o->LifeTime > 0.0f ?
                        o->LifeTime / rise::pegasus::kSpiralChargeSteamParticleLifetime : 0.0f;
                    o->Scale += o->Gravity *
                        rise::pegasus::kSpiralChargeParticleGrowth *
                        FPS_ANIMATION_FACTOR;
                }
                break;
            case rise::pegasus::kSpiralChargeSmokeBitmap:
                if (o->SubType == rise::pegasus::kSpiralChargeSmokeParticleSubtype)
                {
                    const float alphaStep =
                        rise::pegasus::kSpiralChargeSmokeAlphaStep *
                        FPS_ANIMATION_FACTOR;
                    o->Alpha += o->LifeTime >
                        rise::pegasus::kSpiralChargeSmokeFadeSplit ?
                        alphaStep : -alphaStep;
                    if (o->Alpha < 0.0f) o->Alpha = 0.0f;
                    if (o->Alpha > 1.0f) o->Alpha = 1.0f;
                    o->Scale += rise::pegasus::kSpiralChargeParticleGrowth *
                        FPS_ANIMATION_FACTOR;
                }
                break;
            case rise::pegasus::kDexBoosterImpactBitmap:
                if (o->SubType == 0)
                {
                    // Pegasus 0x661ACD..0x661C20. Native lifetime decrement
                    // occurs immediately above this switch.
                    if (o->LifeTime < rise::pegasus::kDexBoosterImpactFadeStart)
                    {
                        o->Alpha -= 0.02f * FPS_ANIMATION_FACTOR;
                        o->Scale -= 0.00075f * FPS_ANIMATION_FACTOR;
                    }
                    else if (o->Alpha < rise::pegasus::kDexBoosterImpactPeakAlpha)
                    {
                        o->Alpha += static_cast<float>((rand() % 5) + 2) *
                            0.1f * FPS_ANIMATION_FACTOR;
                        o->Scale -= 0.00012f * FPS_ANIMATION_FACTOR;
                    }
                    else
                    {
                        o->Alpha = rise::pegasus::kDexBoosterImpactPeakAlpha;
                        o->Scale += static_cast<float>((rand() % 4) + 8) *
                            0.001f * 0.15f * FPS_ANIMATION_FACTOR;
                    }
                    if (o->Alpha < rise::pegasus::kDexBoosterImpactStopAlpha)
                        o->Live = false;
            ...
            case rise::pegasus::kChaosBladeSteamBitmap:
                ...
                o->Position[0] += 4.0f * FPS_ANIMATION_FACTOR;
                break;
            case rise::pegasus::kChaosBladeSmokeBitmap:
                o->Scale += 0.03f * FPS_ANIMATION_FACTOR;
                o->Position[0] += 3.0f * FPS_ANIMATION_FACTOR;
                break;
            case rise::pegasus::kChaosBladeShockwaveBitmap:
            ...
            case rise::pegasus::kChaosBladeSmokeLineBitmap:
                ...
                {
                    o->Light[channel] -= 0.05f * FPS_ANIMATION_FACTOR;
                    if (o->Light[channel] < 0.0f) o->Light[channel] = 0.0f;
                }
                break;
            case rise::pegasus::kChaosBladeSmokeLine03Bitmap:
                o->Scale += 0.003333f * FPS_ANIMATION_FACTOR;
                for (int channel = 0; channel < 3; ++channel)
                {
                    o->Light[channel] -= (1.0f / 30.0f) * FPS_ANIMATION_FACTOR;
                    if (o->Light[channel] < 0.0f) o->Light[channel] = 0.0f;
                }
                break;
            case rise::pegasus::kShiningBirdSmokeBitmap:
            {
                const float lifeRatio = o->LifeTime / 20.0f;
                Vector(lifeRatio * 0.5f, lifeRatio * 0.4f, lifeRatio, o->Light);
                o->Scale += 0.45f * FPS_ANIMATION_FACTOR;
            }
                break;
            case rise::pegasus::kShiningBirdPinStarBitmap:
                o->Scale = o->LifeTime * 0.6f / 25.0f;
                o->Alpha = o->LifeTime > 10.0f ? 1.0f
                    : (o->LifeTime + 1.0f) / 12.0f;
                o->Position[2] -= 6.6f * FPS_ANIMATION_FACTOR;
                break;
            case rise::pegasus::kDragonViolentFlareBitmap:
                if (o->LifeTime < 7.5f)
                    o->Alpha = o->LifeTime / 7.5f;
                break;
            case rise::pegasus::kDragonViolentLightningBitmap:
                o->Alpha -= 0.15f * FPS_ANIMATION_FACTOR;
                if (o->Alpha < 0.1f) o->Alpha = 0.0f;
                break;
            case rise::pegasus::kDragonViolentSmokeBitmap:
            {
                const float life = o->LifeTime / 40.0f * 0.25f;
                Vector(life * 0.45f, life * 0.7f, life, o->Light);
                o->Rotation += 2.0f * FPS_ANIMATION_FACTOR;
                o->Scale += 0.01f * FPS_ANIMATION_FACTOR;
            }
                break;
            case rise::pegasus::kDragonViolentImpactBitmap:
                o->Rotation += 20.0f * FPS_ANIMATION_FACTOR;
                if (o->LifeTime < 4.0f)
                    o->Alpha = o->LifeTime * 0.25f;
                break;
            case rise::pegasus::kDragonViolentComboBitmap:
            ...
            case rise::pegasus::kSwordBlowSparkBitmap:
            {
                // Pegasus 0x678BEC..0x678D06: gravity, terrain collision,
                // timestep-aware 0.6 bounce and direction-speed movement.
                if (o->LifeTime < 0.0f)
                    o->Live = false;
                o->Position[2] += o->Gravity * FPS_ANIMATION_FACTOR;
                o->Gravity -= 2.0f * FPS_ANIMATION_FACTOR;
                const float terrain = RequestTerrainHeight(o->Position[0], o->Position[1]);
                if (o->Position[2] < terrain)
                {
                    o->Position[2] = terrain;
                    o->Gravity = -o->Gravity *
                        powf(0.6f, FPS_ANIMATION_FACTOR);
                    o->LifeTime -= 4.0f * FPS_ANIMATION_FACTOR;
                }
                const float speed = o->TurningForce[0];
                o->Position[0] += o->Angle[0] * speed * FPS_ANIMATION_FACTOR;
                o->Position[1] += o->Angle[1] * speed * FPS_ANIMATION_FACTOR;
                o->Position[2] += o->Angle[2] * speed * FPS_ANIMATION_FACTOR;
            }
                break;
            case rise::pegasus::kSwordBlowGroundSmokeBitmap:
                // Pegasus 0x678B7D..0x678BE7. Native pool decrement/release
                // remains authoritative around this exact field update.
            ...
            case BITMAP_FIRE + 1:
                ...
                else if (o->SubType == 0)
                {
                    o->Gravity += (0.02f) * FPS_ANIMATION_FACTOR;
                    o->Scale += o->Gravity * FPS_ANIMATION_FACTOR;
                    float velocityScale = powf(1.03f, FPS_ANIMATION_FACTOR);
                    VectorScale(o->Velocity, velocityScale, o->Velocity);
                    o->Position[2] += o->Gravity * 20.f * FPS_ANIMATION_FACTOR;
                    Luminosity = (float)(o->LifeTime) * 0.15f;
                    Vector(Luminosity, Luminosity, Luminosity, o->Light);
                }
                break;
            case BITMAP_FLAME:
...
void RenderParticles(BYTE byRenderOneMore)
{
    ...
    for (int i = 0; i < MAX_PARTICLES; i++)
    {
        ...
        if (o->Live)
        {
            ...
            int Frame;
            const bool deathsideSmokeRender =
                (o->Type == BITMAP_WATERFALL_4 &&
                    o->SubType == rise::pegasus::kDeathsideSmokeSubtype) ||
                rise::pegasus::IsDeathsideGroundSmokeBitmap(static_cast<int>(o->Type)) ||
                rise::pegasus::IsDeathsideSmokeBitmap(static_cast<int>(o->Type));
            if (deathsideSmokeRender)
            {
                // Pegasus 7CDBD1..7CDC1B (waterfall subtype116),
                // 7CDE67/7CDE76 -> 7CDC71..7CDCAE (ground/smoke):
                // RGB * age alpha, then ZERO / ONE_MINUS_SRC_COLOR.
                // Preserve stored RGB; repeated rendering must not compound alpha.
                vec3_t deathsideRenderLight;
                VectorScale(o->Light, o->Alpha, deathsideRenderLight);
                EnableAlphaBlendMinus();
                RenderSprite(o->TexType, o->Position, Width, Height,
                    deathsideRenderLight, o->Rotation);
            }
            else if (o->Type == rise::pegasus::kChaosBladeSmokeLine03Bitmap)
            {
                // Pegasus emitter655090 selects7F3A. Renderer7CDE67..
                // 7CDE89 routes it to7CD03D ->87F992: ZERO/ONE_MINUS_SRC_COLOR.
                // Unlike Deathside smoke, this branch uses stored RGB directly.
                EnableAlphaBlendMinus();
#if defined(PEGASUS_ISOLATED_TEST)
                static bool tracedChaosSmokeBlend = false;
                if (!tracedChaosSmokeBlend)
                {
                    tracedChaosSmokeBlend = true;
                    GLint sourceBlend = 0, destinationBlend = 0;
                    glGetIntegerv(GL_BLEND_SRC, &sourceBlend);
                    glGetIntegerv(GL_BLEND_DST, &destinationBlend);
                    rise::pegasus::TraceRuntime("CHAOS_SMOKE03_BLEND", sourceBlend,
                        destinationBlend, glIsEnabled(GL_BLEND) ? "ENABLED" : "DISABLED");
                }
#endif
                RenderSprite(o->TexType, o->Position, Width, Height,
                    o->Light, o->Rotation);
            }
            else if (o->Type == rise::pegasus::kSpiralChargeSteamBitmap ||
                o->Type == rise::pegasus::kSpiralChargeSmokeBitmap)
            {
                // Pegasus7F1B/7F1C ->7CDE94..7CDECC: temporary RGB*Alpha,
                // then the normal sprite draw. Do not compound stored Light.
                vec3_t spiralRenderLight;
                VectorScale(o->Light, o->Alpha, spiralRenderLight);
#if defined(PEGASUS_ISOLATED_TEST)
                static bool tracedSpiralAlpha[2] = {};
                const int spiralVariant = o->Type == rise::pegasus::kSpiralChargeSmokeBitmap ? 1 : 0;
                if (!tracedSpiralAlpha[spiralVariant] && o->Alpha > 0.0f && o->Alpha < 0.9f)
                {
                    tracedSpiralAlpha[spiralVariant] = true;
                    char detail[256];
                    _snprintf_s(detail, sizeof(detail), _TRUNCATE,
                        "alpha=%.4f stored=%.4f,%.4f,%.4f render=%.4f,%.4f,%.4f",
                        o->Alpha, o->Light[0], o->Light[1], o->Light[2],
                        spiralRenderLight[0], spiralRenderLight[1], spiralRenderLight[2]);
                    rise::pegasus::TraceRuntime("SPIRAL_PARTICLE_RENDER_ALPHA", 2014, spiralVariant, detail);
                }
#endif
                RenderSprite(o->TexType, o->Position, Width, Height,
                    spiralRenderLight, o->Rotation);
            }
            else switch (o->Type)
            {
            case BITMAP_WATERFALL_1:
