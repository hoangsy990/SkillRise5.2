...
#include "RISE/PegasusHighSkillInput.h"
...
bool g_bRenderNameMonster = true;
bool g_bRenderImageLogo = true;
...
void RenderTipText(int sx, int sy, const char* Text)
{
	...
	RenderColor((float)sx - 2, (float)sy - 3, (float)TextSize.cx / g_SetForm->x_fScreenRate_x + 4, (float)1);	// À§
	RenderColor((float)sx - 2, (float)sy - 3, (float)1, (float)TextSize.cy / g_SetForm->x_fScreenRate_y + 4);	// ÁÂ
	RenderColor((float)sx - 2 + TextSize.cx / g_SetForm->x_fScreenRate_x + 3, (float)sy - 3, (float)1, (float)TextSize.cy / g_SetForm->x_fScreenRate_y + 4);
	RenderColor((float)sx - 2, (float)sy - 3 + TextSize.cy / g_SetForm->x_fScreenRate_y + 3, (float)TextSize.cx / g_SetForm->x_fScreenRate_x + 4, (float)1);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor((float)sx - 1, (float)sy - 2, (float)TextSize.cx / g_SetForm->x_fScreenRate_x + 2, (float)TextSize.cy / g_SetForm->x_fScreenRate_y + 2);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	glEnable(GL_TEXTURE_2D);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetBgColor(0);
	g_pRenderText->RenderText(sx, sy - 2, Text, 0, 0, 3);
...
void RenderNotices()
{
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INGAMESHOP)		== true
		...
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_BANK)			== true
		...
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NEWSPIN)		== true
		...
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_RENDER_COLOR)	== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NEWSHOP)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MUHELPER)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHATINPUTBOX)	== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_OPTION)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NEWSPIN)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_DUNGLUYEN)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_HARMONY)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CONGHUONG)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_UIITEM_TRADER)	== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_EXTOPTION_CHAR)== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MINI_MAP)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_GUILD_UPGRADE)	== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_THAN_MA)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_EXT_SHOP)		== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHANGECLASS)	== true
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHANGEPASS)	== true
	...
	for (int i = 0; i < MAX_NOTICE; i++)
	{
		...
		if (n->Color == 0)
		{
			...
			{
				g_pRenderText->SetTextColor(255, 200, 80, 200);
			}
			else
			{
		...
		{
			g_pRenderText->RenderText(GetWindowsX() / 2, setPosDown(300 + i * 13), n->Text, 0, 0, RT3_WRITE_CENTER);
		}
		else
		{
			g_pRenderText->RenderText(GetWindowsX() / 2, setPosDown(300 + i * 13), n->Text, 0, 0, RT3_WRITE_CENTER);
...
void SetBooleanPosition(CHAT* c)
{
	...
	c->Width /= g_SetForm->x_fScreenRate_x;
	c->Height /= g_SetForm->x_fScreenRate_y;
}
// RISE color name
void SetPlayerColor(BYTE PK)
{
	switch (PK)
	{
		case 0:g_pRenderText->SetTextColor(150, 255, 240, 255); break;
		case 1:g_pRenderText->SetTextColor(100, 120, 255, 255); break;
		case 2:g_pRenderText->SetTextColor(140, 180, 255, 255); break;
		case 3:g_pRenderText->SetTextColor(255, 255, 255, 255); break;
		case 4:g_pRenderText->SetTextColor(255, 150, 60, 255); break;
		case 5:g_pRenderText->SetTextColor(255, 80, 030, 255); break;
		default:g_pRenderText->SetTextColor(255, 000, 000, 255); break;
	}
}
...
void RenderBoolean(int x, int y, CHAT* c)
{
	...
	int iLineHeight = (FontHeight / g_SetForm->x_fScreenRate_y) + 2;
	...
	if (c->Union && c->Union[0] && gMapManager.WorldActive != WD_87THANMA)
	{
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Union, RenderBoxSize.cx + 2, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
	}
	if (c->Guild && c->Guild[0] && gMapManager.WorldActive != WD_87THANMA)
	{
	...
	else
	{
		...
		if (c->Color == 3 && c->Owner)
		{
			DWORD cColor = pUserRank->GetUserRank(c->Owner->Key)->rColorName;
			if (cColor != 0)
			{
				g_pRenderText->SetTextColor((cColor >> 16) & 0xFF, (cColor >> 8) & 0xFF, cColor & 0xFF, 255);
			}
			else if (g_pCharacterInfoWindow && !strcmp(c->ID, Hero->ID) && g_pCharacterInfoWindow->m_AppliedColorR != 255)
			{
				g_pRenderText->SetTextColor(g_pCharacterInfoWindow->m_AppliedColorR, g_pCharacterInfoWindow->m_AppliedColorG, g_pCharacterInfoWindow->m_AppliedColorB, 255);
			}
		}
	}
	//
	...
	if (g_bRenderImageLogo && c->Owner != NULL)
	{
		int pDanhHieu = pUserRank->GetUserRank(c->Owner->Key)->rDanhHieu;
		int pTuLuyen = pUserRank->GetUserRank(c->Owner->Key)->rTuLuyen;		
		BYTE ExCharLevel = pUserRank->GetUserRank(c->Owner->Key)->rCharOption;

		float logoY = RenderPos.y;
		...
		float BASEY = logoY - 23;
		float STEPY = 22.f;
		float OFFSET = 0.f;

		if (ExCharLevel > 0 && ExCharLevel < 6)
		{
			RenderBitmap((SEASON3B::CNewUIExtOptionChar::BITMAP_FREE_LOAD_6 + ExCharLevel) - 1, (float)(RenderPos.x + (RenderBoxSize.cx / 2)) - 24, (BASEY - OFFSET) + 10, 50.f, 13, 0.f, 0.f, 1.0f, 64.f / 64.f, 1, 1, 0);
			OFFSET += (STEPY - 10);
		}

		if (pDanhHieu >= 1 && pDanhHieu <= g_pDanhHieu->IsData[2])
		{
#if ANIM_RANKING_TITLE
			pUserRank->RenderAnimationRank(g_pDanhHieu->IMAGE_HD_LOGO_START_LEVEL_1, (float)(RenderPos.x + (RenderBoxSize.cx / 2)) - 40.5f, (BASEY - OFFSET));
#else
			RenderBitmap((SEASON3B::CNewUIDanhHieu::IMAGE_HD_LOGO_START_LEVEL_1 + pDanhHieu) - 1, (float)(RenderPos.x + (RenderBoxSize.cx / 2)) - 47.f, BASEY - OFFSET, 100.f, 21.f, 0.f, 0.f, 1.0f, 64.f / 64.f, 1, 1, 0);
#endif
			OFFSET += STEPY;
		}

		if (pTuLuyen >= 1 && pTuLuyen <= g_pTuLuyen->IsData[2])
		{
			RenderBitmap((SEASON3B::CNewUITuLuyen::IMAGE_HD_LOGO_START_LEVEL_1 + pTuLuyen) - 1, (float)(RenderPos.x + (RenderBoxSize.cx / 2)) - 47.f, BASEY - OFFSET, 100.f, 21.f, 0.f, 0.f, 1.0f, 64.f / 64.f, 1, 1, 0);
		}		
	}
	...
	if (c->Owner != NULL && c->Owner->GuildMarkIndex >= 0 && (c->Owner->GuildMarkIndex == MARK_EDIT || g_GuildCache.IsExistGuildMark(GuildMark[c->Owner->GuildMarkIndex].Key) == TRUE) && gMapManager.WorldActive != WD_87THANMA)
	{
		CreateGuildMark(c->Owner->GuildMarkIndex);
		RenderBitmap(BITMAP_GUILD, RenderPos.x - 15, RenderPos.y - 15, 15, 15);

		if (GuildMark[c->Owner->GuildMarkIndex].UnionName[0])
		{
			SEASON3B::CNewUIGuildInfoWindow* pGuildInfo = g_pNewUISystem->GetUI_NewGuildInfoWindow();
			if (pGuildInfo)
			{
				std::deque<UNIONGUILD_TEXT>& textList = pGuildInfo->GetUnionListBox().GetTextList();
				for (std::deque<UNIONGUILD_TEXT>::iterator it = textList.begin(); it != textList.end(); ++it)
				{
					if (strcmp(it->szName, GuildMark[c->Owner->GuildMarkIndex].GuildName) != 0)
					{
						memcpy(GuildMark[MARK_EDIT].Mark, it->GuildMark, 64);
						CreateGuildMark(MARK_EDIT);
						RenderBitmap(BITMAP_GUILD, (float)RenderPos.x - 15, (float)RenderPos.y - 30, 15, 15);
						break;
					}
				}
			}
	...
	//-------------------------------------------------------------------------------------------------
	// Render HP/Shield Bar
	pUserRank->RenderHPPlayer(x, RenderPos.y, c->Width, c->Owner);
	//-------------------------------------------------------------------------------------------------

	// RISE PK Free Mode indicator - visible to all players
	bool bShowPK = false;
	if (c->Owner == Hero)
	{
		bShowPK = pKit->m_PKFreeMode;
	}
	else if (c->Owner != NULL && KIND_PLAYER == c->Owner->Object.Kind && MODEL_PLAYER == c->Owner->Object.Type)
	{
		bShowPK = (pUserRank->GetUserRank(c->Owner->Key)->PKFreeMode == 1);
	}

	float iconOffsetX = RenderPos.x + RenderBoxSize.cx + 4.0f;
	bool hasPrevIcon = false;

	if (bShowPK)
	{
		RenderBitmap(SEASON3B::CUIFormSystemRender::UI_BAR_ICON_ON_PK, iconOffsetX - 1.f, RenderPos.y - 9.5f, 12.f, 10.f, 0.0f, 0.0f, 45.f / 64.f, 40.f / 64.f, 1, 1, 0);
		iconOffsetX += 12.f;
		hasPrevIcon = true;
	}
	
	int pQuanHam = pUserRank->GetUserRank(c->Owner->Key)->rQuanHam;
	bool bShowQuanHam = (pQuanHam >= 1 && pQuanHam <= g_pQuanHam->IsData[2]);
	
	int AccountType = pUserRank->GetUserRank(c->Owner->Key)->AccountType;
	bool bShowAccount = (AccountType > 0 && AccountType < 4);
	...
	if (bShowQuanHam)
	{
		...
		RenderBitmap((SEASON3B::CNewUIQuanHam::IMAGE_HD_LOGO_START_LEVEL_1 + pQuanHam) - 1, iconOffsetX, RenderPos.y - 11.f, 15.f, 12.f, 0.0f, 0.0f, 25.f / 32.f, 22.f / 32.f, 1, 1, 0);
		iconOffsetX += 12.f;
		hasPrevIcon = true;
	}
	...
	if (bShowAccount)
	{
		RenderBitmap((IMAGE_IGS_LOGOV1 + AccountType) - 1, iconOffsetX, RenderPos.y - 11.5f, 13.f, 13.2f, 0.f, 0.f, 1.0f, 16.f / 16.f, 1, 1, 0);
	}

	// RISE Show Guild Name->
...
void AddGuildName(CHAT* c, CHARACTER* Owner)
{
	...
	if (Owner->GuildMarkIndex >= 0 && GuildMark[Owner->GuildMarkIndex].UnionName[0])
	{
		...
		if (pGuildInfo)
		{
			...
			for (std::deque<UNIONGUILD_TEXT>::iterator it = textList.begin(); it != textList.end(); ++it)
			{
				if (strcmp(it->szName, GuildMark[Owner->GuildMarkIndex].GuildName) != 0)
				{
					wsprintf(c->Union, "(%s)", it->szName);
					bFoundPartner = true;
					break;
				}
			}
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	{
		if (((strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[c->GuildMarkIndex].GuildName) == NULL) || (g_pPartyManager->IsPartyMember(SelectedCharacter))) && (pKit->m_AutoCtrl || HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128))
		{
			return true;
		}
	...
	else if (c->Object.Kind == KIND_PLAYER)
	{
		...
		else if (c->PK >= PVP_MURDERER2 || pKit->m_AutoCtrl || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && c != Hero))
		{
			return true;
		}
...
bool CheckAttack()
{
	...
	else if (::IsStrifeMap(gMapManager.WorldActive) && c != Hero && c->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		if (pKit->m_AutoCtrl || HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128)
		{
			if (EnableGuildWar)
			{
				if (c->GuildTeam == 2 && c != Hero)
...
int	getTargetCharacterKey(CHARACTER* c, int selected)
{
	...
	else if (sc->PK >= PVP_MURDERER2 || pKit->m_AutoCtrl || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && sc != Hero))
	{
		return sc->Key;
	}
...
void SetCharacterPos(CHARACTER* c, BYTE posX, BYTE posY, vec3_t position)
{
	...
	SendCharacterMove(c->Key, c->Object.Angle[2], 1, PathX, PathY, PathX[0], PathY[0]); // RISE character move
}
...
bool CastWarriorSkill(CHARACTER* c, OBJECT* o, ITEM* p, int iSkill)
{
	...
	// Pegasus Sword Blow is a stationary ranged cast.  Do not feed it into
	// the native warrior chase path: that path walks the caster into melee
	// range and makes the recovered action look like a lunge/stab.
	if (iSkill == rise::pegasus::kSwordBlowSkill ||
		iSkill == rise::pegasus::kSwordBlowFlySkill)
	{
		// Consume this input even when the target is outside the five-tile
		// catalog range. Returning false would fall through to AttackKnight,
		// which starts the native melee chase seen in owner V4 QA.
		LetHeroStop();
		c->Movement = false;
		if (!c->SafeZone && CheckTile(c, o, Distance))
		{
			UseSkillWarrior(c, o);
		}
		return true;
	...
	if ((gMapManager.InBloodCastle() == true) && (iSkill >= AT_SKILL_SWORD1 && iSkill <= AT_SKILL_SWORD5)
		|| iSkill == MASTER_SKILL_ADD_FALLING_SLASH_IMPROVED
		|| iSkill == MASTER_SKILL_ADD_CYCLONE_IMPROVED1
		|| iSkill == MASTER_SKILL_ADD_CYCLONE_IMPROVED2
		|| iSkill == MASTER_SKILL_ADD_SLASH_IMPROVED
		|| iSkill == MASTER_SKILL_ADD_LUNGE_IMPROVED
		)
	{
		Distance = 1.8f;
	}
...
bool SkillWarrior(CHARACTER* c, ITEM* p)
{
	...
	if (Skill == AT_SKILL_RIDER
		...
		|| Skill == MASTER_SKILL_ADD_DEATH_STAB_IMPROVED
		|| Skill == MASTER_SKILL_ADD_DEATH_STAB_ENHANCED
		|| Skill == MASTER_SKILL_ADD_DEATH_STAB_MASTERED
		|| Skill == MASTER_SKILL_ADD_FALLING_SLASH_IMPROVED
		|| Skill == MASTER_SKILL_ADD_CYCLONE_IMPROVED1
		|| Skill == MASTER_SKILL_ADD_CYCLONE_IMPROVED2
		|| Skill == MASTER_SKILL_ADD_SLASH_IMPROVED
		|| Skill == MASTER_SKILL_ADD_LUNGE_IMPROVED
		|| (Skill == AT_SKILL_SPEAR && (Hero->Helper.Type == MODEL_HELPER + 2 || Hero->Helper.Type == MODEL_HELPER + 3 || Hero->Helper.Type == MODEL_HELPER + 4 || Hero->Helper.Type == MODEL_HELPER + 37  || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 6 || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 5))
		|| Skill == AT_SKILL_STRONG_PIER
		|| Skill == MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED
		|| Skill == AT_SKILL_LONGPIER_ATTACK
		|| Skill == MASTER_SKILL_ADD_FIRE_BURST_IMPROVED
		|| Skill == MASTER_SKILL_ADD_FIRE_BURST_ENHANCED
	...
	for (int i = 0; i < p->SpecialNum; i++)
	{
		if (CharacterAttribute->Skill[Hero->CurrentSkill] == p->Special[i])
		{
			...
			case MASTER_SKILL_ADD_SLASH_IMPROVED:
			case AT_SKILL_RIDER:
				if (CheckAttack())
	...
	if (CharacterAttribute->Skill[Hero->CurrentSkill] == AT_SKILL_DARK_SCREAM
		|| CharacterAttribute->Skill[Hero->CurrentSkill] == MASTER_SKILL_ADD_FIRE_SCREAM_IMPROVED
		|| CharacterAttribute->Skill[Hero->CurrentSkill] == MASTER_SKILL_ADD_FIRE_SCREAM_ENHANCED
		|| Skill == AT_SKILL_WIND_SOUL
		|| Skill == AT_SKILL_GAOTIC
		|| Skill == MASTER_SKILL_ADD_BIRDS_IMPROVED
		)
	{
		int iMana;
...
void UseSkillWarrior(CHARACTER* c, OBJECT* o)
{
	...
	{
		SetAttackSpeed();
		if (Skill == rise::pegasus::kSwordBlowFlySkill)
			rise::pegasus::SynchronizeSwordBlowFlightActionSpeed();
		...
		case MASTER_SKILL_ADD_DEATH_STAB_MASTERED:
			...
			break;
		case rise::pegasus::kSwordBlowFlySkill:
			// The authoritative echo starts the dedicated re-keyed flight clip.
			// Do not flash the original thrust action on the input frame.
			break;
		case AT_SKILL_RIDER:
		...
		case MASTER_SKILL_ADD_BIRDS_IMPROVED:
			break;
		...
		case AT_SKILL_SPACE_SPLIT:
			if (c->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 5)
			{
				SetAction(o, PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE);
			}
			else if ((c->Helper.Type >= MODEL_HELPER + 2) && (c->Helper.Type <= MODEL_HELPER + 4) || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6)
			{
	...
	if (Skill != AT_SKILL_STRONG_PIER && Skill != AT_SKILL_LONGPIER_ATTACK
		...
		&& Skill != MASTER_SKILL_ADD_FIRE_SCREAM_IMPROVED
		&& Skill != MASTER_SKILL_ADD_FIRE_SCREAM_ENHANCED

		&& Skill != AT_SKILL_THRUST
		&& Skill != MASTER_SKILL_ADD_LARGE_RING_BLOWER_IMPROVED
		&& Skill != MASTER_SKILL_ADD_LARGE_RING_BLOWER_ENHANCED
		&& Skill != AT_SKILL_STAMP
		&& Skill != MASTER_SKILL_ADD_UPPER_BEAST_IMPROVED
		&& Skill != MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED)
	{
	...
	/*if (Skill != AT_SKILL_GAOTIC && Skill != MASTER_SKILL_ADD_BIRDS_IMPROVED)
		{
			if (
				Skill == AT_SKILL_ONETOONE
				||Skill == MASTER_SKILL_ADD_DEATH_STAB_IMPROVED
...
void UseSkillWizard(CHARACTER* c, OBJECT* o)
{
	...
	case AT_SKILL_JAVELIN:
	{
		if (
			...
			|| Skill == MASTER_SKILL_ADD_METEORITE_ENHANCED
			)
		{
			BYTE PathX[1];
			BYTE PathY[1];
...
void UseSkillSummon(CHARACTER* pCha, OBJECT* pObj)
{
	...
	case AT_SKILL_ALICE_LIGHTNINGORB:
	{
		LetHeroStop();
		if (
			iSkill == AT_SKILL_ALICE_DRAINLIFE
			|| iSkill == MASTER_SKILL_ADD_DRAIN_LIFE_IMPROVED
			|| iSkill == MASTER_SKILL_ADD_DRAIN_LIFE_ENHANCED)
		{
...
void UseSkillRagefighter(CHARACTER* pCha, OBJECT* pObj)
{
	...
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighterX 1");
		return;
	}

	if (g_CMonkSystem.IsRideNotUseSkill(iSkill, pCha->Helper.Type))
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighterX 2");
		return;
	}

	if (!g_CMonkSystem.IsSwordformGlovesUseSkill(iSkill))
	{
		g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighterX 3");
		return;
	}
	...
	//g_ConsoleDebug->Write(MCD_ERROR, "UseSkillRagefighter %d", iSkill);
	...
	{
		//RISE Test skill Manh Long RF 261 -> Ket qua fix da OK 22.11.2025
		WORD wTargetKey = 0;
		if (g_MovementSkill.m_iTarget > -1)
		{
			wTargetKey = CharactersClient[g_MovementSkill.m_iTarget].Key;
...
void AttackRagefighter(CHARACTER* pCha, int nSkill, float fDistance)
{
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "AttackRagefighter ID : %d/%d , Dis : %.2f | %d %d / %d | %d %d", nSkill, Hero->CurrentSkill, fDistance, iMana, iSkillMana, CharacterAttribute->Mana, gSkillManager.CheckSkillDelay(Hero->CurrentSkill), CharacterAttribute->SkillDelay[Hero->CurrentSkill]);
	...
	if(iSkillMana > CharacterAttribute->SkillMana)
		return;
	// RISE Fix Delay RF
	if (CharacterAttribute->SkillDelay[Hero->CurrentSkill] <= 2850 && nSkill == AT_SKILL_DRAGON_KICK
		|| CharacterAttribute->SkillDelay[Hero->CurrentSkill] <= 620 && nSkill == AT_SKILL_DRAGON_LOWER
		|| CharacterAttribute->SkillDelay[Hero->CurrentSkill] <= 590 && nSkill == AT_SKILL_GIANTSWING
	)
	{
		return;
	}
	...
	if(bSuccess)
	{
		...
		case AT_SKILL_OCCUPY:
		{
			...
			fDistance = gSkillManager.GetSkillDistance(nSkill, pCha) * 1.0f;
...
bool SkillElf(CHARACTER* c, ITEM* p)
{
	...
	for (int i = 0; i < p->SpecialNum; i++)
	{
		...
		if (
			Spe_Num == AT_SKILL_CROSSBOW ||
			Spe_Num == MASTER_SKILL_ADD_TRIPLE_SHOT_IMPROVED ||
			Spe_Num == MASTER_SKILL_ADD_TRIPLE_SHOT_ENHANCED
			)
		...
		if (CharacterAttribute->Skill[Hero->CurrentSkill] == Spe_Num)
		{
			...
			if (
				g_isCharacterBuff(o, eBuff_InfinityArrow) ||
	...
	case MOVEMENT_TALK:
		...
		if (TargetNpc != -1)
		{
			...
			if (CharactersClient[TargetNpc].MonsterIndex >= npcIndex)
			{
				...
				else if (CharactersClient[TargetNpc].MonsterIndex == 581)
				{
					SendRequestTalk(CharactersClient[TargetNpc].Key);
				}
				else
...
void CheckChatText(char* Text)
{
	...
	else if (FindText(Text, "dance") || FindText(Text, "Dance"))
	{
		SetActionClass(c, o, PLAYER_UNKNOWN1, AT_UNKNOWN1);
		SendRequestAction(AT_UNKNOWN1, ((BYTE)((o->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
	}
...
void AttackElf(CHARACTER* c, int Skill, float Distance)
{
	...
	if (!CheckTile(c, o, Distance))
	{
		if (SelectedCharacter != -1 && (
			...
			Skill == MASTER_SKILL_ADD_HEAL_IMPROVED ||
			Skill == AT_SKILL_ATTACK ||
			Skill == MASTER_SKILL_ADD_GREATER_DAMAGE_IMPROVED ||
			Skill == MASTER_SKILL_ADD_GREATER_DAMAGE_ENHANCED ||
			Skill == AT_SKILL_DEFENSE ||
			Skill == MASTER_SKILL_ADD_GREATER_DEFENSE_IMPROVED ||
			Skill == MASTER_SKILL_ADD_GREATER_DEFENSE_ENHANCED ||
			CheckAttack()
			|| Skill == AT_SKILL_DARK_SCREAM
			|| Skill == MASTER_SKILL_ADD_FIRE_SCREAM_IMPROVED
			|| Skill == MASTER_SKILL_ADD_FIRE_SCREAM_ENHANCED
	...
	if (Wall)
	{
		if (SelectedCharacter != -1)
		{
			...
			{
				if ((
					(Skill == AT_SKILL_PARALYZE) ||
					(Skill == MASTER_SKILL_ADD_ICE_ARROW_IMPROVED) ||
					(Skill == AT_SKILL_DEEPIMPACT)) && ((o->Type == MODEL_PLAYER) && (gCharacterManager.GetEquipedBowType(Hero) != BOWTYPE_NONE)))
				{
					UseSkillElf(c, o);
				}
			}
	...
	case MASTER_SKILL_ADD_INFINITY_ARROW_IMPROVED:
	{
		...
		if (g_isCharacterBuff((&Hero->Object), eBuff_InfinityArrow) == false && g_isCharacterBuff((&Hero->Object), EFFECT_INFINITY_ARROW_IMPROVED) == false)
		{
			SendRequestMagic(Skill, HeroKey);
			if ((c->Helper.Type == MODEL_HELPER + 37)
...
void AttackKnight(CHARACTER* c, int Skill, float Distance)
{
	...
	if (((iTypeR != -1 &&
		...
		|| Skill == MASTER_SKILL_ADD_GREATER_LIFE_IMPROVED
		|| Skill == MASTER_SKILL_ADD_GREATER_LIFE_ENHANCED
		|| Skill == MASTER_SKILL_ADD_GREATER_LIFE_MASTERED
		|| Skill == AT_SKILL_ADD_CRITICAL
		|| Skill == MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_EXTENDED
		|| Skill == MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_ENHANCED
		|| Skill == MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_MASTERED
		|| Skill == MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_IMPROVED

		|| Skill == AT_SKILL_PARTY_TELEPORT
		|| Skill == AT_SKILL_THUNDER_STRIKE
		|| Skill == MASTER_SKILL_ADD_ELECTRIC_SPARK_IMPROVED
		|| Skill == AT_SKILL_DARK_HORSE
		|| Skill == MASTER_SKILL_ADD_EARTHQUAKE_IMPROVED
		|| Skill == MASTER_SKILL_ADD_EARTHQUAKE_ENHANCED
		...
		|| Skill == MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED1
		|| Skill == MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED2
		|| Skill == MASTER_SKILL_ADD_TWISTING_SLASH_ENHANCED
		...
		))
	{
		...
		if (g_csItemOption.Special_Option_Check() == false && (
			Skill == AT_SKILL_ICE_BLADE ||
			Skill == MASTER_SKILL_ADD_POWER_SLASH_IMPROVED))
			Success = false;
		...
		if (Success)
		{
			...
			{
				o->Angle[2] = CreateAngle(o->Position[0], o->Position[1],
					c->TargetPosition[0], c->TargetPosition[1]);
				const BYTE byValue = GetDestValue(c->PositionX, c->PositionY,
					TargetX, TargetY);
				const BYTE angle = static_cast<BYTE>(
					((o->Angle[2] + 180.f) / 360.f) * 255.f);
				WORD targetKey = 0xffff;
				if (g_MovementSkill.m_iTarget != -1)
				{
					targetKey = getTargetCharacterKey(c, g_MovementSkill.m_iTarget);
					if (targetKey == 0xffff)
						targetKey = CharactersClient[g_MovementSkill.m_iTarget].Key;
				}
				SendRequestMagicContinue(Skill, c->PositionX, c->PositionY,
					static_cast<BYTE>((o->Angle[2] / 360.f) * 255.f),
					byValue, angle, targetKey, 0);
				SetAttackSpeed();
				c->Movement = 0;
			}
			break;
			...
			case MASTER_SKILL_ADD_BIRDS_IMPROVED:
			{
				o->Angle[2] = CreateAngle(o->Position[0], o->Position[1], c->TargetPosition[0], c->TargetPosition[1]);
				{
					...
					float AttackSpeedDL = CharacterAttribute->AttackSpeed;
					int heso = static_cast<int>(round(-0.0067f * AttackSpeedDL + 17.3f) * 0.6f);
					...
					int bongmathap = static_cast<int>(round(static_cast<float>(bongma) / 2));

					if (FPS_ANIMATION_FACTOR > 0.5f)
					{
						for (int i = 0; i < bongmathap; i++)
						{
							Position[0] = (o->Position[0] - 119.f) + (float)(rand() % 240);
							Position[2] = (o->Position[2] + 5.f) + (float)(rand() % 60);
							CreateJoint(BITMAP_2LINE_GHOST, Position, o->Position, o->Angle, 0, o, 35.f, o->PKKey, 0, o->m_bySkillSerialNum);	//20.f	fix bong ma DL aLongfix6)
						}
					}
					else
					{
						for (int i = 0; i < bongma; i++)
						{
							Position[0] = (o->Position[0] - 119.f) + (float)(rand() % 240);
							Position[2] = (o->Position[2] + 49.f) + (float)(rand() % 60);
							if (rand() % 4 < 2)
							CreateJoint(BITMAP_2LINE_GHOST, Position, o->Position, o->Angle, 0, o, 40.f, o->PKKey, 0, o->m_bySkillSerialNum);	//20.f	fix bong ma DL aLongfix6)
						}
					}

					if (c == Hero && SelectedCharacter != -1 && (CheckTile(c, o, Distance)))
					{
						vec3_t Pos;
			...
			case MASTER_SKILL_ADD_FIRE_SCREAM_ENHANCED:

				{
					...
					SetAttackSpeed();
					{
						if (((c->Helper.Type >= MODEL_HELPER + 2 && c->Helper.Type <= MODEL_HELPER + 4 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6)) && !c->SafeZone)
						{
							SetAction(o, PLAYER_ATTACK_RIDE_STRIKE);
						}
			...
			case MASTER_SKILL_ADD_ELECTRIC_SPARK_IMPROVED:
				...
				if (CheckTile(c, o, Distance))
				{
					...
					if (Skill == AT_SKILL_THUNDER_STRIKE || Skill == MASTER_SKILL_ADD_ELECTRIC_SPARK_IMPROVED)
					{
						if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6) && !c->SafeZone)
						{
							SetAction(o, PLAYER_ATTACK_RIDE_ATTACK_FLASH);
						}
						else if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 5) && !c->SafeZone)
						{
					...
					else if (Skill == AT_SKILL_DARK_HORSE || Skill == MASTER_SKILL_ADD_EARTHQUAKE_IMPROVED || Skill == MASTER_SKILL_ADD_EARTHQUAKE_ENHANCED)
					{
...
{
	g_ConsoleDebug->Write(MCD_RECEIVE, "AttackWizard %d", Skill);
	OBJECT* o = &c->Object;
	int ClassIndex = gCharacterManager.GetBaseClass(c->Class);
	...
	case AT_SKILL_WIZARDDEFENSE:
		if (SelectedCharacter != -1)
		{
			...
			{
				if (!g_pPartyManager->IsPartyMember(SelectedCharacter) && SelectedCharacter != FindCharacterIndex(Hero->Key))
					return;
	...
	{
		SendRequestMagicContinue(Skill, (c->PositionX), (c->PositionY), (BYTE)(o->Angle[2] / 360.f * 256.f), 0, 0, 0xffff, 0);
		SetAttackSpeed();
	...
	case MASTER_SKILL_ADD_MAGIC_CIRCLE_ENHANCED:
	{
		if (g_isCharacterBuff((&Hero->Object), eBuff_SwellOfMagicPower) == false
			&& g_isCharacterBuff((&Hero->Object), EFFECT_MAGIC_CIRCLE_IMPROVED) == false
			&& g_isCharacterBuff((&Hero->Object), EFFECT_MAGIC_CIRCLE_ENHANCED) == false
			)
		{
			SendRequestMagic(Skill, HeroKey);
	...
	if (!CheckTile(c, o, Distance))
	{
		...
		if (Skill != AT_SKILL_STUN && Skill != AT_SKILL_REMOVAL_STUN
			...
			&& Skill != MASTER_SKILL_ADD_SWORD_POWER_IMPROVED
			&& Skill != MASTER_SKILL_ADD_SWORD_POWER_ENHANCED
			&& Skill != MASTER_SKILL_ADD_SWORD_POWER_MASTERED
	...
	if (Wall)
	{
		...
		if (Success)
		{
			...
			case AT_SKILL_STORM:
			{
				...
				SetPlayerMagic(c);
				*/
				// [Modified] Throttle storm khi ViewMagicSpeed >= 1600
				DWORD dwMagicSpeed = CharacterAttribute->ViewMagicSpeed;
				if (dwMagicSpeed >= 1800)
				{
					static DWORD s_dwLastStormCastTime = 0;
					DWORD dwNow = GetTickCount();
					if (dwNow - s_dwLastStormCastTime >= 200)
					{
						SendRequestMagicContinue(Skill, (c->PositionX), (c->PositionY), (BYTE)(o->Angle[2] / 360.f * 256.f), 0, 0, 0xffff, &o->m_bySkillSerialNum);
						SetPlayerMagic(c);
						s_dwLastStormCastTime = dwNow;
					}
				}
				else
				{
	...
	if (ClassIndex == CLASS_WIZARD && Success)
	{
		...
		case AT_SKILL_TELEPORT_B:
			...
			{
				if (SEASON3B::CNewUIInventoryCtrl::GetPickedItem() || g_isCharacterBuff(o, eDeBuff_Stun) || g_isCharacterBuff(o, eDeBuff_Sleep))
				{
					return;
				}
...
static bool SendPegasusHighSkillInput(CHARACTER* c, int skill, float distance)
{
    using namespace rise::pegasus;
    if (!IsPegasusHighSkillInput(skill) || skill == kSwordBlowSkill ||
        skill == kSwordBlowFlySkill)
        return false;

    OBJECT* owner = &c->Object;
    if (!gSkillManager.CheckSkillDelay(Hero->CurrentSkill))
    {
        TraceRuntime("INPUT", skill, Hero->CurrentSkill, "REJECT_DELAY");
        return true;
    }

    WORD targetKey = HeroKey;
    if (IsPegasusTargetAttackInput(skill))
    {
        if (SelectedCharacter < 0 || SelectedCharacter >= MAX_CHARACTERS_CLIENT ||
            !CheckTarget(c))
        {
            TraceRuntime("INPUT", skill, SelectedCharacter, "REJECT_TARGET");
            return true;
        }

        OBJECT* target = &CharactersClient[SelectedCharacter].Object;
        if (!target->Live || !target->Visible ||
            !CheckWall(c->PositionX, c->PositionY, TargetX, TargetY) ||
            !CheckTile(c, owner, distance))
        {
            TraceRuntime("INPUT", skill, SelectedCharacter, "REJECT_PATH_RANGE");
            return true;
        }

        targetKey = CharactersClient[SelectedCharacter].Key;
        owner->Angle[2] = CreateAngle(owner->Position[0], owner->Position[1],
            target->Position[0], target->Position[1]);
        c->TargetCharacter = SelectedCharacter;
    }

    // All fourteen catalog entries use the native target-key request. Visuals
    // remain authoritative on the accepted 0x19 echo in ReceiveMagic.
    LetHeroStop();
    c->Movement = false;
    SetAttackSpeed();
    SendRequestMagic(skill, targetKey);
    TraceRuntime("INPUT", skill, targetKey, "SEND");
    c->SkillSuccess = true;
    return true;
}
...
{
	if ((MouseOnWindow || !SEASON3B::CheckMouseIn(0, 0, GetScreenWidth(), (GetWindowsY() - 51))) && MouseLButtonPush) //toa do chi chuot wide
	{
		MouseRButtonPop = false;
		MouseRButtonPush = false;
		MouseRButton = false;
	...
	if (Success && c->Dead == 0)
	{
		...
		else if (!c->SafeZone || (gMapManager.InBloodCastle() == true) || gMapManager.InChaosCastle() == true)
		{
				...
				&& gMapManager.InChaosCastle() == false)
			{
				if (ClassIndex == CLASS_ELF
					&& (Skill != AT_SKILL_CROSSBOW
						...
						&& Skill != MASTER_SKILL_ADD_PENETRATION_IMPROVED
				...
				else if (ClassIndex == CLASS_KNIGHT && (Skill == AT_SKILL_VITALITY ))
				{
					Attacking = -1;
				}
			...
			if (o->Type == MODEL_PLAYER)
			{
				const bool swordBlowFlyHeld =
					Skill == rise::pegasus::kSwordBlowFlySkill &&
					o->CurrentAction == rise::pegasus::PLAYER_ATTACK_ONETOONE_FLY;
				if (!swordBlowFlyHeld && (o->CurrentAction < PLAYER_STOP_MALE
			...
			if (SendPegasusHighSkillInput(c, Skill, Distance))
			{
				return;
			}
			...
			if (ClassIndex != CLASS_WIZARD)
			{
				...
				if (CheckWall((c->PositionX), (c->PositionY), TargetX, TargetY))
				{
					...
					{
						if (ClassIndex == CLASS_KNIGHT || ClassIndex == CLASS_MAGIC || ClassIndex == CLASS_DARK_LORD || ClassIndex == CLASS_RAGEFIGHTER)
						{
							bool bOk = false;

							if (gProtect->m_MainInfo.ButtonCharracter[16] == 1)
							{
								bOk = true;
							}
							else
			...
			if (ClassIndex == CLASS_KNIGHT || ClassIndex == CLASS_MAGIC || ClassIndex == CLASS_DARK_LORD)
			{
			...
			if (ClassIndex == CLASS_WIZARD || ClassIndex == CLASS_MAGIC || ClassIndex == CLASS_SUMMONER)
			{
		...
		if (gs->Flag == 1 && gs->Map == gMapManager.WorldActive)
		{
				...
				(Hero->PositionX) <= gs->x2 && (Hero->PositionY) <= gs->y2)
			{
				...
				if ((LoadingWorld) == 0 && Hero->JumpTime == 0)
				{
					...
					if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_MAGIC || gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD
					...
					if (Success)
					{
							...
							((CharacterMachine->Equipment[EQUIPMENT_HELPER].Type >= ITEM_HELPER + 2 && CharacterMachine->Equipment[EQUIPMENT_HELPER].Type <= ITEM_HELPER + 3)))
						{
							...
							!((CharacterMachine->Equipment[EQUIPMENT_WING].Type >= ITEM_WING && CharacterMachine->Equipment[EQUIPMENT_WING].Type <= ITEM_WING + 6
								...
								|| gCustomPet.GetInfoPetType(CharacterMachine->Equipment[EQUIPMENT_HELPER].Type) == 5
	...
	if (c->SafeZone)
	{
		if ((gMapManager.InBloodCastle() == true) || gMapManager.InChaosCastle() == true)
		{
			if (Skill != AT_SKILL_HEALING && Skill != AT_SKILL_DEFENSE && Skill != AT_SKILL_ATTACK
				...
				&& Skill != MASTER_SKILL_ADD_GREATER_DEFENSE_IMPROVED
				&& Skill != MASTER_SKILL_ADD_GREATER_DEFENSE_ENHANCED
				&& Skill != MASTER_SKILL_ADD_MANA_SHIELD_IMPROVED
				&& Skill != MASTER_SKILL_ADD_MANA_SHIELD_ENHANCED
				&& Skill != MASTER_SKILL_ADD_MANA_SHIELD_MASTERED
				&& Skill != MASTER_SKILL_ADD_GREATER_LIFE_IMPROVED
				&& Skill != MASTER_SKILL_ADD_GREATER_LIFE_ENHANCED
				&& Skill != MASTER_SKILL_ADD_GREATER_LIFE_MASTERED
				&& Skill != MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_EXTENDED
				...
				&& Skill != MASTER_SKILL_ADD_SWORD_POWER_ENHANCED
				&& Skill != MASTER_SKILL_ADD_SWORD_POWER_MASTERED
				&& Skill != MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_IMPROVED
				&& Skill != MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED
				&& Skill != MASTER_SKILL_ADD_INFINITY_ARROW_IMPROVED
				&& Skill != MASTER_SKILL_ADD_MAGIC_CIRCLE_IMPROVED
				&& Skill != MASTER_SKILL_ADD_MAGIC_CIRCLE_ENHANCED
...
int ExecuteSkill(CHARACTER* c, int Skill, float Distance)
{
	...
	bool skillComplete = ExecuteSkillComplete(c);
	return (int)skillComplete;
}
...
void MoveHero()
{
	...
	else
	{
		...
		if (StandTime >= FPS_ANIMATION_FACTOR && !MouseOnWindow && Hero->Dead == 0 &&
	...
	if (!MouseOnWindow && false == g_pNewUISystem->CheckMouseUse())
	{
				...
				))
		{
			...
			else if (SelectedOperate != -1 && (c->SafeZone || (c->Helper.Type<MODEL_HELPER + 2 || c->Helper.Type>MODEL_HELPER + 4 || c->Helper.Type != MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 5 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6)))
			{
...
void SelectObjects()
{
	...
	if (!MouseOnWindow && false == g_pNewUISystem->CheckMouseUse() && SEASON3B::CheckMouseIn(0, 0, GetScreenWidth(), (GetWindowsY() - 51)))	//toa do chi chuot wide
	{
		if (HIBYTE(GetAsyncKeyState(VK_MENU)) == 128)
		{
		...
		else
		{
			...
			if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_ELF || gCharacterManager.GetBaseClass(Hero->Class) == CLASS_WIZARD)
			{
				...
				if (((Skill >= AT_SKILL_HEALING) && (Skill <= AT_SKILL_ATTACK))
					|| Skill == MASTER_SKILL_ADD_GREATER_DAMAGE_IMPROVED
					|| Skill == MASTER_SKILL_ADD_GREATER_DAMAGE_ENHANCED
					|| ((MASTER_SKILL_ADD_HEAL_IMPROVED == Skill))
...
{
	// RISE Unlock Skill CS
	return true;

	if (Type == AT_SKILL_RUSH || Type == AT_SKILL_JAVELIN || Type == AT_SKILL_DEEPIMPACT || Type == AT_SKILL_ONEFLASH || Type == AT_SKILL_DEATH_CANNON || Type == AT_SKILL_SPACE_SPLIT || Type == AT_SKILL_OCCUPY)
	{
...
void RenderCursor()
{
	...
	g_pNewUISystem->DisEnabledCurMouse();
