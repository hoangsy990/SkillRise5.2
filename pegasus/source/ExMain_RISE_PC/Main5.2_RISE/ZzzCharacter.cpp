...
void SetPlayerStop(CHARACTER* c)
{
    ...
    {
        if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 5) && !c->SafeZone)
        {
        ...
        else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6) && !c->SafeZone)
        {
            if (c->Weapon[0].Type == -1 && c->Weapon[1].Type == -1)
        ...
        else if (c->SafeZone && c->m_PetInfo->m_dwPetType == PET_TYPE_DARK_SPIRIT && !gMapManager.InChaosCastle())
        {
            ...
            else
            {
                ...
                if (c->MonsterIndex == 257 && c->PositionX == 135 && c->PositionY == 127)
                {
                    Fly = false;
                }
                else if(c->MonsterIndex == 257)
                {
                    Fly = true;
                }
...
void SetPlayerWalk(CHARACTER* c)
{
    ...
    if (( c->MonsterIndex >= 534 && c->MonsterIndex <= 539) || c->MonsterIndex == 580)
    {
        iItemBootsLevel = 0;
        iItemGlovesLevel = 0;
        c->Run = 0;
        ...
        {
            if (gCharacterManager.GetBaseClass(c->Class) == CLASS_MAGIC || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD
                ...
                || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 1
...
//				    SetAction(&c->Object,PLAYER_FLY_RIDE_WEAPON);
    ...
    else if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD && c->Helper.Type == MODEL_HELPER + 4 || gCustomPet.GetInfoPetType(c->Helper.Type - MODEL_ITEM) == 6 && !c->SafeZone)
    {
...
void SetAttackSpeed()
{
    float AttackSpeed1 = CharacterAttribute->AttackSpeed    * 0.004f; //* 0.001f;
    float MagicSpeed1 = CharacterAttribute->MagicSpeed      * 0.004f; //* 0.001f;
    float MagicSpeed2 = CharacterAttribute->MagicSpeed      * 0.002f; //* 0.001f;
    ...
    Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SKILL_WHEEL].PlaySpeed = 0.3f + AttackSpeed1;
    ...
    //Models[MODEL_PLAYER].Actions[PLAYER_SKILL_HAND1].PlaySpeed = 4.0 + MagicSpeed2;
    ...
    Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_STRIKE].PlaySpeed = 0.25f + AttackSpeed1 / 1.65;
    Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_RIDE_STRIKE].PlaySpeed = 0.2f + AttackSpeed1 / 1.65;
    ...
    Models[MODEL_PLAYER].Actions[PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE].PlaySpeed = 0.2f + AttackSpeed1 / 1.65;
    ...
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_LIGHTNING_ORB].PlaySpeed = 0.4f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_LIGHTNING_ORB_UNI].PlaySpeed = 0.25f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_LIGHTNING_ORB_DINO].PlaySpeed = 0.25f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_LIGHTNING_ORB_FENRIR].PlaySpeed = 0.25f + MagicSpeed2 * 1.1;

    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_CHAIN_LIGHTNING].PlaySpeed = 0.25f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_CHAIN_LIGHTNING_UNI].PlaySpeed = 0.15f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_CHAIN_LIGHTNING_DINO].PlaySpeed = 0.15f + MagicSpeed2 * 1.1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_CHAIN_LIGHTNING_FENRIR].PlaySpeed = 0.15f + MagicSpeed2 * 1.1;
    ...
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_FLAMESTRIKE].PlaySpeed = 0.69f + MagicSpeed1;
    Models[MODEL_PLAYER].Actions[PLAYER_SKILL_LIGHTNING_SHOCK].PlaySpeed = 0.2f + MagicSpeed2 * 2;
    ...
    float RageAttackSpeed = CharacterAttribute->AttackSpeed * 0.004f;
...
void AttackEffect(CHARACTER* c)
{
    ...
    if (c->TargetCharacter >= 0 && c->TargetCharacter < MAX_CHARACTERS_CLIENT)
    {
        ...
        if ((c->Skill) == AT_SKILL_ENERGYBALL)
        {
            ...
            || (c->Skill) == MASTER_SKILL_ADD_LIGHTNING_IMPROVED1
            || (c->Skill) == MASTER_SKILL_ADD_LIGHTNING_IMPROVED2)
        {
            switch (c->MonsterIndex)
            {
            case 89:   //  마법 해골.
...
bool CheckMonsterSkill(CHARACTER* pCharacter, OBJECT* pObject)
{
    ...
    {
        case 0:        SetAction(pObject, MONSTER01_ATTACK1);        break;
        case 1:        SetAction(pObject, MONSTER01_ATTACK2);        break;
        case 2:        SetAction(pObject, MONSTER01_ATTACK3);        break;
        case 3:        SetAction(pObject, MONSTER01_ATTACK4);        break;
        case 4:        SetAction(pObject, MONSTER01_ATTACK5);        break;
        case 5:        SetAction(pObject, MONSTER01_ATTACK5);        break;
        case 6:        SetAction(pObject, MONSTER01_ATTACK5);        break;
        case 7:        SetAction(pObject, MONSTER01_ATTACK5);        break;
        case 8:        SetAction(pObject, MONSTER01_ATTACK5);        break;
        case 9:        SetAction(pObject, MONSTER01_ATTACK5);        break;
    }
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
    ...
    int iSkill = (c->Skill);
    g_iLimitAttackTime = g_iLimitAttackTime;

    // RISE Agi Fix Getmain
    /*if (c == Hero || c->Object.Kind == KIND_PLAYER)
        g_iLimitAttackTime = GetAttackTimeForClass(c->GetBaseClass());
    else
    ...
    default:
            ...
            ((o->Type >= MODEL_MONSTER01 && o->Type < MODEL_MONSTER_END) && o->CurrentAction >= MONSTER01_ATTACK1 && o->CurrentAction <= MONSTER01_ATTACK2)))
        {
            ...
            if ((int)c->AttackTime >= 1 && LeftType == ITEM_BOW + 21 && o->Type == MODEL_PLAYER && rand_fps_check(1))
            {
                for (int i = 0; i < 20; i++)
                {
...
void OnlyNpcChatProcess(CHARACTER* c, OBJECT* o)
{
    if (o->Kind == KIND_NPC && rand_fps_check(2))
    {
        ...
        case MODEL_PLAYER:
            ...
            {
                if (c->PositionX != 135 || c->PositionY != 127)
                {
...
{
	rise::pegasus::UpdateSwordBlowFlight(*c);
	rise::pegasus::UpdateCasterActionTrace(*c);
	if (o->Type == MODEL_WARCRAFT)
    {
        char Text[100];
        char ID[100];
    ...
    if (gMapManager.InBattleCastle() == false && o->m_byHurtByOneToOne > 0)
    {
        ...
        o->m_byHurtByOneToOne -= FPS_ANIMATION_FACTOR;
    }
    ...
    if (c->AttackTime >= g_iLimitAttackTime || g_iLimitAttackTime == 0x02)
    {
        c->AttackTime = 0;
        o->PKKey = getTargetCharacterKey(c, SelectedCharacter);
        ...
        case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_IMPROVED:
            ...
            if (c->Weapon[0].Type != MODEL_BOW + 15) //CustomArrow Cung
            {
                b->TransformPosition(o->BoneTransform[c->Weapon[0].LinkBone], p, Position, true);
                CreateEffect(MODEL_DARKLORD_SKILL, Position, o->Angle, Light, 0);
            }
            if (c->Weapon[1].Type != MODEL_BOW + 7 && (c->Weapon[1].Type < MODEL_SHIELD || c->Weapon[1].Type >= MODEL_SHIELD + MAX_ITEM_INDEX))//CustomArrow No
            {
        ...
        case AT_SKILL_BLAST_FREEZE:
        {
            ...
            TargetPosition[1] = (float)(c->SkillY - 1 + 0.5f) * TERRAIN_SCALE;
            TargetPosition[2] = RequestTerrainHeight(TargetPosition[0], TargetPosition[1]);

            CreateEffect(MODEL_BLIZZARD, TargetPosition, o->Angle, Light, 0);

            float radius = 120.0f;
            for (int i = 0; i < 4; ++i) 
            {
                float angle = i * (PI / 4.0f);
                Pos[0] = TargetPosition[0] + cosf(angle) * radius;
                Pos[1] = TargetPosition[1] + sinf(angle) * radius;
                Pos[2] = RequestTerrainHeight(Pos[0], Pos[1]);
                CreateEffect(MODEL_BLIZZARD, Pos, o->Angle, Light, 0);
            }

            for (int i = 0; i < 4; ++i)
            {
                Pos[0] = TargetPosition[0] + (rand() % 20 - 10);
                Pos[1] = TargetPosition[1] + (rand() % 20 - 10);
                Pos[2] = TargetPosition[2] + (rand() % 80);
        ...
        case AT_SKILL_BLAST: //13
        case MASTER_SKILL_ADD_BLAST_IMPROVED1: //382
...
//#endif //PBG_ADD_NEWCHAR_MONK_SKILL
        ...
        case AT_SKILL_GIGANTIC_STORM:
        {
            ...
            for (int i = 0; i < 5; ++i)
            {
                Vector(0.f, 270.f, 0.f, vDirection);
...
//#endif //PBG_ADD_NEWCHAR_MONK_SKILL
        ...
        if (c->TargetCharacter == -1)
        {
            BYTE Skill = 0;
            if (
                (c->Skill) == AT_SKILL_CROSSBOW ||
                (c->Skill) == MASTER_SKILL_ADD_TRIPLE_SHOT_IMPROVED ||
                (c->Skill) == MASTER_SKILL_ADD_TRIPLE_SHOT_ENHANCED)
                Skill = 1;
            if ((o->Type == MODEL_PLAYER &&
        ...
        else
        {
            ...
            case MASTER_SKILL_ADD_FLAME_IMPROVED2:
                ...
                Position[1] = to->Position[1];
                Position[2] = RequestTerrainHeight(Position[0], Position[1]);
                CreateEffect(BITMAP_FLAME, Position, o->Angle, o->Light, 5, o, o->PKKey, FindHotKey(c->Skill));
                PlayBuffer(SOUND_FLAME);
                break;
            ...
            case MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED:
                ...
                break;
            case AT_SKILL_LONGPIER_ATTACK: // 00X01
            case MASTER_SKILL_ADD_FIRE_BURST_IMPROVED:
            case MASTER_SKILL_ADD_FIRE_BURST_ENHANCED:
            {
                ...
                Angle[2] = o->Angle[2] + 90 + (rand() % 31 - 15);
                Pos[2] = (to->BoundingBoxMax[2] / 1.f) + (rand() % 51 - 25);
                CreateEffect(MODEL_PIER_PART, Position, Angle, Pos, 0, to);

                Angle[2] = o->Angle[2] + (rand() % 31 - 15);
                Pos[2] = (to->BoundingBoxMax[2] / 2.f) + (rand() % 51 - 25);
                CreateEffect(MODEL_PIER_PART, Position, Angle, Pos, 0, to);
            ...
            case AT_SKILL_ALICE_THORNS:
            {
                ...
                if (iSkillIndex == AT_SKILL_ALICE_SLEEP
                    || iSkillIndex == MASTER_SKILL_ADD_SLEEP_IMPROVED
                    )
                {
                ...
                if (iSkillIndex == AT_SKILL_ALICE_SLEEP
                    || iSkillIndex == MASTER_SKILL_ADD_SLEEP_IMPROVED
                    || iSkillIndex == AT_SKILL_ALICE_THORNS)
                {
...
bool CheckFullSet(CHARACTER* c)
{
    ...
    if (gCharacterManager.GetBaseClass(c->Class) == CLASS_MAGIC)
    {
        end = EQUIPMENT_ARMOR;
    }
    ...
    if (bHero == true)
    {
        ...
        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_MAGIC && Success)
        {
    ...
    if (o->Visible)
    {
        ...
        if (c->PK < PVP_MURDERER2)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                else if (g_pItemColor->GetInfoByItem(c->Weapon[j].Type - MODEL_ITEM) || c->Weapon[j].Type == MODEL_SWORD + 19 || c->Weapon[j].Type == MODEL_BOW + 18 || c->Weapon[j].Type == MODEL_STAFF + 10 || c->Weapon[j].Type == MODEL_MACE + 13)
                {
...
float CharacterMoveSpeed(CHARACTER* c)
{
    ...
    if (o->Type == MODEL_PLAYER && o->Kind == KIND_PLAYER)
    {
        ...
        if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 5 || gCustomPet.GetInfoPetType(Hero->Helper.Type - MODEL_ITEM) == 6) && !c->SafeZone && !isholyitem)
        {
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
    ...
    {
        if (Type >= MODEL_WING && Type <= MODEL_WING + 6)
        {
            return;
        }

        if ((Type >= MODEL_WING + 36 && Type <= MODEL_WING + 43) || gCustomWing->CheckCustomWingByItem(Type - MODEL_ITEM))
        {
            return;
        }

        if (Type >= MODEL_WING + 130 && Type <= MODEL_WING + 135)
        {
            return;
        }

        if (Type >= MODEL_WING + 49 && Type <= MODEL_WING + 50)
        {
            return;
        }
    }
    ...
    if ((c->MonsterIndex >= 529 && c->MonsterIndex <= 539) || c->MonsterIndex == 580)
    {
    ...
    if (Link)
    {
        ...
        else if (Type >= MODEL_WING + 40)
        {
            ...
            || gCustomBow.CheckIsCrossBow(Type - MODEL_ITEM)
    ...
    if ((c->Skill) == AT_SKILL_PIERCING || (c->Skill) == MASTER_SKILL_ADD_PENETRATION_IMPROVED &&
    ...
    case MODEL_MACE + 14:
    {
        const bool ex702LoginScene = SceneFlag == LOG_IN_SCENE && gMapManager.WorldActive == WD_94EX702_LOGIN_SCENE;
        const float ex702GlowScale = ex702LoginScene ? 0.38f : 1.0f;
        const float ex702SpriteScale = ex702LoginScene ? 0.58f : 1.0f;
        Vector(1.0f * ex702GlowScale, 0.3f * ex702GlowScale, 0.0f, Light);

        float fRendomPos = (float)(rand() % 60) / 20.0f - 1.5f;
        float fRendomScale = ((float)(rand() % 10) / 20.0f + 1.4f) * ex702SpriteScale;
        ...
        fRendomScale = ((float)(rand() % 10) / 20.0f + 1.0f) * ex702SpriteScale;
        Vector(0.f, -100.f + fRendomPos, fRendomPos, p);
        b->TransformPosition(BoneTransform[0], p, Position, true);
        CreateSprite(BITMAP_LIGHT, Position, fRendomScale + 0.3f, Light, o);

        fRendomPos = (float)(rand() % 40) / 20.0f - 1.0f;
        fRendomScale = ((float)(rand() % 8) / 20.0f + 0.4f) * ex702SpriteScale;
        Vector(0.f, 100.f + fRendomPos, fRendomPos, p);
        b->TransformPosition(BoneTransform[0], p, Position, true);
        CreateSprite(BITMAP_SHINY + 1, Position, fRendomScale, Light, o);
    ...
    if (gMapManager.WorldActive != WD_10HEAVEN && gMapManager.InHellas() == FALSE && !g_Direction.m_CKanturu.IsMayaScene())
    {
        ...
        if (gCapeRender.IncludeCape(Type))
        {
            b->RenderBodyShadow(-1, -1, -1, -1, o->m_pCloth, o->m_byNumCloth);
        }
    }
...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
    ...
    if (gMapManager.WorldActive != WD_10HEAVEN && (o->Type == MODEL_PLAYER) && (!(MODEL_HELPER + 2 <= c->Helper.Type && c->Helper.Type <= MODEL_HELPER + 3) || c->SafeZone)
            ...
            )
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                if (IsFilteredWeapon(c->Weapon[i].Type))
                {
                    continue;
                }
    ...
    int MonsterGolden = g_pMonsterGlow->IsGolden(c->MonsterIndex);
    if (MonsterGolden != 0 || c->MonsterIndex == 38 || c->MonsterIndex == 43 || c->MonsterIndex == 44 || c->MonsterIndex == 52 || c->MonsterIndex == 59 || c->MonsterIndex == 67 || (78 <= c->MonsterIndex && c->MonsterIndex <= 83) || (c->MonsterIndex >= 493 && c->MonsterIndex <= 502))
    {
        ...
        if (c->MonsterIndex == 59)
            Bright = 0.5f;
        if (c->MonsterIndex == 43 || c->MonsterIndex == 44 || (78 <= c->MonsterIndex && c->MonsterIndex <= 83) || (c->MonsterIndex >= 493 && c->MonsterIndex <= 502))
        {
            if (c->MonsterIndex == 44)
            {
                VectorCopy(Models[o->Type].BodyLight, vBackupBodyLight);
                Vector(0.f, 0.0f, 1.0f, Models[o->Type].BodyLight);

                float	fEffectScale = o->Scale * 1.6f;
                vec3_t	v3EffectLightColor, v3EffectPosition;

                Vector(0.0f, 0.6f, 0.1f, v3EffectLightColor);
                ...
                Vector(0.0f, 0.8f, 0.1f, v3EffectLightColor);
    ...
    if ((gCharacterManager.GetCharacterClass(c->Class) == CLASS_MAGIC 
        || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD
    ...
    if (c->MonsterIndex == 55 || c->MonsterIndex == 361 || c->MonsterIndex == 754 || c->MonsterIndex == 756)
    {
        bCloak = true;
    }
    ...
    if ((c->MonsterIndex >= 529 && c->MonsterIndex <= 539) || c->MonsterIndex == 580)
    {
        ...
        if (gMapManager.WorldActive == WD_65DOPPLEGANGER1);
            ...
            else
            {
                ...
                if (rand_fps_check(1) && iNumBones > 0)
                {
...
#endif// GUILD_WAR_EVENT
        {
            for (int i = MAX_BODYPART - 1; i >= 0; i--)
            {
                ...
                if (p->Type != -1)
                {
                    ...
                    {
                        // RISE Glow Light Antilag 2 F7
                        /*if (!g_pOption->GetRenderEndGlow() && SceneFlag == MAIN_SCENE && c != Hero)
                        {
                            RenderPartObject(&c->Object, Type, p, c->Light, o->Alpha, p->Level << 3, p->Option1, p->ExtOption, false, false, Translate, Select, RENDER_DOPPELGANGER | RENDER_BRIGHT | RENDER_TEXTURE);
                        }
                        else*/
                        {
                            ...
                            else if ((o->CurrentAction == PLAYER_SKILL_DARKSIDE_READY) && (gCharacterManager.GetBaseClass(c->Class) == CLASS_RAGEFIGHTER))
                            {
                            ...
                            else if (o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && g_pOption->GetRenderEquipMent() == 0)
                            {
    ...
    if (bCloak)
    {
        if (!c->Object.m_pCloth)
        {
            if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD || c->MonsterIndex == 754 || gCapeRender.IncludeCape(c->Wing.Type, 0))
            {
                int numCloth = 4;

                if (c->Wing.Type == MODEL_WING + 40 || gCapeRender.IncludeCape(c->Wing.Type, 0))
                {
                    numCloth = 6;
                }
                else
                ...
                gCapeRender.RenderModelCapeDL(c->Wing.Type, pCloth, o);
            ...
            else if (gCharacterManager.GetBaseClass(c->Class) == CLASS_RAGEFIGHTER || c->MonsterIndex == 756)
            {
                ...
                gCapeRender.RenderModelCapeRF(c->Wing.Type, pCloth, o);
            ...
            && gCapeRender.IncludeCape(c->Wing.Type, 0)
            )
        {
            if ((c->BodyPart[BODYPART_ARMOR].Type != MODEL_BODY_ARMOR + CLASS_DARK_LORD
        ...
        if (!g_isCharacterBuff(o, eBuff_Cloaking))
        {
            for (int i = 0; i < o->m_byNumCloth; i++)
            {
                if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD)
                {
                    if (i == 2 && ((c->Wing.Type != MODEL_HELPER + 30 
                ...
                if (gCharacterManager.GetBaseClass(c->Class) == CLASS_RAGEFIGHTER)
                {
                    if (i == 0 && ((c->Wing.Type != MODEL_WING + 49
                        ...
                        && !gCapeRender.IncludeCape(c->Wing.Type, 2)
    ...
    case MODEL_PLAYER:
        ...
        else if (o->CurrentAction >= PLAYER_SKILL_SLEEP && o->CurrentAction <= PLAYER_SKILL_SLEEP_FENRIR)
        {
            ...
            else if (iSkillType == AT_SKILL_ALICE_THORNS)
            {
                ...
                || iSkillType == MASTER_SKILL_ADD_SWORD_POWER_IMPROVED
                || iSkillType == MASTER_SKILL_ADD_SWORD_POWER_MASTERED
                || iSkillType == MASTER_SKILL_ADD_SWORD_POWER_ENHANCED)
            {
            ...
            if (iSkillType == AT_SKILL_ALICE_SLEEP
                || iSkillType == MASTER_SKILL_ADD_SLEEP_IMPROVED
        ...
        if (gMapManager.InChaosCastle() == false)
        {
            ...
            if (fullset && g_pOption->GetRenderLevel() >= 2)
            {
                ...
                CUSTOM_SET_INFO* SetInfo = g_pSetEffect->GetInfoByItem(c->BodyPart[BODYPART_BOOTS].Type - MODEL_ITEM);

                if (SetInfo && EquipmentLevelSet > 9)
                {
                    VectorCopy(o->Light, Light);
                    Vector(SetInfo->Color[0], SetInfo->Color[1], SetInfo->Color[2], o->Light);
...
void RenderCharactersClient()
{
    for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
    {
        ...
        // RISE Run Effect -> For Hon Hoan
        g_pHonHoan->RenderHonHoanEffect(c, o);
...
void SetCharacterScale(CHARACTER* c)
{
    ...
    if (SceneFlag == CHARACTER_SCENE)
    {
        if (c->Skin == 0)
        {
            ...
            case CLASS_MAGIC:    c->Object.Scale = 1.2f; break;
    ...
    else
    {
        ...
        if (c->Skin == 0)
        {
            ...
            case CLASS_MAGIC:    c->Object.Scale = 0.95f; break;
...
static void ApplyExtendedPetCharSet(CHARACTER* character, const BYTE* petCharSet)
{
    if (character == nullptr || petCharSet == nullptr)
        return;

    OBJECT* owner = &character->Object;
    for (int i = 0; i < 2; ++i)
    {
        const int renderSlot = i + 1;
        for (int mount = 0; mount < MAX_MOUNTS; ++mount)
        {
            if (Mounts[mount].Live && Mounts[mount].Owner == owner && Mounts[mount].LinkBone == renderSlot)
                Mounts[mount].Live = false;
        }

        const WORD itemIndex = (WORD)(petCharSet[4 + (i * 2)] | (petCharSet[5 + (i * 2)] << 8));
        if (itemIndex == 0xFFFF || itemIndex >= MAX_ITEM || !gCustomPet.CheckCustomPetByItem(itemIndex))
        {
            character->MuunHelper[i].Type = -1;
            continue;
        }

        character->MuunHelper[i].Type = MODEL_ITEM + itemIndex;
        CreateMount(character->MuunHelper[i].Type, owner->Position, owner, 0, renderSlot);
    }
}
...
void ChangeCharacterExt(int Key, BYTE* Equipment, CHARACTER* pCharacter, OBJECT* pHelper)
{
    ...
    if (SceneFlag == 4)
    {
        if (SelectCharSet > 0)
        {
            ...
            if (sNewCharSet0 > 0 && sNewCharSet0 < 112)
            {
                ...
                c->Helper.Type = (MODEL_HELPER + 147) + (sNewCharSet0 - 1);
                if (pHelper == NULL)
                    CreateMount(c->Helper.Type, o->Position, o);
                else
                    CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
            }
            if (sNewCharSet1 > 0 && sNewCharSet1 < 94)
            {
                ...
                c->Helper.Type = (MODEL_HELPER + 258) + (sNewCharSet1 - 1);
                if (pHelper == NULL)
                    CreateMount(c->Helper.Type, o->Position, o);
                else
                    CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
            ...
            if (sNewCharSet2 > 0 && sNewCharSet2 < 100)
            {
                ...
                c->Helper.Type = MODEL_HELPER + 351 + (sNewCharSet2 - 1);
                if (pHelper == NULL)
                    CreateMount(c->Helper.Type, o->Position, o);
                else
                    CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
            ...
            if (sNewCharSet3 > 0 && sNewCharSet3 < 100)
            {
                ...
                c->Helper.Type = MODEL_HELPER + 451 + (sNewCharSet3 - 1);
                if (pHelper == NULL)
                    CreateMount(c->Helper.Type, o->Position, o);
                else
                    CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
    ...
    NEW_PET_STRUCT* lpCharSet = GetNewPetCharSet(c->Key);
    ...
    if (lpCharSet > 0)
    {
        ...
        if (NewCharSet0 > 0 && NewCharSet0 < 112)
        {
            ...
            c->Helper.Type = (MODEL_HELPER + 147) + (NewCharSet0 - 1);
            if (pHelper == NULL)
                CreateMount(c->Helper.Type, o->Position, o);
            else
                CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
        ...
        if (NewCharSet1 > 0 && NewCharSet1 < 94)
        {
            ...
            c->Helper.Type = (MODEL_HELPER + 258) + (NewCharSet1 - 1);
            if (pHelper == NULL)
            {
                CreateMount(c->Helper.Type, o->Position, o);
            }
        ...
        if (NewCharSet2 > 0 && NewCharSet2 < 100)
        {
            ...
            c->Helper.Type = MODEL_HELPER + 351 + (NewCharSet2 - 1);
            if (pHelper == NULL)
                CreateMount(c->Helper.Type, o->Position, o);
            else
                CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
        ...
        if (NewCharSet3 > 0 && NewCharSet3 < 100)
        {
            ...
            c->Helper.Type = MODEL_HELPER + 451 + (NewCharSet3 - 1);
            if (pHelper == NULL)
                CreateMount(c->Helper.Type, o->Position, o);
            else
                CreateMountSub(c->Helper.Type, o->Position, o, pHelper);
    ...
    const BYTE* extendedPetCharSet = nullptr;
    if (SceneFlag == 4 && SelectCharSet != nullptr)
        extendedPetCharSet = SelectCharSet->PetCharSet;
    else if (lpCharSet != nullptr)
        extendedPetCharSet = lpCharSet->PetCharSet;
    ApplyExtendedPetCharSet(c, extendedPetCharSet);
    ...
    if (SceneFlag != 4)
    {
        auto lpList = gPreview.GetCustomPreviewList(c->Key);

        if (lpList != 0)
        {
            if (lpList->WingIndex > 0)
            {
                if (gCustomWing->CheckCustomWingByItem(GET_ITEM(12, lpList->WingIndex)))
                {
                    c->Wing.Type = GET_ITEM_MODEL(12, lpList->WingIndex);
                }
            }
        }
    }
    ...
    {
        auto lpCharList = gPreview.GetCustomPreviewCharList(c->ID);

        if (lpCharList != 0)
        {
            if (lpCharList->WingIndex > 0)
            {
                if (gCustomWing->CheckCustomWingByItem(GET_ITEM(12, lpCharList->WingIndex)))
                {
                    c->Wing.Type = GET_ITEM_MODEL(12, lpCharList->WingIndex);
                }
            }
        }
    }
...
void Setting_Monster(CHARACTER* c, int Type, int PositionX, int PositionY)
{
    ...
    if (c != NULL)
    {
        ...
        if (CUSTOM_MONSTER_DATA* pInfo = gCustomMonster->GetMonsterData(Type))
        {
            o->Kind = KIND_NPC;
        }
    }

    g_pNPCName->SetNPCNameAttribute(c, Type, PositionX, PositionY);
}

void CreateMonsterCustomSkin(CHARACTER* c, BYTE Slot, int SlotElement, BYTE ItemLevel)
{
    c->BodyPart[Slot].Type = SlotElement;
    c->BodyPart[Slot].Level = ItemLevel;
    c->BodyPart[Slot].Option1 = 1;
    c->BodyPart[Slot].ExtOption = 1;
}
...
CHARACTER* CreateMonster(int Type, int PositionX, int PositionY, int Key)
{
    ...
    CUSTOM_BOSS_DATA* lpBoss = gCustomMonster->GetBossData(Type);
    ...
    if (lpBoss)
    {
        ...
        CreateMonsterCustomSkin(c, BODYPART_HELM,   MODEL_HELM      + lpBoss->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_ARMOR,  MODEL_ARMOR     + lpBoss->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_PANTS,  MODEL_PANTS     + lpBoss->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_GLOVES, MODEL_GLOVES    + lpBoss->SlotElement[0], 7);
        CreateMonsterCustomSkin(c, BODYPART_BOOTS,  MODEL_BOOTS     + lpBoss->SlotElement[0], 7);

        if (lpBoss->SlotElement[3] != -1)
        {
            c->Wing.Type = MODEL_WING + lpBoss->SlotElement[3];
            c->Wing.Level = 7;
            c->Wing.Option1 = 1;
            c->Wing.ExtOption = 1;
        }

        if (lpBoss->SlotElement[1] != -1)
        {
            c->Weapon[EQUIPMENT_WEAPON_RIGHT].Type = MODEL_ITEM + lpBoss->SlotElement[1];
            c->Weapon[EQUIPMENT_WEAPON_RIGHT].Level = 9;
            c->Weapon[EQUIPMENT_WEAPON_RIGHT].Option1 = 1;
            c->Weapon[EQUIPMENT_WEAPON_RIGHT].ExtOption = 1;
        }

        if (lpBoss->SlotElement[2] != -1)
        {
            c->Weapon[EQUIPMENT_WEAPON_LEFT].Type = MODEL_ITEM + lpBoss->SlotElement[2];
            c->Weapon[EQUIPMENT_WEAPON_LEFT].Level = 9;
            c->Weapon[EQUIPMENT_WEAPON_LEFT].Option1 = 1;
            c->Weapon[EQUIPMENT_WEAPON_LEFT].ExtOption = 1;
        }

        if (lpBoss->SlotElement[4] != -1)
        {
            c->Skill = lpBoss->SlotElement[4];
        }

        c->Object.Scale = lpBoss->ModelScale;
    ...
    if (lpInfo != nullptr)
    {
        if (lpInfo->Type == 0) //==NPC
        {
            int ModelID = Type + MODEL_MONSTER01;
            BMD* b = &Models[ModelID];
            if (b->NumActions > 0)
            {
                goto NexCustomMonter;
            }
            else
            {
                gLoadData.AccessModel(ModelID, lpInfo->ModelDir, lpInfo->ModelFile);
                gLoadData.OpenTexture(ModelID, lpInfo->ModelFolder);
                ...
                {
                    g_ConsoleDebug->Write(MCD_ERROR, "Custom NPC %d has no Actions!", Type);
                }

                if (b->NumMeshs > 0) gLoadData.OpenTexture(ModelID, lpInfo->ModelFolder);
            }
            ...
            strcpy(c->ID, getMonsterName(Type));
            c->Object.Scale = lpInfo->ModelScale;
            if (c != NULL)
            {
                OBJECT* o = &c->Object;
                c->MonsterIndex = Type;
        ...
        else
        {
            ...
            int ModelID = Type + MODEL_MONSTER01;
            c = CreateCharacter(Key, ModelID, PositionX, PositionY);
            strcpy(c->ID, getMonsterName(Type)); // Fix CustomMonsterName
            c->Object.Scale = lpInfo->ModelScale;
            if (c != NULL)
            {
                OBJECT* o = &c->Object;
    ...
    case 257:
        c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
        g_pModelNPC->MakeNpcModel(c, 257);
        strcpy(c->ID, "페이아");
        o = &c->Object;
        break;
    case 242:
    ...
    case 243:
        ...
        /*c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
        g_pModelNPC->MakeNpcModel(c, 243);*/
        strcpy(c->ID, "페이아");
        o = &c->Object;
        break;
    case 244:
    ...
    case 249:
        c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
        g_pModelNPC->MakeNpcModel(c, 249);
        SetCharacterScale(c);
        break;
    case 250:
        OpenNpc(MODEL_MERCHANT_MAN);
...
CHARACTER* CreateHero(int Index, int Class, int Skin, float x, float y, float Rotate)
{
    ...
    if (SceneFlag == LOG_IN_SCENE && gMapManager.WorldActive == WD_94EX702_LOGIN_SCENE)
    {
        Vector(0.12f, 0.12f, 0.12f, o->Light);
    }
    else
    {
...
bool RenderCharacterBackItem(CHARACTER* c, OBJECT* o, bool bTranslate)
{
    ...
    if (o->Type == MODEL_PLAYER)
    {
        ...
        if (gMapManager.InBloodCastle() && c->EtcPart != 0) // BCFIX
        {
            PART_t* w = &c->Wing;
        ...
        if (gMapManager.InChaosCastle() == false)
        {
            ...
            if (w->Type != -1)
            {
                ...
                if (!g_pOption->GetRenderHideWing())
                {
                    return bBindBack;
                }

                if (g_pOption->IsFpsWingMotion())
                {
                    if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW)
                    {
                        w->PlaySpeed = 0.4f; // 1
                    ...
                    {
                        w->PlaySpeed = 0.25f * 0.33f;// FPS_ANIMATION_FACTOR; // RISE Animation Wing
                    }
                }
                else
                {
                ...
                if (w->Type == MODEL_WING + 40 || w->Type == MODEL_WING + 50 || gCapeRender.IncludeCape(w->Type))
                {
                    w->LinkBone = 19;   
