...
namespace
{
bool g_pegasusQaJointSlots[MAX_JOINTS] = {};

bool IsPegasusQaJointCreation(int type, int subtype, WORD skill)
{
    return rise::pegasus::IsPegasusPrivateEffectTexture(type) ||
        rise::pegasus::IsSwordWrathLineBitmap(type) ||
        (type == BITMAP_2LINE_GHOST &&
            subtype == rise::windsoul::Base737Contract::RootSubtype) ||
        (type == BITMAP_FLARE &&
            subtype == rise::pegasus::kDexBoosterFlareJointSubtype &&
            skill == rise::pegasus::kDexBoosterSkill) ||
        (type == BITMAP_JOINT_SPIRIT2 &&
            subtype == rise::pegasus::kDragonViolentHeadJointSubtype) ||
        (type == 0x7DE0 && subtype == 19 && skill == 2012);
}
}

bool IsPegasusQaJointSlot(int index)
{
    return index >= 0 && index < MAX_JOINTS &&
        g_pegasusQaJointSlots[index];
}

void CreatePegasusRibbonSegment(int Type, vec3_t Position, vec3_t TargetPosition,
    vec3_t Angle, int SubType, OBJECT* Target, float Scale, float LifeTime)
{
    for (int i = 0; i < MAX_JOINTS; ++i)
    {
        ...
        ZeroMemory(o, sizeof(JOINT));
        g_pegasusQaJointSlots[i] = true;
        o->Live = true;
        o->Type = Type;
        o->TexType = Type;
        o->SubType = SubType;
        ...
        {
            const float* center = tail == 0 ? Position : TargetPosition;
            vec3_t local;
            vec3_t offset;
            Vector(-Scale * 0.5f, 0.0f, 0.0f, local);
            VectorRotate(local, matrix, offset);
            VectorAdd(center, offset, o->Tails[tail][0]);
            Vector(Scale * 0.5f, 0.0f, 0.0f, local);
            VectorRotate(local, matrix, offset);
            VectorAdd(center, offset, o->Tails[tail][1]);
            Vector(0.0f, 0.0f, -Scale * 0.5f, local);
            VectorRotate(local, matrix, offset);
            VectorAdd(center, offset, o->Tails[tail][2]);
            Vector(0.0f, 0.0f, Scale * 0.5f, local);
            VectorRotate(local, matrix, offset);
            VectorAdd(center, offset, o->Tails[tail][3]);
        }
        return;
    }
}
    ...
    WORD SkillIndex, WORD SkillSerialNum, int iChaIndex, const float* vPriorColor, short int sTargetindex)
{
    for (int i = 0; i < MAX_JOINTS; i++)
    {
        ...
        if (!o->Live)
        {
            g_pegasusQaJointSlots[i] =
                IsPegasusQaJointCreation(Type, SubType, SkillIndex);
            o->Live = true;
            o->Type = Type;
            o->TexType = o->Type;
            o->SubType = SubType;
            ...
            if (g_pegasusQaJointSlots[i])
                rise::pegasus::TraceRuntime("PEGASUS_JOINT_CREATE",
            ...
            if (Type == BITMAP_FLARE &&
                SubType == rise::pegasus::kDexBoosterFlareJointSubtype &&
                SkillIndex == rise::pegasus::kDexBoosterSkill)
            {
                // Pegasus 0x661804: subtype 48 is a short Dex Booster orbit,
                // not RISE's permanent equipment flare using the same native
                // subtype. Skill marks this isolated route without changing
                // the behavior of existing subtype-48 joints.
                o->Scale = rise::pegasus::kDexBoosterFlareJointRuntimeScale;
                o->LifeTime = rise::pegasus::kDexBoosterFlareJointLifetime;
                o->NumTails = 0;
                o->MaxTails = min(MAX_TAILS,
                    rise::pegasus::kDexBoosterFlareJointMaxTails);
                o->m_bCreateTails = true;
                VectorCopy(Position, o->TargetPosition);
                Vector(rise::pegasus::kDexBoosterFlareJointLightR,
                    rise::pegasus::kDexBoosterFlareJointLightG,
                    rise::pegasus::kDexBoosterFlareJointLightB, o->Light);
                return;
            }

            if (Type == rise::pegasus::kSpiralChargeFlareBitmap &&
                SubType == 0 && Target != NULL)
            {
                // Pegasus joint initializer 0x79C9E4 -> 0x67739D.
                o->Scale = Scale;
                o->LifeTime = rise::pegasus::kSpiralChargeRootJointLifetime;
                o->Velocity = 0.0f;
                o->m_iChaIndex = iChaIndex;
                o->m_bCreateTails = false;
                o->NumTails = -1;
                o->MaxTails = rise::pegasus::kSpiralChargeRootJointMaxTails;
                return;
            }

            if (Type == rise::pegasus::kShiningBirdWindBitmap)
            {
                // Pegasus 0x796FE8 and 0x79CA5B..0x79CAD5: wind01 is a
                // travelling joint with 5..8 tails, not an unhandled bitmap.
                o->LifeTime = 12.0f;
                o->NumTails = -1;
                o->MaxTails = (rand() % 4) + 5;
                o->Scale = static_cast<float>((rand() % 120) + 20) *
                    0.01f * Scale;
                o->Velocity = 60.0f;
                o->MultiUse = 1.0f;
            ...
            if (Type == rise::pegasus::kShiningBirdPillarBitmap)
            {
                // Pegasus 0x796FE8, 0x79CADA..0x79CB2B: no initial tail;
                // owner-render positioning feeds a stationary 23-tick ribbon.
                o->Scale = Scale;
                o->LifeTime = 23.0f;
                o->NumTails = -1;
                o->MaxTails = SubType == 0 ? 8 : 6;
                o->Velocity = 0.0f;
                o->MultiUse = 1.0f;
                VectorCopy(o->Light, o->StartPosition);
                o->m_bCreateTails = true;
            ...
            if (Type == rise::pegasus::kHavocSomasiJointBitmap ||
                Type == rise::pegasus::kHavocAdJointBitmap)
            {
                // Pegasus 0x665ED6: these two custom joints start without an
                // initial tail, then travel and build an eight-segment trail.
                o->Scale = Scale;
                o->LifeTime = 10;
                o->Velocity = static_cast<float>((rand() % 3) + 4);
                VectorCopy(o->Light, o->Direction);
                o->MultiUse = 1.0f;
                o->m_bCreateTails = true;
                o->NumTails = -1;
                o->MaxTails = min(MAX_TAILS,
                    static_cast<int>(8.0f / max(FPS_ANIMATION_FACTOR, 0.5f)));
                return;
            }

            if (rise::pegasus::IsSwordWrathLineBitmap(Type))
            {
                // Pegasus 0x796F74 -> 0x67A848: this is a joint, not a
                // particle. It starts in a 1000x1000 square around the root,
                // flies back toward it and owns a short randomized ribbon.
                o->Scale = Scale * (1.0f +
                    static_cast<float>(rand() % 1001) * 0.004f);
                o->LifeTime = static_cast<float>(
                    rise::pegasus::kSwordWrathLineLifetime);
                o->NumTails = -1;
                const int sourceTails = rise::pegasus::kSwordWrathLineTailMin +
                    rand() % (rise::pegasus::kSwordWrathLineTailMax -
                        rise::pegasus::kSwordWrathLineTailMin + 1);
                o->MaxTails = min(MAX_TAILS, max(1,
                    static_cast<int>(sourceTails /
                        max(FPS_ANIMATION_FACTOR, 0.5f))));
                o->m_bCreateTails = true;
                o->Position[0] += static_cast<float>((rand() % 1001) - 500);
                o->Position[1] += static_cast<float>((rand() % 1001) - 500);
                o->Position[2] += static_cast<float>((rand() % 31) - 10);
                const float x = o->Position[0] - o->TargetPosition[0];
                const float y = o->Position[1] - o->TargetPosition[1];
                const float z = o->Position[2] - o->TargetPosition[2];
                o->Velocity = sqrtf(x * x + y * y + z * z) *
                    rise::pegasus::kSwordWrathLineVelocityFactor;
                o->Angle[2] = CreateAngle(o->Position[0], o->Position[1],
                    o->TargetPosition[0], o->TargetPosition[1]);
                VectorCopy(o->Light, o->Direction);
                return;
            }
            ...
            case BITMAP_JOINT_ENERGY:
                ...
                case 33:
                    ...
                    if (o->SubType == 5)
					o->MaxTails = (int)(10.0f / max(FPS_ANIMATION_FACTOR, 0.5f));
            ...
            case BITMAP_2LINE_GHOST:
            {
                if (o->SubType == 0)
                {
                    ...
                    if (pros < 15)  
                    {
                        o->Velocity = 30.f + rand() % 10;
                    }
                    else if (pros > 15 && pros < 50)
                    {
                        o->Velocity = 40.f + rand() % 20;
                    }
                    else if (pros > 50 && pros < 80 )
                    {
                        o->Velocity = 60.f + rand() % 20;
                    }
                    else  
                    {
                        o->Velocity = 80.f + rand() % 10;
                    }

                    float AttackSpeedDL1 = CharacterAttribute->AttackSpeed;
                    int heso1 = static_cast<int>(round(-0.0067f * AttackSpeedDL1 + 17.3f));
                    int duoibongma;
                    if (heso1 < 2)
                        duoibongma = 2;
                    else if (heso1 > 16)
                    ...
                    if (Target != nullptr)
                    {
                        ...
                        if (c != nullptr)
                        {
                            if (c == Hero)
                            {
                                o->MaxTails = 4 + rand() % 6 + duoibongma;
                            }
                            else
                            {
                                o->MaxTails = 6 + rand() % 6;
                    ...
                    Vector(0.85f, 0.85f, 0.85f, o->Light);
                ...
                {
                    // Wind Soul 737, traced S21 constructor 0x15CBBD3.
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND;
                    o->Scale = Scale;
                    o->MaxTails = rise::windsoul::Base737Contract::MaxTails;
                    o->LifeTime = rise::windsoul::Base737Contract::LifeTime;
                    o->Velocity = rise::windsoul::Base737Contract::InitialVelocity;
                    o->Weapon = 0.f;
                    o->TargetIndex[0] = -1;
                    Vector(0.f, rise::windsoul::Base737Contract::DirectionY, 0.f, o->Direction);
                    VectorCopy(TargetPosition, o->TargetPosition);
                    o->TargetPosition[2] += rise::windsoul::Base737Contract::TargetZOffset;
                    ...
                    if (rand() % 3 < 2)
                    {
                        vec3_t vLight, vDesairPosition;
                        Vector(0.6f, 0.8f, 0.9f, vLight);
                        GetNearRandomPos(o->Position, 120, vDesairPosition);
                        CreateEffect(MODEL_DESAIR, vDesairPosition, o->Angle,
                            vLight, 2, NULL, -1, 0, 0, 0, 1.f, i);
                    }
                }
            }
            break;
            ...
            case MODEL_SPEARSKILL:
                ...
                case 2:
                    ...
                    //Vector(1.0f, .3f, .3f, o->Light);     //RISE Skill dam gio mau color
                    o->LifeTime = 20;
                    o->MaxTails = 5;
            ...
            case BITMAP_FLARE_BLUE:
                ...
                else if (o->SubType == 25)
                {
                    ...
                    o->MultiUse = 0;
                    o->LifeTime = 30;
                    o->MaxTails = (int)(15.0f / max(FPS_ANIMATION_FACTOR, 0.5f));
                    o->Direction[0] = (float)(rand() % 3000);
                    o->Scale = 30.f;
                }
                else if (o->SubType == 13)
            ...
            case BITMAP_FLARE + 1:
                ...
                case 13:
                    ...
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
                    o->Scale = Scale;
                    o->MaxTails = (int)(16.0f / max(FPS_ANIMATION_FACTOR, 0.5f));
                    o->Velocity = 70.f;
                    break;
                case 14:
                    Vector(1.0f, 1.0f, 1.0f, o->Light);
                ...
                case 15:
                    ...
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND;
                    o->Scale = Scale;
                    o->MaxTails = (int)(8.0f / max(FPS_ANIMATION_FACTOR, 0.5f));
                    o->Velocity = 70.f;
                    break;
                case 16:
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
                ...
                case 18:
                {
                    ...
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND;
                    o->Scale = Scale * 1.15;
                    o->MaxTails = (int)(7.0f / min(FPS_ANIMATION_FACTOR, 0.5f));
                    o->Velocity = 70.f;
                }
                break;
                case 19:
...
#if defined(PEGASUS_ISOLATED_TEST)
    if (IsPegasusQaJointCreation(Type, SubType, SkillIndex))
        rise::pegasus::TraceRuntime("PEGASUS_JOINT_CREATE",
...
void MoveJoint(JOINT* o, int iIndex)
{
    ...
    if (o->Type == BITMAP_FLARE &&
        o->SubType == rise::pegasus::kDexBoosterFlareJointSubtype &&
        o->Skill == rise::pegasus::kDexBoosterSkill)
    {
        // Pegasus 0x661980 and caller 0x7A62AF..0x7A62ED: orbit on a
        // spherical path around the live owner, append one ribbon tail, then
        // use the ordinary FPS-adjusted lifetime. Do not fall through to the
        // native subtype-48 bone-24 attachment below.
        if (!o->Target || !o->Target->Live)
        {
            o->Live = false;
            return;
        }
        rise::pegasus::ComputeDexBoosterFlareJointPosition(WorldTime, iIndex,
            o->Target->Position, o->Position);
        VectorCopy(o->Target->Position, o->TargetPosition);
        AngleMatrix(o->Angle, Matrix);
        CreateTail(o, Matrix);
    ...
    if (o->Type == rise::pegasus::kSpiralChargeFlareBitmap &&
        o->SubType == 0)
    {
        OBJECT* root = o->Target;
        if (!root || !root->Live ||
            root->Type != rise::pegasus::kSpiralChargeRootModel ||
            !rise::pegasus::EnsureSpiralChargeModel(root->Type))
        {
            o->Live = false;
            return;
        }
        if (root->AnimationFrame <
            rise::pegasus::kSpiralChargeRootJointActivationFrame)
        {
            o->m_bCreateTails = false;
        }
        else
        {
            BMD& model = Models[root->Type];
            model.CurrentAction = root->CurrentAction;
            model.BodyScale = root->Scale;
            model.Animation(BoneTransform, root->AnimationFrame,
                root->PriorAnimationFrame, root->PriorAction, root->Angle,
                root->HeadAngle, false, false);
            model.TransformByObjectBone(o->Position, root, o->m_iChaIndex);
            o->m_bCreateTails = true;
        }
    }
    ...
    if (o->Type == rise::pegasus::kShiningBirdWindBitmap)
    {
        // 0x7ABA69..0x7ABA89: acceleration occurs AFTER the common movement
        // step, then the native tail/lifetime path runs without a local fade.
        o->Velocity += 2.0f * FPS_ANIMATION_FACTOR;
    }

    if (o->Type == rise::pegasus::kHavocSomasiJointBitmap ||
        o->Type == rise::pegasus::kHavocAdJointBitmap)
    {
        // Pegasus 0x66637A: preserve the authored RGB and fade only during
        // the final five lifetime ticks.
        if (o->LifeTime < 5)
        {
            o->MultiUse -= FPS_ANIMATION_FACTOR * 0.2f;
            if (o->MultiUse < 0.0f) o->MultiUse = 0.0f;
        }
        VectorScale(o->Direction, o->MultiUse, o->Light);
    }

    if (rise::pegasus::IsSwordWrathLineBitmap(o->Type))
    {
        // Pegasus 0x7ABA8E -> 0x67AD92. Movement/tail creation stays in the
        // native joint loop; this callback owns only the 20-tick RGB fade.
        VectorScale(o->Direction,
            rise::pegasus::SwordWrathLineLightFactor(o->LifeTime), o->Light);
    }
    ...
    case BITMAP_2LINE_GHOST:
    {
        if (o->SubType == 0)
        {
            ...
            if ((int)o->LifeTime > 40 &&(rand() % 16 == 0))
            {
        ...
        {
            // Wind Soul 737, traced S21 move case 0x15EE884.
            const int targetCharacterIndex = o->m_sTargetIndex;
            if (targetCharacterIndex < 0 ||
                targetCharacterIndex >= MAX_CHARACTERS_CLIENT)
            {
                o->Live = false;
                return;
            }

            OBJECT* targetObject = &CharactersClient[targetCharacterIndex].Object;
            if (!targetObject->Live)
            {
                o->Live = false;
                return;
            }
            VectorCopy(targetObject->Position, o->TargetPosition);
            o->TargetPosition[2] += rise::windsoul::Base737Contract::TargetZOffset;

            const int lifeTick = (int)o->LifeTime;
            if (lifeTick < (int)rise::windsoul::Base737Contract::FadeStartLife)
            {
                const float luminosity = lifeTick * rise::windsoul::Base737Contract::FadeFactor;
                Vector(luminosity * rise::windsoul::Base737Contract::JointLightR,
                    luminosity * rise::windsoul::Base737Contract::JointLightG,
                    luminosity * rise::windsoul::Base737Contract::JointLightB, o->Light);
            }
            else
            {
                Vector(rise::windsoul::Base737Contract::JointLightR,
                    rise::windsoul::Base737Contract::JointLightG,
                    rise::windsoul::Base737Contract::JointLightB, o->Light);
            }

            if ((lifeTick % 8) == 0 && o->TargetIndex[0] != lifeTick)
            {
                vec3_t smokeLight;
                Vector(rise::windsoul::Base737Contract::SmokeLightR,
                    rise::windsoul::Base737Contract::SmokeLightG,
                    rise::windsoul::Base737Contract::SmokeLightB, smokeLight);
                // S21 calls the particle allocator directly here.  The previous
                // FPS-checked wrapper discarded most of the four scheduled puffs.
                CreateParticle(BITMAP_SMOKE, o->Position, o->Angle, smokeLight,
                    rise::windsoul::Base737Contract::SmokeSubtype,
                    rise::windsoul::Base737Contract::SmokeScale, NULL);
                o->TargetIndex[0] = lifeTick;
            }

            const int movementSteps = (o->LifeTime <= 10.f) ? 2 : 1;
            float targetDistance = 0.f;
            for (int step = 0; step < movementSteps; ++step)
            {
                if (rand_fps_check(2))
                {
                    const float jitter = (float)(rand() % rise::windsoul::Base737Contract::PitchJitterModulo +
                        rise::windsoul::Base737Contract::PitchJitterBase);
                    o->Angle[0] += (o->Angle[0] < rise::windsoul::Base737Contract::PitchThreshold) ? jitter : -jitter;
                }
                targetDistance = MoveHumming(o->Position, o->Angle,
                    o->TargetPosition, o->Velocity);
                // This branch is already reached from the engine's FPS-scaled
                // joint step.  S21 adds these per-step values without another
                // frame multiplier.
                o->Velocity += rise::windsoul::Base737Contract::VelocityStep;
                if (o->LifeTime < 10.f)
                    o->Velocity += rise::windsoul::Base737Contract::FinalVelocityStep;
            ...
            if (targetDistance < rise::windsoul::Base737Contract::ImpactDistance &&
                lifeTick == rise::windsoul::Base737Contract::ImpactLife && o->Weapon == 0.f)
            {
                vec3_t smokeLight;
                Vector(rise::windsoul::Base737Contract::ImpactLight,
                    rise::windsoul::Base737Contract::ImpactLight,
                    rise::windsoul::Base737Contract::ImpactLight, smokeLight);
                o->Scale *= rise::windsoul::Base737Contract::ImpactScaleFactor;
                CreateParticle(BITMAP_SMOKE, o->TargetPosition,
                    o->Angle, smokeLight, 2, 1.f, NULL);
                CreateParticle(BITMAP_SMOKE, o->TargetPosition,
                    o->Angle, smokeLight, 5, 1.f, NULL);
                o->Weapon = 1.f;
            }
        }
    }
    ...
    case MODEL_SPEARSKILL:	// 방어막
        ...
        //    {
        //        VectorScale(MagicPos, 60.f, MagicPos);
        //    }
        //    else
        //    {
        //        VectorScale(MagicPos, 30.f, MagicPos);
        //    }
        //    VectorAdd(MagicPos, o->TargetPosition, MagicPos);
        //    vec3_t TargetPos;
        //    //VectorCopy(o->Target->m_vPosSword, TargetPos); //Goc
        //    VectorCopy(o->StartPosition, TargetPos);
    ...
    case BITMAP_FLARE + 1:
        ...
        else if (o->SubType == 12 || o->SubType == 13 || o->SubType == 14 || o->SubType == 17 || o->SubType == 15 || o->SubType == 18)
        {
            if (o->Target != NULL)
            {
                ...
                if (o->SubType == 14 && o->Live == true && rand() % 3 == 0)
                {
                    CreateParticle(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 34, 1.0f);
                    if (rand() % 2 == 0)
                    {
                        CreateParticleFpsChecked(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 12, 1.0f);
