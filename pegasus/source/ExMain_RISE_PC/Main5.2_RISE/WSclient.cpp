...
#include "RISE/PegasusCasterActionTrace.h"
#include "RISE/PegasusHighSkillDispatch.h"
#include "RISE/PegasusCastRootGate.h"
#include "RISE/PegasusRuntimeTrace.h"
#include "RISE/PegasusLocalVisualQa.h"
#include "RISE/WindSoulRuntime.h"
#include "../../Shared/PegasusHighSkillCatalogData.h"
#include "../../Shared/PegasusRuntimeQa15.h"
#if defined(PEGASUS_ISOLATED_TEST)
#include "RISE/PegasusQaCastBoundary.h"
static rise::pegasus::QaCastBoundary g_PegasusQaCastBoundary;
#endif
...
extern void CrashReportOnConnectServer();
extern void CrashReportReceiveAck(const BYTE* receiveBuffer, int size);
...
#include <RISE/CustomReadGuildClient.h>
#include <RISE/NewUICharInfoStats.h>
#include <RISE/NewUIItemTrader.h>
#include <RISE/NewUIChaosMixRate.h>
#include <RISE/MainProtocol.h>
#include <RISE/CustomPrice.h>
...
static bool IsRisePrivateServerAddress(const char* ip)
{
	if (ip == NULL || ip[0] == '\0')
	{
		return false;
	...
	if (strncmp(ip, "10.", 3) == 0 || strncmp(ip, "192.168.", 8) == 0 ||
		strncmp(ip, "127.", 4) == 0 || strncmp(ip, "0.", 2) == 0)
	{
		return true;
	}
	...
	{
		const int secondOctet = atoi(ip + 4);
...
void ReceiveServerConnect(const BYTE* ReceiveBuffer) //Recebe informação do ConnectServer sobre a sala e envia a conexão para a sala escolhida
{
	...
	if (IsRisePrivateServerAddress(IP) && !IsRisePrivateServerAddress(szServerIpAddress))
	{
		strncpy(IP, szServerIpAddress, sizeof(IP) - 1);
		IP[sizeof(IP) - 1] = '\0';
	}
	SocketClient.Close();
...
static bool GetRiseCharacterSelectPlacement(int index, float* pos, float& angle)
{
	if (pos == NULL)
	{
		return false;
...
#if RISE_ENABLE_EX702_SELECT_SCENE
	switch (index)
	{
	case 0: pos[0] = 13379.0f; pos[1] = 11958.0f; angle = 115.0f; return true;
	case 1: pos[0] = 13367.0f; pos[1] = 12229.0f; angle = 90.0f;  return true;
	case 2: pos[0] = 13384.0f; pos[1] = 12503.0f; angle = 75.0f;  return true;
	case 3: pos[0] = 13468.0f; pos[1] = 12764.0f; angle = 60.0f;  return true;
	case 4: pos[0] = 13566.0f; pos[1] = 13018.0f; angle = 35.0f;  return true;
	...
	switch (index)
	{
	case 0: pos[0] = 8008.0f; pos[1] = 18885.0f; angle = 115.0f; return true;
	case 1: pos[0] = 7986.0f; pos[1] = 19145.0f; angle = 90.0f;  return true;
	case 2: pos[0] = 8046.0f; pos[1] = 19400.0f; angle = 75.0f;  return true;
	case 3: pos[0] = 8133.0f; pos[1] = 19645.0f; angle = 60.0f;  return true;
	case 4: pos[0] = 8282.0f; pos[1] = 19845.0f; angle = 35.0f;  return true;
	default: return false;
	}
}
...
void ReceiveCharacterList(const BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->CharacterCount; i++)
	{
		...
		if (!GetRiseCharacterSelectPlacement(Data2->Index, fPos, fAngle))
		{
			return;
		}
		...
		if (c)
		{
			...
			c->rCharOption = Data2->rCharOption;
		}
...
void ReceiveCharacterCard_New(const BYTE* ReceiveBuffer)
{
	...
	if ((Data->CharacterCard & CLASS_MAGIC_CARD) == CLASS_MAGIC_CARD)
...
void ReceiveCreateCharacter(const BYTE* ReceiveBuffer)
{
	...
	if (Data->Result == 1)
	{
		...
		if (!GetRiseCharacterSelectPlacement(Data->Index, fPos, fAngle))
		{
			return;
		}
...
{
	WriteCrashBreadcrumb("ReceiveLogOut: packet received");
	LogOut = false;
	auto Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
	g_pReconnect->ReconnectOnCloseClient(Data->Value);
	...
	case 0:
		...
		break;
	case 1:
		WriteCrashBreadcrumb("ReceiveLogOut: select character begin");
		StopMusic();
		AllStopSound();

		SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();

		WriteCrashBreadcrumb("ReceiveLogOut: ReleaseMainData begin");
		ReleaseMainData();
		WriteCrashBreadcrumb("ReceiveLogOut: ReleaseMainData complete");
		CryWolfMVPInit();
		...
		WriteCrashBreadcrumb("ReceiveLogOut: InitGame begin");
		InitGame();
		WriteCrashBreadcrumb("ReceiveLogOut: InitGame complete");
		break;
	case 2:
...
{
	g_pReconnect->ReconnectOnCharacterInfo();
	...
	CharacterAttribute->ViewReset			= Data->ViewReset;
	CharacterAttribute->ViewMReset			= Data->ViewMReset;
	CharacterAttribute->ViewMaxReset		= Data->ViewMaxReset;
	CharacterAttribute->MuHelperLevel		= Data->MuHelperLevel;
	CharacterAttribute->AutoResetEnable		= Data->AutoResetEnable;

	CharacterAttribute->AttackSpeed			= Data->SpeedAttack;
	CharacterAttribute->MagicSpeed			= Data->MagicAttack;
	
	CharacterAttribute->ManaClassReg[0]		= Data->ManaClassReg[0]; // DK
	CharacterAttribute->ManaClassReg[1]		= Data->ManaClassReg[1]; // DW
	CharacterAttribute->ManaClassReg[2]		= Data->ManaClassReg[2]; // EF
	CharacterAttribute->ManaClassReg[3]		= Data->ManaClassReg[3]; // MG
	CharacterAttribute->ManaClassReg[4]		= Data->ManaClassReg[4]; // DL
	CharacterAttribute->ManaClassReg[5]		= Data->ManaClassReg[5]; // SU
	CharacterAttribute->ManaClassReg[6]		= Data->ManaClassReg[6]; // RF A
	CharacterAttribute->ManaClassReg[7]		= Data->ManaClassReg[7]; // RF B

	CharacterMachine->Gold					= Data->Gold;
	gMapManager.WorldActive					= Data->Map;

	memcpy(CharacterAttribute->PersonalCode, Data->PersonalCode, sizeof(CharacterAttribute->PersonalCode));
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x03 [ReceiveJoinMapServer] %s: Key: %d Map: %d X: %d Y:%d",Data->PersonalCode, c->Key, Data->Map, Data->PositionX, Data->PositionY);
	...
	g_pNewUIMiniMap->ResetMapMarkers();
	gCustomItem.ServerEquipmentOptions.Reset();

	// AntiStream can be busy uploading screenshot blocks. Do not stall the
	// join-map packet handler while waiting for its socket lock.
	AntiStream_SendHardwareInfoAsync();

	// RISE: Auto Load Guild & Union List
...
void ReceiveMagicList(BYTE* ReceiveBuffer)
{
	...
	if (Data->Value == 0xFF)
	{
		...
		if (rise::pegasus::FindHighSkillById(Data2->Type) != nullptr ||
			Data2->Type == AT_SKILL_WIND_SOUL ||
			Data2->Type == rise::pegasus::kSwordBlowFlySkill)
		{
			rise::pegasus::TraceRuntime("SKILL_LIST_DEL", Data2->Type,
				Data2->Index, "RECEIVED");
		}
	...
	else if (Data->Value == 0xFE)
	{
		...
		if (rise::pegasus::FindHighSkillById(Data2->Type) != nullptr ||
			...
			Data2->Type == rise::pegasus::kSwordBlowFlySkill)
		{
			rise::pegasus::TraceRuntime("SKILL_LIST_ADD", Data2->Type,
				Data2->Index, "RECEIVED");
			rise::pegasus::TraceRuntime("SKILL_LIST_STATE", Data2->Type,
				Data2->Index,
				CharacterAttribute->Skill[Data2->Index] == Data2->Type ?
				"PRESENT" : "MISSING");
		}
	...
	else
	{
		...
		for (int i = 0; i < Data->Value; i++)
		{
			...
			if (rise::pegasus::FindHighSkillById(Data2->Type) != nullptr ||
				...
				Data2->Type == rise::pegasus::kSwordBlowFlySkill)
			{
				rise::pegasus::TraceRuntime("SKILL_LIST_FULL", Data2->Type,
					Data2->Index, "RECEIVED");
				rise::pegasus::TraceRuntime("SKILL_LIST_STATE", Data2->Type,
					Data2->Index,
					CharacterAttribute->Skill[Data2->Index] == Data2->Type ?
	...
	for (int i = 0; i < MAX_SKILLS; ++i)
	{
		if (CharacterAttribute->Skill[i] == rise::pegasus::kSwordBlowFlySkill)
		{
			rise::pegasus::TraceRuntime("SKILL_LIST_STATE",
				rise::pegasus::kSwordBlowFlySkill, i, "PRESENT");
			break;
		}
	}
	...
	for (int i = 0; i < MAX_SKILLS; i++)
	{
		...
		if (AT_SKILL_ICE_BLADE == Skill
			|| MASTER_SKILL_ADD_POWER_SLASH_IMPROVED == Skill
			|| MASTER_SKILL_ADD_TRIPLE_SHOT_IMPROVED == Skill
			|| MASTER_SKILL_ADD_TRIPLE_SHOT_ENHANCED == Skill
...
BOOL ReceiveInventory(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	for (int i = 0; i < Data->Value + 1; i++)
	{
		...
		if (CharacterMachine->CheckEquipment(itemindex))
		{
		...
		// RISE Debug InventoryExt 1
		//g_ConsoleDebug->Write(MCD_ERROR, "RISE Debug InventoryExt 1 %d / %d", itemindex, Data->Value);
	}
...
#if defined(PEGASUS_ISOLATED_TEST)
    const size_t noticeOffset = offsetof(PRECEIVE_NOTICE, Notice);
    const size_t noticeBytes = Data->Header.Size > noticeOffset
        ? Data->Header.Size - noticeOffset : 0;
    if (Data->Result == 1 && IsPegasusServerQaActive() && noticeBytes > 12 &&
        memcmp(Data->Notice, "QA15 SERVER ", 12) == 0 &&
        memchr(Data->Notice, 0, noticeBytes) != nullptr)
    {
        // Records what the connected server announces, not its EXE hash.
        rise::pegasus::TraceRuntime("SERVER_QA_BUILD_DECLARATION", 0, 0,
            reinterpret_cast<const char*>(Data->Notice));
    }
	if (Data->Result == 1 && IsPegasusServerQaActive() &&
		g_PegasusQaCastBoundary.Notice(reinterpret_cast<const char*>(Data->Notice)))
		rise::pegasus::TraceRuntime("SERVER_QA_CAST_BOUNDARY", 0, 0, "ACCEPTED");
#endif
	if (Data->Result == 0)
	{
...
void ReceiveChangePlayer(const BYTE* ReceiveBuffer)
{
	...
	case 7:
		if (Type == 0x1FFF)
		{
			if (c->Wing.Type == MODEL_WING + 39 ||
				...
				gCapeRender.IncludeCape(c->Wing.Type) ||
	...
	case 8:
		...
		else
		{
			...
			if (gCustomPet.CheckCustomPetByItem(Type))
			{
				CreateMount(Type + MODEL_ITEM, o->Position, o);
			}
		}
		break;
	case 237:
	case 238:
	{
		const int helperIndex = (Data->iSrcIndex == 237) ? 0 : 1;
		const int renderSlot = helperIndex + 1;
		for (int i = 0; i < MAX_MOUNTS; ++i)
		{
			if (Mounts[i].Live && Mounts[i].Owner == o && Mounts[i].LinkBone == renderSlot)
			{
				Mounts[i].Live = false;
			}
		}
		...
		if (Type == 0x1FFF)
		{
			c->MuunHelper[helperIndex].Type = -1;
		}
		else
		{
			c->MuunHelper[helperIndex].Type = MODEL_ITEM + Type;
			c->MuunHelper[helperIndex].Level = LevelConvert(Level);
			c->MuunHelper[helperIndex].Option1 = Option;
			c->MuunHelper[helperIndex].ExtOption = ExtOption;
			CreateMount(MODEL_ITEM + Type, o->Position, o, 0, renderSlot);
		}
		break;
	}
	}
...
#if defined(PEGASUS_ISOLATED_TEST)
static bool IsPegasusAutoQaEnabledForCastCleanup()
{
	const DWORD serverAttributes = GetFileAttributesA("PegasusAutoQA.enable");
	const DWORD castOnlyAttributes = GetFileAttributesA("PegasusAutoQACastOnly.enable");
	const DWORD localAttributes = GetFileAttributesA("PegasusLocalVisualQA.enable");
	const DWORD previewAttributes = GetFileAttributesA("PegasusSwordBlowPreview.enable");
	return (serverAttributes != INVALID_FILE_ATTRIBUTES &&
		(serverAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) ||
		(castOnlyAttributes != INVALID_FILE_ATTRIBUTES &&
		(castOnlyAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) ||
		(localAttributes != INVALID_FILE_ATTRIBUTES &&
		(localAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) ||
		(previewAttributes != INVALID_FILE_ATTRIBUTES &&
		(previewAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0);
}

static void ClearPegasusAutoQaStaleBuffs(int skill, OBJECT* owner)
{
	if (!owner || !IsPegasusAutoQaEnabledForCastCleanup())
		return;
	int keep = rise::pegasus::NativeEffectForHighSkillBuff(skill);
	if (skill == rise::pegasus::kSwordWrathSkill ||
		skill == rise::pegasus::kSwordBlowSkill ||
		skill == rise::pegasus::kSwordBlowFlySkill)
		keep = rise::pegasus::kSwordWrathNativeEffect;
	if (skill != rise::pegasus::kDeathsideSkill &&
		SearchEffect(rise::pegasus::kDeathsideReaperModel, owner))
	{
		DeleteEffect(rise::pegasus::kDeathsideReaperModel, owner);
		rise::pegasus::TraceRuntime("AUTO_QA_DEATHSIDE_CLEANUP", skill,
			rise::pegasus::kDeathsideReaperModel, "LOCAL_STALE_REMOVED");
	}
	static const int effects[] = {
		rise::pegasus::kSwordWrathNativeEffect,
		rise::pegasus::kSpiralChargeNativeEffect,
		rise::pegasus::kCrusherChargeNativeEffect,
		rise::pegasus::kElementalChargeNativeEffect,
		rise::pegasus::kDexBoosterNativeEffect,
		rise::pegasus::kDarknessNativeEffect,
	};
	int removed = 0;
	for (std::size_t n = 0; n < sizeof(effects) / sizeof(effects[0]); ++n)
	{
		const int effect = effects[n];
		if (effect == keep || !g_isCharacterBuff(owner,
			static_cast<eBuffState>(effect)))
			continue;
		UnRegisterBuff(static_cast<eBuffState>(effect), owner);
		++removed;
	}
	rise::pegasus::TraceRuntime("AUTO_QA_BUFF_CLEANUP", skill, removed,
		removed != 0 ? "LOCAL_STALE_REMOVED" : "CLEAN");
}
#endif
...
void ReceiveCreateMonsterViewport(const BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		// RISE ViewPort 1
		c->HPRate = Data2->CurHP;
		c->pLevel = Data2->Level;
		c->IsLife = Data2->Life;
		#if OFF_DEBUG_CONSOLE
		g_ConsoleDebug->Write(MCD_NORMAL, "ReceiveAttackDamage HPRate:%d%% / Life:%f / MonsterIndex:%d / pLevel:%d"
			, c->HPRate
			, c->IsLife
			, c->MonsterIndex
			, c->pLevel
...
#if defined(PEGASUS_ISOLATED_TEST)
static int g_PegasusQaDamageSkill = -1;
static int g_PegasusQaDamageTargetIndex = MAX_CHARACTERS_CLIENT;
static DWORD g_PegasusQaDamageArmTick = 0;

static bool IsPegasusQaDamageSkill(int skill)
{
	return skill == rise::pegasus::kRainingArrowSkill ||
		skill == rise::pegasus::kChaosBladeSkill ||
		skill == rise::pegasus::kHavocSpearSkill ||
		skill == rise::pegasus::kSwordBlowSkill ||
		skill == rise::pegasus::kShiningBirdSkill ||
		skill == rise::pegasus::kDragonViolentSkill ||
		skill == rise::pegasus::kSpearstormSkill;
}

static void ArmPegasusQaDamageTrace(int skill, int targetIndex)
{
	if (!IsPegasusQaDamageSkill(skill))
		return;
	// Multi-target cast echoes arrive as a burst. Retain the first authoritative
	// target so a later echo cannot move the reaction gate to another viewport.
	if (g_PegasusQaDamageSkill == skill &&
		(GetTickCount() - g_PegasusQaDamageArmTick) <= 3000)
		return;
	g_PegasusQaDamageSkill = skill;
	g_PegasusQaDamageTargetIndex = targetIndex;
	g_PegasusQaDamageArmTick = GetTickCount();
}

static void TracePegasusQaDamagePacket(int targetIndex, __int64 damage)
{
	if (g_PegasusQaDamageSkill < 0 ||
		targetIndex != g_PegasusQaDamageTargetIndex ||
		(GetTickCount() - g_PegasusQaDamageArmTick) > 3000)
		return;
	const int tracedDamage = damage > INT_MAX ? INT_MAX :
		(damage < INT_MIN ? INT_MIN : static_cast<int>(damage));
	rise::pegasus::TraceRuntime("DAMAGE_PACKET", g_PegasusQaDamageSkill,
		tracedDamage, damage > 0 ? "POSITIVE" : "ZERO");
}

static void TracePegasusQaTargetReaction(int targetIndex)
{
	if (g_PegasusQaDamageSkill < 0 ||
		targetIndex != g_PegasusQaDamageTargetIndex ||
		(GetTickCount() - g_PegasusQaDamageArmTick) > 3000)
		return;
	rise::pegasus::TraceRuntime("TARGET_REACTION", g_PegasusQaDamageSkill,
		targetIndex,
		(CharactersClient[targetIndex].Object.CurrentAction == PLAYER_SHOCK ||
		 CharactersClient[targetIndex].Object.CurrentAction == MONSTER01_SHOCK)
		 ? "APPLIED" : "NATIVE_CALL_WITHOUT_SHOCK_ACTION");
}

static void TracePegasusQaReactionGate(int targetIndex, const char* reason)
{
	if (g_PegasusQaDamageSkill < 0 ||
		targetIndex != g_PegasusQaDamageTargetIndex ||
		(GetTickCount() - g_PegasusQaDamageArmTick) > 3000)
		return;
	rise::pegasus::TraceRuntime("TARGET_REACTION_GATE", g_PegasusQaDamageSkill,
		targetIndex, reason);
}
#endif
...
void ReceiveDamage(const BYTE* ReceiveBuffer)
{
	...
	CharacterAttribute->Life = Data->GETDAME[0];
	CharacterAttribute->Shield = Data->GETDAME[1];
}
...
void ProcessDamageCastle(LPPRECEIVE_ATTACK Data)
{
	...
	__int64 Damage = Data->ViewDamageHP;
	...
	__int64 accumDamage = ShieldDamage + Damage;
	int	rstDamage = -1;
	float rstScale = 0.8f;
...
void ReceiveAttackDamage(const BYTE* ReceiveBuffer)
{
	...
	if (c)
	{
		...
		__int64 ShieldDamage = Data->ViewDamageSD;
		...
		CharacterAttribute->Experience = Data->CurExp;
		CharacterAttribute->NextExperince = Data->MaxExp;
		...
		if (Success)
		{
			...
			if (Key == HeroKey)
			{
				...
				CharacterAttribute->Life = Data->ViewCurHP;

				CharacterAttribute->Shield = Data->ViewCurSD;
			}
		}
		else
		{
			...
			else
			{
				...
				{
#if defined(PEGASUS_ISOLATED_TEST)
					TracePegasusQaReactionGate(Index, "EXCLUDED_MONSTER_275");
#endif
				}
				else if (rand_fps_check(2))
				{
					...
					TracePegasusQaReactionGate(Index, "NATIVE_RANDOM_GATE");
#endif
			}
			float scale = 15.f;
			if (Damage == 0)
		...
		g_ConsoleDebug->Write(MCD_RECEIVE, "0x11 [ReceiveAttackDamage(%d %s)]", AttackPlayer, pKit->QN(Damage));
	}
}
...
void ReceiveMagicFinish(BYTE* ReceiveBuffer)
{
	...
	case MASTER_SKILL_ADD_POISON_IMPROVED:
		...
		break;
	case AT_SKILL_SLOW:
	case MASTER_SKILL_ADD_ICE_IMPROVED1:
	case MASTER_SKILL_ADD_ICE_IMPROVED2:
		UnRegisterBuff(eDeBuff_Freeze, o);
		break;
	case AT_SKILL_BLOW_OF_DESTRUCTION:
	case MASTER_SKILL_ADD_FROZEN_STAB_IMPROVED:
	case MASTER_SKILL_ADD_FROZEN_STAB_ENHANCED:
	case MASTER_SKILL_ADD_FROZEN_STAB_MASTERED:
	...
	case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_EXTENDED:
	case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_ENHANCED:
	case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_MASTERED:
	...
	case MASTER_SKILL_ADD_FIRE_SLASH_ENHANCED:
...
#if defined(PEGASUS_ISOLATED_TEST)
static rise::pegasus::CastRootStamp g_PegasusCastRootStamps[9] = {};

void ResetPegasusCastRootTracking()
{
	g_PegasusQaCastBoundary.Reset();
	for (int slot = 0; slot < 9; ++slot)
		g_PegasusCastRootStamps[slot] = rise::pegasus::CastRootStamp();
}

static int PegasusCastRootSlot(WORD skill)
{
	switch (skill)
	{
	case rise::pegasus::kDeathsideSkill: return 0;
	case rise::pegasus::kRainingArrowSkill: return 1;
	case rise::pegasus::kChaosBladeSkill: return 2;
	case rise::pegasus::kHavocSpearSkill: return 3;
	case rise::pegasus::kSwordBlowSkill: return 4;
	case rise::pegasus::kSwordBlowFlySkill: return 8;
	case rise::pegasus::kShiningBirdSkill: return 5;
	case rise::pegasus::kDragonViolentSkill: return 6;
	case rise::pegasus::kSpearstormSkill: return 7;
	default: return -1;
	}
}

static bool ShouldDispatchPegasusCastRoot(WORD skill, int sourceKey)
{
	const int slot = PegasusCastRootSlot(skill);
	if (slot < 0)
		return true;

	const DWORD now = GetTickCount();
	rise::pegasus::CastRootStamp& stamp = g_PegasusCastRootStamps[slot];
	const int qaBoundary = IsPegasusServerQaActive() && Hero && Hero->Key == sourceKey
		? g_PegasusQaCastBoundary.Claim(skill) : 0;
	if (qaBoundary == 2 || (qaBoundary == 0 && stamp.sourceKey == sourceKey &&
		rise::pegasus::IsDuplicateCastRoot(now, stamp.tick, stamp.valid))
		)
	{
		rise::pegasus::TraceRuntime("CAST_ROOT_GATE", skill, sourceKey,
			"DUPLICATE_SKIPPED");
		return false;
	}

	stamp.sourceKey = sourceKey;
	stamp.tick = now;
	stamp.valid = true;
	// /qa15 bypasses mouse input, which normally establishes Hero heading.
	// Only turn for an accepted root, never later multi-target echoes or
	// ordinary gameplay. Server damage remains independently authoritative.
	if (IsPegasusServerQaActive() && Hero && Hero->Key == sourceKey &&
		Hero->TargetCharacter >= 0 && Hero->TargetCharacter < MAX_CHARACTERS_CLIENT)
	{
		CHARACTER& target = CharactersClient[Hero->TargetCharacter];
		if (&target != Hero && target.Object.Live)
		{
			OBJECT& source = Hero->Object;
			source.Angle[2] = CreateAngle(source.Position[0], source.Position[1],
				target.Object.Position[0], target.Object.Position[1]);
			rise::pegasus::TraceRuntime("SERVER_QA_FACING", skill, target.Key,
				"INPUT_PRECONDITION_RESTORED");
		}
	}
	return true;
}
#else
static bool ShouldDispatchPegasusCastRoot(WORD, int)
{
	return true;
}
#endif
...
#if defined(PEGASUS_ISOLATED_TEST)
static bool g_PegasusLocalVisualInjection = false;
#endif

static const char* PegasusCastEchoTraceEvent()
{
#if defined(PEGASUS_ISOLATED_TEST)
	return g_PegasusLocalVisualInjection ? "LOCAL_VISUAL_ECHO" : "CAST_ECHO";
#else
	return "CAST_ECHO";
#endif
}

static const char* PegasusCastDispatchTraceEvent()
{
#if defined(PEGASUS_ISOLATED_TEST)
	return g_PegasusLocalVisualInjection ? "LOCAL_VISUAL_DISPATCH" : "CAST_DISPATCH";
#else
	return "CAST_DISPATCH";
#endif
}
...
BOOL ReceiveMagic(BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	if (
		MagicNumber == AT_SKILL_ATTACK ||
		MagicNumber == MASTER_SKILL_ADD_GREATER_DAMAGE_IMPROVED ||
		MagicNumber == MASTER_SKILL_ADD_GREATER_DAMAGE_ENHANCED ||
		MagicNumber == AT_SKILL_DEFENSE ||
		MagicNumber == MASTER_SKILL_ADD_GREATER_DEFENSE_IMPROVED ||
		MagicNumber == MASTER_SKILL_ADD_GREATER_DEFENSE_ENHANCED
		)
	{
		if (Success == false)
		{
	...
	if (rise::pegasus::FindHighSkillById(MagicNumber) != nullptr ||
		MagicNumber == AT_SKILL_WIND_SOUL)
	{
#if defined(PEGASUS_ISOLATED_TEST)
		ClearPegasusAutoQaStaleBuffs(MagicNumber, so);
		if (sc == Hero)
			ArmPegasusQaDamageTrace(MagicNumber, TargetIndex);
#endif
		rise::pegasus::TraceRuntime(PegasusCastEchoTraceEvent(), MagicNumber, TargetIndex,
			Success != 0 ? "HIT" : "MISS");
#if defined(PEGASUS_ISOLATED_TEST)
		SchedulePegasusQaCapture(MagicNumber);
#endif
	}
	switch (MagicNumber)
	{
	case rise::pegasus::kDeathsideSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kDeathsideReaperModel, "DISPATCHED");
		rise::pegasus::ApplyDeathsideCasterAction(*sc);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			so->CurrentAction);
		sc->AttackTime = 1;
		rise::pegasus::CastDeathside(*sc, MagicNumber);
		break;
	case rise::pegasus::kDarknessSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kDarknessNativeEffect, "DISPATCHED");
		if (Models[so->Type].NumActions > rise::pegasus::kPegasusDarknessAction)
			SetAction(so, rise::pegasus::kPegasusDarknessAction);
		else
			SetAction(so, rise::pegasus::kNativePegasusBuffAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusDarknessAction);
		sc->AttackTime = 1;
		break;
	case rise::pegasus::kRainingArrowSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kRainingArrowRootModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kRainingArrowSound, so);
		if (Models[so->Type].NumActions > rise::pegasus::kPegasusRainingArrowDefaultAction)
			SetAction(so, rise::pegasus::kPegasusRainingArrowDefaultAction);
		else
			SetPlayerAttack(sc);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusRainingArrowDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateRainingArrowRoot(*so, MagicNumber,
			static_cast<short>(Index));
		break;
	case rise::pegasus::kDexBoosterSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kDexBoosterAuraModel, "DISPATCHED");
		SetAction(so, rise::pegasus::kNativePegasusBuffAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusDexBoosterDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateDexBooster(*so, MagicNumber);
		break;
	case rise::pegasus::kSpiralChargeSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kSpiralChargeRootModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kSpiralChargeSound, so);
		SetAction(so, rise::pegasus::SpiralChargeCasterAction(sc->Helper.Type, sc->SafeZone));
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::SpiralChargeCasterAction(sc->Helper.Type, sc->SafeZone));
		sc->AttackTime = 1;
		rise::pegasus::CreateSpiralChargeRoot(*so, MagicNumber);
		break;
	case rise::pegasus::kCrusherChargeSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kCrusherChargeCoreModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kCrusherChargeSound, so);
		SetAction(so, rise::pegasus::kNativePegasusBuffAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusCrusherChargeDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateCrusherChargeRoots(*so, MagicNumber);
		break;
	case rise::pegasus::kElementalChargeSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kElementalChargeModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kElementalChargeSound, so);
		SetAction(so, rise::pegasus::kNativePegasusBuffAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusElementalChargeDefaultAction);
		sc->AttackTime = 1;
		// Pegasus 0x664C95: the same ESI receives SetAction at 0x664CC4
		// and owns the cast aura at 0x664CEC. Buff-recipient sync is separate.
		rise::pegasus::CreateElementalCharge(*so, MagicNumber);
		break;
	case rise::pegasus::kChaosBladeSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kChaosBladeRootModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kChaosBladeSound, so);
		if (Models[so->Type].NumActions > rise::pegasus::kPegasusChaosBladeExtendedAction)
			SetAction(so, rise::pegasus::kPegasusChaosBladeExtendedAction);
		else
			SetAction(so, rise::pegasus::kNativeChaosBladeAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			Models[so->Type].NumActions > rise::pegasus::kPegasusChaosBladeExtendedAction
			? rise::pegasus::kPegasusChaosBladeExtendedAction
			: rise::pegasus::kPegasusChaosBladeDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateChaosBladeRoot(*so, MagicNumber);
		break;
	case rise::pegasus::kHavocSpearSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kHavocRootModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kHavocSpearSound, so);
		if (Models[so->Type].NumActions > rise::pegasus::kPegasusHavocSpearExtendedAction)
			SetAction(so, rise::pegasus::kPegasusHavocSpearExtendedAction);
		else
			SetAction(so, rise::pegasus::kNativeHavocSpearAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			Models[so->Type].NumActions > rise::pegasus::kPegasusHavocSpearExtendedAction
			? rise::pegasus::kPegasusHavocSpearExtendedAction
			: rise::pegasus::kPegasusHavocSpearDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateHavocSpearRoot(*so, MagicNumber);
		break;
	case rise::pegasus::kDragonViolentSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kDragonViolentHeadModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kDragonViolentSound, so);
		SetAction(so, rise::pegasus::kNativeDragonViolentAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusDragonViolentDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateDragonViolentRoots(*so, MagicNumber);
		break;
	case rise::pegasus::kSpearstormSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kSpearstormRootModel, "DISPATCHED");
		PlayBuffer(rise::pegasus::kSpearstormSound, so);
		SetAction(so, rise::pegasus::kNativeSpearstormAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber,
			rise::pegasus::kPegasusSpearstormDefaultAction);
		sc->AttackTime = 1;
		rise::pegasus::CreateSpearstormRoot(*so, MagicNumber);
		break;
	case rise::pegasus::kShiningBirdSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kShiningBirdModel, "DISPATCHED");
		// Pegasus 0x672AB9 calls its common equipment-aware attack selector.
		SetPlayerAttack(sc);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber, -1);
		PlayBuffer(rise::pegasus::kShiningBirdSound, so);
		sc->AttackTime = 1;
		rise::pegasus::CreateShiningBird(*so, MagicNumber);
		break;
	case rise::pegasus::kSwordWrathSkill:
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			rise::pegasus::kSwordWrathRootModel, "DISPATCHED");
		SetAction(so, rise::pegasus::kNativePegasusBuffAction);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber, 67);
		sc->AttackTime = 1;
		{
			vec3_t wrathLight;
			Vector(1.0f, 0.2f, 0.19f, wrathLight);
			CreateEffect(rise::pegasus::kSwordWrathRootModel, so->Position, so->Angle,
				wrathLight, 1, so, -1, 0, MagicNumber);
		}
		break;
	case rise::pegasus::kSwordBlowSkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			MODEL_PEGASUS_SWORD_BLOW_BIG, "DISPATCHED");
		SetAction(so, PLAYER_ATTACK_ONETOONE);
		rise::pegasus::BeginCasterActionTrace(*sc, MagicNumber, 71);
		rise::pegasus::CreateSwordBlowCasterRoot(*sc);
		sc->AttackTime = 1;
		break;
	case rise::pegasus::kSwordBlowFlySkill:
		if (!ShouldDispatchPegasusCastRoot(MagicNumber, SourceKey))
			break;
		rise::pegasus::TraceRuntime("SWORD_BLOW_FLY_DISPATCH", MagicNumber,
			MODEL_PEGASUS_SWORD_BLOW_BIG, "DISPATCHED");
		rise::pegasus::StartSwordBlowFlight(*sc, *to);
		sc->AttackTime = 1;
		break;
	...
	case AT_SKILL_RECOVER:
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			if (iNumBones <= 0) continue; // RISE Fix Crash Login BONE
	...
	case MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED:

		if ((sc->Helper.Type >= MODEL_HELPER + 2 && sc->Helper.Type <= MODEL_HELPER + 4) || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 6 && !sc->SafeZone)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_STRIKE);
		}
		else if (sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 5 && !sc->SafeZone)
		{
	...
	case MASTER_SKILL_ADD_FALLING_SLASH_IMPROVED:
		SetAction(so, PLAYER_ATTACK_SKILL_SWORD1 + AT_SKILL_SWORD1 - AT_SKILL_SWORD1);
		sc->AttackTime = 1;
		PlayBuffer(SOUND_SKILL_SWORD1);
		break;

	case AT_SKILL_SWORD2:
	case MASTER_SKILL_ADD_LUNGE_IMPROVED:
		SetAction(so, PLAYER_ATTACK_SKILL_SWORD1 + AT_SKILL_SWORD2 - AT_SKILL_SWORD1);
		sc->AttackTime = 1;
		PlayBuffer(SOUND_SKILL_SWORD2);
		break;

	case AT_SKILL_SWORD3:
		SetAction(so, PLAYER_ATTACK_SKILL_SWORD1 + AT_SKILL_SWORD3 - AT_SKILL_SWORD1);
		sc->AttackTime = 1;
		PlayBuffer(SOUND_SKILL_SWORD3);
		break;

	case AT_SKILL_SWORD4:
	case MASTER_SKILL_ADD_CYCLONE_IMPROVED1:
	case MASTER_SKILL_ADD_CYCLONE_IMPROVED2:
		SetAction(so, PLAYER_ATTACK_SKILL_SWORD1 + AT_SKILL_SWORD4 - AT_SKILL_SWORD1);
		sc->AttackTime = 1;
		PlayBuffer(SOUND_SKILL_SWORD4);
		break;

	case AT_SKILL_SWORD5://º£±â
	case MASTER_SKILL_ADD_SLASH_IMPROVED://º£±â
		if (sc->SwordCount % 2 == 0)
		{
			SetAction(so, PLAYER_ATTACK_SKILL_SWORD1 + AT_SKILL_SWORD5 - AT_SKILL_SWORD1);
		}
		else
		{
	...
	case AT_SKILL_BRAND_OF_SKILL:
		...
		if (sc->Helper.Type == MODEL_HELPER + 4 || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 6 && !sc->SafeZone)
		{
	...
	case MASTER_SKILL_ADD_GREATER_CRITICAL_DAMAGE_IMPROVED:
		...
		if (MagicNumber == AT_SKILL_ADD_CRITICAL)g_CharacterRegisterBuff(to, eBuff_AddCriticalDamage);
	...
	case MASTER_SKILL_ADD_GREATER_LIFE_MASTERED:
		if (
			...
			|| !g_isCharacterBuff(to, EFFECT_GREATER_LIFE_MASTERED)) //eBuff_Life
		{
	...
	{
		if (sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 5)
		{
			SetAction(so, PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE);
		}
		else if ((sc->Helper.Type >= MODEL_HELPER + 2) && (sc->Helper.Type <= MODEL_HELPER + 4) || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 6)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_STRIKE);
		}
		else
	...
	break;
	case AT_SKILL_WIND_SOUL:
	{
		// The local input path is send-only. The accepted 0x19 echo owns the
		// caster action, sound, and three visual roots exactly once.
		rise::pegasus::TraceRuntime(PegasusCastDispatchTraceEvent(), MagicNumber,
			BITMAP_2LINE_GHOST, "DISPATCHED");
		rise::windsoul::ApplyBase737CasterAction(*sc);
		rise::windsoul::CreateBase737Root(*sc, TargetIndex);
#if defined(PEGASUS_ISOLATED_TEST)
		MarkPegasusAutoQaSequenceComplete();
#endif
	}
	break;
	...
	case MASTER_SKILL_ADD_SWORD_POWER_MASTERED:
	{
		...
		if (MagicNumber == AT_SKILL_ALICE_SLEEP ||
			MagicNumber == MASTER_SKILL_ADD_FITNESS_IMPROVED)
		{
	...
	case AT_SKILL_GAOTIC: //Attack 1
	case MASTER_SKILL_ADD_BIRDS_IMPROVED: //Attack 1
	{
		if (so->Type == MODEL_PLAYER)
		{
			if (sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 5)
	...
	case MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED:
	{
		...
		if (
			MagicNumber == AT_SKILL_HP_UP_OURFORCES ||
			MagicNumber == MASTER_SKILL_ADD_FITNESS_IMPROVED
			)
		{
			...
			|| MagicNumber == MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_IMPROVED
			|| MagicNumber == MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED
			)
	...
	case MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED:
	{
		g_CMonkSystem.SetRageSkillAni(MagicNumber, so);
		so->m_sTargetIndex = TargetIndex;
		sc->AttackTime = 1;
...
static void ReceivePegasusTargets(BYTE* receiveBuffer, int size)
{
	if (!receiveBuffer || size < 10)
		return;
	const int headerSize = (receiveBuffer[0] == 0xC1 || receiveBuffer[0] == 0xC3) ? 4 : 5;
	if (headerSize + static_cast<int>(rise::pegasus::kSwordBlowPayloadFixedSize) > size)
		return;
	const BYTE* payload = receiveBuffer + headerSize;
	rise::pegasus::SwordBlowTargetPayload decoded = {};
	if (!rise::pegasus::DecodeSwordBlowPayload(payload, size - headerSize, decoded) ||
		(decoded.skill != rise::pegasus::kSwordBlowSkill &&
		 decoded.skill != rise::pegasus::kSwordBlowFlySkill &&
		 decoded.skill != rise::pegasus::kDragonViolentTargetSkill &&
		 decoded.skill != rise::pegasus::kHavocSpearTargetSkill &&
		 decoded.skill != rise::pegasus::kRainingArrowTargetSkill &&
		 decoded.skill != rise::pegasus::kChaosBladeTargetSkill))
		return;
	// F3:35 is the multi-target impact list. The caster-side sword root is
	// created by the authoritative 0x19 skill-cast path, so this packet cannot
	// suppress it through caster-key/viewport timing or create a duplicate.
#if defined(PEGASUS_ISOLATED_TEST)
	// Server sends this list after ALL echoes of one accepted cast. Close that
	// cast, so the next complete batch need not wait for a wall-clock timeout.
	// Only exact, nonempty payloads can serve as the end marker; the tolerant
	// legacy impact-list decoder must not turn truncated packets into markers.
	const int rootSlot = PegasusCastRootSlot(decoded.skill);
	if (rootSlot >= 0 && decoded.targetCount > 0 &&
		rise::pegasus::ReadWordLE(payload + 4) == decoded.targetCount &&
		static_cast<std::size_t>(size - headerSize) ==
			rise::pegasus::SwordBlowPayloadSize(decoded.targetCount) &&
		g_PegasusCastRootStamps[rootSlot].Complete(decoded.caster))
		rise::pegasus::TraceRuntime("CAST_ROOT_BOUNDARY", decoded.skill,
			decoded.caster, "TARGET_LIST_COMPLETE");
#endif
	for (std::size_t n = 0; n < decoded.targetCount; ++n)
	{
		const WORD key = decoded.targets[n];
		...
		if (!object->Live || !object->Visible)
			continue;
		if (decoded.skill == rise::pegasus::kSwordBlowSkill ||
			decoded.skill == rise::pegasus::kSwordBlowFlySkill)
		{
			CreateEffect(MODEL_PEGASUS_SWORD_BLOW_TARGET1, object->Position, object->Angle,
				object->Light, 0, object);
			CreateEffect(MODEL_PEGASUS_SWORD_BLOW_TARGET2, object->Position, object->Angle,
				object->Light, 0, object);
		}
		else if (decoded.skill == rise::pegasus::kHavocSpearTargetSkill)
		{
			rise::pegasus::CreateHavocSpearTargetImpact(*object,
				static_cast<int>(n), decoded.skill);
		}
		else if (decoded.skill == rise::pegasus::kRainingArrowTargetSkill)
		{
			rise::pegasus::CreateRainingArrowTarget(*object, decoded.skill);
		}
		else if (decoded.skill == rise::pegasus::kChaosBladeTargetSkill)
		{
			rise::pegasus::CreateChaosBladeTargetImpact(*object, decoded.skill);
		}
		else
		{
			rise::pegasus::CreateDragonViolentTargetImpact(*object, decoded.skill);
		}
	}
}

#if defined(PEGASUS_ISOLATED_TEST)
static CHARACTER* FindPegasusLocalVisualTarget()
{
	if (!Hero)
		return NULL;
	CHARACTER* nearest = NULL;
	float nearestDistanceSquared = 3.4e38f;
	for (int index = 0; index < MAX_CHARACTERS_CLIENT; ++index)
	{
		CHARACTER* candidate = &CharactersClient[index];
		OBJECT* object = &candidate->Object;
		if (candidate == Hero || candidate->Dead != 0 || !object->Live ||
			!object->Visible || object->Kind != KIND_MONSTER)
			continue;
		const float dx = object->Position[0] - Hero->Object.Position[0];
		const float dy = object->Position[1] - Hero->Object.Position[1];
...
static void CreatePegasusLocalVisualTargetLayers(int skill, OBJECT& target)
{
	if (skill == rise::pegasus::kSwordBlowSkill ||
		skill == rise::pegasus::kSwordBlowFlySkill)
	{
		CreateEffect(MODEL_PEGASUS_SWORD_BLOW_TARGET1, target.Position, target.Angle,
			target.Light, 0, &target);
		CreateEffect(MODEL_PEGASUS_SWORD_BLOW_TARGET2, target.Position, target.Angle,
			target.Light, 0, &target);
	}
	else if (skill == rise::pegasus::kHavocSpearTargetSkill)
	{
		rise::pegasus::CreateHavocSpearTargetImpact(target, 0, skill);
	}
	else if (skill == rise::pegasus::kRainingArrowTargetSkill)
	{
		rise::pegasus::CreateRainingArrowTarget(target, skill);
	}
	else if (skill == rise::pegasus::kChaosBladeTargetSkill)
	{
		rise::pegasus::CreateChaosBladeTargetImpact(target, skill);
	}
	else if (skill == rise::pegasus::kDragonViolentTargetSkill)
	{
		rise::pegasus::CreateDragonViolentTargetImpact(target, skill);
	}
}

static void FacePegasusLocalVisualTarget(CHARACTER& target, int skill)
{
	// ReceiveMagic intentionally does not turn Hero: ordinary input already
	// did that. Injected local QA bypasses input and must establish the same
	// facing before creating heading-dependent roots. Never move the actor.
	if (&target == Hero)
		return;
	OBJECT& source = Hero->Object;
	source.Angle[2] = CreateAngle(source.Position[0], source.Position[1],
	...
	_snprintf_s(detail, sizeof(detail), _TRUNCATE,
		"yaw=%.3f source=%.1f,%.1f target=%.1f,%.1f",
		source.Angle[2], source.Position[0], source.Position[1],
		target.Object.Position[0], target.Object.Position[1]);
	rise::pegasus::TraceRuntime("LOCAL_VISUAL_FACING", skill, target.Key, detail);
}

bool RunPegasusLocalVisualQaStep(unsigned step, unsigned castOrdinal)
{
	if (!Hero || !Hero->Object.Live || step >= rise::pegasus::kRuntimeQaSkillCount)
		return false;
	const int skill = rise::pegasus::RuntimeQaSkillId(step);
	const bool selfBuff = rise::pegasus::RuntimeQaSkillIsSelfBuff(step);
	CHARACTER* target = selfBuff ? Hero : FindPegasusLocalVisualTarget();
	if (!target)
	{
		rise::pegasus::TraceRuntime("LOCAL_VISUAL_TARGET", skill,
			static_cast<int>(step), "NO_VISIBLE_MONSTER");
		return false;
	}

	PRECEIVE_MAGIC packet = {};
	packet.MagicH = HIBYTE(static_cast<WORD>(skill));
	packet.MagicL = LOBYTE(static_cast<WORD>(skill));
	packet.SourceKeyH = HIBYTE(static_cast<WORD>(Hero->Key));
	packet.SourceKeyL = LOBYTE(static_cast<WORD>(Hero->Key));
	const WORD targetKey = static_cast<WORD>(target->Key) | 0x8000;
	packet.TargetKeyH = HIBYTE(targetKey);
	packet.TargetKeyL = LOBYTE(targetKey);

	rise::pegasus::TraceRuntime("LOCAL_VISUAL_STEP", skill,
		static_cast<int>(castOrdinal), selfBuff ? "SELF" : "MONSTER");
	FacePegasusLocalVisualTarget(*target, skill);
	g_PegasusLocalVisualInjection = true;
	const BOOL received = ReceiveMagic(reinterpret_cast<BYTE*>(&packet),
		sizeof(packet), FALSE);
	g_PegasusLocalVisualInjection = false;
	if (!received)
		return false;

	CreatePegasusLocalVisualTargetLayers(skill, target->Object);
	int buffEffect = rise::pegasus::NativeEffectForHighSkillBuff(skill);
	if (skill == rise::pegasus::kSwordWrathSkill)
		buffEffect = rise::pegasus::kSwordWrathNativeEffect;
	if (buffEffect >= 0)
	{
		RegisterBuff(static_cast<eBuffState>(buffEffect), &Hero->Object,
			rise::pegasus::HighSkillBuffDurationSeconds(skill));
	}
	return true;
}
...
bool RunSwordBlowFlyLocalPreview(unsigned castOrdinal)
{
	if (!Hero || !Hero->Object.Live)
		return false;
	CHARACTER* target = FindPegasusLocalVisualTarget();
	if (!target)
	{
		rise::pegasus::TraceRuntime("SWORD_BLOW_FLY_PREVIEW",
			rise::pegasus::kSwordBlowFlySkill, 0, "NO_VISIBLE_MONSTER");
		return false;
	}
	PRECEIVE_MAGIC packet = {};
	packet.MagicH = HIBYTE(rise::pegasus::kSwordBlowFlySkill);
	packet.MagicL = LOBYTE(rise::pegasus::kSwordBlowFlySkill);
	packet.SourceKeyH = HIBYTE(static_cast<WORD>(Hero->Key));
	packet.SourceKeyL = LOBYTE(static_cast<WORD>(Hero->Key));
	const WORD targetKey = static_cast<WORD>(target->Key) | 0x8000;
	packet.TargetKeyH = HIBYTE(targetKey);
	packet.TargetKeyL = LOBYTE(targetKey);
	rise::pegasus::TraceRuntime("SWORD_BLOW_FLY_PREVIEW",
		rise::pegasus::kSwordBlowFlySkill, static_cast<int>(castOrdinal), "CAST");
	...
	if (!received)
		return false;
	CreatePegasusLocalVisualTargetLayers(rise::pegasus::kSwordBlowFlySkill,
		target->Object);
	return true;
}
#endif
...
BOOL ReceiveMagicContinue(BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	if (so->Type == MODEL_PLAYER)
	{
		if (sc != Hero)
		{
			...
			case MASTER_SKILL_ADD_ELECTRIC_SPARK_IMPROVED:
				if (sc->Helper.Type == MODEL_HELPER + 37)
			...
			case MASTER_SKILL_ADD_RAGEFUL_BLOW_IMPROVED:
			case MASTER_SKILL_ADD_RAGEFUL_BLOW_ENHANCED:
			...
			case AT_SKILL_PLASMA_STORM_FENRIR:
				...
				break;
			case AT_SKILL_GAOTIC: //Attack 2
			case MASTER_SKILL_ADD_BIRDS_IMPROVED: //Attack 2
			{
				if (sc->Helper.Type == MODEL_HELPER + 37 || gCustomPet.GetInfoPetType(sc->Helper.Type - MODEL_ITEM) == 5)
				{
					SetAction(&sc->Object, PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE);
...
// RISE Case nay khong ton tai o GS
BOOL ReceiveDieExp(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	int     Damage = (Data->ViewDamageHP > 0) ? Data->ViewDamageHP : (((int)(Data->DamageH) << 8) + Data->DamageL);
	int     Success = (Key >> 15);
	Key &= 0x7FFF;
	...
	if (Exp > 0)
	{
		if (gCharacterManager.IsMasterLevel(Hero->Class) == true)
		{
			sprintf(Text, "iID1: %s %s", GlobalText[1750], pKit->QN(Exp));
		}
		else
		{
			sprintf(Text, "mID1: %s %s", GlobalText[486], pKit->QN(Exp));
		}

		g_pChatListBox->AddText("", Text, SEASON3B::TYPE_SYSTEM_MESSAGE);
	}

	g_ConsoleDebug->Write(MCD_RECEIVE, "0x16 [ReceiveDieExp : %d - %s]", Exp, Text);
...
BOOL ReceiveDieExpLarge(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (Exp > 0)
	{
		if (gCharacterManager.IsMasterLevel(Hero->Class) == true && CharacterAttribute->Level >= 400)
		{
			sprintf(Text, GlobalText[1750], pKit->QN(Exp));
			//sprintf(Text, "iID2: Ban nhan duoc %s Master", pKit->QN(Exp));
		}
		else
		{
			sprintf(Text, GlobalText[486], pKit->QN(Exp));
			//sprintf(Text, "mID2: %s Ex thuong", pKit->QN(Exp));
		}
...
void ReceiveCreateItemViewport(const BYTE* ReceiveBuffer)
{
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x2F [ReceiveCreateItemViewport]");
}
...
void ReceiveGetItem(const BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		else
		{
			...
			//g_ConsoleDebug->Write(3, "%d", Items[ItemKey].Item.Level);
			char szMessage[128];
		...
		int Type = ConvertItemType(Data->Item);
		if (Type == ITEM_POTION + 13 || g_pCustomJewel->CheckCustomJewelByItem(Type) || Type == ITEM_POTION + 14 || Type == ITEM_POTION + 16 || Type == ITEM_WING + 15 || Type == ITEM_POTION + 22
...
void ReceiveDropItem(const BYTE* ReceiveBuffer)
{
	...
	if (Data->KeyH == 1)
	{
		if (CharacterMachine->CheckEquipment(Data->KeyL))
		{
...
BOOL ReceiveEquipmentItem(const BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (Data->SubCode != 255)
	{
		...
		if (storageType == STORAGE_TYPE::INVENTORY)
		{
			...
			else if (itemindex >= MAX_EQUIPMENT_INDEX && itemindex < MAX_MY_INVENTORY_INDEX)
			{
				...
				g_pTrade->ProcessTradeItemAutoMoveSuccess();
				g_pMixInventory->ProcessMixItemAutoMoveSuccess();
				g_pMyInventory->InsertItem(itemindex, Data->Item);
			}
			else if (itemindex >= MAX_MY_INVENTORY_INDEX && itemindex < MAX_MY_INVENTORY_EX_INDEX)
			{
	...
	else
	{
		...
		if (g_pTrade->IsItemAutoMove())
		{
			g_pTrade->ProcessMyInvenItemAutoMoveFailure();
		}

		if (g_pMixInventory && g_pMixInventory->IsItemAutoMove())
		{
			g_pMixInventory->ProcessMixItemAutoMoveFailure();
		}
	}
...
void ReceiveModifyItem(const BYTE* ReceiveBuffer)
{
	...
	{
		SEASON3B::CNewUIPickedItem* pPickedItem = SEASON3B::CNewUIInventoryCtrl::GetPickedItem();
		if (pPickedItem && pPickedItem->GetSourceLinealPos() == Data->Index)
		{
			int iNewDur = Data->Item[2];
			if (iNewDur > 0)
			{
				ITEM* pItem = pPickedItem->GetItem();
				if (pItem)
				{
					pItem->Durability = iNewDur;
				}
				return;
			}
			else
...
void ReceiveSell(const BYTE* ReceiveBuffer)
{
	...
	// Every terminal reply releases the pending sale, including rejection.
	g_pNPCShop->SetSellingItem(false);
	// Only explicit success may consume the picked item or replace the balance.
	if (Data->Flag == 1)
	{
		SEASON3B::CNewUIInventoryCtrl::DeletePickedItem();
		CharacterMachine->Gold = Data->Gold;
...
void ReceiveLevelUp(const BYTE* ReceiveBuffer)
{
	...
	CharacterAttribute->NextExperince	= Data->NextExperince;

	pMain->pPrint.Experience			= Data->Experience;
	pMain->pPrint.NextExperince		= Data->NextExperince;
	...
	WORD CurLevel = CharacterAttribute->Level;

	if (iExp > 0 && CurLevel < 400)
	{
		sprintf(szText, GlobalText[486], pKit->QN(iExp));
	...
	g_ConsoleDebug->Write(3, "0xF3, 0x05 LPPRECEIVE_LEVEL_UP size: %d bytes", sizeof(LPPRECEIVE_LEVEL_UP));
}
//
void ReceiveAddPoint(const BYTE* ReceiveBuffer)
{
	...
	if (Data->Result >> 4)
	{
		...
		{
			case 0:			CharacterAttribute->Strength++;			break;
			case 1:			CharacterAttribute->Dexterity++;		break;
			case 2:
			CharacterAttribute->Vitality++;
			CharacterAttribute->LifeMax = Data->ViewMaxHP;
			break;
			case 3:
			CharacterAttribute->Energy++;
			CharacterAttribute->ManaMax = Data->ViewMaxMP;
			break;
			case 4:			CharacterAttribute->Charisma++;			break;
		}

		CharacterAttribute->SkillManaMax	= Data->ViewMaxBP;
		CharacterAttribute->ShieldMax		= Data->ViewMaxSD;
	}
	...
	g_ConsoleDebug->Write(3, "0xF3, 0x06 LPPRECEIVE_ADD_POINT size: %d bytes", sizeof(LPPRECEIVE_ADD_POINT));
...
#else
	case 0xFE:
		CharacterAttribute->LifeMax = Data->ViewHP;
		CharacterAttribute->ShieldMax = Data->ViewSD;
		break;
	case 0xFF:
		CharacterAttribute->Life = ((CharacterAttribute->Life == 0) ? CharacterAttribute->Life : Data->ViewHP);
		CharacterAttribute->Shield = Data->ViewSD;
		break;
#endif
	case 0xFD:
		EnableUse = 0;
...
{
	auto Data = (LPPRECEIVE_MANA)ReceiveBuffer;
	switch (Data->Index)
	{
#if MU_REMOVE_CLASS_CHECK
	case 0xFF:
...
#else
	case 0xFE:
		CharacterAttribute->ManaMax = Data->ViewMP;
		CharacterAttribute->SkillManaMax = Data->ViewBP;
		break;
	case 0xFF:
		CharacterAttribute->Mana = Data->ViewMP;
		CharacterAttribute->SkillMana = Data->ViewBP;
		break;
	default:
		CharacterAttribute->Mana = Data->ViewMP;
#endif
	...
	else
	{
		...
		if (pPickedItem)
		{
			...
			if (pPickItem)
			{
				...
				{
					// Neu la item dang duoc cam => update truc tiep
		...
		if (!pItem)
		{
			pItem = g_pMyInventoryExt->FindItem(iLinealPos);
		}
...
void ReceiveStorageGold(const BYTE* ReceiveBuffer)
{
	...
	{
		g_pChatListBox->AddText("", "Giao dich Zen bi tu choi. Kiem tra so du va gioi han hom do.", SEASON3B::TYPE_ERROR_MESSAGE);
	}
...
void ReceivePartyList(const BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->Count; i++)
	{
		...
		p->maxHP = Data2->maxHP;
		p->cClass = Data2->cClass;
		p->cLevel = Data2->cLevel;
...
void Receive_Master_LevelUp(const BYTE* ReceiveBuffer)
{
	...
	Master_Level_Data.ViewMaxHP = Data->ViewMaxHP;
	Master_Level_Data.ViewMaxMP = Data->ViewMaxMP;
	Master_Level_Data.ViewMaxBP = Data->ViewMaxBP;
	Master_Level_Data.ViewMaxSD = Data->ViewMaxSD;
	Master_Level_Data.lMasterLevel_Experince = Data->ViewMasterExperience;
	Master_Level_Data.lNext_MasterLevel_Experince = Data->ViewMasterNextExperience;

	/* Thong bao: Nhan Exp khi len cap Master

	unicode::t_char szText[256] = { NULL, };
	DWORD iExp = Master_Level_Data.lNext_MasterLevel_Experince - Master_Level_Data.lMasterLevel_Experince;
	if (iExp > 0)
	{
		sprintf(szText, GlobalText[1750], pKit->QN(iExp));
		g_pChatListBox->AddText("", szText, SEASON3B::TYPE_SYSTEM_MESSAGE);
	}
	*/
...
void ReceiveDuelStart(const BYTE* ReceiveBuffer)
{
	...
	g_ConsoleDebug->Write(3, "nResult Start %d", Data->nResult);
...
void ReceiveChangeMapServerInfo(const BYTE* ReceiveBuffer)
{
	...
	g_pReconnect->ReconnectOnMapServerMove(Data->m_vSvrInfo.m_szMapSvrIpAddress, Data->m_vSvrInfo.m_wMapSvrPort);
...
void ReceiveChangeMapServerResult(const BYTE* ReceiveBuffer)
{
	...
	g_pReconnect->ReconnectOnMapServerMoveAuth(Data->Value);
...
{
	auto Data = safe_cast<CREATE_MONEY>(ReceiveBuffer, sizeof(CREATE_MONEY));
	if (Data == nullptr)
	{
		assert(false);
		return;
	...
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x2F [ReceiveCreateMoney]");
}
...
void ReceiveMuHelperConfigurationData(const BYTE* ReceiveBuffer)
{
	...
	if (!lpMsg->result)
	{
		...
		g_pAIControll->LoadConfig(BData);

		MUHelper::ConfigData config;
		MUHelper::ConfigDataDoSave::InsertDataMuHelper(config);

		g_pAIControll->InitAllConfigData(config);

		g_ConsoleDebug->Write(MCD_RECEIVE, "0xAE [LoadConfig] %d", BData[0]);
	}
	else
	{
		g_pAIControll->LoadConfig(NULL);
		MUHelper::ConfigData config;
		MUHelper::ConfigDataDoSave::InsertDataMuHelper(config);
		g_pAIControll->InitAllConfigData(config);
	}
...
void ReceiveMuHelperStatusUpdate(const BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		if (g_pAIControll->AI.bUseDarkRaven && g_pAIControll->AI.iDarkRavenMode != 0)
		{
			SendRequestPetCommand(PET_TYPE_DARK_SPIRIT, g_pAIControll->AI.iDarkRavenMode, 0xFFFF);
			g_ConsoleDebug->Write(3, "PET_TYPE_DARK_SPIRIT %d", g_pAIControll->AI.iDarkRavenMode);
		}
...
bool ReceiveRegistedLuckyCoin(const BYTE* ReceiveBuffer, int size)
{
	const int legacySize = sizeof(PBMSG_HEADER2) + sizeof(int);
	if (size < legacySize) return false;
	int batches[3] = { 1, 10, 30 };
	if (size >= legacySize + (int)(sizeof(DWORD) * 3))
		memcpy(batches, ReceiveBuffer + legacySize, sizeof(batches));
	g_pLuckyCoinRegistration->SetRegisterBatches(batches);
...
bool ReceiveResultEmpireGuardian(const BYTE* ReceiveBuffer)
{
	...
	case 2:
	{
		...
		sprintf(szText, GlobalText[861], pKit->QN(Data->Exp));
		pMsgBox->AddMsg(szText, RGBA(255, 255, 255, 255), SEASON3B::MSGBOX_FONT_NORMAL);
	}break;
	}
...
bool ReceivePeriodItemList(const BYTE* pReceiveBuffer)
{
	...
	if (CharacterMachine->CheckEquipment(Data->wItemSlotIndex))
	{
		CharacterMachine->GetOnlyEquipment(Data->wItemSlotIndex)->lExpireTime = (long)Data->lExpireDate;
	}
	else
	{
...
BOOL ReceiveStraightAttack(BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	if (TargetIndex == MAX_CHARACTERS_CLIENT &&
		AttackNumber != AT_SKILL_DARKSIDE
		&& AttackNumber != MASTER_SKILL_ADD_DARK_SIDE_IMPROVED
		&& AttackNumber != MASTER_SKILL_ADD_DARK_SIDE_ENHANCED
		)
		return (TRUE);
	...
	//g_ConsoleDebug->Write(3, "ReceiveStraightAttack AttackNumber %d TargetIndex %d", AttackNumber, TargetIndex);
	...
	{
		if (sc != Hero &&
			AttackNumber == AT_SKILL_STAMP
			&& AttackNumber == MASTER_SKILL_ADD_UPPER_BEAST_IMPROVED
			&& AttackNumber == MASTER_SKILL_ADD_UPPER_BEAST_ENHANCED
			)
		{
...
void ReceiveDarkside(const BYTE* ReceiveBuffer)
{
	...
	if (
		Data->MagicNumber == AT_SKILL_DARKSIDE
		|| Data->MagicNumber == MASTER_SKILL_ADD_DARK_SIDE_IMPROVED
		|| Data->MagicNumber == MASTER_SKILL_ADD_DARK_SIDE_ENHANCED
		)
	{
		g_CMonkSystem.InitDarksideTarget();
		g_CMonkSystem.SetDarksideTargetIndex(Data->TargerIndex);
		g_ConsoleDebug->Write(MCD_ERROR, " ReceiveDarkside %d ,  %d ,  %d , ", Data->TargerIndex[0], Data->TargerIndex[1], Data->TargerIndex[2]);
	}
}

#if(NEW_MASTER_SKILL_TREE)
void Receive_Master_SkillList(BYTE* ReceiveBuffer)
{

	PMSG_MASTER_SKILL_LIST_SEND* lpMsg = (PMSG_MASTER_SKILL_LIST_SEND*)ReceiveBuffer;
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		g_pMasterSkillTreeInterface->SetMasterSkillTreeInfo(lpInfo->skill, lpInfo->level, lpInfo->MainValue, lpInfo->NextValue);
	}
}
#endif


void RecvPostItem(const BYTE* ReceiveBuffer)
{
	PMSG_POSTITEM* lpMsg = (PMSG_POSTITEM*)ReceiveBuffer;

	ITEM* pNewItem = new ITEM;
	memset(pNewItem, 0, sizeof(ITEM));
	memcpy_s(pNewItem, sizeof(ITEM), lpMsg->item_data, sizeof(lpMsg->item_data));

	g_pChatListBox->AddItemPost(pNewItem);

	g_ConsoleDebug->Write(3, "[RecvPostItem] [%s] %s %d", lpMsg->chatid, lpMsg->chatmsg, g_pChatListBox->GetCurrentRenderEndLine());
	char Name[11] = { '\0' };
	char Messase[61] = { '\0' };
	...
	sprintf_s(Messase_API, "%s", Messase);
	...
	switch (HeadCode)
	{
		case 0xF1:
		{
			...
			switch (Data->SubCode)
			{
				...
				case 0x01: 
				g_pReconnect->ReconnectOnConnectAccount(Data->Value);
				switch (Data->Value)
				{
					case 0x20: CurrentProtocolState = RECEIVE_LOG_IN_SUCCESS; LogIn = 2; CheckHack();		break;
					case 0x00: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_PASSWORD);				break;
					case 0x01: CurrentProtocolState = RECEIVE_LOG_IN_SUCCESS;LogIn = 2;CheckHack();			break;
					case 0x02: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_ID);						break;
					case 0x03: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_ID_CONNECTED);			break;
					case 0x04: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_SERVER_BUSY);				break;
					case 0x05: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_ID_BLOCK);				break;
					case 0x06: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_VERSION);					break;
					case 0x07:
					case 0x08: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_ERROR);					break;
					case 0x09: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_NO_PAYMENT_INFO);			break;
					case 0x0a: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_USER_TIME1);				break;
					case 0x0b: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_USER_TIME2);				break;
					case 0x0c: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_PC_TIME1);				break;
					case 0x0d: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_PC_TIME2);				break;
					case 0x11: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_ONLY_OVER_15);			break;
					case 0x40: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_CHARGED_CHANNEL);			break;
					case 0xc0: 
					case 0xd0: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_POINT_DATE);				break;
					case 0xc1: 
					case 0xd1: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_POINT_HOUR);				break;
					case 0xc2: 
					case 0xd2: CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_INVALID_IP);				break;

					default:   CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_CONNECT);					break;
				}
				break;
				...
				switch (Data->Value)
				{
					case 0x00: CurrentProtocolState = RECEIVE_CREATE_ACCOUNT_FAIL_ID;						break;
					case 0x01: CurrentProtocolState = RECEIVE_CREATE_ACCOUNT_SUCCESS;						break;
					case 0x02: CurrentProtocolState = RECEIVE_CREATE_ACCOUNT_FAIL_RESIDENT;				break;
				}
				break;

				case 0x03: ReceiveConfirmPassword(ReceiveBuffer);			break;
				case 0x04: ReceiveConfirmPassword2(ReceiveBuffer);			break;
				case 0x05: ReceiveChangePassword(ReceiveBuffer);			break;
			}
			break;
		}
			...
			case 0xF3:
		{
			...
			g_ConsoleDebug->Write(MCD_RECEIVE, "Recv [0xF3][0x%02X]", subcode);
				...
				switch (subcode)
				{
				case rise::pegasus::kPegasusTargetPacketSub:
					ReceivePegasusTargets(ReceiveBuffer, Size);
					break;
				case 0x00: 
					gCustomItem.ServerEquipmentOptions.Reset();
					ReceiveCharacterList(ReceiveBuffer);
					g_pIconBuff->DeleteAllBuff();
				break;

				case 0x01: ReceiveCreateCharacter(ReceiveBuffer);				break;
				case 0x02: ReceiveDeleteCharacter(ReceiveBuffer);				break;
				...
				case 0x04: ReceiveRevival(ReceiveBuffer);						break;
				case 0x05: ReceiveLevelUp(ReceiveBuffer);						break;
				case 0x06: ReceiveAddPoint(ReceiveBuffer);						break;
				case 0x07: ReceiveDamage(ReceiveBuffer);						break;
				case 0x08: ReceivePK(ReceiveBuffer);							break;
				...
				case 0x11: ReceiveMagicList(ReceiveBuffer);													break;
				case 0x13: ReceiveEquipment(ReceiveBuffer);													break;
				case 0x14: ReceiveModifyItem(ReceiveBuffer);												break;			
				case 0x20: ReceiveSummonLife(ReceiveBuffer);												break;
				case 0x22: ReceiveWTTimeLeft(ReceiveBuffer);												break;
				case 0x24: ReceiveWTMatchResult(ReceiveBuffer);												break;
				case 0x25: ReceiveWTBattleSoccerGoalIn(ReceiveBuffer);										break;
				case 0x23: ReceiveSoccerScore(ReceiveBuffer);												break;
				case 0x30: ReceiveOption(ReceiveBuffer);													break;
				case 0x40: ReceiveServerCommand(ReceiveBuffer);												break;
				case 0x50: Receive_Master_Level_Exp(ReceiveBuffer);											break;
				case 0x51: Receive_Master_LevelUp(ReceiveBuffer);											break;
				case 0x52: Receive_Master_LevelGetSkill(ReceiveBuffer);										break;
				case 0x53: Receive_Master_SkillList(ReceiveBuffer);											break;
			
				case 0xE0: GCNewCharacterInfoRecv(ReceiveBuffer);											break;
				case 0xE1: GCNewCharacterCalcSend(ReceiveBuffer);											break;

				case 0x15: g_pDameBoss->DamageGetInfo(ReceiveBuffer);										break;
				case 0x16: g_pDameBoss->SendProtocolClient(ReceiveBuffer);									break;
				case 0x26: g_pIconBuff->BuffInfoValueRegister(ReceiveBuffer);								break;

				case 0x42: g_pCastleEvent->CastleEventMini(ReceiveBuffer);									break;
				case 0x43: g_pCastleEvent->CaslteEventData(ReceiveBuffer);									break;

				case 0x6B: g_pNewUIRelife->TaiSinhCheckRecv(ReceiveBuffer);									break;
				case 0x6C: g_pNewUIRelife->TaiSinhDiemRecv(ReceiveBuffer);									break;
				case 0x6D: g_pNewUIRelife->SaveFormGs(ReceiveBuffer);										break;

				case 0x7E: gPreview.PreviewCharListRecv(ReceiveBuffer);										return 1;
				case 0x7F: gPreview.PreviewCharRecv(ReceiveBuffer);											return 1;

				case 0x70: g_pDanhHieu->SendWsClient(ReceiveBuffer);										break;
				case 0x71: g_pHonHoan->SendWsClient(ReceiveBuffer);											break;
				case 0x72: g_pQuanHam->SendWsClient(ReceiveBuffer);											break;
				case 0x73: g_pTuLuyen->SendWsClient(ReceiveBuffer);											break;

				case 0x80: g_ExWinQuestSystem.GC_MiniInfoRecvEx((GC_ExWinMiniInfoEx*)ReceiveBuffer);		return 1;
				case 0x81: g_ExWinQuestSystem.GC_Start((GC_ExWinStart*)ReceiveBuffer);						return 1;
				case 0x82: g_ExWinQuestSystem.GC_RecvMain((GC_MainPacket*)ReceiveBuffer);					return 1;
				case 0x83: g_ExWinQuestSystem.GC_MiniInfoRecv((GC_ExWinMiniInfo*)ReceiveBuffer);			return 1;

				case 0x98: g_pNewUIInvasion->GetInfoReceived(ReceiveBuffer);								break;
				case 0x99: g_pNewUIInvasion->GetUpdateMonster(ReceiveBuffer);								break;

				case 0xE2: pUserRank->ReceivedLife(ReceiveBuffer);											return 1;
				case 0xE5: pUserRank->GCReqRankLevelUser(ReceiveBuffer);									return 1;
				case 0xE9: gItemPrice.GCItemValueRecv((PMSG_ITEM_VALUE_RECV*)ReceiveBuffer);				return 1;
				case 0xE7: g_pNewUIMiniMap->ReceiveMapMarkers(ReceiveBuffer, Size); return 1;
				case 0xE6: gCustomItem.ServerEquipmentOptions.Receive(ReceiveBuffer, Size); return 1;
				case 0xE8: g_pEventTime->GCReqEventTime(ReceiveBuffer);										return 1;
				case 0xEA: pMain->GCThisCoin(ReceiveBuffer);												return 1;

				case 0xEB: pMain->RecvPShop(ReceiveBuffer);													return 1;
				case 0xEC: pMain->PShopActiveRecv(ReceiveBuffer);											return 1;
				case 0xED: pMain->SpeedAttack(ReceiveBuffer);												return 1;
				case 0xF1: GCPetCharSetRecv((PMSG_NEW_PET_CHARSET_RECV*)ReceiveBuffer);						break;
				case 0xF2: GCPetChartSelectRecv((PMSG_NEW_SELECT_CHARACTER_CHARSET_RECV*)ReceiveBuffer);	break;
				case 0xF3: g_StatsInfo.CharacterAttributeInfo(ReceiveBuffer);								return 1;
				case 0xF5: g_pBank->GCCustomBankInfoRecv(ReceiveBuffer);									break;
				case 0xFA: g_pIDLevel->GCRecvBuyVipCoin(ReceiveBuffer);							return 1;
		...
		case 0xF4:
		{
			...
			switch (subcode)
			{
				...
				case 0xE2:	CrashReportReceiveAck(ReceiveBuffer, Size);									break;
			}
			break;
		}

		case 0x00: ReceiveChat(ReceiveBuffer);																break;
		case 0x01: ReceiveChatKey(ReceiveBuffer);															break;
		case 0x02: ReceiveChatWhisper(ReceiveBuffer);														break;
		case 0x03: ReceiveCheckSumRequest(ReceiveBuffer);													break;
		case 0x0B: AddDebugText(ReceiveBuffer, Size); ReceiveEvent(ReceiveBuffer);							break;
		case 0x0C: ReceiveChatWhisperResult(ReceiveBuffer);													break;

		case 0x0D: 
			ReceiveNotice(ReceiveBuffer);	
			gPKSys.Handle(ReceiveBuffer);
			break;	

		case 0x0F: ReceiveWeather(ReceiveBuffer);															break;
		case 0xD4: ReceiveMoveCharacter(ReceiveBuffer);														break;
		case 0x15: ReceiveMovePosition(ReceiveBuffer);														break;
		case 0x12: AddDebugText(ReceiveBuffer, Size);ReceiveCreatePlayerViewport(ReceiveBuffer, Size);		break;
		case 0x13: ReceiveCreateMonsterViewport(ReceiveBuffer);												break;
		case 0x1F: ReceiveCreateSummonViewport(ReceiveBuffer);												break;
		case 0x45: ReceiveCreateTransformViewport(ReceiveBuffer);											break;
		case 0x14: ReceiveDeleteCharacterViewport(ReceiveBuffer);											break;
		case 0x20: ReceiveCreateItemViewport(ReceiveBuffer);												break;
		case 0x2F: ReceiveCreateMoney(ReceiveBuffer);														break;
		case 0x21: ReceiveDeleteItemViewport(ReceiveBuffer);												break;
		case 0x22: ReceiveGetItem(ReceiveBuffer);															break;
		case 0x23: ReceiveDropItem(ReceiveBuffer);															break;
		...
		case 0x25: ReceiveChangePlayer(ReceiveBuffer);														break;
		case 0x11: ReceiveAttackDamage(ReceiveBuffer);														break;
		case 0x18: ReceiveAction(ReceiveBuffer, Size);														break;
		...
		break;
		case 0x1A: ReceiveMagicPosition(ReceiveBuffer, Size);									break;
		...
		case 0x1B: ReceiveMagicFinish(ReceiveBuffer);											break;
		case 0x07: ReceiveSkillStatus(ReceiveBuffer);											break;
		...
		case 0x17: ReceiveDie(ReceiveBuffer, Size);												break;
		case 0x2A: AddDebugText(ReceiveBuffer, Size); ReceiveDurability(ReceiveBuffer);			break;
		case 0x26: ReceiveLife(ReceiveBuffer);													break;
		case 0x27: ReceiveMana(ReceiveBuffer);													break;
		case 0x28: ReceiveDeleteInventory(ReceiveBuffer);										break;
		...
		case 0x2c: ReceiveUseStateItem(ReceiveBuffer);											break;
		...
		case 0x31: ReceiveTradeInventory(ReceiveBuffer);										break;
		case 0x32: ReceiveBuy(ReceiveBuffer);													break;
		case 0x33: ReceiveSell(ReceiveBuffer);													break;
		case 0x34: ReceiveRepair(ReceiveBuffer);												break;
		...
		case 0x37: ReceiveTradeResult(ReceiveBuffer);											break;
		case 0x38: ReceiveTradeYourInventoryDelete(ReceiveBuffer);								break;
		case 0x39: ReceiveTradeYourInventory(ReceiveBuffer);									break;
		case 0x3A: ReceiveTradeMyGold(ReceiveBuffer);											break;
		case 0x3B: ReceiveTradeYourGold(ReceiveBuffer);											break;
		case 0x3C: ReceiveTradeYourResult(ReceiveBuffer);										break;
		case 0x3D: ReceiveTradeExit(ReceiveBuffer);												break;
		...
		case 0x40: ReceiveParty(ReceiveBuffer);													break;
		case 0x41: ReceivePartyResult(ReceiveBuffer);											break;
		case 0x42: ReceivePartyList(ReceiveBuffer);												break;
		case 0x43: ReceivePartyLeave(ReceiveBuffer);											break;
		case 0x44: ReceivePartyInfo(ReceiveBuffer);												break;
		case 0x46: ReceiveSetAttribute(ReceiveBuffer);											break;
		case 0x47: ReceivePartyGetItem(ReceiveBuffer);											break;
		case 0x48: ReceiveDisplayEffectViewport(ReceiveBuffer);									break;
		case 0x50: ReceiveGuild(ReceiveBuffer);													break;
		case 0x51: ReceiveGuildResult(ReceiveBuffer);											break;
		case 0x52: ReceiveGuildList(ReceiveBuffer);												break;
		case 0x53: ReceiveGuildLeave(ReceiveBuffer);											break;
		case 0x54: ReceiveCreateGuildInterface(ReceiveBuffer);									break;
		case 0x55: ReceiveCreateGuildMasterInterface(ReceiveBuffer);							break;
		case 0x56: ReceiveCreateGuildResult(ReceiveBuffer);										break;
		case 0x65: ReceiveGuildIDViewport(ReceiveBuffer);										break;
		case 0x66: ReceiveGuildInfo(ReceiveBuffer);												break;
		case 0x5D: ReceiveDeleteGuildViewport(ReceiveBuffer);									break;
		case 0x60: ReceiveDeclareWarResult(ReceiveBuffer);										break;
		case 0x61: ReceiveDeclareWar(ReceiveBuffer);											break;
		case 0x62: ReceiveGuildBeginWar(ReceiveBuffer);											break;
		case 0x63: ReceiveGuildEndWar(ReceiveBuffer);											break;
		case 0x64: ReceiveGuildWarScore(ReceiveBuffer);											break;
		case 0xE1: ReceiveGuildAssign(ReceiveBuffer);											break;
		case 0xE5: ReceiveGuildRelationShip(ReceiveBuffer);										break;
		case 0xE6: ReceiveGuildRelationShipResult(ReceiveBuffer);								break;
		...
		case 0xEB:
		{
			...
			switch (Data->SubCode)
			{
				case 0x01: ReceiveBanUnionGuildResult(ReceiveBuffer);							break;
			}
		}
		break;
		case 0x67: ReceiveUnionViewportNotify(ReceiveBuffer);									break;
		case 0xE9: ReceiveUnionList(ReceiveBuffer);												break;
		...
		case 0xBC:
		{
			...
			switch (Data->SubCode)
			{
				case 0x00: ReceiveGemMixResult(ReceiveBuffer);									break;
				case 0x01: ReceiveGemUnMixResult(ReceiveBuffer);								break;
			}
		}
		break;

		case 0x68: ReceivePreviewPort(ReceiveBuffer);											break;
		case 0x71: ReceivePing(ReceiveBuffer);													break;
		case 0x81: ReceiveStorageGold(ReceiveBuffer);											break;
		case 0x82: ReceiveStorageExit(ReceiveBuffer);											break;
		case 0x83: ReceiveStorageStatus(ReceiveBuffer);											break;
		case 0x86: ReceiveMix(ReceiveBuffer);													break;
		case 0x87: ReceiveMixExit(ReceiveBuffer);												break;
		...
		case 0x8E:
		{
			...
			switch (Data->SubCode)
			{
				case 0x01: ReceiveMoveMapChecksum(ReceiveBuffer);								break;
				case 0x03: ReceiveRequestMoveMap(ReceiveBuffer);								break;
			}
		}
		break;

		case 0x90: ReceiveMoveToDevilSquareResult(ReceiveBuffer);								break;
		case 0x91: ReceiveEventZoneOpenTime(ReceiveBuffer);										break;
		case 0x92: ReceiveDevilSquareCountDown(ReceiveBuffer);									break;
		case 0x93: ReceiveDevilSquareRank(ReceiveBuffer);										break;
		case 0x9A: ReceiveMoveToEventMatchResult(ReceiveBuffer);								break;
		case 0x9B: ReceiveMatchGameCommand(ReceiveBuffer);										break;
		case 0x94: ReceiveEventChipInfomation(ReceiveBuffer);									break;
		case 0x95: ReceiveEventChip(ReceiveBuffer);												break;
		case 0x96: ReceiveMutoNumber(ReceiveBuffer);											break;
		case 0x99: ReceiveServerImmigration(ReceiveBuffer);										break;
		case 0x9D: ReceiveScratchResult(ReceiveBuffer);											break;
		case 0x9E: ReceivePlaySoundEffect(ReceiveBuffer);										break;
		case 0x9F: ReceiveEventCount(ReceiveBuffer);											break;
		case 0xA0: ReceiveQuestHistory(ReceiveBuffer);											break;
		case 0xA1: ReceiveQuestState(ReceiveBuffer);											break;
		case 0xA2: ReceiveQuestResult(ReceiveBuffer);											break;
		case 0xA3: ReceiveQuestPrize(ReceiveBuffer);											break;
		case 0xA4: ReceiveQuestMonKillInfo(ReceiveBuffer);										break;
		...
		case 0xF6:
		{
			...
			{
				#ifdef ASG_ADD_TIME_LIMIT_QUEST
				case 0x00: ReceiveQuestLimitResult(ReceiveBuffer);								break;
				#endif
				case 0x03: ReceiveQuestByEtcEPList(ReceiveBuffer);								break;
				#ifdef ASG_ADD_TIME_LIMIT_QUEST
				case 0x04: ReceiveQuestByItemUseEP(ReceiveBuffer);								break;
				#endif
				case 0x0A:ReceiveQuestByNPCEPList(ReceiveBuffer);								break;
				case 0x0B:ReceiveQuestQSSelSentence(ReceiveBuffer);								break;
				case 0x0C:ReceiveQuestQSRequestReward(ReceiveBuffer);							break;
				case 0x0D:ReceiveQuestCompleteResult(ReceiveBuffer);							break;
				case 0x0F:ReceiveQuestGiveUp(ReceiveBuffer);									break;
				case 0x1A:ReceiveProgressQuestList(ReceiveBuffer);								break;
				case 0x1B:ReceiveProgressQuestRequestReward(ReceiveBuffer);						break;
				case 0x20:ReceiveProgressQuestListReady(ReceiveBuffer);							break;
			}
		}
		break;
		...
		case 0xF8:
		{
			...
			switch (bySubcode)
			{
			case 0x02: ReceiveGensJoining(ReceiveBuffer);									break;
			case 0x04: ReceiveGensSecession(ReceiveBuffer);									break;
			case 0x07: ReceivePlayerGensInfluence(ReceiveBuffer);							break;
			case 0x05: ReceiveOtherPlayerGensInfluenceViewport(ReceiveBuffer);				break;
			#ifdef PBG_ADD_GENSRANKING
			case 0x0A: ReceiveReward(ReceiveBuffer);										break;
			#endif
			}
		}
		break;
		...
		case 0xF9:
		{
			...
			switch (Data->SubCode)
			{
			case 0x01: ReceiveNPCDlgUIStart(ReceiveBuffer);			break;
			}
		}
		break;

		case 0xA7: ReceivePetCommand(ReceiveBuffer);										break;
		case 0xA8: ReceivePetAttack(ReceiveBuffer);											break;
		case 0xA9: ReceivePetInfo(ReceiveBuffer);											break;
		...
		case 0xAA:
		{
			...
			switch (Data->SubCode)
			{
				case 0x01: ReceiveDuelStart(ReceiveBuffer);									break;
				case 0x02: ReceiveDuelRequest(ReceiveBuffer);								break;
				case 0x03: ReceiveDuelEnd(ReceiveBuffer);									break;
				case 0x04: ReceiveDuelScore(ReceiveBuffer);									break;
				case 0x05: ReceiveDuelHP(ReceiveBuffer);									break;
				case 0x06: ReceiveDuelChannelList(ReceiveBuffer);							break;
				case 0x07: ReceiveDuelWatchRequestReply(ReceiveBuffer);						break;
				case 0x08: ReceiveDuelWatcherJoin(ReceiveBuffer);							break;
				case 0x09: ReceiveDuelWatchEnd(ReceiveBuffer);								break;
				case 0x0A: ReceiveDuelWatcherQuit(ReceiveBuffer);							break;
				case 0x0B: ReceiveDuelWatcherList(ReceiveBuffer);							break;
				case 0x0C: ReceiveDuelResult(ReceiveBuffer);								break;
				case 0x0D: ReceiveDuelRound(ReceiveBuffer);									break;
			}
		}
		break;
		...
		case 0xF7:
		{
			...
			switch (Data->SubCode)
			{
				case 0x02:			ReceiveEnterEmpireGuardianEvent(ReceiveBuffer);			break;
				case 0x04:			ReceiveRemainTickEmpireGuardian(ReceiveBuffer);			break;
				case 0x06:			ReceiveResultEmpireGuardian(ReceiveBuffer);				break;
			}
		}
		break;
		...
		{
			case 0x61: g_ThanMa->SendProtocol61();				break;
			case 0x62: g_ThanMa->SendProtocol62(ReceiveBuffer); break;
			case 0x63: g_ThanMa->SendProtocol63(ReceiveBuffer); break;
			case 0x64: g_ThanMa->SendProtocol64(ReceiveBuffer); break;
		}
		break;
		...
		switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
		{
			case 0x08: if (g_BossGuild) g_BossGuild->RecvProtocol(ReceiveBuffer); break;
			case 0x09: if (g_BossGuild) g_BossGuild->BossGuildRecv(ReceiveBuffer); break;
			case 0x0A: if (g_BossGuild) g_BossGuild->BossGuildTimeRecv(ReceiveBuffer); break;
		}
		break;
		...
		case 0x3F:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveCreateShopTitleViewport(ReceiveBuffer);			break;
				case 0x01: ReceiveSetPriceResult(ReceiveBuffer);					break;
				case 0x02: ReceiveCreatePersonalShop(ReceiveBuffer);				break;
				case 0x03: ReceiveDestroyPersonalShop(ReceiveBuffer);				break;
				case 0x05: ReceivePersonalShopItemList(ReceiveBuffer);				break;
				case 0x06: ReceivePurchaseItem(ReceiveBuffer);						break;
				case 0x08: NotifySoldItem(ReceiveBuffer);							break;
				case 0x10: ReceiveShopTitleChange(ReceiveBuffer);					break;
				case 0x12: NotifyClosePersonalShop(ReceiveBuffer);					break;
				case 0x13: ReceiveRefreshItemList(ReceiveBuffer);					break;
			}
		}
		break;
		...
		case 0xAF:
		{
			...
			switch (Data->SubCode)
			{
			case 0x01: ReceiveMoveToEventMatchResult2(ReceiveBuffer);			break;
			}
		}
		break;
		...
		case 0xB2:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveBCStatus(ReceiveBuffer);										break;
				case 0x01: ReceiveBCReg(ReceiveBuffer);											break;
				case 0x02: ReceiveBCGiveUp(ReceiveBuffer);										break;
				case 0x03: ReceiveBCRegInfo(ReceiveBuffer);										break;
				case 0x04: ReceiveBCRegMark(ReceiveBuffer);										break;
				case 0x05: ReceiveBCNPCBuy(ReceiveBuffer);										break;
				case 0x06: ReceiveBCNPCRepair(ReceiveBuffer);									break;
				case 0x07: ReceiveBCNPCUpgrade(ReceiveBuffer);									break;
				case 0x08: ReceiveBCGetTaxInfo(ReceiveBuffer);									break;
				case 0x09: ReceiveBCChangeTaxRate(ReceiveBuffer);								break;
				case 0x10: ReceiveBCWithdraw(ReceiveBuffer);									break;
				case 0x1A: ReceiveTaxInfo(ReceiveBuffer);										break;
				case 0x1F: ReceiveHuntZoneEnter(ReceiveBuffer);									break;
				case 0x11: ReceiveGateState(ReceiveBuffer);										break;
				case 0x12: ReceiveGateOperator(ReceiveBuffer);									break;
				case 0x13: ReceiveGateCurrentState(ReceiveBuffer);								break;
				case 0x14: ReceiveCrownSwitchState(ReceiveBuffer);								break;
				case 0x15: ReceiveCrownRegist(ReceiveBuffer);									break;
				case 0x16: ReceiveCrownState(ReceiveBuffer);									break;
				case 0x17: ReceiveBattleCastleStart(ReceiveBuffer);								break;
				case 0x18: ReceiveBattleCastleProcess(ReceiveBuffer);							break;
				case 0x19: ReceiveBattleCastleRegiment(ReceiveBuffer);							break;
				case 0x20: ReceiveBattleCasleSwitchInfo(ReceiveBuffer);							break;
				case 0x1B: ReceiveMapInfoResult(ReceiveBuffer);									break;
				case 0x1D: ReceiveGuildCommand(ReceiveBuffer);									break;
				case 0x1E: ReceiveMatchTimer(ReceiveBuffer);									break;
			}
		}
		break;

		case 0xB3: ReceiveBCNPCList(ReceiveBuffer);												break;
		case 0xB4: ReceiveBCDeclareGuildList(ReceiveBuffer);									break;
		case 0xB5: ReceiveBCGuildList(ReceiveBuffer);											break;
		case 0xB6: ReceiveGuildMemberLocation(ReceiveBuffer);									break;
		case 0xBB: ReceiveGuildNpcLocation(ReceiveBuffer);										break;
		...
		case 0xB7:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveCatapultState(ReceiveBuffer);									break;
				case 0x01: ReceiveCatapultFire(ReceiveBuffer);									break;
				case 0x02: ReceiveCatapultFireToMe(ReceiveBuffer);								break;
				case 0x03: /*ReceiveDamageFrmeCatapult(ReceiveBuffer);*/						break;
			}
		}
		break;
		...
		case 0xB8:
		{
			...
			switch (subcode)
			{
				case 0x01:ReceiveKillCount(ReceiveBuffer);										break;
				case 0x02:break;
			}
		}
		break;
		...
		case 0xB9:
		{
			...
			switch (subcode)
			{
				case 0x01: ReceiveBuildTime(ReceiveBuffer);										break;
				case 0x02: ReceiveCastleGuildMark(ReceiveBuffer);								break;
				case 0x03: ReceiveCastleHuntZoneInfo(ReceiveBuffer);							break;
				case 0x05: ReceiveCastleHuntZoneResult(ReceiveBuffer);							break;
			}
		}
		break;

		case 0xBA: ReceiveSkillCount(ReceiveBuffer);		break;
		...
		case 0xBD:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveCrywolfInfo(ReceiveBuffer);									break;
				case 0x02: ReceiveCrywolStateAltarfInfo(ReceiveBuffer);							break;
				case 0x03: ReceiveCrywolfAltarContract(ReceiveBuffer);							break;
				case 0x04: ReceiveCrywolfLifeTime(ReceiveBuffer);								break;
				case 0x0C: ReceiveCrywolfTankerHit(ReceiveBuffer);								break;
				case 0x05: ReceiveCrywolfBossMonsterInfo(ReceiveBuffer);						break;
				case 0x07: ReceiveCrywolfPersonalRank(ReceiveBuffer);							break;
				case 0x09: ReceiveCrywolfBenefitPlusChaosRate(ReceiveBuffer);					break;
				case 0x08: ReceiveCrywolfHeroList(ReceiveBuffer);								break;
			}
			break;
		}

		case 0xC0: ReceiveFriendList(ReceiveBuffer);										break;
		case 0xC1: ReceiveAddFriendResult(ReceiveBuffer);									break;
		case 0xC2: ReceiveRequestAcceptAddFriend(ReceiveBuffer);							break;
		case 0xC3: ReceiveDeleteFriendResult(ReceiveBuffer);								break;
		case 0xC4: ReceiveFriendStateChange(ReceiveBuffer);									break;
		case 0xC5: ReceiveLetterSendResult(ReceiveBuffer);									break;
		case 0xC6: ReceiveLetter(ReceiveBuffer);											break;
		case 0xC7: ReceiveLetterText(ReceiveBuffer);										break;
		case 0xC8: ReceiveLetterDeleteResult(ReceiveBuffer);								break;
		case 0xCA: ReceiveCreateChatRoomResult(ReceiveBuffer);								break;
		case 0xCB: ReceiveChatRoomInviteResult(ReceiveBuffer);								break;

		case 0x2D: 
			ReceiveBuffState(ReceiveBuffer); 
			g_pIconBuff->Recv((PMSG_SEND_BUFF_ICON*)ReceiveBuffer);	
		break;
		...
		case 0xD1:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveKanturu3rdStateInfo(ReceiveBuffer);						break;
				case 0x01: ReceiveKanturu3rdEnterBossMap(ReceiveBuffer);					break;
				case 0x02: ReceiveKanturu3rdCurrentState(ReceiveBuffer);					break;
				case 0x03: ReceiveKanturu3rdState(ReceiveBuffer);							break;
				case 0x04: ReceiveKanturu3rdResult(ReceiveBuffer);							break;
				case 0x05: ReceiveKanturu3rdTimer(ReceiveBuffer);							break;
				case 0x06: RecevieKanturu3rdMayaSKill(ReceiveBuffer);						break;
				case 0x07: RecevieKanturu3rdLeftUserandMonsterCount(ReceiveBuffer);			break;
				case 0x10: ReceiveRaklionStateInfo(ReceiveBuffer);							break;
				case 0x11: ReceiveRaklionCurrentState(ReceiveBuffer);						break;
				case 0x12: RecevieRaklionStateChange(ReceiveBuffer);						break;
				case 0x13: RecevieRaklionBattleResult(ReceiveBuffer);						break;
				case 0x14: RecevieRaklionWideAreaAttack(ReceiveBuffer);						break;
				case 0x15: RecevieRaklionUserMonsterCount(ReceiveBuffer);					break;
			}
			break;
		}
		...
		case 0xBF:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveCursedTempleEnterResult(ReceiveBuffer);			break;
				case 0x01: ReceiveCursedTempleInfo(ReceiveBuffer);					break;
				case 0x02: ReceiveCursedTempMagicResult(ReceiveBuffer);				break;
				case 0x03: ReceiveCursedTempleEnterInfo(ReceiveBuffer);				break;
				case 0x04: ReceiveCursedTempleGameResult(ReceiveBuffer);			break;
				case 0x06: ReceiveCursedTempSkillPoint(ReceiveBuffer);				break;
				case 0x07: ReceiveCursedTempSkillEnd(ReceiveBuffer);				break;
				case 0x08: ReceiveCursedTempleHolyItemRelics(ReceiveBuffer);		break;
				case 0x09: ReceiveCursedTempleState(ReceiveBuffer);					break;
				case 0x0a: ReceiveChainMagic(ReceiveBuffer);						break;
				case 0x0B: ReceiveRegistedLuckyCoin(ReceiveBuffer, Size);					break;
				case 0x0C: ReceiveRegistLuckyCoin(ReceiveBuffer);					break;
				case 0x0D: ReceiveRequestExChangeLuckyCoin(ReceiveBuffer);			break;
				case 0x0E: ReceiveEnterDoppelGangerEvent(ReceiveBuffer);			break;
				case 0x0F: ReceiveDoppelGangerMonsterPosition(ReceiveBuffer);		break;
				case 0x10: ReceiveDoppelGangerState(ReceiveBuffer);					break;
				case 0x11: ReceiveDoppelGangerIcewalkerState(ReceiveBuffer);		break;
				case 0x12: ReceiveDoppelGangerTimePartyState(ReceiveBuffer);		break;
				case 0x13: ReceiveDoppelGangerResult(ReceiveBuffer);				break;
				case 0x14: ReceiveDoppelGangerMonsterGoal(ReceiveBuffer);			break;

				#ifdef PBG_ADD_SECRETBUFF
				case 0x15: ReceiveFatigueTime(ReceiveBuffer);						break;
				#endif
				#ifdef LJH_ADD_SYSTEM_OF_EQUIPPING_ITEM_FROM_INVENTORY
				case 0x20: ReceiveEquippingInventoryItem(ReceiveBuffer);			break;
				#endif
		...
		case 0xDE:
		{
			...
			switch (subcode)
			{
				case 0x00: ReceiveCharacterCard_New(ReceiveBuffer);			break;
			}
		}
		break;
		...
		case 0xD2:
		{
			...
			{
				case 0x01: ReceiveIGS_CashPoint(ReceiveBuffer);					break;
				case 0x02: ReceiveIGS_ShopOpenResult(ReceiveBuffer);			break;
				case 0x03: ReceiveIGS_BuyItem(ReceiveBuffer);					break;
				case 0x04: ReceiveIGS_SendItemGift(ReceiveBuffer);				break;
				case 0x06: ReceiveIGS_StorageItemListCount(ReceiveBuffer);		break;
				case 0x07: ReceiveIGS_SendCashGift(ReceiveBuffer);				break;
				case 0x08: ReceiveIGS_PossibleBuy(ReceiveBuffer);				break;
				case 0x09: ReceiveIGS_LeftCountItem(ReceiveBuffer);				break;
				case 0x0B: ReceiveIGS_UseStorageItem(ReceiveBuffer);			break;
				case 0x0C: ReceiveIGS_UpdateScript(ReceiveBuffer);				break;
				case 0x0D: ReceiveIGS_StorageItemList(ReceiveBuffer);			break;
				case 0x0E: ReceiveIGS_StorageGiftItemList(ReceiveBuffer);		break;
				case 0x13: ReceiveIGS_EventItemlistCnt(ReceiveBuffer);			break;
				case 0x14: ReceiveIGS_EventItemlist(ReceiveBuffer);				break;
				case 0x15: ReceiveIGS_UpdateBanner(ReceiveBuffer);				break;
				case 0x11: ReceivePeriodItemListCount(ReceiveBuffer);			break;
				case 0x12: ReceivePeriodItemList(ReceiveBuffer);				break;
			}
		}
		break;

		case 0x4A:	ReceiveStraightAttack(ReceiveBuffer, Size, bEncrypted);		break;
		case 0x4B:	ReceiveDarkside(ReceiveBuffer);								break;
		case 0xAE:	ReceiveMuHelperConfigurationData(ReceiveBuffer);			break;
		case 0x78:	RecvPostItem(ReceiveBuffer);								break;

		case 0x88:
		{
			if (gUiChaosMixRate)
			{
				gUiChaosMixRate->GCRecvMixInfo(ReceiveBuffer, Size);
			}
			break;
		}
		...
		case 0xD3:
		{
			switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
			{
				case 0x01: g_pViewItem->RecvProtocol(ReceiveBuffer);						break;
				case 0x02: g_pChangeClass->GCSendToClient(ReceiveBuffer);					break;
				case 0x03: g_pExtOptionChar->SaveAndSetInfo(ReceiveBuffer);					break;
				case 0x05: g_pExtOptionChar->LoadSetInfo(ReceiveBuffer);					break;
				case 0x07: g_pDungLuyen->Protocol2A(ReceiveBuffer);							break;
				case 0x08: g_pDungLuyen->Protocol2B(ReceiveBuffer);							break;
				case 0x09: g_pDungLuyen->ListMainItem(ReceiveBuffer);						break;
				case 0x0A: g_pDungLuyen->ListItem(ReceiveBuffer);							break;

				case 0x0C: g_pHarmony->ListItem(ReceiveBuffer);								break;
				case 0x0D: g_pHarmony->Protocol3A(ReceiveBuffer);							break;
				case 0x0E: g_pHarmony->Protocol3B(ReceiveBuffer);							break;

				case 0x12: g_pItemTrader->ItemTradeProtocol6A(ReceiveBuffer);				break;
				case 0x13: g_pItemTrader->ItemTradeProtocol6B(ReceiveBuffer);				break;
				case 0x35: g_pItemTrader->ListMainItem(ReceiveBuffer);						break;
				case 0x16: g_pMocNap->WsClientSend9A(ReceiveBuffer);						break;
				case 0x17: g_pMocNap->WsClientSend9B(ReceiveBuffer);						break;
				case 0x1A:
				{
					if (g_pMyInventory)
						g_pMyInventory->PassLock(ReceiveBuffer);
				}
				break;

				case 0x1C: g_pLoanChien->ProtocolCase34(ReceiveBuffer);						break;
				case 0x1D: g_pLoanChien->ProtocolCase36(ReceiveBuffer);						break;

				case 0x1F: g_pNewSpin->SendProtocol8A(ReceiveBuffer);						break;
				case 0x20: g_pNewSpin->SendProtocol8B(ReceiveBuffer);						break;
				case 0x21: g_pNewSpin->SendProtocol8C(ReceiveBuffer);						break;
				case 0x22: g_pResetChange->GCSendToClient(ReceiveBuffer);					break;

				case 0x23:
				{
					if (g_pMixTrader) g_pMixTrader->RecvProtocol(ReceiveBuffer);			return 1;
				}
				break;
				
				case 0x40: gCustomReadGuild.GCReqInfoGuild((ReadFromLeach*)ReceiveBuffer);	return 1;
				case 0x3E: pKit->ReceiveAutoHpState(ReceiveBuffer);							break;

				case 0x80:	g_pRanking->RankingDataRequest(ReceiveBuffer, ALL);				break;
				case 0x81:	g_pRanking->RankingDataRequest(ReceiveBuffer, TP);				break;
				case 0x82:	g_pRanking->RankingDataRequest(ReceiveBuffer, TG); 				break;
				case 0x83:	g_pRanking->RankingDataRequest(ReceiveBuffer, PK);				break;
				case 0x84:	g_pRanking->RankingDataRequest(ReceiveBuffer, ET);				break;
				case 0x85:	g_pRanking->RankingDataRequest(ReceiveBuffer, MT);				break;
				case 0x86:	g_pRanking->RankingDataRequest(ReceiveBuffer, WQ);				break;

				case 0x90: g_NangCapGuild->ParseSelfClanData(ReceiveBuffer);				return true;
				case 0x91: g_NangCapGuild->ParseRankClanData(ReceiveBuffer);				return true;
				case 0x92: g_NangCapGuild->ParseGroupClanData(ReceiveBuffer);				return true;
				case 0x93: g_NangCapGuild->ParseContribClanData(ReceiveBuffer);				return true;
				case 0x94: g_NangCapGuild->ParseStoreClanData(ReceiveBuffer);				return true;
				case 0x95: g_NangCapGuild->ParseContribResultData(ReceiveBuffer);			return true;

				case 0x9C:
				{
					if (g_pCharacterInfoWindow)
						g_pCharacterInfoWindow->ReceiveNameColorCoin(ReceiveBuffer);
					break;
				}
			}
			break;
		...
		case 0xD5:
		{
			switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
			{
				case 0x11: g_pNewShop->SendPacketToProtocol11(ReceiveBuffer);	break;
				case 0x12: g_pNewShop->SendPacketToProtocol12(ReceiveBuffer);	break;
				case 0x14: g_pExtendShop->RecvList(ReceiveBuffer);				break;
			}
			break;
		}
		...
		switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
		{
			case 0x14: 
			{
				SolvedPacket* mRecv = (SolvedPacket*)ReceiveBuffer;
				...
				g_pNewUISystem->Show(SEASON3B::INTERFACE_INVENTORY);
				g_pNewUISystem->Toggle(SEASON3B::INTERFACE_CONGHUONG);

			}
			break;

			case 0x15: g_pCongHuong->RecvInfoGSSend((PMSG_GSSendInfoClient*)ReceiveBuffer);		break;
		}
		break;
...
void GCPetChartSelectRecv(PMSG_NEW_SELECT_CHARACTER_CHARSET_RECV* lpMsg)
{
	...
	if (o && o == &Hero->Object)
	{
		...
		case eBuff_Hellowin5:
		{
			...
			else if (buff == eBuff_Hellowin2)
			{
				...
				if (iBaseClass == CLASS_WIZARD || iBaseClass == CLASS_MAGIC || iBaseClass == CLASS_SUMMONER)
				{
					CharacterMachine->CalculateMagicDamage();
				}
				else
...
void InsertBuffPhysicalEffect(eBuffState buff, OBJECT* o)
{
	switch (buff)
	{
	case static_cast<eBuffState>(rise::pegasus::kSpiralChargeNativeEffect):
		rise::pegasus::CreateSpiralChargeAnchor(*o);
		break;
	case static_cast<eBuffState>(rise::pegasus::kCrusherChargeNativeEffect):
		rise::pegasus::CreateCrusherChargeAura(*o);
		break;
	case static_cast<eBuffState>(rise::pegasus::kElementalChargeNativeEffect):
		rise::pegasus::CreateElementalChargeBuffAura(*o);
		break;
...
void ClearBuffPhysicalEffect(eBuffState buff, OBJECT* o)
{
	...
	{
	case static_cast<eBuffState>(rise::pegasus::kSpiralChargeNativeEffect):
		DeleteEffect(rise::pegasus::kSpiralChargeAnchorModel, o);
		break;
	case static_cast<eBuffState>(rise::pegasus::kCrusherChargeNativeEffect):
		rise::pegasus::DeleteCrusherChargeAura(*o);
		break;
	case static_cast<eBuffState>(rise::pegasus::kElementalChargeNativeEffect):
		rise::pegasus::DeleteElementalChargeAuras(*o);
		break;
	case eBuff_CursedTempleProdection:
	{
...
void RegisterBuff(eBuffState buff, OBJECT* o, const int bufftime)
{
	...
	const bool pegasusMarker = rise::pegasus::IsPegasusNativeBuffEffect(
		static_cast<int>(buff));
	if (pegasusMarker)
		rise::pegasus::TraceRuntime("BUFF_REGISTER",
			static_cast<int>(buff), bufftime, o ? "RECEIVED" : "NO_OWNER");
	if (buffclasstype == eBuffClass_Count && !pegasusMarker) return;
	...
	if (pegasusMarker)
	{
		...
		rise::pegasus::TraceRuntime("BUFF_STATE",
			static_cast<int>(buff), bufftime,
			g_isCharacterBuff(o, buff) ? "ACTIVE" : "REGISTER_FAILED");
		return;
	}
...
void UnRegisterBuff(eBuffState buff, OBJECT* o)
{
	...
	const bool pegasusMarker = rise::pegasus::IsPegasusNativeBuffEffect(
		static_cast<int>(buff));
	if (pegasusMarker)
		rise::pegasus::TraceRuntime("BUFF_UNREGISTER",
			static_cast<int>(buff), 0, o ? "RECEIVED" : "NO_OWNER");
	if (buffclasstype == eBuffClass_Count && !pegasusMarker) return;
	...
	if (pegasusMarker)
	{
		...
		rise::pegasus::TraceRuntime("BUFF_STATE",
			static_cast<int>(buff), 0,
			g_isCharacterBuff(o, buff) ? "UNREGISTER_FAILED" : "CLEARED");
		return;
	}
