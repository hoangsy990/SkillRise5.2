...
CAttack gAttack;
bool GetMapCanKillAll(int map)
{
	switch (map)
	{
	case MAP_DUEL_ARENA:
	case MAP_CHAOS_CASTLE1:
	case MAP_CHAOS_CASTLE2:
	case MAP_CHAOS_CASTLE3:
	case MAP_CHAOS_CASTLE4:
	case MAP_CHAOS_CASTLE5:
	case MAP_CHAOS_CASTLE6:
	case MAP_CHAOS_CASTLE7:
		return true;
	default:
		return false;
	}
}
bool CAttack::Attack(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, bool send, BYTE flag, __int64 damage, int count, bool combo)
{
	...
	if (g_BossGuild->CheckPlayerTarget(lpObj, lpTarget, 1))
	{
		return 0;
	}
	if (lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		if (lpObj->DisablePvp == 1 || lpTarget->DisablePvp == 1)
		{
			if (gObjCheckEventJoin(lpTarget))
			{
				...
				{
					gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, pMessage->Text(672));
	...
	if (lpTarget->SummonIndex == lpObj->Index)
	{
		return 0;
	}

	if (lpTarget->Index == lpObj->SummonIndex)
	{
		return 0;
	}

	if (gCastleStartGuild.CheckPlayerTarget(lpObj, lpTarget, 1))
	{
		return 0;
	}
	...
	if (OBJECT_RANGE(lpObj->SummonIndex) != 0)
	{
		if (lpTarget->SummonIndex == lpObj->SummonIndex)
		{
			return 0;
		}
	}
...
#pragma region DAMAGE_CALC
	...
	if (damage == 0)
	{
		...
		{
			damage -= (damage * lpTarget->DamageReduction[n]) / gServerInfo.MaxDameReduct;
		}
		...
		else if (skill == SKILL_FORCE || skill == SKILL_FIRE_BURST || skill == SKILL_EARTHQUAKE || skill == SKILL_ELECTRIC_SPARK || skill == SKILL_FIRE_BLAST || skill == SKILL_FIRE_SCREAM || skill == SKILL_BIRDS || skill == rise_windsoul::kSkillId)
		{
		...
		else if (skill == SKILL_LARGE_RING_BLOWER || skill == SKILL_PHOENIX_SHOT)
		{
		...
		else if (skill == SKILL_CHAIN_DRIVER)
		{
			...
			damage = (damage * lpObj->RFDamageMultiplierRate[0]) /  gServerInfo.SKILL_CHAIN_DRIVER;
		}
		else if (skill == SKILL_DARK_SIDE)
		{
			...
			damage = (damage * lpObj->RFDamageMultiplierRate[2]) / gServerInfo.SKILL_DARK_SIDE;
		}
...
#pragma region CHECK_SELF_DEFENSE
	...
	if (damage > 0)
	{
		if (lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			if (g_ThanMaChien->CheckPlayerTarget(lpObj) && g_ThanMaChien->CheckPlayerTarget(lpTarget))
			{
				CheckSelfDefense = 0;
			}
			#if(GAMESERVER_TYPE==1)
...
#pragma region ATTACK_FINISH
	...
	{
		gGetInfoCharacter.GCDamageSend(lpObj->Index, lpTarget->Index, 0, 0, effect, 0);
	}
...
bool CAttack::AttackElemental(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, bool send, BYTE flag, __int64 damage, int count, bool combo)
{
	return 0;
}
...
void CAttack::WingSprite(LPOBJ lpObj, LPOBJ lpTarget, __int64* damage)
{
...
void CAttack::HelperSprite(LPOBJ lpObj, LPOBJ lpTarget, __int64* damage)
{
	const int petSlots[] = { 8, EQUIPMENT_MUUN, EQUIPMENT_EAGLE };

	if (lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		for (int slot : petSlots)
		{
	...
	if (lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		for (int slot : petSlots)
		{
			CItem* lpItem = &lpTarget->Inventory[slot];
			if (lpItem->IsItem() == 0 || lpItem->m_Durability <= 0)
			{
				continue;
			}
			...
			{
				(*damage) = ((*damage) * (100 - gCustomPet.GetCustomPetDecDamageRate(lpItem->m_Index))) / 100;
			}
		}
	}
}
void CAttack::DamageSprite(LPOBJ lpObj, __int64 damage)
{
	if (lpObj->Type != OBJECT_USER)
	{
		return;
	}
	const int extendedPetSlots[] = { EQUIPMENT_MUUN, EQUIPMENT_EAGLE };
	for (int slot : extendedPetSlots)
	{
		CItem* pet = &lpObj->Inventory[slot];
		if (pet->IsItem() != 0 && pet->m_IsPeriodicItem == 0 && gCustomPet.CheckCustomPetByItem(pet->m_Index) != 0)
		{
			this->MuunPet(lpObj, damage, slot);
		}
	}
...
bool CAttack::DarkHorseSprite(LPOBJ lpObj, __int64 damage)
{
	CItem* lpItem = &lpObj->Inventory[8];
	if (lpItem->m_Durability < 1)
	{
...
bool CAttack::FenrirSprite(LPOBJ lpObj, __int64 damage)
{
	CItem* lpItem = &lpObj->Inventory[8];
	if (lpItem->m_Durability < 1)
	{
...
void CAttack::WeaponDurabilityDown(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	for (int n = 0; n < 2; n++)
	{
		if (lpObj->Inventory[n].IsItem() != 0)
		{
			...
			{
				case 0: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, 0);				break;
				case 1: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, 0);				break;
				case 2: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, 0);				break;
				case 3: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, 0);				break;
				case 4: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, 1);				break;
				case 5: result = lpObj->Inventory[n].WeaponDurabilityDown(lpObj->Index, lpTarget->Defense, ((lpObj->Inventory[n].m_Slot == 0) ? 2 : 3));				break;
			}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj, LPOBJ lpTarget)
{
	...
	if (lpObj->Map == MAP_THANMA && lpTarget->Map == MAP_THANMA)
	{
		if (g_ThanMaChien->CheckSelfTeam(lpObj, lpTarget))
		{
			return 0;
		}
		return 1;
	...
	{
		if (1 != 0 && gParty.IsMember(lpObj->PartyNumber, lpTarget->Index) != 0)
		{
			return 0;
		}
	}
	if (lpObj->Map == g_BossGuild->m_MapBossGuild)
	{
		if (lpObj->Guild != 0 && lpTarget->Guild != 0)
		{
			...
			if (lpObj->Guild->Number == lpTarget->Guild->Number && 1 != 0 && gDuel.CheckDuel(lpObj, lpTarget) == 0)
			{
				return 0;
			}
		}
	...
	if (!GetMapCanKillAll(lpObj->Map))
	{
		if (gObjGetRelationShip(lpObj, lpTarget) == 1 && g_LoanChien->CheckPlayerJoined(lpObj, lpTarget) == 0)
		{
		...
		if (lpObj->Guild != 0 && lpTarget->Guild != 0)
		{
			if (lpObj->Guild->Number == lpTarget->Guild->Number && gServerInfo.m_GuildPK != 0 && gDuel.CheckDuel(lpObj, lpTarget) == 0 && g_LoanChien->CheckPlayerJoined(lpObj, lpTarget) == 0)
			{
				return 0; // PK Cung Guid
			}
		}
	}

	if (gDevilSquare.m_DevilSquarePK == 0)
	{
	...
	if (gBloodCastle.m_BloodCastlePK == 0)
	{
	...
	if (gMapManager.GetMapNonPK(lpTarget->Map) != 0 && (lpObj->PKFreeMode == 0 || lpTarget->PKFreeMode == 0))
	{
		return 0;
	}
	if (g_ThanMaChien->CheckPlayerJoined(lpObj, lpObj) && !g_ThanMaChien->CheckPlayerJoined(lpTarget, lpTarget))
	{
		return 0;
	}

	if (g_ThanMaChien->CheckPlayerJoined(lpTarget, lpTarget) && !g_ThanMaChien->CheckPlayerJoined(lpObj, lpObj))
	{
		return 0;
	}
...
bool CAttack::ApplySkillEffect(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, __int64 damage)
{
	...
	case SKILL_EARTH_PRISON:
		...
		break;
	case SKILL_INFERNO:
		gSkillManager.ApplyVongTronLuaEffect(lpObj, lpTarget, lpSkill, damage);
		break;
	}
...
__int64 CAttack::GetAttackDamage(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense)
{
	...
	else
	{
		...
		DamageMin += lpObj->EffectOption.AddMinPhysiDamage;
		DamageMax += lpObj->EffectOption.AddMaxPhysiDamage;
		{ int tmpMin = (int)DamageMin, tmpMax = (int)DamageMax; gSkillManager.SkillSwordPowerGetPhysiDamage(lpObj->Index, &tmpMin, &tmpMax); DamageMin = tmpMin; DamageMax = tmpMax; }
		DamageMin += (DamageMin * lpObj->EffectOption.MulPhysiDamage) / 100;
		DamageMax += (DamageMax * lpObj->EffectOption.MulPhysiDamage) / 100;
...
__int64 CAttack::GetAttackDamageWizard(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense)
{
	CItem* Right = &lpObj->Inventory[0];
	CItem* Left = &lpObj->Inventory[1];
	__int64 DamageMin = lpObj->MagicDamageMin;
	__int64 DamageMax = lpObj->MagicDamageMax;
	...
	{ int tmpMin = (int)DamageMin, tmpMax = (int)DamageMax; gSkillManager.SkillSwordPowerGetMagicDamage(lpObj->Index, &tmpMin, &tmpMax); DamageMin = tmpMin; DamageMax = tmpMax; }
	int type = gSkillManager.GetSkillType(lpSkill->m_index);
	if (CHECK_RANGE(type, MAX_RESISTANCE_TYPE) != 0)
	{
	...
	__int64 range = (DamageMax - DamageMin);
	range = ((range < 1) ? 1 : range);
	__int64 damage = DamageMin + (GetLargeRand() % range);
	if ((GetLargeRand() % gServerInfo.GioiHanTyLeSatThuongChiMang) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
	{
		(*effect) = 3;
		damage = DamageMax;
...
__int64 CAttack::GetAttackDamageCursed(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense)
{
	CItem* Right = &lpObj->Inventory[0];
	CItem* Left = &lpObj->Inventory[1];
	__int64 DamageMin = lpObj->CurseDamageMin;
	__int64 DamageMax = lpObj->CurseDamageMax;
	DamageMin += lpSkill->m_DamageMin;
	DamageMax += lpSkill->m_DamageMax;
	DamageMin += gMasterSkillTree.GetMasterSkillDamageMin(lpObj, lpSkill->m_skill);
	DamageMax += gMasterSkillTree.GetMasterSkillDamageMax(lpObj, lpSkill->m_skill);
	...
	{ int tmpMin = (int)DamageMin, tmpMax = (int)DamageMax; gSkillManager.SkillSwordPowerGetCurseDamage(lpObj->Index, &tmpMin, &tmpMax); DamageMin = tmpMin; DamageMax = tmpMax; }
	int type = gSkillManager.GetSkillType(lpSkill->m_index);
	if (CHECK_RANGE(type, MAX_RESISTANCE_TYPE) != 0)
	{
		DamageMin += lpObj->AddResistance[type];
...
__int64 CAttack::GetAttackDamageFenrir(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense)
{
	int BaseDamage = 0;
	...
	__int64 range = (lpSkill->m_DamageMax - lpSkill->m_DamageMin);
	range = ((range < 1) ? 1 : range);
	__int64 damage = (BaseDamage + lpSkill->m_DamageMin) + (GetLargeRand() % range);
	if ((GetLargeRand() % 100) < ((lpObj->CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
	{
		(*effect) = 3;
...
__int64 CAttack::GetAttackDamageElemental(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int AttackDamage, int TargetDefense)
{
	return 0;
}
__int64 CAttack::GetShieldDamage(LPOBJ lpObj, LPOBJ lpTarget, __int64 damage)
{
	...
	__int64 SDDamage = (damage * rate) / 100;
	__int64 HPDamage = damage - SDDamage;
...
void CAttack::GetPreviewDamageMultiplier(LPOBJ lpObj, DWORD* DamageMultiplier, DWORD* RFDamageMultiplierA, DWORD* RFDamageMultiplierB, DWORD* RFDamageMultiplierC)
{
	...
	case CLASS_FE:
		...
		break;
	case CLASS_MG:
		(*DamageMultiplier) = lpObj->MGDamageMultiplierRate; //(*DamageMultiplier) = 200;
		(*RFDamageMultiplierA) = 100;
		(*RFDamageMultiplierB) = 100;
		(*RFDamageMultiplierC) = 100;
		break;
...
bool CAttack::MuunPet(LPOBJ lpObj, __int64 damage, int slot)
{
	CItem* lpItem = &lpObj->Inventory[slot];
	...
	if (lpItem->m_DurabilitySmall > MaxSmallDur)
	{
		...
		gItemManager.GCItemDurSend(lpObj->Index, slot, (BYTE)lpItem->m_Durability, 0);
