...
{
	int skill, slot;
	if (lpItem->m_Index == GET_ITEM(rise_windsoul::kScrollGroup,
		rise_windsoul::kScrollIndex))
	{
		if (lpObj->Class != CLASS_DL ||
			(lpObj->ChangeUp + 1) < rise_windsoul::kRequiredClassTier ||
			lpObj->Level < rise_windsoul::kRequiredLevel ||
			(lpObj->Strength + lpObj->AddStrength) < rise_windsoul::kRequiredStrength ||
			gSkillManager.GetSkill(lpObj,
				rise_windsoul::kRequiredRegularSkillId) == 0 ||
			gMasterSkillTree.GetMasterSkillLevel(lpObj,
				rise_windsoul::kRequiredMasterSkillId) <
				rise_windsoul::kRequiredMasterSkillPoints)
		{
			return 0;
		}
	}
