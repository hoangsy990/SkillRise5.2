...
namespace
{
bool IsIsolatedPegasusRuntimeSkill(int skill)
{
	return skill == rise_windsoul::kSkillId ||
		rise::pegasus::FindHighSkillById(skill) != nullptr;
}

void TraceIsolatedPegasusServerCast(LPOBJ lpObj, int skill, int target,
	const char* result)
{
#if defined(PEGASUS_ISOLATED_TEST)
	if (IsIsolatedPegasusRuntimeSkill(skill))
	{
		LogAdd(LOG_BLACK,
			"[PegasusCast] account=%s character=%s skill=%d target=%d result=%s",
			lpObj->Account, lpObj->Name, skill, target, result);
	}
#else
	(void)lpObj;
	(void)skill;
...
bool CheckPacketSkillClassForIsolatedQA(LPOBJ lpObj, int skill)
{
#if defined(PEGASUS_ISOLATED_TEST)
	const bool isQaSkill = IsIsolatedPegasusRuntimeSkill(skill);
	const bool isQaGameMaster = (lpObj->Authority & 32) != 0 ||
		gGameMaster.CheckGameMasterLevel(lpObj, 1) != 0;
	if (isQaSkill && isQaGameMaster)
	{
		return true;
	}
#endif
	return gSkillManager.CheckSkillRequireClass(lpObj, skill) != 0;
}

bool CheckPacketSkillWeaponForIsolatedQA(LPOBJ lpObj, int skill)
{
#if defined(PEGASUS_ISOLATED_TEST)
	const bool isQaSkill = IsIsolatedPegasusRuntimeSkill(skill);
	const bool isQaGameMaster = (lpObj->Authority & 32) != 0 ||
	...
	if (isQaSkill && isQaGameMaster)
		return true;
#endif
	return gSkillManager.CheckSkillRequireWeapon(lpObj, skill) != 0;
}
}
...
void CSkillManager::Load(char* path)
{
	...
	try
	{
		while (true)
		{
			...
			if (info.Index >= 0 && info.Index < MAX_SKILL_TYPES)
			{
	...
	// Exact transferable fields are sourced from the hash-pinned Pegasus XML.
	// Server-only routing/effect fields use the native generic target default;
	// dedicated cast/buff handlers remain gated as IN_PROCESS work.
	for (const rise::pegasus::HighSkillSeed& seed : rise::pegasus::kHighSkillSeeds)
	{
		SKILL_INFO info = {};
		info.Index = seed.skillId;
		strcpy_s(info.Name, seed.name);
		info.Damage = seed.damage;
		info.Mana = seed.mana;
		info.BP = seed.abilityGauge;
		info.Range = seed.distance;
		info.Radio = seed.distance;
		info.Delay = seed.delay;
		info.Type = -1;
		info.Effect = 0;
		info.RequireLevel = seed.level;
		info.RequireEnergy = seed.energy;
		info.RequireLeadership = seed.leadership;
		for (int n = 0; n < MAX_CLASS; ++n) info.RequireClass[n] = seed.requiredClass[n];
		this->m_SkillInfo[info.Index] = info;
	}
#if defined(PEGASUS_ISOLATED_TEST)
	// Owner custom clone: same server contract/damage as Sword Blow, separate
	// identity so the recovered 1501 behavior remains untouched.
	{
		SKILL_INFO info = this->m_SkillInfo[rise::pegasus::kSwordBlowSkill];
		info.Index = rise::pegasus::kSwordBlowFlySkill;
		strcpy_s(info.Name, "Sword Blow Fly");
		this->m_SkillInfo[info.Index] = info;
	}
#endif
	// Standalone Wind Soul 737 seed from the verified SS21 server config. It is
	// intentionally separate from the Pegasus group-15 catalog.
	{
		SKILL_INFO info = {};
		info.Index = rise_windsoul::kSkillId;
		strcpy_s(info.Name, "Wind Soul");
		info.Damage = 130;
		info.Mana = 35;
		info.BP = 17;
		info.Range = 6;
		...
		info.Type = -1;
		info.Effect = 0;
		info.RequireLevel = rise_windsoul::kRequiredLevel;
		info.RequireClass[CLASS_DL] = rise_windsoul::kRequiredClassTier;
		this->m_SkillInfo[info.Index] = info;
	}
}
...
{
	if (index == GET_ITEM(rise_windsoul::kScrollGroup,
		rise_windsoul::kScrollIndex))
	{
		return SKILL_WIND_SOUL;
	}
	if (index >= GET_ITEM(15, 102) && index <= GET_ITEM(15, 113))
	{
		const rise::pegasus::HighSkillSeed* seed =
			rise::pegasus::FindHighSkillByItem(15, index % MAX_ITEM_TYPE);
...
{
	if (index < 0 || index >= MAX_SKILL_TYPES)
	{
		return 0;
	}
	SKILL_INFO SkillInfo;
...
{
	if (index == SKILL_SPIRAL_CHARGE || index == SKILL_CRUSHER_CHARGE ||
		index == SKILL_ELEMENTAL_CHARGE)
	{
		CItem* weapon = &lpObj->Inventory[0];
		rise::pegasus::PegasusChargeWeaponFamily family =
			rise::pegasus::PegasusChargeWeaponFamily::None;
		if (weapon->IsItem() != 0 && weapon->m_IsValidItem != 0)
		{
			const int section = weapon->m_Index / MAX_ITEM_TYPE;
			family = section == 0 ? rise::pegasus::PegasusChargeWeaponFamily::Sword :
				section == 5 ? rise::pegasus::PegasusChargeWeaponFamily::Staff :
				rise::pegasus::PegasusChargeWeaponFamily::Other;
		}
		return rise::pegasus::ChargeBuffWeaponAllowed(index, family,
			weapon->m_TwoHand != 0);
	}
...
INT_PTR CALLBACK RISE_SkillProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: {
		...
		case IDM_SKILL_INSERT: {
			if (gSkillManager.m_SkillInfo.size() < MAX_SKILL_CATALOG_ENTRIES) {
				int newIndex = 1;
...
void CSkillManager::UseAttackSkill(int aIndex, int bIndex, CSkill* lpSkill)
{
	...
	if (lpObj->Type == OBJECT_USER &&
		!CheckPacketSkillWeaponForIsolatedQA(lpObj, lpSkill->m_skill))
	{
		return;
	}
...
bool CSkillManager::RunningSkill(int aIndex, int bIndex, CSkill* lpSkill, BYTE x, BYTE y, BYTE angle, bool combo)
{
	...
	case SKILL_WIND_SOUL:
		return this->SkillWindSoul(aIndex, bIndex, lpSkill, combo);
	case SKILL_SHINING_BIRD:
		return this->SkillShiningBird(aIndex, bIndex, lpSkill, angle, combo);
	...
	case SKILL_DEATHSIDE:
		return this->SkillDeathside(aIndex, bIndex, lpSkill, combo);
	case SKILL_DARKNESS:
		return this->SkillPegasusSelfBuff(aIndex, lpSkill);
	case SKILL_SWORD_BLOW:
	case SKILL_SWORD_BLOW_FLY:
		return this->SkillSwordBlow(aIndex, bIndex, lpSkill, combo);
	case SKILL_DRAGON_VIOLENT:
		return this->SkillDragonViolent(aIndex, bIndex, lpSkill, combo);
	case SKILL_HAVOC_SPEAR:
		return this->SkillHavocSpear(aIndex, bIndex, lpSkill, combo);
	case SKILL_CHAOS_BLADE:
		// Pegasus F3:35 dispatch calls 0x654BA3, which applies the 0x654702
		// impact helper once for every authoritative target.
		return this->SkillDragonViolent(aIndex, bIndex, lpSkill, combo);
	case SKILL_RAINING_ARROW:
		// The recovered 0x670D19 client chain consumes an authoritative
		// per-target list. Reuse the native target-centred area route and the
		// bounded isolated F3:35 transport instead of a single basic hit.
		return this->SkillDragonViolent(aIndex, bIndex, lpSkill, combo);
	case SKILL_SPEARSTORM:
		return this->SkillSpearstorm(aIndex, bIndex, lpSkill, combo);
	case SKILL_SWORD_WRATH:
		return this->SkillSwordWrath(aIndex, lpSkill);
	case SKILL_SPIRAL_CHARGE:
	case SKILL_CRUSHER_CHARGE:
	case SKILL_ELEMENTAL_CHARGE:
	case SKILL_DEX_BOOSTER:
		return this->SkillPegasusSelfBuff(aIndex, lpSkill);
	default:
...
bool CSkillManager::SkillDeathside(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	LPOBJ lpObj = &gObj[aIndex];
	// Pegasus Skill.xml marks 288 as a targeted damage skill. Preserve native
	// validation/damage, then echo the accepted cast so the recovered Reaper
	// client chain owns action and visuals instead of the input path.
	if (this->BasicSkillAttack(aIndex, bIndex, lpSkill, combo) == 0)
	{
		return 0;
	}
...
bool CSkillManager::SkillSwordBlow(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	LPOBJ lpObj = &gObj[aIndex];
	if (OBJECT_RANGE(bIndex) == 0)
	{
	...
	if (this->CheckSkillTarget(lpObj, bIndex, -1, gObj[bIndex].Type) == 0)
	{
		return 0;
	}
	SKILL_INFO skillInfo;
	...
	const bool swordWrathActive =
		gEffectManager.CheckEffect(lpObj, rise::pegasus::kSwordWrathNativeEffect) != 0;
	const int effectiveRange = rise::pegasus::SwordWrathEffectiveRange(
		skillInfo.Range, swordWrathActive);
	const int effectiveRadius = rise::pegasus::SwordWrathEffectiveDamageRadius(
		skillInfo.Radio, swordWrathActive);
	if (lpObj->Type == OBJECT_USER && !rise::pegasus::IsWithinPegasusDistance(
		lpObj->X, lpObj->Y, gObj[bIndex].X, gObj[bIndex].Y, effectiveRange))
	{
		return 0;
	}
	WORD targets[rise::pegasus::kSwordBlowTargetLimit];
	BYTE count = 0;
	targets[count++] = static_cast<WORD>(bIndex);
	gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 1, 0, 0, 0, combo);
	for (int n = 0; n < MAX_VIEWPORT && count < rise::pegasus::kSwordBlowTargetLimit; ++n)
	{
		if (lpObj->VpPlayer2[n].state == VIEWPORT_NONE || lpObj->VpPlayer2[n].index == bIndex)
		{
			continue;
		}
		const int index = lpObj->VpPlayer2[n].index;
		if (this->CheckSkillTarget(lpObj, index, bIndex, lpObj->VpPlayer2[n].type) == 0 ||
			!rise::pegasus::IsWithinPegasusDistance(gObj[bIndex].X, gObj[bIndex].Y,
				gObj[index].X, gObj[index].Y, effectiveRadius))
		{
			continue;
		}
		gAttack.Attack(lpObj, &gObj[index], lpSkill, 0, 0, 0, 0, combo);
		targets[count++] = static_cast<WORD>(index);
	}
	this->GCSkillAttackSend(lpObj, lpSkill->m_index, bIndex, 1);
	this->GCPegasusTargetsSend(lpObj, lpSkill->m_index, targets, count);
	return 1;
}
bool CSkillManager::SkillDragonViolent(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	LPOBJ lpObj = &gObj[aIndex];
	const bool doubleHit = (lpSkill->m_index == SKILL_DRAGON_VIOLENT &&
		rise::pegasus::kDragonViolentBaseDamageInstances == 2);
	if (OBJECT_RANGE(bIndex) == 0 ||
		this->CheckSkillTarget(lpObj, bIndex, -1, gObj[bIndex].Type) == 0)
	{
		return 0;
	}
	SKILL_INFO skillInfo;
	...
	const int effectiveRange = rise::pegasus::ChargeEffectiveCastRange(
		lpSkill->m_index, skillInfo.Range,
		gEffectManager.CheckEffect(lpObj, rise::pegasus::kSpiralChargeNativeEffect) != 0,
		gEffectManager.CheckEffect(lpObj, rise::pegasus::kCrusherChargeNativeEffect) != 0,
		gEffectManager.CheckEffect(lpObj, rise::pegasus::kElementalChargeNativeEffect) != 0);
	if (lpObj->Type == OBJECT_USER && !rise::pegasus::IsWithinPegasusDistance(
		lpObj->X, lpObj->Y, gObj[bIndex].X, gObj[bIndex].Y, effectiveRange))
	{
		return 0;
	}
	WORD targets[rise::pegasus::kPegasusTargetLimit];
	BYTE count = 0;
	targets[count++] = static_cast<WORD>(bIndex);
	gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 1, 0, 0, 1, combo);
	if (doubleHit)
		gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 0, 0, 0, 2, combo);
	for (int n = 0; n < MAX_VIEWPORT && count < rise::pegasus::kPegasusTargetLimit; ++n)
	{
		if (lpObj->VpPlayer2[n].state == VIEWPORT_NONE ||
		...
		const int index = lpObj->VpPlayer2[n].index;
		if (this->CheckSkillTarget(lpObj, index, bIndex, lpObj->VpPlayer2[n].type) == 0 ||
			this->CheckSkillRadio(lpSkill->m_index, gObj[bIndex].X, gObj[bIndex].Y,
...
bool CSkillManager::SkillSpearstorm(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	// Spearstorm damages nearby targets twice and has a 5% third instance.
	// The visual storm and its damage radio are centred on the selected target.
	LPOBJ lpObj = &gObj[aIndex];
	if (OBJECT_RANGE(bIndex) == 0 ||
		this->CheckSkillTarget(lpObj, bIndex, -1, gObj[bIndex].Type) == 0)
	{
	...
	if (lpObj->Type == OBJECT_USER && this->CheckSkillRange(lpSkill->m_index,
		lpObj->X, lpObj->Y, gObj[bIndex].X, gObj[bIndex].Y) == 0)
	{
		return 0;
	}
	gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 1, 0, 0, 1, combo);
	gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 0, 0, 0, 2, combo);
	if ((GetLargeRand() % 100) < rise::pegasus::kSpearstormAdditionalDamageRate)
		gAttack.Attack(lpObj, &gObj[bIndex], lpSkill, 0, 0, 0, 3, combo);
	int count = 1;
	for (int n = 0; n < MAX_VIEWPORT; ++n)
	{
		if (lpObj->VpPlayer2[n].state == VIEWPORT_NONE ||
	...
	// The target-centred storm is created by the authoritative cast response.
	// Without this echo the server dealt damage while the client showed nothing.
	this->GCSkillAttackSend(lpObj, lpSkill->m_index, bIndex, 1);
	return 1;
}
bool CSkillManager::SkillHavocSpear(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	// Pegasus 0x666DA1 iterates the authoritative target list and invokes the
	// per-target impact chain once for every valid entry.
	return this->SkillDragonViolent(aIndex, bIndex, lpSkill, combo);
}
bool CSkillManager::SkillSwordWrath(int aIndex, CSkill* lpSkill)
{
	LPOBJ lpObj = &gObj[aIndex];
	EFFECT_INFO* effectInfo = gEffectManager.GetInfo(rise::pegasus::kSwordWrathNativeEffect);
	if (effectInfo == 0 || strcmp(effectInfo->Name, "Pegasus Sword Wrath") != 0)
	{
		LogAdd(LOG_RED, "[Pegasus] Sword Wrath effect contract is unavailable");
		return 0;
	}
	if (gEffectManager.AddEffect(lpObj, 0, rise::pegasus::kSwordWrathNativeEffect,
		rise::pegasus::kSwordWrathDefaultDurationSeconds, 0, 0, 0, 0) == 0)
	{
		return 0;
	}
...
bool CSkillManager::SkillPegasusSelfBuff(int aIndex, CSkill* lpSkill)
{
	LPOBJ lpObj = &gObj[aIndex];
	const int effect = rise::pegasus::NativeEffectForHighSkillBuff(lpSkill->m_index);
	const rise::pegasus::HighSkillBuffEvidence* evidence =
		rise::pegasus::FindHighSkillBuffEvidence(lpSkill->m_index);
	EFFECT_INFO* effectInfo = gEffectManager.GetInfo(effect);
	if (effect < 0 || evidence == 0 || effectInfo == 0 ||
		strncmp(effectInfo->Name, "Pegasus ", 8) != 0)
	{
		LogAdd(LOG_RED, "[Pegasus] Self-buff effect contract is unavailable for skill %d", lpSkill->m_index);
		return 0;
	}
	// Marker-only until the original server-side stat formula is evidenced.
	// This prevents TypeSkill=1 entries from falling through to BasicSkillAttack.
	if (gEffectManager.AddEffect(lpObj, 0, effect,
		rise::pegasus::HighSkillBuffDurationSeconds(lpSkill->m_index),
		0, 0, 0, 0) == 0)
	{
		return 0;
	}
...
bool CSkillManager::SkillDeathStab(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	...
	const int effectiveRadius = rise::pegasus::SwordWrathEffectiveDamageRadius(
		skillInfo.Radio, swordWrathActive);
	if (lpObj->Type == OBJECT_USER && !rise::pegasus::IsWithinPegasusDistance(
		lpObj->X, lpObj->Y, lpTarget->X, lpTarget->Y, effectiveRange))
	{
		return 0;
	}
	this->GCSkillAttackSend(lpObj, lpSkill->m_index, bIndex, 1);
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (!rise::pegasus::IsWithinPegasusDistance(lpTarget->X, lpTarget->Y,
			gObj[index].X, gObj[index].Y, effectiveRadius))
		{
			continue;
		}
...
bool CSkillManager::SkillShiningBird(int aIndex, int bIndex, CSkill* lpSkill, BYTE angle, bool combo)
{
	if (OBJECT_RANGE(bIndex) == 0)
		return 0;
	// Keep the native Birds 1.5 x 6.0 directional frustum, but Shining Bird
	// applies exactly one damage instance to each affected target. Unlike the
	// native duration-skill Birds route, Pegasus Shining Bird arrives through
	// the ordinary target-skill packet. Echo the accepted cast here so the
	// client-owned action, sound and 0x27C1 visual root are actually dispatched.
	LPOBJ lpObj = &gObj[aIndex];
	// The target-skill route passes angle=0 to RunningSkill: unlike duration
	// packets it has no heading field. Derive heading from the validated target
	// using native byte-angle encoding instead of attacking world-north.
	angle = static_cast<BYTE>((this->GetSkillAngle(lpObj->X, lpObj->Y,
		gObj[bIndex].X, gObj[bIndex].Y) * 255) / 360);
	int SkillFrustrumX[4], SkillFrustrumY[4];
	this->GetSkillFrustrum(SkillFrustrumX, SkillFrustrumY, angle,
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		const int index = lpObj->VpPlayer2[n].index;
		if (this->CheckSkillTarget(lpObj, index, bIndex, lpObj->VpPlayer2[n].type) == 0 ||
			this->CheckSkillRadio(lpSkill->m_index, lpObj->X, lpObj->Y,
				gObj[index].X, gObj[index].Y) == 0 ||
			this->CheckSkillFrustrum(SkillFrustrumX, SkillFrustrumY,
				gObj[index].X, gObj[index].Y) == 0)
		{
			continue;
		}
		gObjAddAttackProcMsgSendDelay(lpObj, 50, index, 200,
			lpSkill->m_index, combo);
		if (combo != 0)
			this->GCSkillAttackSend(lpObj, SKILL_COMBO, index, 1);
		if (CHECK_SKILL_ATTACK_COUNT(count) == 0)
			break;
...
bool CSkillManager::SkillWindSoul(int aIndex, int bIndex, CSkill* lpSkill, bool combo)
{
	LPOBJ lpObj = &gObj[aIndex];
	if (OBJECT_RANGE(bIndex) == 0)
	{
	...
	LPOBJ lpTarget = &gObj[bIndex];
	if (this->CheckSkillRange(lpSkill->m_index, lpObj->X, lpObj->Y,
		lpTarget->X, lpTarget->Y) == 0)
	{
		return 0;
	}
	this->GCSkillAttackSend(lpObj, lpSkill->m_index, bIndex, 1);
	for (int barrage = 1; barrage <= rise_windsoul::kPrimaryBarrageCount;
		++barrage)
	{
		gAttack.Attack(lpObj, lpTarget, lpSkill, 0, 0, 0, barrage, combo);
	}
	return 1;
}
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg, int aIndex, bool type)
{
	...
	if (lpObj->Type == OBJECT_USER)
	{
		...
		if (!CheckPacketSkillClassForIsolatedQA(lpObj, lpSkill->m_index))
		{
			return;
		}
	}
...
{
	LPOBJ lpObj = &gObj[aIndex];
	const int skill = MAKE_NUMBERW(lpMsg->skill[0], lpMsg->skill[1]);
	const int packetTarget = MAKE_NUMBERW(lpMsg->index[0], lpMsg->index[1]);
	TraceIsolatedPegasusServerCast(lpObj, skill, packetTarget, "RECEIVED");
	if (gObjIsConnectedGS(aIndex) == 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, packetTarget, "REJECT_SOURCE_DISCONNECTED");
		return;
	}
	if (lpObj->Live == 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, packetTarget, "REJECT_SOURCE_DEAD");
		return;
	}
	int bIndex = packetTarget;
	if (OBJECT_RANGE(bIndex) == 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_TARGET_RANGE");
		return;
	}
	LPOBJ lpTarget = &gObj[bIndex];
	if (gObjIsConnectedGS(bIndex) == 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_TARGET_DISCONNECTED");
		return;
	}
	if (lpTarget->Live == 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_TARGET_DEAD");
		return;
	}
	if (gMap[lpObj->Map].CheckAttr(lpObj->X, lpObj->Y, 1) != 0 || gMap[lpTarget->Map].CheckAttr(lpTarget->X, lpTarget->Y, 1) != 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_SAFE_ZONE");
		return;
	}
	if (gDuel.GetDuelArenaBySpectator(aIndex) != 0 || gDuel.GetDuelArenaBySpectator(bIndex) != 0)
	{
		TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_DUEL_SPECTATOR");
		return;
	}
	CSkill* lpSkill = 0;
	if (lpObj->Type == OBJECT_USER)
	{
		...
		if (lpSkill == 0)
		{
			TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_SKILL_MISSING");
			return;
		}
		if (this->CheckSkillDelay(lpObj, lpSkill->m_index) == 0)
		{
			TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_DELAY");
			return;
		}
		if (this->CheckSkillRequireKillPoint(lpObj, lpSkill->m_index) == 0)
		{
			TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_KILL_POINT");
			return;
		}
		if (this->CheckSkillRequireGuildStatus(lpObj, lpSkill->m_index) == 0)
		{
			TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_GUILD_STATUS");
			return;
		}
		if (!CheckPacketSkillClassForIsolatedQA(lpObj, lpSkill->m_index))
		{
			TraceIsolatedPegasusServerCast(lpObj, skill, bIndex, "REJECT_CLASS");
			return;
		}
	}
	else
...
{
	TraceIsolatedPegasusServerCast(lpObj, skill, aIndex, "ECHO_SEND");
...
void CSkillManager::GCPegasusTargetsSend(LPOBJ lpObj, int skill, const WORD* targets, BYTE count)
{
	BYTE packet[4 + rise::pegasus::kPegasusTargetPayloadFixedSize +
		(rise::pegasus::kPegasusTargetLimit * 2)] = { 0 };
	const BYTE boundedCount = (count > rise::pegasus::kPegasusTargetLimit)
		? static_cast<BYTE>(rise::pegasus::kPegasusTargetLimit) : count;
	const BYTE packetSize = static_cast<BYTE>(4 + rise::pegasus::SwordBlowPayloadSize(boundedCount));
	packet[0] = 0xC1;
	packet[1] = packetSize;
	packet[2] = rise::pegasus::kPegasusTargetPacketHead;
	packet[3] = rise::pegasus::kPegasusTargetPacketSub;
	rise::pegasus::WriteWordLE(packet + 4, static_cast<WORD>(skill));
	rise::pegasus::WriteWordLE(packet + 6, static_cast<WORD>(lpObj->Index));
	rise::pegasus::WriteWordLE(packet + 8, boundedCount);
	for (BYTE n = 0; n < boundedCount; ++n)
	{
		rise::pegasus::WriteWordLE(packet + 10 + (n * 2), targets[n]);
	}
	if (lpObj->Type == OBJECT_USER)
	{
		DataSend(lpObj->Index, packet, packetSize);
	}
	MsgSendV2(lpObj, packet, packetSize);
}
